...
namespace Server.Engines.CityLoyalty
{
	public class GuardCaptain : BaseCreature
	{
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			...
			{
				Raider raider =
					((PlayerMobile)m).AllFollowers.FirstOrDefault(mob => mob is Raider && mob.InRange(this.Location, 2))
					as Raider;
				...
				if (theirSystem != null && thisSystem != null && CityLoyaltySystem.HasCitizenship(Player, Guard.City))
				{
					if (Guard.IsInBannerCooldown(Player))
						Guard.SayTo(
							Player,
							1152364,
							String.Format("#{0}", CityLoyaltySystem.BannerLocalization(thisSystem.City))
					...
					if (theirSystem.GetLoyaltyRating(Player) < LoyaltyRating.Adored)
						Guard.SayTo(
							Player,
							1152363,
							String.Format("#{0}", CityLoyaltySystem.GetCityLocalization(thisSystem.City))
						); // I apologize, but you are not well-enough renowned in the city of ~1_CITY~ to make this purchase.
					else
					{
						string args = String.Format(
							"#{0}\t{1}",
							CityLoyaltySystem.BannerLocalization(thisSystem.City),
							CityLoyaltySystem.BannerCost.ToString("N0", CultureInfo.GetCultureInfo("en-US"))
						);
						Player.SendGump(
							new ConfirmCallbackGump(Player, 1049004, 1152365, Player, args, Guard.OnConfirm)
						);
					}
				}
			}
		...
		public GuardCaptain(Serial serial)
			: base(serial) { }
