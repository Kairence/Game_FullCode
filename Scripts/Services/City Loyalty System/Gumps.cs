...
namespace Server.Engines.CityLoyalty
{
	public class BaseCityGump : BaseGump
	{
		...
		public BaseCityGump(PlayerMobile pm)
			: base(pm, 120, 120)
		{
	...
	{
		public CityLoyaltyGump(PlayerMobile pm)
			: base(pm) { }
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < CityLoyaltySystem.Cities.Count; i++)
			{
				...
				AddHtmlLocalized(
					...
					200,
					16,
					CityLoyaltySystem.RatingLocalization(city.GetLoyaltyRating(User)),
					false,
					false
				);
			...
			AddHtmlLocalized(
				...
				100,
				16,
				Citizenship != null ? CityLoyaltySystem.CityLocalization(Citizenship.City) : 1152884,
				false,
				false
			);
			...
			if (!CityLoyaltySystem.IsSetup())
			{
				AddHtml(
					...
					250,
					60,
					"City Loyalty System has not been enabled by your server owner yet.",
					false,
					false
				);
			}
			...
			else
			{
				...
				else
				{
					AddHtmlLocalized(
						...
						80,
						1152886,
						CityLoyaltySystem.NextJoinCity(User).ToString(),
						0,
						false,
						false
					);
	...
	public class DeclareCitizenshipGump : BaseCityGump
	{
		...
		public DeclareCitizenshipGump(CityLoyaltySystem toDeclare, PlayerMobile pm)
			: base(pm)
		{
			City = toDeclare;
		}
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				200,
				1152891,
				String.Format("#{0}", CityLoyaltySystem.GetCityLocalization(City.City).ToString()),
				1,
				false,
				true
			);
	...
	public class RenounceCitizenshipGump : BaseCityGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				200,
				1152887,
				String.Format("#{0}", CityLoyaltySystem.GetCityLocalization(Citizenship.City).ToString()),
				1,
				false,
				true
			);
	...
	{
		public CityTitlesGump(PlayerMobile pm)
			: base(pm) { }
			...
			foreach (int i in Enum.GetValues(typeof(CityTitle)))
			{
				...
				if (Citizenship.HasTitle(User, title))
				{
					AddHtmlLocalized(
						...
						300,
						16,
						CityLoyaltySystem.GetTitleLocalization(User, title, Citizenship.City),
						false,
						false
					);
					y += 20;
	...
	public class CityTitlesInfoGump : BaseCityGump
	{
		...
		public CityTitlesInfoGump(PlayerMobile pm, CityTitle title)
			: base(pm)
		{
			Title = title;
		}
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				150,
				16,
				CityLoyaltySystem.RatingLocalization(Citizenship.GetMinimumRating(Title)),
				false,
				false
			);
			AddHtml(
		...
		public override void AddGumpLayout()
		{
			if (City.Election == null || !City.Election.Ongoing)
			{
				...
				AddHtmlLocalized(
					...
					false,
					false
				); // The current Governor of ~1_CITY~ is ~2_PLAYER~.  The next election cycle begins after ~3_DAYS~.
			}
			else
			{
		...
		public override void OnResponse(RelayInfo info)
		{
			switch (info.ButtonID)
			{
				case 0:
					break;
				case 1:
					City.Election.TryNominate(User);
					break;
				case 2:
					BaseGump.SendGump(new NomineesGump(User, City));
					break;
				case 3:
					BaseGump.SendGump(new CandidatesGump(User, City));
					break;
			}
		}
	}
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < City.Election.Candidates.Count; i++)
			{
				...
				AddHtml(
					...
					200,
					20,
					String.Format("<basefont color=#EEE8AA>{0}", entry.Player == null ? "Unknown" : entry.Player.Name),
					false,
					false
				);
				AddHtml(
					...
					200,
					20,
					String.Format("<basefont color=#EEE8AA>{0}", g != null ? g.Name : ""),
					false,
					false
				);
				pageIndex++;
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < Candidates.Count; i++)
			{
				...
				AddHtml(
					...
					20,
					String.Format(
						"<basefont color=#EEE8AA>{0}%",
						City.Election.GetStanding(entry)
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			if (id >= 0 && id < _Deals.Length)
			{
				if (City.Treasury < CityLoyaltySystem.TradeDealCost)
				{
					...
					User.AccessLevel == AccessLevel.Player
					&& City.TradeDealStart != DateTime.MinValue
					&& City.TradeDealStart + TimeSpan.FromDays(CityLoyaltySystem.TradeDealCooldown) > DateTime.UtcNow
				)
				{
			...
			else
			{
				...
				if (Server.Guilds.BaseGuildGump.CheckProfanity(text) && text.Trim().Length > 3)
				{
					if (entry != null && entry.IsCitizen)
					{
						...
						if (User != Citizen)
							User.SendMessage(
								"You have bestowed {0} the title: {1} of {2}.",
								Citizen.Name,
								text,
								City.Definition.Name
							);
	...
	public class AcceptOfficeGump : BaseGump
	{
		...
		public AcceptOfficeGump(PlayerMobile pm, CityLoyaltySystem city)
			: base(pm, 50, 50)
		{
			City = city;
	...
	{
		public ElectionStartTimeGump(PlayerMobile pm)
			: base(pm, 50, 50)
		{
		...
		public override void AddGumpLayout()
		{
			...
			if (CityLoyaltySystem.Britain.Election != null && CityLoyaltySystem.Britain.Election.Ongoing)
			{
				AddHtml(
					...
					370,
					40,
					"You will have to wait until the current election is over before you can start another one.",
					false,
					true
				);
			}
			else
			{
				AddHtml(
					...
					370,
					120,
					"You can have up to 4 election times.  Per EA, default are March, June, September and December, "
						+ "or 3, 6, 9, 12 as the system uses the numeric value for the month. Each election time must be at "
						+ "least 1 month apart, and each will begin on the first day of that month.",
					false,
					false
				);
				...
				for (int i = 0; i < 4; i++)
				{
					...
					if (
						CityLoyaltySystem.Britain.Election != null
						&& CityLoyaltySystem.Britain.Election.StartTimes.Length >= i
						&& CityLoyaltySystem.Britain.Election.StartTimes[i] != DateTime.MinValue
					)
		...
		public override void OnResponse(RelayInfo info)
		{
			if (
				info.ButtonID < 1
				|| (CityLoyaltySystem.Britain.Election != null && CityLoyaltySystem.Britain.Election.Ongoing)
			)
				return;
	...
	public class OpenInventoryGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				250,
				20,
				String.Format("<center>Inventory - {0}</center>", City.Definition.Name),
				false,
				false
			); // Inventory
				...
				if (id >= 0 && id < City.Stone.Boxes.Count)
				{
					...
					m.SendGump(
						new WarningGump(
							1111873,
							30720,
							"This will permanently delete the ballot box. Proceed?",
							32512,
							300,
							200,
							(mob, ok, obj) =>
	...
	public class CityMessageBoardGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				1154645,
				"#1154911",
				Server.Engines.Quests.BaseQuestGump.C32216(_Red),
				false,
				false
			);
			...
			for (int i = 0; i < CityLoyaltySystem.Cities.Count; i++)
			{
				...
				AddHtml(
					...
					150,
					20,
					String.Format("<basefont color=#8B0000>{0}", city.Definition.Name),
					false,
					false
				);
		...
		public override void AddGumpLayout()
		{
			...
			if (City.PostedOn != DateTime.MinValue)
				AddHtmlLocalized(
					...
					String.Format(
						"{0}\t{1}",
						City.Governor != null ? City.Governor.Name : "somebody",
						City.PostedOn.ToShortDateString()
					),
					0,
					false,
					false
				); // Posted by ~1_NAME~ on ~2_date~
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < CityLoyaltySystem.Cities.Count; i++)
			{
				...
				AddHtml(
					...
					100,
					20,
					city.Treasury.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US")),
					false,
					false
				);
	...
	public class CityInfoGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				400,
				20,
				String.Format("<center>City Info - {0}</center>", City.Definition.Name),
				false,
				false
			);
			...
			for (int i = 0; i < City.PlayerTable.Count; i++)
			{
				...
				AddHtml(
					...
					String.Format(
						"{0}{1}",
						entry.Player != null ? entry.Player.Name : "Unknown",
						entry.IsCitizen ? "(Citizen)" : ""
					),
					false,
					false
				);
				...
				AddHtml(
					...
					150,
					20,
					String.IsNullOrEmpty(entry.CustomTitle) ? "" : entry.CustomTitle,
					false,
					false
				);
