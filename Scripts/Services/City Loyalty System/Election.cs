...
namespace Server.Engines.CityLoyalty
{
	...
	public class CityElection
	{
		...
		public bool Ongoing
		{
			get { return CanNominate() || CanVote(); }
		}
		...
		public bool TryEndorse(PlayerMobile pm, PlayerMobile nominee)
		{
			...
			else
			{
				...
				for (int i = 0; i < a.Length; i++)
				{
					...
					BallotEntry ballot = Candidates.FirstOrDefault(entry =>
						entry.Endorsements.Contains(m as PlayerMobile)
					);
		...
		public void OnTick()
		{
			...
			if (CanNominate())
			{
				...
				{
					if (
						entry.TimeOfNomination + TimeSpan.FromHours(NominationDeadline) < DateTime.Now
						&& entry.Endorsements.Count == 0
					)
						Candidates.Remove(entry);
				});
			}
		...
		public static bool CheckConflict(DateTime[] times)
		{
			...
			for (int i = 0; i < times.Length; i++)
			{
				...
				for (int j = 0; j < times.Length; j++)
				{
					...
					if (
						(times[j] > t && times[j] - t < TimeSpan.FromDays(30))
						|| times[j] < t && t - times[j] < TimeSpan.FromDays(30)
					)
						return true;
				}
			}
		...
		public bool CanNominate(out DateTime until)
		{
			...
			foreach (DateTime dt in StartTimes)
			{
				if (
					dt.Year == DateTime.Now.Year
					&& DateTime.Now.Month == dt.Month
					&& DateTime.Now <= dt.AddDays(VotePeriod)
				)
				{
		...
		public bool CanVote(out DateTime until)
		{
			...
			foreach (DateTime dt in StartTimes)
			{
				if (
					...
					&& DateTime.Now.Month == dt.Month
					&& DateTime.Now > dt.AddDays(VotePeriod)
					&& DateTime.Now <= dt.AddDays(VotePeriod * 2)
				)
				{
