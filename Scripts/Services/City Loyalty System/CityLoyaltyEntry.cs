...
namespace Server.Engines.CityLoyalty
{
	...
	public class CityLoyaltyEntry : PointsEntry
	{
		...
		{
			get { return CityLoyaltySystem.GetCityInstance(City).GetLoyaltyRating(Player, this); }
		}
		...
		public bool UtilizingTradeDeal
		{
			...
			set
			{
				...
				if (!_Utilizing && value)
					TradeDealExpires =
						DateTime.UtcNow + TimeSpan.FromHours(CityLoyaltySystem.TradeDealUtilizationPeriod);
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool TradeDealExpired
		{
			get { return TradeDealExpires < DateTime.UtcNow; }
		}

		public CityLoyaltyEntry(PlayerMobile pm, City city)
			: base(pm)
		{
			City = city;
