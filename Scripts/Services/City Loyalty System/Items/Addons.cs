...
namespace Server.Items
{
	public abstract class InterchangeableAddonDeed : BaseAddonDeed
	{
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(InterchangeableAddonDeed deed)
				...
				if (Deed.IsChildOf(from.Backpack))
				{
					...
					switch (result)
					{
						...
						case AddonFitResult.Blocked:
							...
							break; // You cannot build that there.
						case AddonFitResult.NotInHouse:
							from.SendLocalizedMessage(500274);
							break; // You can only place this in a house that you own!
						case AddonFitResult.NoWall:
							from.SendLocalizedMessage(500268);
							break; // This object needs to be mounted on something.
					}
				}
				else
				{
	...
	public abstract class InterchangeableAddon : BaseAddon
	{
		...
		public InterchangeableAddon(Serial serial)
			: base(serial) { }
	...
	public class AddonInterchangeableGump : Gump
	{
		...
		public AddonInterchangeableGump(Mobile user, InterchangeableAddonDeed deed, IPoint3D p, Map map)
			: base(50, 50)
		{
			Deed = deed;
			Point = p;
