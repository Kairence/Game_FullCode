...
namespace Server.Engines.CityLoyalty
{
	public class CityStone : Item
	{
		...
		public CityStone(CityLoyaltySystem city)
			: base(0xED4)
		{
			City = city;
			Movable = false;
		...
		{
			if (
				CityLoyaltySystem.Enabled
				&& CityLoyaltySystem.IsSetup()
				&& from is PlayerMobile
				&& from.InRange(from.Location, 3)
			)
			{
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (City.GovernorElect != null)
				...
				list.Add(
					1154067,
					City.PendingGovernor ? "#1154102"
						: City.Governor != null ? City.Governor.Name
						: "#1154072"
				); // Governor: ~1_NAME~
			...
			list.Add(
				1154023,
				City.Treasury > 0
					? City.Treasury.ToString("N0", CultureInfo.GetCultureInfo("en-US"))
					: City.Treasury.ToString()
			); // City Treasury Balance: ~1_AMT~
			list.Add(
				1154059,
				String.Format("#{0}", City.ActiveTradeDeal == TradeDeal.None ? 1011051 : (int)City.ActiveTradeDeal - 12)
			); // Current Trade Deal: ~1_GUILD~
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
								...
								(mob, targeted) =>
								{
									if (targeted is IPoint3D)
									{
										...
										if (CheckLocation(m, box, p))
										{
											...
											m.SendMessage(
												"{0} of {1} ballot boxes placed.",
												Boxes.Count.ToString(),
												CityLoyaltySystem.MaxBallotBoxes.ToString()
											);
										}
										else
											box.Delete();
			...
			list.Add(
				new SimpleContextMenuEntry(
					...
					enabled: City.GovernorElect == from
						&& City.Governor == null
						&& City.GetLoyaltyRating(from) >= LoyaltyRating.Unknown
				)
			);
		}
