...
namespace Server.Engines.CityLoyalty
{
	public class TradeOrderCrate : Container
	{
		...
		{
			get { return CityTradeSystem.KrampusEncounterActive ? 1123594 : base.LabelNumber; }
		}
				...
				for (int i = 0; i < Entry.Details.Count; i++)
				{
					if (Utility.ToInt32(Entry.Details[i].Name) > 0)
						list.Add(
							...
							String.Format(
								"#{0}\t{1}\t{2}",
								Entry.Details[i].Name,
								GetAmount(Entry.Details[i].ItemType),
								Entry.Details[i].Amount
							)
						); // ~1_val~: ~2_val~/~3_val~
					else
						list.Add(
		...
		private class CancelOrderEntry : ContextMenuEntry
		{
			...
			public CancelOrderEntry(TradeOrderCrate crate, Mobile player)
				: base(1151727, 3) // cancel trade order
			{
				Crate = crate;
				Player = player;
			}
		...
		public TradeOrderCrate(Serial serial)
			: base(serial) { }
