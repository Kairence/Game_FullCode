...
namespace Server.Engines.CityLoyalty
{
	...
	public class CityTradeSystem : PointsSystem
	{
		...
		{
			get { return new TextDefinition("City Trading"); }
		}
		public override PointsType Loyalty
		{
			get { return PointsType.CityTrading; }
		}
		...
		public static bool KrampusEncounterActive
		{
			get { return KrampusEncounter.Enabled && KrampusEncounter.Encounter != null; }
		}
		...
		public bool TryOfferTrade(Mobile from, TradeMinister minister)
		{
			...
			if (ActiveTrades.ContainsKey(from))
			{
				...
				KrampusEncounterActive
				&& (KrampusEncounter.Encounter.Krampus != null || KrampusEncounter.Encounter.KrampusSpawning)
			)
			{
				...
				minister.SayTo(
					from,
					1158790,
					String.Format("{0}\t{1}", WorldLocationInfo.GetLocationString(p, map), Sextant.GetCoords(p, map)),
					1150
				);
		...
		public bool TryTurnIn(Mobile from, TradeOrderCrate order, Mobile turninMobile)
		{
			...
			if (from.AccessLevel == AccessLevel.Player && minister != null && minister.City != entry.Destination)
				turninMobile.SayTo(
					from,
					1151738,
					String.Format("#{0}", CityLoyaltySystem.GetCityLocalization(entry.Destination))
				); // Begging thy pardon, but those goods are destined for the City of ~1_city~
		...
		public void CheckTitle(CityTradeEntry entry)
		{
			...
			{
				case 1:
					entry.Player.AddRewardTitle((int)TradeTitle.Trader);
					break;
				case 25:
					entry.Player.AddRewardTitle((int)TradeTitle.Exporter);
					break;
				case 50:
					entry.Player.AddRewardTitle((int)TradeTitle.Broker);
					break;
				case 100:
					entry.Player.AddRewardTitle((int)TradeTitle.Tycoon);
					break;
				case 150:
					entry.Player.AddRewardTitle((int)TradeTitle.Magnate);
					break;
			}
		...
		public static void CancelTradeOrder(Mobile from, TradeOrderCrate crate)
		{
			...
			if (from != null)
			{
				...
				CityTradeEntry entry = CityLoyaltySystem.CityTrading.GetPlayerEntry<CityTradeEntry>(
		...
		public static Type GetRandomTrade(City originCity, City dest, ref int worth, ref string name)
		{
			...
			List<BaseVendor> list = new List<BaseVendor>(
				region
					.GetEnumeratedMobiles()
					.OfType<BaseVendor>()
					.Where(bv => bv.GetBuyInfo() != null && bv.GetBuyInfo().Length > 0)
			);
			...
			do
			{
				...
				if (
					!(info is BeverageBuyInfo)
					&& !(info is AnimalBuyInfo)
					&& info != null
					&& info.Type != null
					&& info.Args == null
					&& info.Price < 5000
		...
		public static void CheckAmbush(TradeOrderCrate crate)
		{
			if (
				...
				|| crate.Entry == null
				|| crate.Expired
				|| crate.Entry.LastAmbush + TimeSpan.FromMinutes(AmbushWaitDuration) > DateTime.UtcNow
			)
				return;
		...
		private static Type[] _LandTypes = { typeof(Troll), typeof(Ettin), typeof(GiantSpider), typeof(Brigand) };
		...
		public class CityTradeEntry : PointsEntry
		{
			...
			public CityTradeEntry(PlayerMobile pm)
				: base(pm) { }
