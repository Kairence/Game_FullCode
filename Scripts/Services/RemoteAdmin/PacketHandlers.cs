...
namespace Server.RemoteAdmin
{
	public class RemoteAdminHandlers
	{
		...
		private static void AccountSearch(NetState state, PacketReader pvSrc)
		{
			...
			{
				state.Send(
					new MessageBoxMessage("There were no results to your search.\nPlease try again.", "No Matches")
				);
			}
		}
		...
		private static void RemoveAccount(NetState state, PacketReader pvSrc)
		{
			...
			{
				state.Send(
					new MessageBoxMessage("You do not have permission to delete accounts.", "Account Access Exception")
				);
				return;
			}
		...
		private static void UpdateAccount(NetState state, PacketReader pvSrc)
		{
			...
			{
				state.Send(
					new MessageBoxMessage("You do not have permission to edit accounts.", "Account Access Exception")
				);
				return;
			}
			...
			else
			{
				...
				if (CreatedAccount)
					RemoteAdminLogging.WriteLine(
						state,
						"Created account {0} with Access Level {1}",
						a.Username,
						a.AccessLevel
					);
				else
				{
					...
					if (oldAcessLevel != a.AccessLevel)
						changes = string.Format(
							"{0} Access level changed from {1} to {2}.",
							changes,
							oldAcessLevel,
							a.AccessLevel
						);
