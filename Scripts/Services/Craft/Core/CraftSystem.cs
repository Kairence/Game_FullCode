...
namespace Server.Engines.Craft
{
	...
	public abstract class CraftSystem
	{
		...
		{
			get { return m_CanAlter; }
			set { m_CanAlter = value; }
		}
		...
		{
			typeof(CapturedEssence),
			typeof(EyeOfTheTravesty),
			typeof(DiseasedBark),
			typeof(LardOfParoxysmus),
			typeof(GrizzledBones),
			typeof(DreadHornMane),
			typeof(Blight),
			typeof(Corruption),
			typeof(Muculent),
			typeof(Scourge),
			...
			// Tinkering
			typeof(Server.Factions.Silver),
			typeof(RingOfTheElements),
			typeof(HatOfTheMagi),
			typeof(AutomatonActuator),
			// Inscription
		...
		public virtual bool ConsumeOnFailure(
			Mobile from,
			Type resourceType,
			CraftItem craftItem,
			ref MasterCraftsmanTalisman talisman
		)
		{
		...
		)
		{
			...
			int hunger = 50;
			if (
				skillToMake is SkillName.Blacksmith
				|| skillToMake is SkillName.Carpentry
				|| skillToMake is SkillName.Tailoring
				|| skillToMake is SkillName.Fletching
			)
				hunger *= 2;
		...
		public void CraftSkillCheck(Mobile from, Type typeItem, SkillName skillToMake)
		{
			int point = 0;
		}
