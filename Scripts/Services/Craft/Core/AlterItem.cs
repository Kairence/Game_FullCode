	...
	public class AlterItemTarget : Target
	{
			...
			else
			{
				...
				else if (Server.Engines.VendorSearching.VendorSearch.StringList != null)
				{
					if (origItem.LabelNumber > 0 && RetainsName(origItem))
						newitem.Name = Server.Engines.VendorSearching.VendorSearch.StringList.GetString(
							origItem.LabelNumber
						);
				}
				...
				EventSink.InvokeAlterItem(
					new AlterItemEventArgs(from, m_Tool is Item ? (Item)m_Tool : m_Contract, origItem, newitem)
				);
		...
		private bool RetainsName(Item item)
		{
			...
			return (item.LabelNumber >= 1073505 && item.LabelNumber <= 1073552)
				|| (item.LabelNumber >= 1073111 && item.LabelNumber <= 1075040);
		}
