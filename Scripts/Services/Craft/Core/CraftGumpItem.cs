...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
			...
			: base(40, 40)
		{
			...
			bool needsRecipe = (
				craftItem.Recipe != null && from is PlayerMobile && !((PlayerMobile)from).HasRecipe(craftItem.Recipe)
			);
			...
			* */
			...
			{
				bool supportsEx = (
					from.NetState != null && from.NetState.SupportsExpansion(craftItem.RequiredExpansion)
				);
			...
			if (craftItem.RequiredThemePack != ThemePack.None)
			{
				TextDefinition.AddHtmlText(
					...
					310,
					18,
					RequiredThemePackMessage(craftItem.RequiredThemePack),
					false,
					false,
					LabelColor,
					LabelHue
		...
		public void DrawSkill()
		{
			for (int i = 0; i < m_CraftItem.Skills.Count; i++)
			{
				...
				AddHtmlLocalized(
					...
					200,
					18,
					AosSkillBonuses.GetLabel(skill.SkillToMake),
					LabelColor,
					false,
					false
				);
		...
		public void DrawResource()
		{
			...
			if (m_CraftItem.NameNumber == 1041267) // runebook
			{
				AddHtmlLocalized(
					170,
					219 + (m_CraftItem.Resources.Count * 20),
					310,
					18,
					1044447,
					LabelColor,
			...
			switch (info.ButtonID)
			{
				...
				case 3: //Make Max
					...
					new AutoCraftTimer(
						...
						m_Tool,
						9999,
						TimeSpan.FromSeconds(m_CraftSystem.Delay * m_CraftSystem.MaxCraftEffect + 1.0),
						TimeSpan.FromSeconds(m_CraftSystem.Delay * m_CraftSystem.MaxCraftEffect + 1.0)
					);
					break;
			}
