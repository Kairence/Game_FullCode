	...
	public class Repair
	{
		...
		private class InternalTarget : Target
		{
			...
			private int GetWeakenChance(Mobile mob, SkillName skill, int curHits, int maxHits)
			{
				...
				{
					value = Misc.Util.RepairSkillCheck(m_Addon.Tools.Find(x => x.System == m_CraftSystem).SkillValue);
				}
				else
				{
					value = Misc.Util.RepairSkillCheck(mob.Skills[skill].Value);
				}

				// 40% - (1% per hp lost) - (1% per 10 craft skill)
				//Tier 1당 수리 내구 5배수로 고침. 무조건 내구 1은 하락 됨

				int repairlevel = maxHits - curHits;

				return 1 + (maxHits - curHits) / ((value + 1) * 5);
			}
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (m_Addon != null)
				{
					...
					value = Misc.Util.RepairSkillCheck(tool.SkillValue);
				}
				else
				{
					value = Misc.Util.RepairSkillCheck(from.Skills[m_CraftSystem.MainSkill].Value);
				}
				...
				else if (targeted is Item)
				{
					...
					if (from.InRange(((Item)targeted).GetWorldLocation(), 2))
					{
						...
						else if (targeted is BaseWeapon)
						{
							...
							int toWeaken = GetWeakenChance(from, skill, weapon.HitPoints, weapon.MaxHitPoints);
						...
						else if (targeted is BaseArmor)
						{
							...
							int toWeaken = GetWeakenChance(from, skill, armor.HitPoints, armor.MaxHitPoints);
						...
						else if (targeted is BaseJewel)
						{
							...
							int toWeaken = GetWeakenChance(from, skill, jewel.HitPoints, jewel.MaxHitPoints);

							if (
								m_CraftSystem.CraftItems.SearchForSubclass(jewel.GetType()) == null
								&& !CheckSpecial(jewel)
							)
							{
								number = (usingDeed) ? 1061136 : 1044277; // That item cannot be repaired. // You cannot repair that item with this type of repair contract.
						...
						else if (targeted is BaseClothing)
						{
							...
							int toWeaken = GetWeakenChance(from, skill, clothing.HitPoints, clothing.MaxHitPoints);

							if (
								m_CraftSystem.CraftItems.SearchForSubclass(clothing.GetType()) == null
								&& !CheckSpecial(clothing)
							)
							{
								number = (usingDeed) ? 1061136 : 1044277; // That item cannot be repaired. // You cannot repair that item with this type of repair contract.
							}
						...
						else if (targeted is BaseTalisman)
						{
							...
							int toWeaken = GetWeakenChance(from, skill, talisman.HitPoints, talisman.MaxHitPoints);
						...
						{
							Spellbook spellbook = (Spellbook)targeted;
							int toWeaken = GetWeakenChance(from, skill, spellbook.HitPoints, spellbook.MaxHitPoints);

							if (!(m_CraftSystem is DefTinkering))
							{
								number = (usingDeed) ? 1061136 : 1044277; // That item cannot be repaired. // You cannot repair that item with this type of repair contract.
							}
							else if (!spellbook.IsChildOf(from.Backpack) && (!Core.ML || spellbook.Parent != from))
							{
								number = 1044275; // The item must be in your backpack to repair it.
							}
							...
							{
								spellbook.MaxHitPoints--;
								toDelete = true;
								number = 1044278; // That item has been repaired many times, and will break if repairs are attempted again.
							}
							else if (!spellbook.CanRepair) // quick fix
							{
								number = 1044277; // That item cannot be repaired.
							}
							else
						...
						{
							BaseAddon ba = ((AddonComponent)targeted).Addon as BaseAddon;

							int toWeaken = GetWeakenChance(from, skill, ba.Hits, ba.MaxHits);

							if (ba.MaxHits <= 0 || ba.Hits == ba.MaxHits)
							{
								number = 1044281; // That item is in full repair
							}
							else if (ba.MaxHits <= toWeaken)
							{
								ba.MaxHits--;
								toDelete = true;
								number = 1044278; // That item has been repaired many times, and will break if repairs are attempted again.
							...
							{
								ba.MaxHits -= toWeaken;
								ba.Hits = Math.Max(0, ba.Hits - toWeaken);

								number = 1044279; // You repair the item.
								m_CraftSystem.PlayCraftEffect(from);
								ba.Hits = ba.MaxHits;

								if (ba.MaxHits > 0 && ba.Hits >= 0)
								{
									from.SendMessage(
										"{0} 포인트가 부숴졌습니다. 현재 내구도는 {1}/{2} 입니다.",
										toWeaken,
										ba.Hits,
										ba.MaxHits
									);
						...
						else if (targeted is BlankScroll)
						{
							if (!usingDeed)
							{
								...
								{
									BlankScroll bs = targeted as BlankScroll;
									...
									{
										deed.Amount = bs.Amount;
										bs.Consume(bs.Amount);
									}
			...
			public bool TryRepairMobile(Mobile from, IRepairableMobile m, bool usingDeed, out bool toDelete)
			{
				...
				else
				{
					...
					else if (!from.CanBeginAction(typeof(IRepairableMobile)))
					{
						...
						&& bc.GetMaster() != null
						&& bc.GetMaster() != from
						&& !bc.GetMaster().InRange(from.Location, 10)
					)
					{
					...
					else
					{
						...
						if (!from.CheckSkill(SkillName.Tinkering, 0.0, 250.0))
							damage /= 6;
						...
						if (pack != null)
						{
							...
							if (v > 0)
							{
								...
								Timer.DelayCall(
									...
									new TimerStateCallback(EndMobileRepair),
									from
								);
		...
		public static bool AllowsRepair(object targeted, CraftSystem system)
		{
			...
			if (targeted is BaseAddon || targeted is AddonComponent)
				return true;

			return (
				targeted is BlankScroll
				...
				|| (targeted is Spellbook && ((Spellbook)targeted).CanRepair)
