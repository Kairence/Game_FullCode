...
namespace Server.Items
{
	...
	public class CompassionPigment : Item, IUsesRemaining
	{
		...
		{
			get { return CompassionPigmentInfo.GetInfo(m_Type).LabelNumber; }
		}
		...
		private class DyeTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (!item.IsChildOf(from.Backpack))
				{
					...
					item is MetalPigmentsOfTokuno
					|| item is LesserPigmentsOfTokuno
					|| item is PigmentsOfTokuno
					|| item is CompassionPigment
				)
				{
