...
namespace Server.Engines.XmlSpawner2
{
	public class XmlEditDialogGump : Gump
	{
					...
					default:
					{
						...
						{ // accept
							...
							state.Mobile.SendGump(
								new XmlEditDialogGump(state.Mobile, true, xd, -1, 0, null, false, null, 0, 0)
							);
						}
						break;
					}
		...
		public static void ProcessXmlEditBookEntry(Mobile from, object[] args, string text)
		{
			...
			from.SendGump(
				new XmlEditDialogGump(from, true, dialog, selected, displayfrom, savefile, false, null, 0, 0)
			);
		}
					...
					default:
					{
						...
						{ // accept
							...
							for (int i = 0; i < SearchList.Count; i++)
							{
								int index = i - DisplayFrom;
								if (
									(index >= 0 && index < SelectedList.Length && SelectedList[index] == true)
									|| selectAll
								)
								{
