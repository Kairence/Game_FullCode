	...
	public class XmlSpawnerGump : Gump
	{
		...
		*/
			...
			AddButton(
				280,
				395,
				m_Spawner.DisableGlobalAutoReset ? 0xD3 : 0xD2,
				m_Spawner.DisableGlobalAutoReset ? 0xD2 : 0xD3,
				9997,
					...
					if (m_ShowGump > 1)
					{
						...
						if (m_Spawner.SpawnObjects[i].NextSpawn > DateTime.UtcNow)
						{
							...
							else
							{
								...
								strnext = (
									m_Spawner.SpawnObjects[i].NextSpawn - DateTime.UtcNow + m_Spawner.NextSpawn
								).ToString();
							}
						}
						else
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 200: // gump extension
				{
					if (this.m_ShowGump > 1)
						state.Mobile.SendGump(
							new XmlSpawnerGump(m_Spawner, this.X, this.Y, -1, this.xoffset, this.page)
						);
					else
						state.Mobile.SendGump(
							new XmlSpawnerGump(m_Spawner, this.X, this.Y, this.m_ShowGump + 2, this.xoffset, this.page)
						);
					return;
				}
				case 201: // gump text extension
				{
					if (this.xoffset > 0)
						state.Mobile.SendGump(
							new XmlSpawnerGump(m_Spawner, this.X, this.Y, this.m_ShowGump, 0, this.page)
						);
					else
						state.Mobile.SendGump(
							new XmlSpawnerGump(m_Spawner, this.X, this.Y, this.m_ShowGump, 250, this.page)
						);
					return;
				}
				...
				{
					state.Mobile.SendGump(
						new XmlSpawnerGump(m_Spawner, this.X, this.Y, this.m_ShowGump, this.xoffset, this.page)
					);
					return;
				}
				case 9999:
				...
				default:
				{
					...
					else if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxSpawnEntries)
					{
						...
						else
						{
							...
							state.Mobile.SendGump(
								new XmlPartialCategorizedAddGump(state.Mobile, categorystring, 0, types, true, i, newg)
							);
			...
			//m_Spawner.OnDoubleClick( state.Mobile);
			state.Mobile.SendGump(
				new XmlSpawnerGump(m_Spawner, this.X, this.Y, this.m_ShowGump, this.xoffset, this.page, this.Rentry)
			);
		}
	}
}
