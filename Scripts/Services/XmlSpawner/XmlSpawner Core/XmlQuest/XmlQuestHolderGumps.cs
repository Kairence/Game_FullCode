			...
			: base(X, Y)
		{
			...
			else
			{
				...
				if ((questitem.RewardItem != null && !questitem.RewardItem.Deleted))
				{
					...
					else if (questitem.RewardItem is Container)
					{
						...
						AddLabel(
							...
							373,
							55,
							String.Format("Weight: {0}", questitem.RewardItem.TotalWeight + questitem.RewardItem.Weight)
						);
					}
					else
					{
				...
				else if (questitem.AlreadyDone)
				{
					...
					{
						List<XmlAttachment> a = XmlAttach.FindAttachments(
							questitem.Owner,
			...
			switch (info.ButtonID)
			{
				case 700:
					...
					state.Mobile.SendGump(
						new XmlQuestStatusGump(m_questitem, m_gumptitle, m_X, m_Y, m_solid, m_screen)
					);
					break;
				case 800:
					if (!m_questitem.CanSeeReward)
						return;
