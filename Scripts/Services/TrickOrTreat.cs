...
namespace Server.Engines.Events
{
	public class TrickOrTreat
	{
		...
		{
			return (
				mobile != null && mobile.Map != null && !mobile.Deleted && mobile.Alive && mobile.Map != Map.Internal
			);
		}
							...
							else
							{
								...
								if (m_Action == 0)
								{
									Timer.DelayCall<Mobile>(
										...
										OneSecond,
										10,
										new TimerStateCallback<Mobile>(Bleeding),
										from
									);
								}
								else if (m_Action == 1)
								{
									Timer.DelayCall<Mobile>(
										...
										new TimerStateCallback<Mobile>(SolidHueMobile),
										from
									);
								}
								else
								{
									Timer.DelayCall<Mobile>(
										TimeSpan.FromSeconds(2),
										new TimerStateCallback<Mobile>(MakeTwin),
										from
									);
								}
							}
	...
	public class NaughtyTwin : BaseCreature
	{
			...
			: base(AIType.AI_Melee, FightMode.None, 10, 1, 0.2, 0.4)
		{
			if (TrickOrTreat.CheckMobile(from))
			{
				...
				Timer.DelayCall<Mobile>(
					TrickOrTreat.OneSecond,
					Utility.RandomBool()
						? new TimerStateCallback<Mobile>(StealCandy)
						: new TimerStateCallback<Mobile>(ToGate),
					this.m_From
				);
			}
		}
