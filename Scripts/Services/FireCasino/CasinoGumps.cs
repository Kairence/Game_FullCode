...
namespace Server.Engines.ResortAndCasino
{
				...
				case Section.None:
					...
					if (Bought > 0 || CashedOut > 0)
					{
						...
						AddHtmlLocalized(
							...
							200,
							16,
							Bought > 0 ? 1153189 : 1153190,
							amount.ToString("N0", CultureInfo.GetCultureInfo("en-US")),
							Yellow,
							false,
							false
						); // You purchased ~1_AMT~ chips
						y += 30;
					}
					...
					AddHtml(
						...
						150,
						16,
						Color("#FF69B4", total.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
						false,
						false
					);
					y += 20;
					...
					AddHtml(
						...
						150,
						16,
						Color("#FF69B4", chips.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
						false,
						false
					);
					y += 30;
						...
						if (num2 > 0)
						{
							if (num2 <= (int)PointsSystem.CasinoData.GetPoints(User))
							{
								...
								User.SendLocalizedMessage(
									1060397,
									(num2 * CasinoData.ChipCost).ToString(CultureInfo.GetCultureInfo("en-US"))
	...
	public class ChucklesLuckGump : BaseCasinoGump
	{
		...
		public ChucklesLuck Game
		{
			get { return DiceGame as ChucklesLuck; }
		}
		...
		public override void BuildBetting()
		{
			...
			AddHtmlLocalized(
				...
				16,
				1153370,
				chips.ToString(CultureInfo.GetCultureInfo("en-US")),
				Yellow,
				false,
				false
			); // You have ~1_VAL~ chips
		...
		public override void BuildRolling()
		{
			...
			AddHtml(
				...
				150,
				16,
				Color("#FFFF00", Game.CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
		...
		public override void BuildResults()
		{
			...
			if (!Game.Winner)
				AddHtmlLocalized(
					...
					32,
					1153385,
					Game.CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US")),
					Yellow,
					false,
					false
				); // The dice did not match your number. You lose your bet of ~1_AMT~.
			else
			{
				...
				AddHtmlLocalized(
					...
					32,
					1153384,
					String.Format("{0}\t{1}", matches.ToString(), win.ToString(CultureInfo.GetCultureInfo("en-US"))),
					Yellow,
					false,
					false
				); // The dice matched your number ~1_COUNT~ times. You win ~2_AMT~ chips!
			}
	...
	public class HiMiddleLowGump : BaseCasinoGump
	{
		...
		public HiMiddleLow Game
		{
			get { return DiceGame as HiMiddleLow; }
		}
		...
		public override void BuildResults()
		{
			...
			AddHtmlLocalized(
				...
				16,
				1154645,
				Game.WinsHi(total) ? "#1153400" : "#1153399",
				Yellow,
				false,
				false
			); // Hi wins/lose
			AddHtmlLocalized(
				...
				16,
				1154645,
				Game.WinsLow(total) ? "#1153404" : "#1153403",
				Yellow,
				false,
				false
			); // Low wins/lose
			AddHtmlLocalized(
				...
				16,
				1154645,
				Game.WinsMiddle(total) ? "#1153402" : "#1153401",
				Yellow,
				false,
				false
			); // middle wins/lose
			AddHtmlLocalized(
				...
				16,
				1154645,
				Game.WinsOutside(total) ? "#1153406" : "#1153405",
				Yellow,
				false,
				false
			); // outside wins/lose
			...
			if (!Game.Winner)
			{
				...
				AddHtmlLocalized(
					...
					false,
					false
				); // You lost ~1_AMT~ chips. Better luck next time!
			}
			else
			{
				...
				AddHtmlLocalized(
					...
					16,
					1153414,
					winnings.ToString(CultureInfo.GetCultureInfo("en-US")),
					Yellow,
					false,
					false
				); // You won ~1_AMT~ chips!
	...
	public class DiceRiderGump : BaseCasinoGump
	{
		...
		public DiceRider Game
		{
			get { return DiceGame as DiceRider; }
		}
		...
		public override void BuildRolling()
		{
			...
			AddHtml(
				...
				100,
				16,
				Color("#FFFF00", Game.Bet1.ToString(CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
			AddHtml(
				...
				100,
				16,
				Color("#FFFF00", Game.Bet2.ToString(CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
			AddHtml(
				...
				100,
				16,
				Color("#FFFF00", Game.Bet3.ToString(CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
			...
			else
			{
				...
				if (!Game.Winner)
				{
					AddHtmlLocalized(
						...
						16,
						1153639,
						totalbet.ToString(CultureInfo.GetCultureInfo("en-US")),
						Yellow,
						false,
						false
					); // You did not make a winning hand. You lost ~1_AMT~.
				}
				else
				{
					AddHtmlLocalized(
						...
						16,
						1153640,
						String.Format("{0}\t#{1}", Game.WinningTotal, WinningHand()),
						Yellow,
						false,
						false
					); // You won ~1_AMT~ for making a ~2_HAND_NAME~!
				}
