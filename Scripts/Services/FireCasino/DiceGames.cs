...
namespace Server.Engines.ResortAndCasino
{
	...
	{
		public ChucklesLuck(PlayerMobile player, CasinoDealer dealer)
			: base(player, dealer) { }
		...
		public override void BeginRollDice()
		{
			...
			Player.PrivateOverheadMessage(
				...
				0x35,
				1153375,
				String.Format("{0}\t{1}", CurrentBet.ToString(), BettingOn.ToString()),
				Player.NetState
			); // *bets ~1_AMT~ chips on ~2_PROP~*
		}
		...
		public override void OnDiceRolled()
		{
			...
			Dealer.PrivateOverheadMessage(
				MessageType.Regular,
				0x35,
				1153391,
				String.Format("{0}\t{1}\t{2}", Roll[0], Roll[1], Roll[2]),
				Player.NetState
			); // *rolls the dice; they land on ~1_FIRST~ ~2_SECOND~ ~3_THIRD~*
			...
			if (matches == 0)
			{
				Dealer.PrivateOverheadMessage(
					...
					0x35,
					1153376,
					String.Format("{0}\t{1}", Player.Name, CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US"))),
					Player.NetState
				); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
			}
			else
			{
				...
				Dealer.PrivateOverheadMessage(
					...
					0x35,
					1153377,
					String.Format("{0}\t{1}", Player.Name, winnings.ToString(CultureInfo.GetCultureInfo("en-US"))),
					Player.NetState
				); // *pays out ~2_VAL~ chips to ~1_NAME~*
			}
		}
	...
	public class HiMiddleLow : BaseDiceGame
	{
		...
		{
			get { return (HighMiddleLowType)BettingOn; }
		}
		...
		public override void BeginRollDice()
		{
			...
			Player.PrivateOverheadMessage(
				...
				0x35,
				1153375,
				String.Format("{0}\t{1}", CurrentBet.ToString(), ((HighMiddleLowType)BettingOn).ToString()),
				Player.NetState
			); // *bets ~1_AMT~ chips on ~2_PROP~*
		}
				...
				else
				{
					Dealer.PrivateOverheadMessage(
						...
						String.Format(
							"{0}\t{1}",
							Player.Name,
							CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US"))
						),
						Player.NetState
					); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
				}
		...
		public override void BeginRollDice()
		{
			...
			if (RollNumber == 1)
				Player.PrivateOverheadMessage(
					...
					0x35,
					1153631,
					(CurrentBet / 3).ToString(CultureInfo.GetCultureInfo("en-US")),
					Player.NetState
				); // *bets ~1_AMT~ chips on ~2_PROP~*
		}
		...
		public override void OnDiceRolled()
		{
			if (RollNumber == 1)
			{
				...
				Dealer.PrivateOverheadMessage(
					...
					0x35,
					1153391,
					String.Format("{0}\t{1}\t{2}", GetRoll(0), GetRoll(1), GetRoll(2)),
					Player.NetState
				); // *rolls the dice; they land on ~1_FIRST~ ~2_SECOND~ ~3_THIRD~*
			}
			else
			{
				...
				Dealer.PrivateOverheadMessage(
					MessageType.Regular,
					0x35,
					RollNumber == 2 ? 1153633 : 1153634,
					RollNumber == 2 ? GetRoll(3).ToString() : GetRoll(4).ToString(),
					Player.NetState
				); // *rolls the fourth die: ~1_DIE4~*
			}
			...
			if (RollNumber == 3)
			{
				...
				else
				{
					Dealer.PrivateOverheadMessage(
						...
						0x35,
						1153376,
						String.Format("{0}\t{1}", Player.Name, TotalBet.ToString(CultureInfo.GetCultureInfo("en-US"))),
						Player.NetState
					); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
				}
			}
