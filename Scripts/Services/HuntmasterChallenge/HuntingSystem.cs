...
namespace Server.Engines.HuntsmasterChallenge
{
	public class HuntingSystem : Item
	{
		...
		public static HuntingSystem Instance
		{
		...
		public DateTime SeasonBegins
		{
			get { return m_SeasonBegins; }
			set { m_SeasonBegins = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime SeasonEnds
		{
			get { return m_SeasonEnds; }
			set { m_SeasonEnds = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int BonusIndex
		{
			get { return m_BonusIndex; }
		}
		...
		private void CheckHint(Map map)
		{
			...
			foreach (Mobile m in eable)
			{
				if (m is BaseVendor)
				{
					...
					{
						if (
							player is PlayerMobile
							&& ((PlayerMobile)player).NpcGuild == NpcGuild.RangersGuild
							&& !player.Hidden
						)
						{
							GiveHint(player, m);
		...
		private void GiveHint(Mobile player, Mobile vendor)
		{
			vendor.SayTo(
				player,
				"A good place to hunt these days is {0}.",
				WorldLocationInfo.Locations[0][m_BonusIndex].RegionName
			);
		...
		{
			if (
				permit.KillEntry == null
				|| permit.KillEntry.KillIndex < 0
				|| permit.KillEntry.KillIndex > HuntingTrophyInfo.Infos.Count
			)
			...
			else
			{
				...
				if (info != null)
				{
					...
					if (leaders.Count == 0 || permit.KillEntry.Measurement >= leaders[0].Measurement)
					{
						...
						leaders.Add(
							new HuntingKillEntry(
								permit.Owner,
								permit.KillEntry.Measurement,
								permit.KillEntry.DateKilled,
								permit.KillEntry.KillIndex,
								permit.KillEntry.Location
							)
						);
		...
		public HuntingSystem(Serial serial)
			: base(serial) { }

		private Dictionary<HuntType, List<HuntingKillEntry>> m_Leaders;
		public Dictionary<HuntType, List<HuntingKillEntry>> Leaders
		{
			get { return m_Leaders; }
		}

		private Dictionary<Mobile, int> m_UnclaimedWinners = new Dictionary<Mobile, int>();
		public Dictionary<Mobile, int> UnclaimedWinners
		{
			get { return m_UnclaimedWinners; }
		}

		private Dictionary<HuntType, List<HuntingKillEntry>> m_Top10;
		public Dictionary<HuntType, List<HuntingKillEntry>> Top10
		{
			get { return m_Top10; }
		}
