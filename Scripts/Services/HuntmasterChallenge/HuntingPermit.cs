...
namespace Server.Items
{
	public class HuntingPermit : Item
	{
		...
		public static List<HuntingPermit> Permits
		{
			get { return m_Permits; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool ProducedTrophy
		{
			get { return m_ProducedTrophy; }
			set { m_ProducedTrophy = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public HuntingKillEntry KillEntry
		{
			get { return m_KillEntry; }
			set { m_KillEntry = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public bool HasDocumentedKill
		{
			get { return m_KillEntry != null; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public bool CanUseTaxidermyOn
		{
			get
			{
				return HasDocumentedKill && m_KillEntry.KillIndex < HuntingTrophyInfo.Infos.Count && !m_ProducedTrophy;
			}
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (
				m_KillEntry == null
				|| m_KillEntry.KillIndex < 0
				|| m_KillEntry.KillIndex >= HuntingTrophyInfo.Infos.Count
			)
				return;
					...
					else
					{
						...
						for (int i = 0; i < HuntingTrophyInfo.Infos.Count; i++)
						{
							...
							if (t == info.CreatureType)
							{
								...
								if (
									HuntingSystem.Instance != null
									&& HuntingSystem.Instance.IsPrimeHunt(from, c.Location)
								)
								{
								...
								int measurement =
									info.MinMeasurement
									+ (int)(
										(double)(info.MaxMeasurement - info.MinMeasurement)
										* (double)((double)v / 100.0)
									);
								m_Permit.KillEntry = new HuntingKillEntry(
									m_Permit.Owner,
									measurement,
									DateTime.Now,
									i,
									WorldLocationInfo.GetLocationString(c.Location, c.Map)
								);
								...
								from.PrivateOverheadMessage(
									...
									1155713,
									from.NetState
								); // *You document your kill on the permit*
