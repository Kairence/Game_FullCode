	...
	public class Imbuing
	{
		public static void Initialize()
		{
			...
			CommandSystem.Register(
				...
				new CommandEventHandler(GetTotalWeight_OnCommand)
			);
			CommandSystem.Register(
				"GetTotalMods",
				AccessLevel.GameMaster,
				new CommandEventHandler(GetTotalMods_OnCommand)
			);
		}
		...
		public static Dictionary<Mobile, ImbuingContext> ContextTable
		{
			get { return m_ContextTable; }
		}
		...
		public static TimeSpan OnUse(Mobile from)
		{
			...
			else if (from is PlayerMobile)
			{
				...
				if( )
				{
					from.Target = new InternalTargetImbuing();

					//from.SendLocalizedMessage( 500343 ); // What do you wish to appraise and identify?
					from.SendMessage( "임뷰잉 보석 혹은 추출할 아이템(악세사리와 마법책)을 선택하십시오." );
					from.CloseGump(typeof(ImbuingGump));
					BaseGump.SendGump(new ImbuingGump((PlayerMobile)from));
					from.BeginAction(typeof(Imbuing));
				}
				else
					from.SendMessage( "임뷰잉을 하기 위해서는 아케인 잼이 필요합니다!" );
				*/
			}
		...
		private class InternalTargetImbuing : Target
		{
			public InternalTargetImbuing()
				: base(2, false, TargetFlags.None) { }

			private bool DiceCheck(double skill, double item)
			{
				return false;
				//if( skill >= item * 15 )
			}
			...
			{
				if ( targeted is ImbuingJewel )
				{
					from.SendMessage( "어느 장비에 임뷰잉을 하시겠습니까?" ); // To what do you wish to apply the poison?
					from.Target = new InternalTarget( (ImbuingJewel)targeted );
				}
				else if ( targeted is BaseWeapon && !((BaseWeapon)targeted).IsImbued )
				{
					BaseWeapon imbuitem = targeted as imbuitem;
					double chance = from.Skills.Imbuing.Value;
					if( DiceCheck(chance, (int)imbeitem.ItemPower) )
					{
						if( chance >= 100 )
							chance += 30;
					}
		...
		{
			return OnBeforeImbue(
				from,
				item,
				id,
				value,
				GetTotalMods(item, id),
				GetMaxProps(item),
				GetTotalWeight(item, id, false, true),
				Imbuing.GetMaxWeight(item)
			);
		}

		public static bool OnBeforeImbue(
			Mobile from,
			Item item,
			int id,
			int value,
		...
		private static Type[] _SpecialImbuable =
		{
			...
			typeof(VirtuososCap),
			typeof(VirtuososCollar),
			typeof(VirtuososEarpieces),
			typeof(VirtuososKidGloves),
			typeof(VirtuososKilt),
			typeof(VirtuososNecklace),
			typeof(VirtuososTunic),
			typeof(BestialArms),
			typeof(BestialEarrings),
			typeof(BestialGloves),
			typeof(BestialGorget),
			typeof(BestialHelm),
			typeof(BestialKilt),
			typeof(BestialLegs),
			typeof(BestialNecklace),
			typeof(BarbedWhip),
			typeof(BladedWhip),
			typeof(SpikedWhip),
		};

		private static Type[] _NonCraftables = { typeof(SilverRing), typeof(SilverBracelet) };
		...
		public static double GetSuccessChance(
			Mobile from,
			Item item,
			int totalItemIntensity,
			int propintensity,
			double bonus
		)
		{
			...
			return Math.Max(
				0,
				Math.Round(
					Math.Floor(20 * skill + 10 * a * Math.Pow(e, (b / (resultWeight + c))) + 10 * w - 2400) / 1000 * (i)
						+ bonus,
					3
				) * 100
			);
		...
		public static void TryImbueItem(Mobile from, Item i, int id, int value)
		{
			...
			if (
				...
				&& (
					from.Backpack == null
					|| from.Backpack.GetAmount(gem) < gemAmount
		...
		public static void ImbueItem(Mobile from, Item item, int id, int value)
		{
			...
			if (id >= 51 && id <= 55)
			{
				if (item is BaseArmor)
				{
					...
					{
						case 51:
							arm.PhysNonImbuing = 0;
							break;
						case 52:
							arm.FireNonImbuing = 0;
							break;
						case 53:
							arm.ColdNonImbuing = 0;
							break;
						case 54:
							arm.PoisonNonImbuing = 0;
							break;
						case 55:
							arm.EnergyNonImbuing = 0;
							break;
					}
				}
				else if (item is BaseClothing)
				{
					...
					{
						case 51:
							hat.PhysNonImbuing = 0;
							break;
						case 52:
							hat.FireNonImbuing = 0;
							break;
						case 53:
							hat.ColdNonImbuing = 0;
							break;
						case 54:
							hat.PoisonNonImbuing = 0;
							break;
						case 55:
							hat.EnergyNonImbuing = 0;
							break;
					}
				}
			}
		...
		public static void SetProperty(Item item, int id, int value)
		{
				...
				else if (prop is AosElementAttribute)
				{
					...
					switch (attr)
					{
						...
						case AosElementAttribute.Cold:
							...
							break;
						case AosElementAttribute.Poison:
							arm.PoisonBonus = value;
							break;
						case AosElementAttribute.Energy:
							arm.EnergyBonus = value;
							break;
					}
				}
				else if (prop is SAAbsorptionAttribute)
			...
			else if (item is BaseClothing)
			{
				...
				else if (prop is AosElementAttribute)
				{
					...
					{
						case AosElementAttribute.Physical:
							clothing.Resistances.Physical = value;
							break;
						case AosElementAttribute.Fire:
							clothing.Resistances.Fire = value;
							break;
						case AosElementAttribute.Cold:
							clothing.Resistances.Cold = value;
							break;
						case AosElementAttribute.Poison:
							clothing.Resistances.Poison = value;
							break;
						case AosElementAttribute.Energy:
							clothing.Resistances.Energy = value;
							break;
					}
				}
			}
			else if (item is BaseJewel)
			{
				...
				else if (prop is AosElementAttribute)
				{
					...
					{
						case AosElementAttribute.Physical:
							jewel.Resistances.Physical = value;
							break;
						case AosElementAttribute.Fire:
							jewel.Resistances.Fire = value;
							break;
						case AosElementAttribute.Cold:
							jewel.Resistances.Cold = value;
							break;
						case AosElementAttribute.Poison:
							jewel.Resistances.Poison = value;
							break;
						case AosElementAttribute.Energy:
							jewel.Resistances.Energy = value;
							break;
					}
				}
		...
		public static int GetTotalMods(Item item, int id = -1)
		{
			...
			if (item is BaseWeapon)
			{
				...
				foreach (int i in Enum.GetValues(typeof(AosAttribute)))
				{
					...
					if (wep.Attributes[attr] > 0)
					{
						...
						wep.Attributes[attr] == 0
						&& attr == AosAttribute.CastSpeed
						&& wep.Attributes[AosAttribute.SpellChanneling] > 0
					)
					{
						if (!(prop is AosAttribute) || (AosAttribute)prop != attr)
							total++;
			...
			else if (item is BaseArmor)
			{
				...
				foreach (int i in Enum.GetValues(typeof(AosAttribute)))
				{
					...
					if (armor.Attributes[attr] > 0)
					{
						...
						armor.Attributes[attr] == 0
						&& attr == AosAttribute.CastSpeed
						&& armor.Attributes[AosAttribute.SpellChanneling] > 0
					)
					{
						if (!(prop is AosAttribute) || (AosAttribute)prop == attr)
							total++;
				...
				if (armor.PhysicalBonus > armor.PhysNonImbuing && id != 51)
				{
					total++;
				}
				if (armor.FireBonus > armor.FireNonImbuing && id != 52)
				{
					total++;
				}
				if (armor.ColdBonus > armor.ColdNonImbuing && id != 53)
				{
					total++;
				}
				if (armor.PoisonBonus > armor.PoisonNonImbuing && id != 54)
				{
					total++;
				}
				if (armor.EnergyBonus > armor.EnergyNonImbuing && id != 55)
				{
					total++;
				}
			...
			else if (item is BaseJewel)
			{
				...
				if (j.Resistances.Physical > 0 && id != 51)
				{
					total++;
				}
				if (j.Resistances.Fire > 0 && id != 52)
				{
					total++;
				}
				if (j.Resistances.Cold > 0 && id != 53)
				{
					total++;
				}
				if (j.Resistances.Poison > 0 && id != 54)
				{
					total++;
				}
				if (j.Resistances.Energy > 0 && id != 55)
				{
					total++;
				}
			}
			else if (item is BaseClothing)
			{
				...
				if (clothing.Resistances.Physical > clothing.PhysNonImbuing && id != 51)
				{
					total++;
				}
				if (clothing.Resistances.Fire > clothing.FireNonImbuing && id != 52)
				{
					total++;
				}
				if (clothing.Resistances.Cold > clothing.ColdNonImbuing && id != 53)
				{
					total++;
				}
				if (clothing.Resistances.Poison > clothing.PoisonNonImbuing && id != 54)
				{
					total++;
				}
				if (clothing.Resistances.Energy > clothing.EnergyNonImbuing && id != 55)
				{
					total++;
				}
			}
				...
				else
				{
					...
					if (IsDerivedArmorOrClothing(baseType))
					{
						...
						if (temp != null)
						{
							...
							resists[0] =
								GetBaseResistBonus(item, AosElementAttribute.Physical)
								- GetBaseResistBonus(temp, AosElementAttribute.Physical);
							resists[1] =
								GetBaseResistBonus(item, AosElementAttribute.Fire)
								- GetBaseResistBonus(temp, AosElementAttribute.Fire);
							resists[2] =
								GetBaseResistBonus(item, AosElementAttribute.Cold)
								- GetBaseResistBonus(temp, AosElementAttribute.Cold);
							resists[3] =
								GetBaseResistBonus(item, AosElementAttribute.Poison)
								- GetBaseResistBonus(temp, AosElementAttribute.Poison);
							resists[4] =
								GetBaseResistBonus(item, AosElementAttribute.Energy)
								- GetBaseResistBonus(temp, AosElementAttribute.Energy);
		...
		public static int GetTotalWeight(Item item, int id, bool trueWeight, bool imbuing)
		{
			...
			if (item is BaseWeapon)
			{
				...
				if (item is BaseRanged)
				{
					...
					if (ranged.Velocity > 0)
						weight += GetIntensityForAttribute(
							item,
							"WeaponVelocity",
							id,
							ranged.Velocity,
			...
			else if (item is BaseArmor)
			{
				...
				if (arm.PhysicalBonus > arm.PhysNonImbuing)
				{
					if (id != 51)
					{
						weight += ((double)(100.0 / 15) * (double)(arm.PhysicalBonus - arm.PhysNonImbuing));
					}
				}
				if (arm.FireBonus > arm.FireNonImbuing)
				{
					if (id != 52)
					{
						weight += ((double)(100.0 / 15) * (double)(arm.FireBonus - arm.FireNonImbuing));
					}
				}
				if (arm.ColdBonus > arm.ColdNonImbuing)
				{
					if (id != 53)
					{
						weight += ((double)(100.0 / 15) * (double)(arm.ColdBonus - arm.ColdNonImbuing));
					}
				}
				if (arm.PoisonBonus > arm.PoisonNonImbuing)
				{
					if (id != 54)
					{
						weight += ((double)(100.0 / 15) * (double)(arm.PoisonBonus - arm.PoisonNonImbuing));
					}
				}
				if (arm.EnergyBonus > arm.EnergyNonImbuing)
				{
					if (id != 55)
					{
						weight += ((double)(100.0 / 15) * (double)(arm.EnergyBonus - arm.EnergyNonImbuing));
					}
				}
			}
			...
			if (wepAttrs != null)
				foreach (long i in Enum.GetValues(typeof(AosWeaponAttribute)))
					weight += GetIntensityForAttribute(
						item,
						(AosWeaponAttribute)i,
						id,
						wepAttrs[(AosWeaponAttribute)i],
						trueWeight,
						imbuing
					);
			...
			if (saAttrs != null)
				foreach (int i in Enum.GetValues(typeof(SAAbsorptionAttribute)))
					weight += GetIntensityForAttribute(
						item,
						(SAAbsorptionAttribute)i,
						id,
						saAttrs[(SAAbsorptionAttribute)i],
						trueWeight,
						imbuing
					);
			...
			if (armorAttrs != null)
				foreach (int i in Enum.GetValues(typeof(AosArmorAttribute)))
					weight += GetIntensityForAttribute(
						...
						(AosArmorAttribute)i,
						id,
						armorAttrs[(AosArmorAttribute)i],
						trueWeight,
						imbuing
					);
			...
			if (resistAttrs != null && !(item is BaseWeapon))
				foreach (int i in Enum.GetValues(typeof(AosElementAttribute)))
					weight += GetIntensityForAttribute(
						item,
						(AosElementAttribute)i,
						id,
						resistAttrs[(AosElementAttribute)i],
						trueWeight,
						imbuing
					);
			...
			if (extattrs != null)
				foreach (int i in Enum.GetValues(typeof(ExtendedWeaponAttribute)))
					weight += GetIntensityForAttribute(
						item,
						(ExtendedWeaponAttribute)i,
						id,
						extattrs[(ExtendedWeaponAttribute)i],
						trueWeight,
						imbuing
					);
		...
		private static int CheckSkillBonuses(Item item, int check, bool trueWeight, bool imbuing)
		{
			...
			if (skills != null)
			{
				if (skills.GetBonus(0) > 0)
				{
					if (id < 151 || id > 155)
					{
						weight += ((double)(totalWeight / maxInt) * (double)skills.GetBonus(0));
					}
				}
				if (skills.GetBonus(1) > 0)
				{
					if (id < 156 || id > 160)
					{
						weight += ((double)(totalWeight / maxInt) * (double)skills.GetBonus(1));
					}
				}
				if (skills.GetBonus(2) > 0)
				{
					if (id < 161 || id > 166)
					{
						weight += ((double)(totalWeight / maxInt) * (double)skills.GetBonus(2));
					}
				}
				if (skills.GetBonus(3) > 0)
				{
					if (id < 167 || id > 173)
					{
						weight += ((double)(totalWeight / maxInt) * (double)skills.GetBonus(3));
					}
				}
				if (skills.GetBonus(4) > 0)
				{
					if (id < 174 || id > 180)
					{
						weight += ((double)(totalWeight / maxInt) * (double)skills.GetBonus(4));
					}
				}
			}
		...
		public static SkillName[] PossibleSkills
		{
			get { return m_PossibleSkills; }
		}
		...
		public static bool CheckSoulForge(Mobile from, int range, bool message, bool checkqueen, out double bonus)
		{
			...
			{
				if (
					(item.ItemID >= 0x4277 && item.ItemID <= 0x4286)
		...
		public static Type[] IngredTypes
		{
			get { return m_IngredTypes; }
		}
		private static Type[] m_IngredTypes = new Type[]
		{
			typeof(MagicalResidue),
			typeof(EnchantedEssence),
			...
			typeof(GoblinBlood),
			typeof(LavaSerpentCrust),
			typeof(UndyingFlesh),
			typeof(CrushedGlass),
			typeof(CrystallineBlackrock),
			typeof(PowderedIron),
			typeof(ElvenFletching),
		...
		public static int GetValueForID(Item item, int id)
		{
			...
			else if (item is BaseClothing)
			{
				...
				else if (attr is AosElementAttribute)
				{
					...
					return c.ArmorAttributes[(AosArmorAttribute)attr];
				else if (attr is SAAbsorptionAttribute)
		...
		public static int GetIntensityForAttribute(
			Item item,
			object attr,
			int checkID,
			int value,
