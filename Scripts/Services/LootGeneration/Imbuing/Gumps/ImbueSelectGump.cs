			...
			if (menuCat == 1) // == CASTING ==
			{
				if (itemRef == 1 || itemRef == 2)
				{
					...
					if (
						...
						&& (
							((BaseWeapon)m_Item).Attributes.SpellChanneling == 0
							|| ((BaseWeapon)m_Item).Attributes.CastSpeed < 0
						)
					)
					{
				...
				else if (itemRef == 4)
				{
					...
					if (
						...
						&& (
							((BaseShield)m_Item).Attributes.SpellChanneling == 0
							|| ((BaseShield)m_Item).Attributes.CastSpeed < 0
						)
					)
					{
						AddButton(250, 90 + (yOffset * 20), 4005, 4007, 10116, GumpButtonType.Reply, 0);
