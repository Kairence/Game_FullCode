	...
	public class ImbueGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddLabel(
				430,
				220,
				GetColor(m_TotalProps + 1, 5),
				String.Format("{0}/{1}", m_TotalProps + 1, Imbuing.GetMaxProps(m_Item))
			);
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 10100: // Imbue the Item
				{
					...
					if (
						Imbuing.OnBeforeImbue(
							User,
							m_Item,
							m_ID,
							m_Value,
							m_TotalProps,
							Imbuing.GetMaxProps(m_Item),
							m_TotalItemWeight,
							m_MaxWeight
						)
					)
		...
		public static TextDefinition GetNameForAttribute(object attribute)
		{
			...
			{
				if (
					attribute is SlayerName
					&& info.Attribute is SlayerName
					&& (SlayerName)attribute == (SlayerName)info.Attribute
				)
					return info.AttributeName;

				if (
					attribute is AosAttribute
					&& info.Attribute is AosAttribute
					&& (AosAttribute)attribute == (AosAttribute)info.Attribute
				)
					return info.AttributeName;

				if (
					attribute is AosWeaponAttribute
					&& info.Attribute is AosWeaponAttribute
					&& (AosWeaponAttribute)attribute == (AosWeaponAttribute)info.Attribute
				)
					return info.AttributeName;

				if (
					attribute is SkillName
					&& info.Attribute is SkillName
					&& (SkillName)attribute == (SkillName)info.Attribute
				)
					return info.AttributeName;
