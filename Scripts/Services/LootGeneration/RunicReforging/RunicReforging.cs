...
namespace Server.Items
{
	...
	public static class RunicReforging
	{
		public static bool CanReforge(Mobile from, Item item, CraftSystem crsystem)
		{
			...
			int maxmods =
				...
				|| (
					...
					&& BaseArmor.IsMageArmorType((BaseArmor)item)
				)
					? 1
					: 0;
			...
			if (mods > maxmods)
				...
				item is BaseWeapon
				&& Server.Spells.Mysticism.EnchantSpell.IsUnderSpellEffects(from, (BaseWeapon)item)
			)
				goodtogo = false;
		...
		public static void ApplyReforgedProperties(
			Item item,
			List<int> props,
			ReforgedPrefix prefix,
			ReforgedSuffix suffix,
			int budget,
			int perclow,
			int perchigh,
			int maxmods,
			...
			else
			{
				...
				)
				{
					...
					GetNamedModCount(
						...
						prefixCol.Count,
						suffixCol.Count,
						ref specialAddPrefix,
						ref specialAddSuffix
					);
				...
				switch (tool.Resource)
				{
					...
					case CraftResource.ShadowIron:
						if (
							...
							&& HasOption(
								options,
								ReforgingOption.PowerfulAndFundamental,
								ReforgingOption.StructuralAndFundamental
							)
						)
							return false;
						else if (
							index >= 8
							&& index <= 10
							&& HasOption(options, ReforgingOption.PowerfulStructuralAndFundamental)
						)
							return false;
						break;
					case CraftResource.Copper:
						if (
							...
							&& HasOption(
								options,
								ReforgingOption.PowerfulAndStructural,
								ReforgingOption.PowerfulAndFundamental
							)
						)
							return false;
						if (
							...
							&& HasOption(
								options,
								ReforgingOption.StructuralAndFundamental,
								ReforgingOption.PowerfulStructuralAndFundamental
							)
						)
							return false;
						break;
				...
				switch (tool.Resource)
				{
					case CraftResource.OakWood:
						...
						if (
							...
							&& (
								HasOption(options, ReforgingOption.PowerfulStructuralAndFundamental)
								|| HasOption(options, ReforgingOption.StructuralAndFundamental)
							)
						)
							return false;
						break;
				...
				switch (tool.Resource)
				{
					...
					case CraftResource.Copper:
						if (
							...
							(index == 8 || index == 9) && HasOption(options, ReforgingOption.StructuralAndFundamental)
						)
							return false;
						else if (
					...
					case CraftResource.Bronze:
						if (index == 8)
							...
							(index == 9 || index == 5) && HasOption(options, ReforgingOption.PowerfulAndFundamental)
						)
							return false;
						else if (
							(index == 9 || index == 5 || index == 11)
							&& HasOption(
								options,
								ReforgingOption.StructuralAndFundamental,
								ReforgingOption.PowerfulStructuralAndFundamental
					...
					case CraftResource.Gold:
						if (index == 8)
							...
							(index == 5 || index == 9) && HasOption(options, ReforgingOption.PowerfulAndStructural)
						)
							return false;
						else if (
							(index == 9 || index == 5 || index == 11)
				...
				switch (tool.Resource)
				{
					...
					case CraftResource.ShadowIron:
						if ((index == 10 || index == 11) && HasOption(options, ReforgingOption.PowerfulAndStructural))
							...
							(index == 5 || index == 9 || index == 10 || index == 11)
							&& HasOption(options, ReforgingOption.PowerfulStructuralAndFundamental)
						)
							return false;
						break;
					case CraftResource.Copper:
						if (index == 10 || index == 11)
							...
							(index == 5 || index == 9) && HasOption(options, ReforgingOption.StructuralAndFundamental)
						)
							return false;
						else if (
							(index == 5 || index == 9 || index == 5)
							&& HasOption(options, ReforgingOption.PowerfulStructuralAndFundamental)
						)
							return false;
						break;
					...
					case CraftResource.SpinedLeather:
						if ((index == 10 || index == 11) && HasOption(options, ReforgingOption.PowerfulAndStructural))
							...
							(index == 9 || index == 10 || index == 11 || index == 5)
							&& HasOption(options, ReforgingOption.PowerfulStructuralAndFundamental)
						)
							return false;
						break;
		...
		private static void CheckAttributes(Item item, List<NamedInfoCol> list, bool reforged)
		{
			...
			list.IterateReverse(col =>
			{
				if (
					col != null
					&& list.Contains(col)
					&& !ItemPropertyInfo.ValidateProperty(item, col.Attribute, reforged)
				)
				{
					list.Remove(col);
				}
		...
		private static void GetNamedModCount(
			int itemIndex,
			int prefixID,
			int suffixID,
			int maxmods,
		...
		)
		{
			...
			else
			{
				...
				switch (maxmods)
				{
					...
					case 5:
					case 4:
						mods = Utility.RandomBool() ? 2 : 3;
						break;
					case 3:
						mods = Utility.RandomBool() ? 1 : 2;
						break;
					case 2:
					case 1:
						mods = 1;
		...
		public static int GetPropertyCount(BaseRunicTool tool)
		{
			switch (tool.Resource)
			{
				...
				case CraftResource.Agapite:
				case CraftResource.Verite:
					return Utility.RandomMinMax(3, 4);
				case CraftResource.Valorite:
					return 5;
		...
		private static bool ApplyPrefixSuffixAttribute(
			Item item,
			NamedInfoCol col,
			int resIndex,
			int preIndex,
		...
		)
		{
			...
			if (attribute is string)
			{
				...
				{
					case "RandomEater":
						attribute = GetRandomEater();
						break;
					case "HitSpell":
						attribute = GetRandomHitSpell();
						break;
					case "HitArea":
						attribute = GetRandomHitArea();
						break;
					case "Slayer":
						attribute = BaseRunicTool.GetRandomSlayer();
						break;
					case "WeaponVelocity":
						break;
					case "ElementalDamage":
						attribute = GetRandomElemental();
						break;
				}
			}
			...
			if (
				id == -1
				|| Imbuing.GetValueForID(item, id) > 0
				|| !ItemPropertyInfo.ValidateProperty(item, id, reforged)
			)
			{
				return false;
			}
		...
		public static bool ApplyResistance(Item item, int value, AosElementAttribute attribute)
		{
			...
			if (!_Elements.ContainsKey(item))
			{
				...
				else if (item is BaseWeapon)
				{
					_Elements[item] = new int[]
					{
						((BaseWeapon)item).WeaponAttributes.ResistPhysicalBonus,
						((BaseWeapon)item).WeaponAttributes.ResistFireBonus,
						((BaseWeapon)item).WeaponAttributes.ResistColdBonus,
						((BaseWeapon)item).WeaponAttributes.ResistPoisonBonus,
						((BaseWeapon)item).WeaponAttributes.ResistEnergyBonus,
					};
				}
				else if (resists != null)
				{
					_Elements[item] = new int[]
					{
						resists[AosElementAttribute.Physical],
						resists[AosElementAttribute.Fire],
						resists[AosElementAttribute.Cold],
						resists[AosElementAttribute.Poison],
						resists[AosElementAttribute.Energy],
					};
				}
				else
				{
			...
			switch (attribute)
			{
				...
				case AosElementAttribute.Physical:
					if (
						item is BaseArmor
						&& (!_Elements.ContainsKey(item) || ((BaseArmor)item).PhysicalBonus == _Elements[item][0])
					)
					{
						...
						item is BaseWeapon
						&& (
							!_Elements.ContainsKey(item)
							|| ((BaseWeapon)item).WeaponAttributes.ResistPhysicalBonus == _Elements[item][0]
						)
					)
					{
						...
						resists != null
						&& (!_Elements.ContainsKey(item) || resists[attribute] == _Elements[item][0])
					)
					{
				...
				case AosElementAttribute.Fire:
					if (
						item is BaseArmor
						&& (!_Elements.ContainsKey(item) || ((BaseArmor)item).FireBonus == _Elements[item][1])
					)
					{
						...
						&& (
							!_Elements.ContainsKey(item)
							|| ((BaseWeapon)item).WeaponAttributes.ResistFireBonus == _Elements[item][1]
						)
					)
					{
						...
						resists != null
						&& (!_Elements.ContainsKey(item) || resists[attribute] == _Elements[item][1])
					)
					{
						resists[attribute] = value;
						return true;
				...
				case AosElementAttribute.Cold:
					if (
						item is BaseArmor
						&& (!_Elements.ContainsKey(item) || ((BaseArmor)item).ColdBonus == _Elements[item][2])
					)
					{
						...
						&& (
							!_Elements.ContainsKey(item)
							|| ((BaseWeapon)item).WeaponAttributes.ResistColdBonus == _Elements[item][2]
						)
					)
					{
						...
						resists != null
						&& (!_Elements.ContainsKey(item) || resists[attribute] == _Elements[item][2])
					)
					{
						resists[attribute] = value;
						return true;
				...
				case AosElementAttribute.Poison:
					if (
						item is BaseArmor
						&& (!_Elements.ContainsKey(item) || ((BaseArmor)item).PoisonBonus == _Elements[item][3])
					)
					{
						...
						&& (
							!_Elements.ContainsKey(item)
							|| ((BaseWeapon)item).WeaponAttributes.ResistPoisonBonus == _Elements[item][3]
						)
					)
					{
						...
						resists != null
						&& (!_Elements.ContainsKey(item) || resists[attribute] == _Elements[item][3])
					)
					{
						resists[attribute] = value;
						return true;
				...
				case AosElementAttribute.Energy:
					if (
						item is BaseArmor
						&& (!_Elements.ContainsKey(item) || ((BaseArmor)item).EnergyBonus == _Elements[item][4])
					)
					{
						...
						&& (
							!_Elements.ContainsKey(item)
							|| ((BaseWeapon)item).WeaponAttributes.ResistEnergyBonus == _Elements[item][4]
						)
					)
					{
						...
						resists != null
						&& (!_Elements.ContainsKey(item) || resists[attribute] == _Elements[item][4])
					)
					{
						resists[attribute] = value;
						return true;
		...
		{
			if (
				(option & ReforgingOption.Powerful) != 0
				&& (option & ReforgingOption.Structural) != 0
				&& (option & ReforgingOption.Fundamental) != 0
			)
				return 6;

			if ((option & ReforgingOption.Structural) != 0 && (option & ReforgingOption.Fundamental) != 0)
				return 5;

			if ((option & ReforgingOption.Powerful) != 0 && (option & ReforgingOption.Structural) != 0)
				return 4;
		...
		public static Dictionary<int, NamedInfoCol[][]> PrefixSuffixInfo
		{
			get { return m_PrefixSuffixInfo; }
		}
		...
		public static void Configure()
		{
				...
				e =>
				{
					e.Mobile.BeginTarget(
						...
						(from, targeted) =>
						{
							...
							{
								((BaseCreature)targeted).PrivateOverheadMessage(
									Server.Network.MessageType.Regular,
									0x25,
									false,
									GetDifficultyFor((BaseCreature)targeted).ToString(),
									e.Mobile.NetState
								);
							}
						}
			...
			m_PrefixSuffixInfo[9] = new NamedInfoCol[][] // Auspicious
			{
				...
				new NamedInfoCol[] { new NamedInfoCol(AosAttribute.Luck, LuckTable) },
				new NamedInfoCol[] { new NamedInfoCol(AosAttribute.Luck, LuckTable) },
			};
			m_PrefixSuffixInfo[10] = new NamedInfoCol[][] // Charmed
			{
				...
				new NamedInfoCol[] { },
				new NamedInfoCol[] { new NamedInfoCol(AosAttribute.EnhancePotions, ArmorEnhancePotsTable) },
			};
			m_PrefixSuffixInfo[11] = new NamedInfoCol[][] //Vicious
			{
				...
				new NamedInfoCol[] { new NamedInfoCol(AosAttribute.AttackChance, WeaponHCITable) },
				new NamedInfoCol[]
				{
		...
		public static object GetRandomHitSpell()
		{
			switch (Utility.Random(4))
			{
				...
				case 1:
					return AosWeaponAttribute.HitFireball;
				case 2:
					return AosWeaponAttribute.HitHarm;
				case 3:
		...
		private static object GetRandomHitArea()
		{
			switch (Utility.Random(5))
			{
				...
				case 1:
					return AosWeaponAttribute.HitFireArea;
				case 2:
					return AosWeaponAttribute.HitColdArea;
				case 3:
					return AosWeaponAttribute.HitPoisonArea;
				case 4:
					return AosWeaponAttribute.HitEnergyArea;
			}
		}
		...
		private static object GetRandomEater()
		{
			switch (Utility.Random(6))
			{
				...
				case 1:
					return SAAbsorptionAttribute.EaterFire;
				case 2:
					return SAAbsorptionAttribute.EaterCold;
				case 3:
					return SAAbsorptionAttribute.EaterPoison;
				case 4:
					return SAAbsorptionAttribute.EaterEnergy;
				case 5:
					return SAAbsorptionAttribute.EaterDamage;
			}
		}
		...
		public static void AddPrefixName(ObjectPropertyList list, ReforgedSuffix prefix, string name)
		{
			list.Add(1151757, String.Format("{0}\t#{1}", name, GetSuffixName(prefix))); // ~1_ITEM~ of ~2_SUFFIX~
		}
		...
		public static Item GenerateRandomItem(IEntity e)
		{
			Item item = Loot.RandomArmorOrShieldOrWeaponOrJewelry(
				LootPackEntry.IsInTokuno(e),
				LootPackEntry.IsMondain(e),
				LootPackEntry.IsStygian(e)
			);
		...
		/// <returns></returns>
		public static bool GenerateRandomArtifactItem(
			Item item,
			int luck,
			int budget,
		...
		{
			Item item = Loot.RandomArmorOrShieldOrWeaponOrJewelry(
				LootPackEntry.IsInTokuno(killer),
				LootPackEntry.IsMondain(killer),
				LootPackEntry.IsStygian(killer)
			);
			...
			if (item != null)
				GenerateRandomItem(
					item,
					killer,
					Math.Max(100, GetDifficultyFor(creature)),
					LootPack.GetLuckChance(GetLuckForKiller(creature)),
					ReforgedPrefix.None,
					ReforgedSuffix.None
				);
		...
		public static bool GenerateRandomItem(
			Item item,
			Mobile killer,
		...
		public static void GenerateRandomItem(
			...
			int basebudget,
			int luckchance,
			ReforgedPrefix forcedprefix,
			ReforgedSuffix forcedsuffix,
			Map map = null,
			bool artifact = false
		)
		{
			...
			if (item != null)
			{
				...
				int rawLuck =
					killer != null
						? killer is PlayerMobile
							? ((PlayerMobile)killer).RealLuck
							: killer.Luck
						: 0;
				...
				else
				{
					...
					budget =
						Utility.RandomMinMax(basebudget - (basebudget / divisor), (int)(basebudget + (toAdd * perc)))
						+ budgetBonus;
					...
					if (
						forcedprefix == ReforgedPrefix.None
						&& budget >= Utility.Random(2700)
						&& suffix < ReforgedSuffix.Minax
					)
						prefix = ChooseRandomPrefix(item, budget);
					...
					{
						int maxmods = Math.Max(
							5,
							Math.Min(
								RandomItemGenerator.MaxProps - 1,
								(int)Math.Ceiling((double)budget / (double)Utility.RandomMinMax(100, 140))
							)
						);
				...
				switch (suffix)
				{
					...
					case ReforgedSuffix.Kotl:
						item.Hue = 2591;
						break;
					case ReforgedSuffix.EnchantedOrigin:
						item.Hue = 1171;
						break;
					case ReforgedSuffix.Doom:
						item.Hue = 2301;
						break;
				}
		...
		private static int TryApplyRandomDisadvantage(Item item)
		{
			...
			ItemPower power = GetItemPower(
				item,
				Imbuing.GetTotalWeight(item, -1, false, false),
				Imbuing.GetTotalMods(item),
				false
			);
		...
		private static bool ApplyRandomProperty(
			Item item,
			List<int> props,
			int perclow,
			int perchigh,
		...
		)
		{
			...
			while (true)
			{
				...
				if (
					Imbuing.GetValueForID(item, random) == 0
					&& ItemPropertyInfo.ValidateProperty(item, random, reforged)
				)
				{
					id = random;
					break;
		...
		/// <returns></returns>
		public static bool ApplyProperty(
			Item item,
			int id,
			int perclow,
			int perchigh,
		...
		)
		{
			...
			int value = CalculateValue(
				item,
				ItemPropertyInfo.GetAttribute(id),
				min,
				max,
				perclow,
				perchigh,
		...
		public static bool ApplyReforgedNameProperty(
			Item item,
			int id,
			NamedInfoCol info,
			int resIndex,
		...
		/* switch (resource)
 {
	 ...
	 case CraftResource.AshWood: return 5;
		...
		public static void ItemNerfVersion6()
		{
			...
			SpawnerPersistence.ToConsole(
				String.Format(
					"Cleauned up {0} items: {1} fc2, {2} non-Armor eater, {3} non armor casting focus, {4} brittle jewels converted to Antique.",
					fc2 + eater + focus + brittle,
					fc2,
					eater,
					focus,
					brittle
