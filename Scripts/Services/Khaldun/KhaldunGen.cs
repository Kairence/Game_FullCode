	...
	public class GenKhaldun
	{
		...
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(GenKhaldun_OnCommand)
			);
			CommandSystem.Register(
				"DeleteKhaldun",
				AccessLevel.Administrator,
				new CommandEventHandler(DeleteKhaldun_OnCommand)
			);
		}
		...
		public static bool FindMorphItem(int x, int y, int z, int inactiveItemID, int activeItemID)
		{
			...
			foreach (Item item in eable)
			{
				if (
					item is MorphItem
					&& item.Z == z
					&& ((MorphItem)item).InactiveItemID == inactiveItemID
					&& ((MorphItem)item).ActiveItemID == activeItemID
				)
				{
					eable.Free();
					return true;
		...
		public static void GenKhaldun_OnCommand(CommandEventArgs e)
		{
			...
			e.Mobile.SendMessage(
				String.Format("{0} dynamic Khaldun item{1} generated.", m_Count, m_Count == 1 ? "" : "s")
			);
		}
	}
}
