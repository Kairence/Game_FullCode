	...
	public class SR_Gump : Gump
	{
		...
		public void DisplayHeader()
		{
			...
			AddHtml(
				...
				210,
				20,
				"<CENTER><BASEFONT COLOR=#FFFFFF><BIG>Joeku's Staff Runebook</CENTER>",
				false,
				false
			);
		}
		...
		public void DisplayRunebooks(int y, int h, int w, int tiers, int labelHue)
		{
			...
			AddItem(
				SR_Utilities.ItemOffsetX(rune),
				y + SR_Utilities.ItemOffsetY(rune) + 12,
				SR_Utilities.RunebookID,
				SR_Utilities.ItemHue(rune)
			);
			...
			if (tiers > 0)
			{
				...
				AddItem(
					SR_Utilities.ItemOffsetX(rune) + tiers * 5,
					y + SR_Utilities.ItemOffsetY(rune) + 12 + 37,
					SR_Utilities.RunebookID,
					SR_Utilities.ItemHue(rune)
				);
		...
		public void DisplayRunes(int y, int h, int labelHue)
		{
			...
			AddPage(1);
			int pages = (int)Math.Ceiling((double)count / 9.0),
				temp = 0;
			for (int i = 0, loc = 0, page = 1; i < count; i++, loc++)
			{
				...
				AddItem(
					SR_Utilities.ItemOffsetX(runes[i]),
					2 + SR_Utilities.ItemOffsetY(runes[i]) + temp,
					runes[i].IsRunebook ? SR_Utilities.RunebookID : SR_Utilities.RuneID,
					SR_Utilities.ItemHue(runes[i])
				);
				...
				else
				{
					AddLabelCropped(
						...
						20,
						labelHue,
						String.Format("{0}. {1} ({2})", i + 1 - runebooks, runes[i].Name, runes[i].TargetMap.ToString())
					);
				...
				if (pages > 1 && ((loc == 8 && i < count - 1) || i == count - 1))
				{
					...
					AddHtml(
						...
						90,
						20,
						String.Format("<BASEFONT COLOR=#FFFFFF><CENTER>Page {0}/{1}", page, pages),
						false,
						false
					);
