...
namespace Server.Engines.NewMagincia
{
	public class MaginciaLottoGump : Gump
	{
		...
		public MaginciaLottoGump(Mobile from, MaginciaHousingPlot plot)
			: base(75, 75)
		{
			m_Plot = plot;
			m_From = from;
			...
			AddHtml(
				...
				100,
				16,
				String.Format("<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", LabelColor, plot.Map.ToString()),
				false,
				false
			);
			...
			AddHtml(
				...
				100,
				16,
				String.Format("<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", EntryColor, plot.Identifier),
				false,
				false
			);
			...
			AddHtml(
				...
				100,
				16,
				String.Format("<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", EntryColor, totalTicketsSold.ToString()),
				false,
				false
			);
			...
			AddHtml(
				...
				100,
				20,
				String.Format("<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", EntryColor, ticketsBought.ToString()),
				false,
				false
			);
			...
			if (plot.LottoOngoing)
			{
				...
				AddHtml(
					...
					320,
					40,
					"<BASEFONT COLOR=#{0:X6}>The lottery on this plot is currently disabled.</BASEFONT>",
					false,
					false
				);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (Banker.Withdraw(m_From, total))
			{
				...
				m_From.SendLocalizedMessage(
					1150480,
					String.Format("{0}\t{1}\t{2}", toBuy.ToString(), pricePer.ToString(), total.ToString())
				); // Purchase of ~1_COUNT~ ticket(s) at ~2_PRICE~gp each costs a total of ~3_TOTAL~. The funds have been withdrawn from your bank box and your ticket purchase has been recorded.
				...
				m_From.SendLocalizedMessage(
					1150479,
					String.Format("{0}\t{1}\t{2}", toBuy.ToString(), pricePer.ToString(), total.ToString())
				); // Purchase of ~1_COUNT~ ticket(s) at ~2_PRICE~gp each costs a total of ~3_TOTAL~. You do not have the required funds in your bank box to make the purchase.
		}
	}
}
