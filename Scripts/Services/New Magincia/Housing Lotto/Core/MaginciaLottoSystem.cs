...
namespace Server.Engines.NewMagincia
{
	public class MaginciaLottoSystem : Item
	{
		...
		public static MaginciaLottoSystem Instance
		{
		...
		public static List<MaginciaHousingPlot> Plots
		{
			get { return m_Plots; }
		}

		private static Dictionary<Map, List<Rectangle2D>> m_FreeHousingZones;
		public static Dictionary<Map, List<Rectangle2D>> FreeHousingZones
		{
			get { return m_FreeHousingZones; }
		}

		private static Dictionary<Mobile, List<NewMaginciaMessage>> m_MessageQueue =
			new Dictionary<Mobile, List<NewMaginciaMessage>>();
		public static Dictionary<Mobile, List<NewMaginciaMessage>> MessageQueue
		{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public static int GoldSink
		{
			get { return m_GoldSink; }
			set { m_GoldSink = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public TimeSpan LottoDuration
		{
			get { return m_LottoDuration; }
			set { m_LottoDuration = value; }
		}

		public MaginciaLottoSystem()
			: base(3240)
		{
			Movable = false;
			m_Enabled = true;
		...
		public static void UnregisterPlot(MaginciaHousingPlot plot)
		{
			...
			if (
				plot.Map != null
				&& m_FreeHousingZones.ContainsKey(plot.Map)
				&& !m_FreeHousingZones[plot.Map].Contains(plot.Bounds)
			)
			...
			if (system != null && system.Enabled && from.Backpack != null && IsInMagincia(center.X, center.Y, from.Map))
			{
				...
				foreach (Item item in items)
				{
					...
					if (
						...
						&& !lease.Expired
						&& lease.Plot != null
						&& lease.Plot.Bounds.Contains(center)
						&& from.Map == lease.Plot.Map
					)
					{
						lease.OnExpired();
						return;
		...
		private void LoadPlots()
		{
			for (int i = 0; i < m_MagHousingZones.Length; i++)
			{
				...
				MaginciaHousingPlot tramplot = new MaginciaHousingPlot(
					m_Identifiers[i],
					m_MagHousingZones[i],
					prime,
					Map.Trammel
				);
				MaginciaHousingPlot felplot = new MaginciaHousingPlot(
					m_Identifiers[i],
					m_MagHousingZones[i],
					prime,
					Map.Felucca
		...
		public static Rectangle2D[] MagHousingZones
		{
			get { return m_MagHousingZones; }
		}
		...
		{
			return from != null
				&& m_MessageQueue.ContainsKey(from)
				&& m_MessageQueue[from] != null
				&& m_MessageQueue[from].Count > 0;
		}
		...
		public void ValidatePlots()
		{
			for (int i = 0; i < m_Identifiers.Length; i++)
			{
				...
				{
					Console.WriteLine(
						"Adding {0} to Magincia Free Housing Zone.[{1}]",
						rec,
						"Plot existed, writ expired"
					);
