...
namespace Server.Engines.Distillation
{
	public class DistillationGump : Gump
	{
		...
		public DistillationGump(Mobile from)
			: base(35, 35)
		{
			...
			for (int i = 0; i < DistillationSystem.CraftDefs.Count; i++)
			{
				...
				if (def.Group == group)
				{
					...
					AddHtmlLocalized(
						...
						200,
						20,
						DistillationSystem.GetLabel(def.Liquor, false),
						LabelColor,
						false,
						false
					);
			...
			AddHtmlLocalized(
				...
				20,
				1150735,
				String.Format("#{0}", DistillationSystem.GetLabel(liquor, false)),
				LabelColor,
				false,
				false
				...
				else
				{
					...
					AddHtmlLocalized(
						...
						String.Format(
							"#{0}\t{1}",
							m_Def.Labels[i],
							String.Format("{0}/{1}", amount.ToString(), total.ToString())
						),
						LabelColor,
						false,
						false
			...
			AddHtmlLocalized(
				...
				20,
				1150733,
				String.Format("Label\t{0}", m_Context.Label == null ? "None" : m_Context.Label),
				LabelColor,
				false,
				false
			); // ~1_NAME~ : ~2_NUMBER~
						...
						else
						{
							...
							if (HasTotal(from, perc) && barrel != null)
							{
								...
								if (chance > Utility.Random(100))
								{
									...
									barrel.BeginDistillation(
										m_Def.Liquor,
										m_Def.MaturationDuration,
										m_Context.Label,
										m_Context.MakeStrong,
										marker
									);
								}
								else
