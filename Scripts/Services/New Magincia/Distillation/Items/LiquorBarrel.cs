...
namespace Server.Items
{
	public class LiquorBarrel : Item, ICraftable
	{
		...
		public Liquor Liquor
		{
			...
			set
			{
				BeginDistillation(value);
				InvalidateProperties();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime MaturationBegin
		{
			get { return m_MaturationBegin; }
			set
			{
				m_MaturationBegin = value;
				InvalidateProperties();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public TimeSpan MutrationDuration
		{
			get { return m_MaturationDuration; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Mobile Distiller
		{
			get { return m_Distiller; }
			set
			{
				m_Distiller = value;
				InvalidateProperties();
		...
		public virtual bool IsMature
		{
			get
			{
				return m_Liquor != Liquor.None
					&& (
						m_MaturationDuration == TimeSpan.MinValue
						|| m_MaturationBegin + m_MaturationDuration < DateTime.UtcNow
					);
			}
		}
		...
		public bool IsEmpty
		{
			get { return m_Liquor == Liquor.None; }
		}

		public override int LabelNumber
		{
			get { return m_UsesRemaining == 0 || m_Liquor == Liquor.None ? 1150816 : 1150807; }
		} // liquor barrel
					...
					if (DateTime.UtcNow < m_MaturationBegin + m_MaturationDuration)
					{
						...
						if (remaining.TotalDays > 0)
							from.SendLocalizedMessage(
								1150814,
								String.Format("{0}\t{1}", remaining.Days.ToString(), remaining.Hours.ToString())
							);
						else
