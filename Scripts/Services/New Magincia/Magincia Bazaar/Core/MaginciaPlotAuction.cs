...
namespace Server.Engines.NewMagincia
{
	...
	public class MaginciaPlotAuction
	{
		...
		public Dictionary<Mobile, BidEntry> Auctioners
		{
			get { return m_Auctioners; }
		}
		...
		public MaginciaPlotAuction(MaginciaBazaarPlot plot)
			: this(plot, MaginciaBazaar.GetShortAuctionTime) { }
		...
		public void EndAuction()
		{
			...
			if (
				m_Plot.Owner != null
				&& MaginciaBazaar.Reserve.ContainsKey(m_Plot.Owner)
				&& MaginciaBazaar.Instance != null
				&& !MaginciaBazaar.Instance.HasActiveBid(m_Plot.Owner)
			)
			{
				...
				if (matching >= highest)
				{
					...
					MaginciaLottoSystem.SendMessageTo(
						...
						new NewMaginciaMessage(
							null,
							new TextDefinition(1150427),
							String.Format(
								"@{0}@{1}@{2}@{3}@{4}",
								m_Plot.PlotDef.ID,
								m_Plot.PlotDef.Map.ToString(),
								highest.ToString("N0"),
								matching.ToString("N0"),
								newreserve.ToString("N0")
							)
						)
					);
				}
			...
			else if (m_Plot.Owner != null)
			{
				...
				MaginciaLottoSystem.SendMessageTo(
					...
					new NewMaginciaMessage(
						null,
						new TextDefinition(1150430),
						String.Format("@{0}@{1}", m_Plot.PlotDef.ID, m_Plot.PlotDef.Map.ToString())
					)
				);
			}
