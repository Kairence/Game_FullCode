...
namespace Server.Engines.NewMagincia
{
	...
	public class MaginciaBazaarPlot
	{
			...
			set
			{
				...
				if (m_Merchant != null)
				{
					...
					if (m_PlotMulti != null && m_PlotMulti.Fillers.Count > 0)
					{
						...
						p.Z =
							m_PlotMulti.Fillers[0].Z
							+ TileData.ItemTable[m_PlotMulti.Fillers[0].ItemID & TileData.MaxItemValue].CalcHeight;
					}
		...
		public bool Active
		{
			get { return MaginciaBazaar.IsActivePlot(this); }
		}
		...
		public void AddTempMulti(int idx1, int idx2)
		{
			...
			if (idx1 == 0)
			{
				...
				{
					case 0:
						multi = new CommodityStyle1();
						break;
					case 1:
						multi = new CommodityStyle2();
						break;
					case 2:
						multi = new CommodityStyle3();
						break;
				}
			}
			else
		...
		private class InternalTimer : Timer
		{
			...
			public InternalTimer(MaginciaBazaarPlot plot)
		...
		public bool TrySetShopName(Mobile from, string text)
		{
			if (
				text == null
				|| !Server.Guilds.BaseGuildGump.CheckProfanity(text)
				|| text.Length == 0
				|| text.Length > 40
			)
				return false;
	...
	public class PlotDef
	{
		...
		{
			get { return new Point3D(m_Location.X + 1, m_Location.Y - 2, m_Location.Z); }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public Point3D MultiLocation
		{
			get { return new Point3D(m_Location.X, m_Location.Y, m_Location.Z + 2); }
		}
