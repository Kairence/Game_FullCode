...
namespace Server.Engines.NewMagincia
{
	public class MaginciaBazaar : Item
	{
		...
		public static TimeSpan GetShortAuctionTime
		{
			get { return TimeSpan.FromMinutes(Utility.RandomMinMax(690, 750)); }
		}
		public static TimeSpan GetLongAuctionTime
		{
			get { return TimeSpan.FromHours(Utility.RandomMinMax(168, 180)); }
		}

		private static MaginciaBazaar m_Instance;
		public static MaginciaBazaar Instance
		{
		...
		public static List<MaginciaBazaarPlot> Plots
		{
			get { return m_Plots; }
		}

		private static Dictionary<Mobile, BidEntry> m_NextAvailable = new Dictionary<Mobile, BidEntry>();
		public static Dictionary<Mobile, BidEntry> NextAvailable
		{
			get { return m_NextAvailable; }
		}

		private static Dictionary<Mobile, int> m_Reserve = new Dictionary<Mobile, int>();
		public static Dictionary<Mobile, int> Reserve
		{
			...
			foreach (KeyValuePair<Mobile, StorageEntry> kvp in m_WarehouseStorage)
			{
				...
				if (entry.Expires < DateTime.UtcNow)
				{
					...
					if (stabled)
					{
						...
						{
							message =
								"Because your pets remained in storage for more than a full week, one or all of them have been placed in your stables. "
						...
						MaginciaLottoSystem.SendMessageTo(
							m,
							new NewMaginciaMessage(new TextDefinition(1150676), message, null)
						);
					}
					else if (deleted)
					{
						...
						MaginciaLottoSystem.SendMessageTo(
							m,
							new NewMaginciaMessage(new TextDefinition(1150676), new TextDefinition(1150673), null)
						);
					}
				}
			}
				...
				if (current == null && winner != plot.Owner)
				{
					...
					MaginciaLottoSystem.SendMessageTo(
						...
						new NewMaginciaMessage(
							null,
							new TextDefinition(1150426),
							String.Format(
								"{0}\t{1}\t{2}",
								plot.PlotDef.ID,
								plot.PlotDef.Map.ToString(),
								highest.ToString("###,###,###")
							)
						)
					);
		...
		public void AddInventoryToWarehouse(Mobile owner, BaseBazaarBroker broker)
		{
			...
			if (entry != null)
			{
				...
				MaginciaLottoSystem.SendMessageTo(
					owner,
					new NewMaginciaMessage(1150676, new TextDefinition(1150674), null)
				);
			}
		}
		...
		public MaginciaBazaar(Serial serial)
			: base(serial) { }
