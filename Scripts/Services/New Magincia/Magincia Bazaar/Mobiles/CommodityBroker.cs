...
namespace Server.Engines.NewMagincia
{
	public class CommodityBroker : BaseBazaarBroker
	{
		...
		public List<CommodityBrokerEntry> CommodityEntries
		{
			get { return m_CommodityEntries; }
		}

		public static readonly int MaxEntries = 50;

		public CommodityBroker(MaginciaBazaarPlot plot)
			: base(plot) { }
			...
			if (amount > 0)
				from.SendLocalizedMessage(
					...
					String.Format(
						...
						amount.ToString(),
						entry.Label,
						Plot.ShopName == null ? "an unnamed shop" : Plot.ShopName
					)
				); // You have added ~1_QUANTITY~ units of ~2_ITEMNAME~ to the inventory of "~3_SHOPNAME~"
			...
			if (Plot != null && from == Plot.Owner)
				from.SendLocalizedMessage(
					...
					String.Format(
						...
						amount.ToString(),
						entry.Label,
						Plot.ShopName != null ? Plot.ShopName : "a shop with no name"
					)
				); // You have removed ~1_QUANTITY~ units of ~2_ITEMNAME~ from the inventory of "~3_SHOPNAME~"
		}
		...
		public void TryBuyCommodity(Mobile from, CommodityBrokerEntry entry, int amount)
		{
			...
			if (Banker.Withdraw(from, totalCost, true))
			{
				from.SendLocalizedMessage(
					1150643,
					String.Format("{0}\t#{1}", amount.ToString("###,###,###"), entry.Label)
				); // A commodity deed worth ~1_AMOUNT~ ~2_ITEM~ has been placed in your backpack.
		...
		private void TakeCommodities(Container c, Type type, ref int amount)
		{
			...
			foreach (Item item in items)
			{
				...
				if (
					commodityDeed != null
					&& commodityDeed.Commodity != null
					&& commodityDeed.Commodity.GetType() == type
				)
				{
		...
		public int GetCommodityType(Container c, Type type)
		{
			...
			{
				if (
					item is CommodityDeed
					&& ((CommodityDeed)item).Commodity != null
					&& ((CommodityDeed)item).Commodity.GetType() == type
				)
		...
		public CommodityBroker(Serial serial)
			: base(serial) { }
