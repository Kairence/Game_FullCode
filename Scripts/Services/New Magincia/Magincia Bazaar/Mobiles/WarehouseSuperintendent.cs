...
namespace Server.Engines.NewMagincia
{
	public class WarehouseSuperintendent : BaseCreature
	{
		...
		[Constructable]
		public WarehouseSuperintendent()
		...
		public void TryTransfer(Mobile from, StorageEntry entry)
		{
			...
			if (fees < 0)
			{
				...
				SayTo(
					from,
					String.Format(
						"It looks like you owe {0}gp as back fees. How much would you like to pay now?",
						owed.ToString("###,###,###")
					)
				);
				...
				if (Banker.Withdraw(from, toDeduct))
				{
					...
					else
					{
						SayTo(
							from,
							String.Format(
								"Thank you! You have a remaining balance of {0}gp as backfees!",
								newAmount * -1
							)
						);
					}
		...
		public WarehouseSuperintendent(Serial serial)
			: base(serial) { }
