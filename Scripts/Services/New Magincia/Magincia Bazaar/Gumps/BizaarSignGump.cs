...
namespace Server.Engines.NewMagincia
{
	public class StallLeasingGump : BaseBazaarGump
	{
			...
			: base(520, 520)
		{
			...
			AddHtmlLocalized(
				...
				20,
				1150543,
				((int)MaginciaBazaar.GetLongAuctionTime.TotalDays).ToString(),
				BlueColor16,
				false,
				false
			); // ~1_DAYS~ Days
	...
	public class MyStallLeaseGump : BaseBazaarGump
	{
			...
			: base(520, 520)
		{
			...
			AddHtmlLocalized(
				...
				18,
				1150686,
				m_Plot.Merchant == null ? OrangeColor16 : GrayColor16,
				false,
				false
			); // HIRE ANIMAL BROKER
			...
			switch (info.ButtonID)
			{
				...
				case 2:
					...
					else
					{
						from.SendGump(new HireBrokerGump(from, m_Plot, m_Commodity));
						from.SendMessage(
							"The current preview has timed out. Preview the stall style again, and select confirm."
						);
					}
					break;
				case 3:
	...
	public class StallBidGump : BaseBazaarGump
	{
			...
			: base(520, 520)
		{
			...
			AddHtmlLocalized(
				...
				1150568,
				m_Plot.PlotDef.ID,
				isOwner ? GrayColor16 : BlueColor16,
				false,
				false
			); // BID ON THIS STALL (Stall ~1_STALLID~)
			if (!isOwner)
				...
				case 2: // BID ON THIS STALL
					{
						...
						bool hasBiddingPlot =
							MaginciaBazaar.GetBiddingPlot(from) != null
							|| MaginciaBazaar.NextAvailable.ContainsKey(from);
	...
	public class ConfirmBidGump : BaseBazaarGump
	{
		...
		public ConfirmBidGump(
			Mobile from,
			MaginciaBazaarPlot actualPlot,
			MaginciaBazaarPlot newPlot,
			int newBid,
			bool retract
		)
			: base(520, 520)
	...
	public class BazaarInformationGump : BaseBazaarGump
	{
		...
		public BazaarInformationGump(int title, int message)
			: this(title, message, -1, null) { }

		public BazaarInformationGump(int title, int message, Gump g)
			: this(title, message, -1, g) { }
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			if (info.ButtonID == 1)
			{
				...
				if (from.InRange(m_Sign.Location, 2))
				{
					...
					{
						from.SendMessage(
							"You must have {0} gold in your bankbox to purchase a recall rune to this plot.",
							PlotSign.RuneCost
						);
						return;
					}
