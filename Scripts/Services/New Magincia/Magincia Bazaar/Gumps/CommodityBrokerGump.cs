...
namespace Server.Engines.NewMagincia
{
	public class CommodityBrokerGump : BaseBazaarGump
	{
			...
			: base(520, 520)
		{
			...
			AddTextEntry(
				...
				LabelHueBlue,
				0,
				m_Broker.Plot.ShopName == null ? "" : m_Broker.Plot.ShopName
			);
		...
		public class InternalTarget : Target
		{
			...
			public InternalTarget(CommodityBroker broker)
				: this(broker, false) { }

			public InternalTarget(CommodityBroker broker, bool haspicked)
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is Item && (targeted is ICommodity || targeted is CommodityDeed))
				{
					...
					if (item.IsChildOf(from.Backpack))
					{
						...
						{
							from.SendMessage(
								"You can only have a maximum of {0} commodities on your merchant at a time.",
								CommodityBroker.MaxEntries.ToString()
							);
						}
					}
					else
	...
	public class SetPricesAndLimitsGump : BaseBazaarGump
	{
		...
		public SetPricesAndLimitsGump(CommodityBroker broker)
			: this(broker, -1, 0) { }
			...
			: base(520, 520)
		{
			...
			for (
				int i = start;
				i < broker.CommodityEntries.Count
					&& /*index <= perPage * (m_Page + 1) &&*/
					count <= perPage;
				i++
			)
			{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			{
				from.SendGump(
					new BazaarInformationGump(0, 1150644, new SetPricesAndLimitsGump(m_Broker, m_Index, m_Page))
				);
				return;
			}
	...
	public class ViewInventoryGump : BaseBazaarGump
	{
		...
		public ViewInventoryGump(CommodityBroker broker)
			: this(broker, -1) { }

		public ViewInventoryGump(CommodityBroker broker, int index)
			: this(broker, index, 0) { }
	...
	public class CommodityInventoryGump : BaseBazaarGump
	{
		...
		public CommodityInventoryGump(CommodityBroker broker)
			: this(broker, -1, true, 0, 0) { }
				...
				if (m_Entries.Count != 0)
				{
					...
					)
					{
						...
						AddHtmlLocalized(
							...
							20,
							1114514,
							String.Format("#{0}", entry.Label),
							OrangeColor16,
							false,
							false
						); // <DIV ALIGN=RIGHT>~1_TOKEN~</DIV>
						...
						if (entry.BuyPricePer > 0)
						{
							AddHtml(
								...
								54,
								20,
								Color(AlignRight(FormatAmt(entry.BuyPricePer)), OrangeColor),
								false,
								false
							);
						}
						...
						if (entry.BuyPricePer > 0 && entry.BuyLimit > 0)
						{
							AddHtml(
								...
								109,
								20,
								(Color(AlignRight(FormatAmt(sellLimit)), OrangeColor)),
								false,
								false
							);
						}
				...
				case 500: // TRADE
				{
					...
					if (m_Index >= 0 && m_Index < m_Entries.Count)
					{
						...
						if (amount > 0)
						{
							...
							{
								from.SendGump(
									new ConfirmBuyCommodityGump(
										m_Broker,
										amount,
										entry,
										true,
										new CommodityInventoryGump(m_Broker, m_Index, m_Buy, m_Page, 0)
									)
								);
								return;
							}
	...
	public class ConfirmRemoveEntryGump : BaseConfirmGump
	{
		...
		{
			get { return "Remove Commodity Confirmation"; }
		}
		public override string LabelString
		{
			get
			{
				return "Are you sure you want to remove this entry from your commodity broker? Any unused stock will be placed in your bankbox.";
			}
		}
