...
namespace Server.Engines.NewMagincia
{
	public class PetBrokerGump : BaseBazaarGump
	{
		...
		public PetBrokerGump(PetBroker broker, Mobile from)
		{
			...
			AddHtml(
				...
				173,
				18,
				Color(FormatBrokerName(String.Format("Proprietor: {0}", broker.Name)), BlueColor),
				false,
				false
			);
	...
	public class SelectPetsGump : BaseBazaarGump
	{
		...
		public SelectPetsGump(PetBroker broker, Mobile from)
			: this(broker, from, -1) { }
		...
		public SelectPetsGump(PetBroker broker, Mobile from, int index)
		{
			...
			for (int i = 0; i < m_List.Count; i++)
			{
				...
				AddHtmlLocalized(
					...
					18,
					1150340,
					String.Format("{0}\t{1}", bc.Name, PetBrokerEntry.GetOriginalName(bc)),
					col,
					false,
					false
				); // ~1_NAME~ (~2_type~)
					...
					if (cost > 0)
					{
						...
						if (m_Broker.TryAddEntry(bc, from, cost))
						{
							...
							from.SendLocalizedMessage(
								...
								String.Format(
									"{0}\t{1}\t{2}\t{3}",
									PetBrokerEntry.GetOriginalName(bc),
									bc.Name,
									m_Broker.Name,
									cost
								)
							); // Your pet ~1_TYPE~ named ~2_NAME~ has been transferred to the inventory of your animal broker named ~3_SHOP~ with an asking price of ~4_PRICE~.
						}
					}
					else
	...
	public class RemovePetsGump : BaseBazaarGump
	{
		...
		public RemovePetsGump(PetBroker broker, Mobile from)
			: this(broker, from, -1) { }
	...
	public class SetPetPricesGump : BaseBazaarGump
	{
		...
		public SetPetPricesGump(PetBroker broker)
			: this(broker, -1) { }
			...
			if (m_Entries.Count != 0)
			{
				...
				for (int i = 0; i < m_Entries.Count; i++)
				{
					...
					AddHtml(
						...
						104,
						20,
						Color(AlignRight(FormatAmt(entry.SalePrice)), GreenColor),
						false,
						false
					);
			...
			else if (info.ButtonID < 300) // VIEW
			{
				...
				if (id >= 0 && id < m_Entries.Count)
				{
					...
					if (
						entry != null
						&& entry.Pet != null
						&& m_Broker.BrokerEntries.Contains(entry)
						&& entry.Pet.IsStabled
					)
					{
