...
namespace Server.Engines.Doom
{
	...
	public class GaryRegion : BaseRegion
	{
		...
		private static Rectangle2D[] _Bounds = { new Rectangle2D(388, 3, 16, 12) };
		...
		{
			get { return TimeSpan.FromMinutes(Utility.RandomMinMax(12, 15)); }
		}
		...
		{
			if (
				NextRoll < DateTime.UtcNow /*&& (Spawn == null || !Spawn.Alive)*/
				&& this.GetEnumeratedMobiles().OfType<PlayerMobile>().Where(p => p.Alive).Count() > 0
			)
			{
				DoRoll();
		...
		private DisplayStatue[] GetStatues()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				{
					DisplayStatue s = this.GetEnumeratedItems()
						.OfType<DisplayStatue>()
						.FirstOrDefault(st => Array.IndexOf(Statues, st) == -1);
	...
	public class Sapphired20 : Item
	{
		...
		public void Roll(int roll)
		{
			PublicOverheadMessage(
				...
				0x3B2,
				false,
				String.Format("- {0} -", (roll + 1).ToString())
			);
		}
