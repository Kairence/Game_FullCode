	...
	public class GauntletSpawner : Item
	{
		...
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(GenGauntlet_OnCommand)
			);
			CommandSystem.Register(
				"DeleteGauntlet",
				AccessLevel.Administrator,
				new CommandEventHandler(DeleteGauntlet_OnCommand)
			);
		}

		private static bool FindObject<T>(Map map, Point3D p, int range, out T value)
			where T : IEntity
		{
			...
			where T : IEntity
		{
			...
			foreach (
				var o in (predicate != null ? e.OfType<T>().Where(predicate) : e.OfType<T>()).Where(o => o != null)
			)
			{
				value = o;
				any = true;
		...
		public static void GenGauntlet_OnCommand(CommandEventArgs e)
		{
			...
			GauntletSpawner sp1 = CreateSpawner(
				"DarknightCreeper",
				491,
				456,
				473,
		...
		public virtual int ComputeSpawnCount()
		{
			...
			if (map != null)
			{
				...
				if (reg != null)
					playerCount = reg.GetEnumeratedMobiles()
						.Where(m => m is PlayerMobile && m.AccessLevel == AccessLevel.Player)
						.Count();
			}
