...
namespace Server.Engines.Doom
{
	public class LeverPuzzleController : Item
	{
		...
		public static int[][] MsgParams = { new int[] { 0x66d, 3 }, new int[] { 0x66d, 3 }, new int[] { 0x34, 3 } };
		...
		public static int[][] TA =
		{
			...
			new int[] { 323, 72 },
			new int[] { 468, 92, -1 },
			new int[] { 0x181D, 0x482 },
			/* 3D coord, itemid+hue for L.R. teles */new int[] { 469, 92, -1 },
			new int[] { 0x1821, 0x3fd },
			new int[] { 470, 92, -1 },
			new int[] { 0x1825, 0x66d },
			new int[] { 319, 70, 18 },
			new int[] { 0x12d8 },
			/* 3D coord, itemid for statues */new int[] { 329, 60, 18 },
			new int[] { 0x12d9 },
			new int[] { 469, 96, 6 },
		...
		public UInt16 MyKey
		{
			get { return this.m_MyKey; }
			set { this.m_MyKey = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public UInt16 TheirKey
		{
			get { return this.m_TheirKey; }
			set { this.m_TheirKey = value; }
		}
		...
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(GenLampPuzzle_OnCommand)
			);
			CommandSystem.Register(
				"LampPuzzleDelete",
				AccessLevel.Administrator,
				new CommandEventHandler(LampPuzzleDelete_OnCommand)
			);
		}
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					else if (this.Count == 3)
					{
						...
						for (int i = 0; i < j; i++)
						{
							IEntity m_IEntity = new Entity(
								Serial.Zero,
								RandomPointIn(this.m_Player.Location, 10),
								this.m_Player.Map
							);
		...
		public class LampRoomTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					{
						SendLocationEffect(
							RandomPointIn(lr_Rect, -1),
							0x36B0,
							Utility.Random(150, 200),
							0,
							PA[this.level][2]
						);
					}
