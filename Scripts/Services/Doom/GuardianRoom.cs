...
namespace Server.Engines.Doom
{
	public class DoomGuardianRegion : DungeonRegion
	{
		...
		{
			get { return NextActivate < DateTime.UtcNow; }
		}
		...
		public void Activate(Mobile m)
		{
			...
			int count = 0;
			foreach (
				var mob in this.GetEnumeratedMobiles()
					.Where(mob =>
						mob is PlayerMobile
						|| (mob is BaseCreature && ((BaseCreature)mob).GetMaster() != null && !mob.IsDeadBondedPet)
					)
			)
			{
				...
				else if (NextGas < DateTime.UtcNow)
				{
					for (int i = 0; i < Utility.RandomMinMax(5, 12); i++)
					{
						...
						Effects.SendLocationEffect(
							p,
							Map.Malas,
							Utility.RandomList(0x113C, 0x1147, 0x11A8) - 2,
							16,
							3,
							0,
							0
					...
					foreach (
						var m in Region
							...
							.Where(m =>
								m is PlayerMobile && m.Alive && m.AccessLevel == AccessLevel.Player && m.Poison == null
							)
					)
					{
