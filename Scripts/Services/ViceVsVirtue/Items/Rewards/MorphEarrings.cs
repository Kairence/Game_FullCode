	...
	public class MorphEarrings : GoldEarrings
	{
		...
		private void ValidateEquipment(Mobile m)
		{
			...
			foreach (Item item in list)
			{
				...
				if (
					item is BaseArmor
					&& ((BaseArmor)item).RequiredRace != null
					&& ((BaseArmor)item).RequiredRace != race
				)
					drop = true;
				else if (
					item is BaseWeapon
					&& ((BaseWeapon)item).RequiredRace != null
					&& ((BaseWeapon)item).RequiredRace != race
				)
					drop = true;
				else if (
					item is BaseJewel
					&& ((BaseJewel)item).RequiredRace != null
					&& ((BaseJewel)item).RequiredRace != race
				)
					drop = true;
				else if (
					item is BaseClothing
					&& ((BaseClothing)item).RequiredRace != null
					&& ((BaseClothing)item).RequiredRace != race
				)
					drop = true;
