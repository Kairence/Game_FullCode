	...
	public class ManaSpike : Item
	{
							...
							foreach (Mobile mobile in battle.Region.GetEnumeratedMobiles())
							{
								if (ViceVsVirtueSystem.IsEnemy(mobile, m))
								{
									...
									mobile.PrivateOverheadMessage(
										...
										1155498,
										mobile.NetState
									); // *Your body convulses as energy surges through it!*
								}
							}
		...
		public static bool UnderEffects(Mobile m)
		{
			...
			if (
				battle != null
				&& battle.OnGoing
				&& battle.ManaSpikeEndEffects != DateTime.MinValue
				&& battle.ManaSpikeEndEffects > DateTime.UtcNow
			)
			{
		...
		public ManaSpike(Serial serial)
			: base(serial) { }
