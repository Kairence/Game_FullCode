	...
	public class VvVSteedStatuette : BaseImprisonedMobile
	{
		...
		public override BaseCreature Summon
		{
			get
			{
				...
				{
					default:
					case SteedType.Ostard:
						return new VvVMount("a war ostard", 0xDA, 0x3EA4, this.Hue);
					case SteedType.WarHorse:
						return new VvVMount("a war horse", 0xE2, 0x3EA0, this.Hue);
				}
			}
		}
		...
		public VvVSteedStatuette(Serial serial)
			: base(serial) { }
					...
					Timer.DelayCall(
						...
						() =>
						{
							if (!Deleted && ControlMaster != null)
								ControlMaster.PrivateOverheadMessage(
									Server.Network.MessageType.Regular,
									1154,
									cliloc,
									ControlMaster.NetState
								);
						}
					);
				}
		...
		public void GoPoof()
		{
			...
			if (ControlMaster != null && ControlMaster.NetState != null)
				ControlMaster.PrivateOverheadMessage(
					...
					1155550,
					ControlMaster.NetState
				); // *Your steed has depleted it's battle readiness!*
		...
		public static void Initialize()
		{
			Timer.DelayCall(
				...
				() =>
				{
					...
					{
						if (
							(s.Map != Map.Internal || (s.Rider != null && s.Rider.Map != Map.Internal))
							&& s.NextReadinessAtrophy < DateTime.UtcNow
						)
						{
							s.BattleReadiness--;
