...
namespace Server.Engines.VvV
{
	...
	public class ViceVsVirtueSystem : PointsSystem
	{
		...
		{
			get { return new TextDefinition("Vice Vs Virtue"); }
		}
		public override PointsType Loyalty
		{
			get { return PointsType.ViceVsVirtue; }
		}
		...
		public void SendVvVMessageTo(Mobile m, int cliloc, string args = "")
		{
			m.SendLocalizedMessage(
				...
				"[Guild][VvV] ",
				args,
				m is PlayerMobile ? ((PlayerMobile)m).GuildMessageHue : 0x34
			);
		}
		...
		public static bool IsAllied(Mobile a, Mobile b)
		{
			...
			if (
				tempA != null
				&& (tempA.Friendly == b || (tempA.FriendlyGuild != null && tempA.FriendlyGuild == guildB))
			)
			{
				return true;
			}
			...
			if (
				tempB != null
				&& (tempB.Friendly == a || (tempB.FriendlyGuild != null && tempB.FriendlyGuild == guildA))
			)
			{
				return true;
			}
		...
		public static void CheckBeneficial(Mobile from, Mobile target)
		{
			...
			{
				if (
					target.Aggressors.Any(info => IsVvV(info.Attacker))
		...
		public static void AddTempCombatantTimer()
		{
			...
			{
				TempCombatantTimer = Timer.DelayCall(
		...
		public static TemporaryCombatant GetTempCombatant(Mobile from, Mobile to)
		{
			foreach (var combatant in TempCombatants.Where(c => c.From == from))
			{
				...
				if (
					combatant.Friendly == to
					|| (combatant.FriendlyGuild != null && combatant.FriendlyGuild == from.Guild as Guild)
				)
					return combatant;
			}
		...
		public static int GetCityLocalization(VvVCity city)
		{
			switch (city)
			{
				default:
					return 0;
				case VvVCity.Moonglow:
					return 1011344;
				case VvVCity.Britain:
					return 1011028;
				case VvVCity.Jhelom:
					return 1011343;
				case VvVCity.Yew:
				...
				case VvVCity.Minoc:
					return 1011031;
				case VvVCity.Trinsic:
					return 1011029;
				case VvVCity.SkaraBrae:
					return 1011347;
				case VvVCity.Ocllo:
					return 1076027;
			}
		...
		private static string _EnhancedRulesNotice = String.Format(
			"Notice: The Vice Vs Virtue system has recently enabled enhanced rules. To avoid any issues and "
				...
				+ "- VvV players in the battle region during a battle will be subject to combat heat travel restrictions.",
			VvVBattle.Penalty * 100
		);
	...
	public class VvVPlayerEntry : PointsEntry
	{
		...
		public Guild Guild
		{
			get { return Player != null ? Player.Guild as Guild : null; }
		}
		...
		public bool Resigning
		{
			get { return ResignExpiration > DateTime.MinValue; }
		}
	...
	public class TemporaryCombatant
	{
		...
		public bool Expired
		{
			get { return StartTime + TempCombatTime < DateTime.UtcNow; }
		}
