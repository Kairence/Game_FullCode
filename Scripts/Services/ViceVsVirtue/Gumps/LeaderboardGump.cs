...
namespace Server.Engines.VvV
{
	...
	public class ViceVsVirtueLeaderboardGump : Gump
	{
		...
		public ViceVsVirtueLeaderboardGump(PlayerMobile pm, Filter filter = Filter.Score)
			: base(50, 50)
		{
			User = pm;
			Filter = filter;
			...
			AddHtmlLocalized(
				...
				20,
				1114977,
				Filter == Filter.Score ? Server.Engines.Quests.BaseQuestGump.C32216(0x00FA9A) : 0xFFFF,
				false,
				false
			...
			AddHtmlLocalized(
				...
				20,
				1114975,
				Filter == Filter.Kills ? Server.Engines.Quests.BaseQuestGump.C32216(0x00FA9A) : 0xFFFF,
				false,
				false
			); // <DIV ALIGN=RIGHT>Kills:</DIV>
			...
			AddHtmlLocalized(
				...
				20,
				1155575,
				Filter == Filter.ReturnedSigils ? Server.Engines.Quests.BaseQuestGump.C32216(0x00FA9A) : 0xFFFF,
				false,
				false
			); // <DIV ALIGN=RIGHT>Returned Sigil:</DIV>
			...
			List<VvVPlayerEntry> list = new List<VvVPlayerEntry>(
				ViceVsVirtueSystem.Instance.PlayerTable.OfType<VvVPlayerEntry>()
			);
			...
			switch (Filter)
			{
				...
				case Filter.Score:
					...
					break;
				case Filter.Kills:
					list = list.OrderBy(e => -e.Kills).ToList();
					break;
				case Filter.ReturnedSigils:
					list = list.OrderBy(e => -e.ReturnedSigils).ToList();
					break;
			}
			...
			AddHtmlLocalized(
				...
				20,
				1153561,
				String.Format("{0}\t{1}", page.ToString(), pages.ToString()),
				0xFFFF,
				false,
				false
			); // Page ~1_CUR~ of ~2_MAX~
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				AddHtml(
					...
					100,
					20,
					Filter == Filter.Score ? RightGreen(entry.Score.ToString()) : RightGray(entry.Score.ToString()),
					false,
					false
				);
				AddHtml(
					...
					55,
					20,
					Filter == Filter.Kills ? RightGreen(entry.Kills.ToString()) : RightGray(entry.Kills.ToString()),
					false,
					false
				);
				...
				AddHtml(
					...
					90,
					20,
					Filter == Filter.ReturnedSigils
						? RightGreen(entry.ReturnedSigils.ToString())
						: RightGray(entry.ReturnedSigils.ToString()),
					false,
					false
				);
