...
namespace Server.Items
{
	public class PlunderBeaconAddon : BaseAddon
	{
		...
		{
			get { return Crew.Any(c => c.Alive && !c.Deleted); }
		}
		public bool BeaconVulnerable
		{
			get { return !CannonsOperational; }
		}
		...
		private void AddCannon(Direction d, CannonPower type, int xOffset, int yOffset, int zOffset, bool oper = true)
		{
			...
			switch (type)
			{
				...
				case CannonPower.Light:
					cannon = new MannedCulverin(mob, d);
					break;
				case CannonPower.Heavy:
					cannon = new MannedCarronade(mob, d);
					break;
				case CannonPower.Massive:
					cannon = new MannedBlundercannon(mob, d);
					break;
			}
			...
			if (mob != null)
			{
				...
				switch (d)
				{
					...
					case Direction.South:
						offset = new Point3D(0, -1, 0);
						break;
					case Direction.North:
						offset = new Point3D(0, 1, 0);
						break;
					case Direction.West:
						offset = new Point3D(1, 0, 0);
						break;
					case Direction.East:
						offset = new Point3D(-1, 0, 0);
						break;
		...
		public void OnTick()
		{
			...
			{
				foreach (
					var cannon in Cannons.Where(c =>
						c != null
						&& !c.Deleted
						&& (c.CanFireUnmanned || (c.Operator != null && !c.Operator.Deleted && c.Operator.Alive))
					)
				)
				{
					cannon.Scan(true);
		...
		private void SpawnHelper(bool initial)
		{
			...
			BaseCreature creature =
				Activator.CreateInstance(_SpawnTypes[Utility.Random(_SpawnTypes.Length)]) as BaseCreature;

			for (int i = 0; i < 50; i++)
			{
				var spawnLoc = new Point3D(
					Utility.RandomMinMax(p.X - range, p.X + range),
					Utility.RandomMinMax(p.Y - range, p.Y + range),
					-5
				);
		...
		private Type[] _SpawnTypes = { typeof(WaterElemental), typeof(SeaSerpent), typeof(DeepSeaSerpent) };
		...
		public PlunderBeaconAddon(Serial serial)
			: base(serial) { }
