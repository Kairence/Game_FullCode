...
namespace Server.Engines.Khaldun
{
	public class TrapDoor : Item, IRevealableItem, IForensicTarget
	{
		...
		private bool CheckPrerequisite(GoingGumshoeQuest2 quest)
		{
			...
			{
				case "boreas":
					return quest.VisitedHeastone1; // brit
				case "moriens":
					return quest.VisitedHeastone2; // vesper
				case "carthax":
					return quest.VisitedHeastone3; // moonglow
				case "tenebrae":
					return quest.VisitedHeastone4; // yew
			}
		...
		private class TrapDoorPrompt : Prompt
		{
			...
			{
				if (
					Door.Destination == Point3D.Zero
					|| Door.DestinationMap == null
					|| Door.DestinationMap == Map.Internal
					|| String.IsNullOrEmpty(Door.Keyword)
				)
					return;
				...
				if (!string.IsNullOrEmpty(text) && text.Trim().ToLower() == Door.Keyword.ToLower())
				{
					...
					Effects.SendLocationParticles(
						EffectItem.Create(Door.Destination, Door.DestinationMap, EffectItem.DefaultDuration),
						0x3728,
						10,
						10,
						5023
