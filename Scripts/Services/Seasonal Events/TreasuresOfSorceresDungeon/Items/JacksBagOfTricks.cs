...
namespace Server.Items
{
			...
			if (DateTime.Now.Month == 10 || from.AccessLevel > AccessLevel.Player)
			{
				if (_NextUse < DateTime.UtcNow)
				{
					...
					else
					{
						...
						{
							case 0:
								item = new RancidReindeerMeat();
								break;
							case 1:
								item = new GlassyCandyCane();
								break;
							case 2:
								item = new NamedSeveredElfEars();
								break;
							case 3:
								item = new SuspiciousGiftBox();
								break;
							case 4:
								item = new InsultingDoll();
								break;
							case 5:
								item = new SpikedEggnog();
								break;
						}
					}
	...
	public class NamedSeveredElfEars : Item
	{
		...
		private string[] _Names = { "Alabaster Snowball", "Pepper Minstix", "Wunorse Openslae", "Sugarplum Mary" };
		...
		public override void OnDoubleClick(Mobile m)
		{
			if (IsChildOf(m.Backpack))
			{
				PrivateOverheadMessage(
					...
					0,
					Utility.RandomMinMax(1157642, 1157646),
					m.NetState
				);
			}
		}
