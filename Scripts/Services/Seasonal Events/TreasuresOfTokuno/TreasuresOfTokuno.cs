...
namespace Server.Misc
{
	...
	public class TreasuresOfTokuno : PointsSystem
	{
		...
		{
			get { return m_LesserArtifactsTotal; }
		}
		...
		public override void ProcessKill(Mobile victim, Mobile killer)
		{
			...
			if (
				...
				|| !CheckLocation(bc)
				|| !CheckLocation(pm)
				|| !killer.InRange(victim, 18)
				|| !pm.Alive
				|| bc.GivenSpecialArtifact
			)
				return;
				...
				try
				{
					i =
						Activator.CreateInstance(
							m_LesserArtifacts[(int)DropEra - 1][
								Utility.Random(m_LesserArtifacts[(int)DropEra - 1].Length)
							]
						) as Item;
				}
				catch
	...
	public class IharaSoko : BaseVendor
	{
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (m.Alive && m is PlayerMobile)
			{
				...
				if (
					m.Alive
					&& Math.Abs(this.Z - m.Z) < 16
					&& this.InRange(m, range)
					&& !this.InRange(oldLocation, range)
				)
				{
					...
					else
					{
						if (turnIns == 0)
							...
							this.SayTo(
								pm,
								1070981,
								String.Format("{0}\t{1}", turnIns, TreasuresOfTokuno.ItemsPerReward)
...
namespace Server.Gumps
{
	public class ToTRedeemGump : BaseImageTileButtonsGump
	{
		...
		public class PigmentsTileButtonInfo : ImageTileButtonInfo
		{
			...
			{
				get { return this.m_Pigment; }
				set { this.m_Pigment = value; }
			}
		...
		public ToTRedeemGump(Mobile collector, bool pigments)
			: base(
				...
				pigments
					? (ImageTileButtonInfo[])m_PigmentRewards[(int)TreasuresOfTokuno.RewardEra - 1]
					: (ImageTileButtonInfo[])m_NormalRewards[(int)TreasuresOfTokuno.RewardEra - 1]
			)
		{
			this.m_Collector = collector;
		}
		...
		public override void HandleButtonResponse(NetState sender, int adjustedButton, ImageTileButtonInfo buttonInfo)
		{
			...
			if (
				pm == null
				|| !pm.InRange(this.m_Collector.Location, 7)
				|| !(turnIns >= TreasuresOfTokuno.ItemsPerReward)
			)
				return;
			...
			if (pm.AddToBackpack(item))
			{
				...
				this.m_Collector.SayTo(
					pm,
					1070984,
					(item.Name == null || item.Name.Length <= 0) ? String.Format("#{0}", item.LabelNumber) : item.Name
