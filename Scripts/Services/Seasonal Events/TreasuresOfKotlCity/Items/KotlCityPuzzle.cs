...
namespace Server.Engines.TreasuresOfKotlCity
{
	public class KotlCityPuzzle : BaseAddon
	{
		...
		public int Next
		{
			get { return _Order == null || _Order.Count == 0 ? -1 : _Order[0]; }
		}
		...
		public bool Complete
		{
			...
			{
				foreach (
					var comp in Components
						.OfType<KotlCityPuzzleComponent>()
						.Where(c => (value && c.Active) || (!value && !c.Active))
				)
					comp.Active = !value;
		...
		public override void OnComponentUsed(AddonComponent component, Mobile from)
		{
			...
			if (comp != null && comp.Active)
			{
					...
					if (_Order.Count - 1 == _Index)
					{
						...
						if (KotlDoor.Instance != null)
						{
							...
							from.PrivateOverheadMessage(
								...
								1157019,
								from.NetState
							); // *You hear a low hum as the door to the Time Room unseals...*
				...
				else
				{
					...
					{
						component.PrivateOverheadMessage(
							Server.Network.MessageType.Regular,
							1154,
							1157031,
							from.NetState
						); // *Circuit Fault! Generating new circuit sequence!*
						Reset();
					}
				}
			}
