...
namespace Server.Engines.SeasonalEvents
{
	...
	public class KrampusEncounter
	{
		...
		{
			get { return SeasonalEventSystem.IsActive(EventType.KrampusEncounter); }
		}
		...
		public int Wave
		{
			get { return (int)Math.Max(1, (int)Math.Min(6, (double)TotalTradesComplete / 4.1)); }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool KrampusSpawning
		{
			get { return SpawnMap != null && SpawnLocation != Point3D.Zero; }
		}
		...
		public Type[] GetCreatureTypes(Mobile m, bool wet)
		{
			...
			else
			{
				...
				{
					if (
						(m.Map == Map.Trammel || (Siege.SiegeShard && m.Map == Map.Felucca))
						&& !SpellHelper.IsAnyT2A(m.Map, m.Location)
					)
					{
						SpawnKrampus(m);
			...
			foreach (var ns in NetState.Instances)
			{
				...
				if (mob != null && CityTradeSystem.HasTrade(mob))
				{
					mob.LocalOverheadMessage(
						...
						String.Format(
							"{0}\t{1}",
							WorldLocationInfo.GetLocationString(SpawnLocation, SpawnMap),
							Sextant.GetCoords(SpawnLocation, SpawnMap)
						)
					); // *You sense Krampus has been spotted near ~2_where~ at ~1_coords~!*
				}
			}
