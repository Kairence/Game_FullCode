...
namespace Server.Engines.VendorSearching
{
	public class VendorSearchGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			if (Criteria.EntryPrice)
			{
				...
				AddHtmlLocalized(
					...
					string.Format(
						"@{0}@{1}",
						Criteria.MinPrice.ToString("N0", CultureInfo.GetCultureInfo("en-US")),
						Criteria.MaxPrice.ToString("N0", CultureInfo.GetCultureInfo("en-US"))
					),
					CriteriaColor,
					false,
					false
				...
				if (cliloc > 0)
				{
					if (Criteria.Details[i].Attribute is SkillName)
					{
						AddHtmlLocalized(
							...
							20,
							1060451,
							string.Format("#{0}@{1}", cliloc, Criteria.Details[i].Value),
							CriteriaColor,
							false,
							false
						);
					...
					else
					{
						AddHtmlLocalized(
							...
							20,
							cliloc,
							string.Format("{0}", Criteria.Details[i].Value),
							CriteriaColor,
							false,
							false
						);
			...
			AddHtmlLocalized(
				...
				215,
				20,
				Criteria.SortBy == SortBy.LowToHigh ? 1154696 : 1154697,
				CriteriaColor,
				false,
				false
			);
			...
			AddHtmlLocalized(
				...
				206,
				20,
				Criteria.Auction ? 1159353 : 1159354,
				CriteriaColor,
				false,
				false
			);
			...
			SearchCriteriaCategory
				.AllCategories.OrderByDescending(x => x.PageID == 2)
				.ThenByDescending(x => x.PageID == 6)
				.ToList()
				.ForEach(x =>
				{
			...
			switch (info.ButtonID)
			{
				...
				case 1: // Search
				{
					...
					else
					{
						...
						var pollingTimer = new TaskPollingTimer<List<SearchItem>>(
							resultsTask,
							(results) =>
							{
				...
				default:
					...
					else
					{
						...
						var criteria = SearchCriteriaCategory
							.AllCategories.SelectMany(
								x => x.Criteria,
								(x, c) =>
									new
									{
						...
						if (valuetext != null)
							value = Math.Max(
								o is AosAttribute && (AosAttribute)o == AosAttribute.CastSpeed ? -1 : 0,
		...
		public Task<List<SearchItem>> FindVendorItemsAsync(Mobile m, SearchCriteria criteria)
		{
			...
			{
				return criteria.Auction
					? VendorSearch.DoSearchAuction(m, criteria)
					: VendorSearch.DoSearch(m, criteria);
			});
		}
	}
			...
			for (int i = start; i < start + PerPage && i < Items.Count; i++)
			{
				...
				if (Items[i].IsAuction)
					...
					AddHtmlLocalized(
						...
						102,
						72,
						Items[i].IsChild ? 1154598 : 1154645,
						string.Format("{0}", price <= 0 ? "0" : FormatPrice(price)),
						TextColor,
						false,
						false
					); // <center>~1_val~</center>
			...
			switch (info.ButtonID)
			{
				...
				default: // Buy Map
					...
					if (
						item != null
						&& (
							item.AuctionSafe != null && item.AuctionSafe.CheckAuctionItem(item.Item)
							|| item.Vendor != null && item.Vendor.GetVendorItem(item.Item) != null
						)
					)
					{
						if (_GivenTo == null)
		...
		public override void AddGumpLayout()
		{
			...
			if (VendorMap.SetLocation != Point3D.Zero && (VendorMap.Vendor != null || VendorMap.AuctionSafe != null))
			{
				...
				AddHtmlLocalized(
					...
					string.Format(
						"@{0}@{1}@{2}",
						VendorMap.TeleportCost.ToString(),
						VendorMap.Name()[0],
						VendorMap.DeleteDelayMinutes.ToString()
					),
					0x4E73,
					false,
					false
				); // Please select 'Accept' if you would like to pay ~1_cost~ gold to teleport to vendor ~2_name~.  For this price you will also be able to teleport back to this location within the next ~3_minutes~ minutes.
