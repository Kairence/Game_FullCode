...
namespace Server.Items
{
	public class VendorSearchMap : MapItem
	{
		...
		{
			get { return DeleteTime <= DateTime.UtcNow ? 0 : (int)(DeleteTime - DateTime.UtcNow).TotalMinutes; }
		}
		...
		public new string[] Name()
		{
			...
			if (IsAuction)
			{
				if (SearchItem != null)
				{
					...
					Shop =
						(SearchItem.LabelNumber != 0) ? string.Format("#{0}", SearchItem.LabelNumber) : SearchItem.Name;
				}
			}
			else
			{
		...
		public bool IsSale()
		{
			return SearchItem != null
				&& (
					AuctionSafe != null && AuctionSafe.CheckAuctionItem(SearchItem)
