...
namespace Server.Engines.VendorSearching
{
	public class VendorSearch
	{
		...
		public static List<SearchItem> DoSearchAuction(Mobile m, SearchCriteria criteria)
		{
			...
			bool excludefel =
				criteria.Details.FirstOrDefault(d => d.Attribute is Misc && (Misc)d.Attribute == Misc.ExcludeFel)
				!= null;

			foreach (
				Auction.Auction pv in Auction.Auction.Auctions.Where(pv =>
					pv.AuctionItem != null
			...
			switch (criteria.SortBy)
			{
				case SortBy.LowToHigh:
					list = list.OrderBy(vi => vi.Price).ToList();
					break;
		...
		public static List<SearchItem> DoSearch(Mobile m, SearchCriteria criteria)
		{
			...
			foreach (
				PlayerVendor pv in PlayerVendor.PlayerVendors.Where(pv =>
					pv.Map != Map.Internal
					&& pv.Map != null
					&& pv.Backpack != null
					&& pv.VendorSearch
		...
		public static bool CheckMatch(Item item, int price, SearchCriteria searchCriteria)
		{
			...
			if (!string.IsNullOrEmpty(searchCriteria.SearchName))
			{
				...
				if (
					!CheckKeyword(searchCriteria.SearchName, item)
			...
			foreach (SearchDetail detail in searchCriteria.Details)
			{
				...
				else if (o is AosWeaponAttribute)
				{
					...
					if ((AosWeaponAttribute)o == AosWeaponAttribute.MageWeapon)
					{
						if (
							attrs == null
							|| attrs[(AosWeaponAttribute)o] == 0
							|| attrs[(AosWeaponAttribute)o] > Math.Max(0, 30 - value)
						)
							return false;
					}
					...
					if (detail.Category != Category.RequiredSkill)
					{
						...
						if (skillbonuses != null)
						{
							...
							for (int i = 0; i < 5; i++)
							{
								...
								if (
									skillbonuses.GetValues(i, out check, out bonus)
									&& check == (SkillName)o
									&& bonus >= value
								)
								{
		...
		private static bool CheckSlayer(Item item, object o)
		{
			if (o is TalismanSlayerName && (TalismanSlayerName)o == TalismanSlayerName.Undead)
			{
				if (
					!(item is ISlayer)
					|| ((((ISlayer)item).Slayer != SlayerName.Silver && ((ISlayer)item).Slayer2 != SlayerName.Silver))
				)
				{
			...
			else
			{
				if (
					...
					&& (
						!(item is ISlayer)
						|| (((ISlayer)item).Slayer != (SlayerName)o && ((ISlayer)item).Slayer2 != (SlayerName)o)
					)
				)
				{
					return false;
					...
					o is TalismanSlayerName
					&& (!(item is BaseTalisman) || ((BaseTalisman)item).Slayer != (TalismanSlayerName)o)
				)
				{
					return false;
				}
			...
			try
			{
				if (parms.Length > 1)
				{
					for (int i = 0; i < parms.Length; i++)
					{
						...
						{
							parms[i] = StringList.GetString(
								Convert.ToInt32(parms[i].Substring(1, parms[i].Length - 1))
							);
						}
					}
				}
		...
		public static bool CanSearch(Mobile m)
		{
			...
			return r is GuardedRegion && !((GuardedRegion)r).Disabled
				|| r is HouseRegion && ((HouseRegion)r).House.IsFriend(m);
		}
		...
		private static Type[] _SearchableContainers =
		{
			...
			typeof(FishBowl),
			typeof(FirstAidBelt),
			typeof(Plants.SeedBox),
			typeof(BaseSpecialScrollBook),
	...
	public class SearchCriteria
	{
		...
		public void TryAddDetails(object o, int name, int propname, int value, Category cat)
		{
			...
			if (o is Layer)
			{
				SearchDetail layer = Details.FirstOrDefault(det =>
					det.Attribute is Layer && (Layer)det.Attribute != (Layer)o
				);
		...
		private void WriteAttribute(GenericWriter writer)
		{
			...
			switch (attrID)
			{
				...
				case 5:
					...
					break;
				case 6:
					writer.Write((int)(SAAbsorptionAttribute)Attribute);
					break;
				case 7:
					writer.Write((int)(ExtendedWeaponAttribute)Attribute);
					break;
				case 8:
					writer.Write((int)(NegativeAttribute)Attribute);
					break;
				case 9:
				...
				case 11:
					...
					break;
				case 12:
					writer.Write((int)(TalismanSkill)Attribute);
					break;
				case 13:
		...
		private void ReadAttribute(GenericReader reader)
		{
			switch (reader.ReadInt())
			{
				...
				case 5:
					...
					break;
				case 6:
					Attribute = (SAAbsorptionAttribute)reader.ReadInt();
					break;
				case 7:
					Attribute = (ExtendedWeaponAttribute)reader.ReadInt();
					break;
				case 8:
					Attribute = (NegativeAttribute)reader.ReadInt();
					break;
				case 9:
				...
				case 11:
					...
					break;
				case 12:
					Attribute = (TalismanSkill)reader.ReadInt();
					break;
				case 13:
