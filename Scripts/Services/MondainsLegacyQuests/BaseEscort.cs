...
namespace Server.Engines.Quests
{
	public class BaseEscort : MondainQuester
	{
		...
		public virtual bool AcceptEscorter(Mobile m)
		{
			...
			{
				int minutes = (int)
					Math.Ceiling(((((PlayerMobile)m).LastEscortTime + m_EscortDelay) - DateTime.UtcNow).TotalMinutes);
				...
				if (escort.Region != null && escort.Region.Contains(Location))
				{
					...
					if (Quest.Completed)
					{
						...
						if (QuestHelper.AnyRewards(Quest))
							escorter.SendGump(
								new MondainQuestGump(Quest, MondainQuestGump.Section.Rewards, false, true)
							);
						else
							Quest.GiveRewards();
