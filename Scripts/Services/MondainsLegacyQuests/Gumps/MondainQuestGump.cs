		...
		public virtual void SecObjectives()
		{
			...
			if (!m_Quest.RenderObjective(this, m_Offer))
			{
				...
				for (int i = 0; i < m_Quest.Objectives.Count; i++)
				{
					...
					else if (objective is HarvestObjective)
					{
						HarvestObjective harvest = (HarvestObjective)objective;

						if (harvest != null)
						{
							AddHtmlLocalized(98, offset, 90, 16, 1079430, 0x15F90, false, false); // Obtain
							AddLabel(193, offset, 0x481, harvest.MaxProgress + " " + harvest.ResourceName); // %count% + %name%

							if (harvest.Image > 0)
								AddItem(350, offset, harvest.Image, harvest.Hue); // Image
							...
							if (m_Offer)
							{
								...
								{
									AddHtmlLocalized(103, offset, 120, 16, 1062379, 0x15F90, false, false); // Est. time remaining:
									AddLabel(223, offset, 0x481, FormatSeconds(harvest.Seconds)); // %est. time remaining%

									offset += 16;
								}
								else if (harvest.Image > 0)
									offset += 16;

								continue;
							}
							AddHtmlLocalized(103, offset, 120, 16, 3000087, 0x15F90, false, false); // Total
							AddLabel(223, offset, 0x481, harvest.CurProgress.ToString()); // %current progress%
					...
					else if (objective is ApprenticeObjective)
					{
						...
						if (apprentice != null)
						{
							AddHtmlLocalized(
								...
								16,
								1077485,
								"#" + (1044060 + (int)apprentice.Skill) + "\t" + apprentice.MaxProgress,
								0x15F90,
								false,
								false
							); // Increase ~1_SKILL~ to ~2_VALUE~
					...
					else if (objective.ObjectiveDescription != null)
					{
						if (objective.ObjectiveDescription is int)
						{
							AddHtmlLocalized(
								...
								310,
								300,
								(int)objective.ObjectiveDescription,
								0x15F90,
								false,
								false
							);
						...
						else if (objective.ObjectiveDescription is string)
						{
							AddHtmlObject(
								...
								310,
								300,
								(string)objective.ObjectiveDescription,
								LightGreen,
								false,
								false
							);
			...
			for (int i = 0; i < m_Quest.Rewards.Count; i++)
			{
				...
				if (reward != null)
				{
					...
					AddHtmlObject(
						...
						offset,
						280,
						m_Quest.Rewards.Count == 1 ? 100 : 16,
						reward.Name,
						LightGreen,
						false,
						false
			...
			switch (info.ButtonID)
			{
				...
				default:
					if (
						...
						|| info.ButtonID >= m_From.Quests.Count + ButtonOffset
						|| info.ButtonID < ButtonOffset
					)
						break;

					m_From.SendGump(
						new MondainQuestGump(
							m_From.Quests[(int)info.ButtonID - ButtonOffset],
							Section.Description,
							false
						)
					);
