...
namespace Server.Engines.Quests
{
	public class QuestHelper
	{
					...
					if (DateTime.UtcNow < endTime)
					{
						...
						if (ts.Days > 0)
						{
							player.SendLocalizedMessage(
								1158377,
								String.Format("{0}\t{1}", ts.Days.ToString(), "day[s]")
							);
						}
						else if (ts.Hours > 0)
						{
							player.SendLocalizedMessage(
								1158377,
								String.Format("{0}\t{1}", ts.Hours.ToString(), "hour[s]")
							);
						}
						else
						{
							player.SendLocalizedMessage(
								1158377,
								String.Format("{0}\t{1}", ts.Minutes.ToString(), "minute[s]")
							);
						}
								...
								if (vendor is MondainQuester)
									player.SendGump(
										new MondainQuestGump(
											player,
											quest,
											MondainQuestGump.Section.Complete,
											false,
											true,
											(MondainQuester)vendor
										)
									);
								else
									player.SendGump(
		...
		{
			return quest != null
				&& BaseChain.Chains[(int)quest.ChainID] != null
				&& BaseChain.Chains[(int)quest.ChainID].Length > 0
				&& BaseChain.Chains[(int)quest.ChainID][0] == quest.GetType();
		}
		...
		{
			return pm.Quests.Any(q =>
				q.ChainID != QuestChain.None && q.ChainID == quest.ChainID && q.GetType() != quest.GetType()
			);
		}
		...
		public static bool CheckHarvest(PlayerMobile player, Item item)
		{
			for (int i = player.Quests.Count - 1; i >= 0; i--)
			{
				BaseQuest quest = player.Quests[i];
				...
				for (int j = quest.Objectives.Count - 1; j >= 0; j--)
				{
					...
					if (objective is HarvestObjective)
					{
						...
						if (harvest.Update(item))
						{
							if (quest.Completed)
								quest.OnCompleted();
							else if (harvest.Completed)
								player.PlaySound(quest.UpdateSound);
		...
		public static bool HasQuest<T>(PlayerMobile from)
			where T : BaseQuest
		{
		...
		public static T GetQuest<T>(PlayerMobile pm)
			where T : BaseQuest
		{
