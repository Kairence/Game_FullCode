...
namespace Server.Engines.ArenaSystem
{
	public class PVPArenaSystem : PointsSystem
	{
		...
		public static bool BlockSameIP
		{
			get { return true; }
		}
		...
		{
			get { return PointsType.PVPArena; }
		}
		...
		public void AddBlockedArena(PVPArena arena)
		{
			...
			if (!IsBlocked(arena.Definition))
			{
				Utility.WriteConsoleColor(
					ConsoleColor.Green,
					"Adding blocked EA PVP Arena: {0}",
					arena.Definition.Name
				);
		...
		public static void SendParticipantMessage(
			ArenaDuel duel,
			int message,
			bool inRegion = false,
			string args = "",
		...
		public void CheckTitle(PlayerMobile pm)
		{
			...
			{
				case 1:
					title = 1152068 + (int)ArenaTitle.FledglingGladiator;
					break;
				case 50:
					title = 1152068 + (int)ArenaTitle.BuddingGladiator;
					break;
				case 100:
					title = 1152068 + (int)ArenaTitle.Gladiator;
					break;
				case 250:
					title = 1152068 + (int)ArenaTitle.WellKnownGladiator;
					break;
				case 500:
					title = 1152068 + (int)ArenaTitle.VeteranGladiator;
					break;
			}
		...
		public static bool IsSameIP(Mobile one, Mobile two)
		{
			if (
				one.NetState == null
				|| two.NetState == null
				|| one.AccessLevel > AccessLevel.Player
				|| two.AccessLevel > AccessLevel.Player
			)
				return false;
				...
				(fro, targeted) =>
				{
					if (m is PlayerMobile && targeted is ArenaStone)
					{
						...
						if (stone.Arena != null)
						{
							...
							BaseGump.SendGump(
								new GenericConfirmCallbackGump<PVPArena>(
									(PlayerMobile)m,
	...
	public class PlayerStatsEntry : PointsEntry
	{
		...
		{
			get { return SurvivalWins + SurvivalLosses + SurvivalDraws + TeamWins + TeamLosses + TeamDraws; }
		}
