...
namespace Server.Engines.ArenaSystem
{
	public class PVPArenaSystemSetupGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < ArenaDefinition.Definitions.Length; i++)
			{
				...
				bool exists =
					PVPArenaSystem.Arenas != null && PVPArenaSystem.Arenas.Any(arena => arena.Definition == def);
				...
				AddHtml(
					...
					Color(
						...
						String.Format(
							...
							def.Name,
							exists ? "Enabled"
								: PVPArenaSystem.Instance != null && PVPArenaSystem.Instance.IsBlocked(def) ? "Blocked"
								: "Disabled"
						)
					),
					false,
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			if (id >= 0 && id < ArenaDefinition.Definitions.Length)
			{
				...
				BaseGump.SendGump(
					new GenericConfirmCallbackGump<ArenaDefinition>(
						User,
		...
		private string _DisableBody =
			"By disabling this arena, the arena stone, regions and any stats associated with the arena will be lost forever. Do you wish to proceed?";
		private string _EnableBody =
			"By enabling this region, an arena stone and regions will be placed. Be sure to clear any custom content you have at or near the arena.";
	}
	...
	public class BaseArenaGump : BaseGump
	{
		...
		public override bool CloseOnMapChange
		{
			get { return true; }
		}
	...
	public class BaseDuelGump : BaseArenaGump
	{
		...
		protected string GetTimeLimit()
		{
			switch (Duel.TimeLimit)
			{
				...
				case TimeLimit.FiveMinutes:
					return "5 minutes";
				case TimeLimit.TenMinutes:
					return "10 minutes";
				case TimeLimit.FifteenMinutes:
					return "15 minutes";
				case TimeLimit.TwentyMinutes:
					return "20 minutes";
				case TimeLimit.ThirtyMinutes:
					return "30 minutes";
			}
		}
		...
		protected string GetEntryFee()
		{
			switch (Duel.EntryFee)
			{
				...
				case EntryFee.Zero:
					return "0 gold";
				case EntryFee.OneThousand:
					return "1,000 gold";
				case EntryFee.FiveThousand:
					return "5,000 gold";
				case EntryFee.TenThousand:
					return "10,000 gold";
				case EntryFee.TwentyFiveThousand:
					return "25,000 gold";
				case EntryFee.FiftyThousand:
					return "50,000 gold";
			}
		}
		...
		protected string GetPotion()
		{
			...
			{
				default:
				case PotionRules.All:
					return "ALL OK";
				case PotionRules.None:
					return "NONE";
				case PotionRules.NoHealing:
					return "NO HEALING";
			}
		}
	}
	...
	public class CreateDuelGump : BaseDuelGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				170,
				20,
				Duel.RoomType == RoomType.Public ? 1116494 : 1116495,
				0xFFFF,
				false,
				false
			); // Room Type
			...
			AddHtmlLocalized(
				...
				170,
				20,
				Duel.BattleMode == BattleMode.Survival ? 1116492 : 1116493,
				0xFFFF,
				false,
				false
			); // Battle Mode
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			else
			{
				switch (info.ButtonID)
				{
					...
					case 4:
						Duel.BattleMode =
							Duel.BattleMode == BattleMode.Survival ? BattleMode.Team : BattleMode.Survival;
						Refresh();
						break;
					case 5:
					...
					case 6:
						...
						{
							case TimeLimit.FiveMinutes:
								Duel.TimeLimit = TimeLimit.TenMinutes;
								break;
							case TimeLimit.TenMinutes:
								Duel.TimeLimit = TimeLimit.FifteenMinutes;
								break;
							case TimeLimit.FifteenMinutes:
								Duel.TimeLimit = TimeLimit.TwentyMinutes;
								break;
							case TimeLimit.TwentyMinutes:
								Duel.TimeLimit = TimeLimit.ThirtyMinutes;
								break;
							case TimeLimit.ThirtyMinutes:
								Duel.TimeLimit = TimeLimit.FiveMinutes;
								break;
						}
						Refresh();
						break;
					case 7:
						...
						{
							case EntryFee.Zero:
								Duel.EntryFee = EntryFee.OneThousand;
								break;
							case EntryFee.OneThousand:
								Duel.EntryFee = EntryFee.FiveThousand;
								break;
							case EntryFee.FiveThousand:
								Duel.EntryFee = EntryFee.TenThousand;
								break;
							case EntryFee.TenThousand:
								Duel.EntryFee = EntryFee.TwentyFiveThousand;
								break;
							case EntryFee.TwentyFiveThousand:
								Duel.EntryFee = EntryFee.FiftyThousand;
								break;
							case EntryFee.FiftyThousand:
								Duel.EntryFee = EntryFee.Zero;
								break;
						}
						Refresh();
						break;
					...
					case 13:
						...
						{
							case PotionRules.All:
								Duel.PotionRules = PotionRules.None;
								break;
							case PotionRules.None:
								Duel.PotionRules = PotionRules.NoHealing;
								break;
							case PotionRules.NoHealing:
								Duel.PotionRules = PotionRules.All;
								break;
						}
						Refresh();
						break;
	...
	public class PendingDuelGump : BaseDuelGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				20,
				CenterLoc,
				Duel.BattleMode == BattleMode.Survival ? "#1115763" : "#1116392",
				0xFFFF,
				false,
				false
			); // Participant list : Team: Order (Blue) / Chaos (Red)
			...
			else
			{
				for (int i = 0; i < Participants.Count; i++)
				{
					...
					AddLabelCropped(
						...
						135,
						20,
						Duel.TeamOrder.Contains(Participants[i]) ? 487 : 37,
						Participants[i].Name
					);
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = 0; i < PendingDuels.Count; i++)
			{
				...
				AddLabel(
					...
					y,
					LabelHue,
					String.Format("{0}/{1}", duel.ParticipantCount.ToString(), duel.Entries.ToString())
				);
	...
	public class IndividualStatsGump : BaseArenaGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			object title =
				titleIndex >= 0 && titleIndex < WhosStats.RewardTitles.Count
					? WhosStats.RewardTitles[titleIndex]
					: null;
			...
			AddHtmlLocalized(
				...
				20,
				1149602,
				String.Format("{0}\t{1}", WhosStats.Name, rewardTitle),
				0xFFFF,
				false,
				false
			); // <CENTER>Arena Menu - Stats</CENTER>
	...
	public class ArenaRankingsGump : BaseArenaGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				20,
				CenterLoc,
				ViewType == BattleMode.Team ? "#1150130" : "#1150131",
				0xFFFF,
				false,
				false
			); // Arena Menu - Survival Rankings : Arena Menu - Team Rankings
