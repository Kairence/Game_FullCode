...
namespace Server.Engines.ArenaSystem
{
	...
	public class ArenaDuel
	{
		...
		{
			get { return StartTime + TimeSpan.FromMinutes((int)TimeLimit); }
		}

		// used in team mode
		public ArenaTeam TeamOrder
		{
			get { return Teams != null && Teams.Count > 0 ? Teams[0] : null; }
		}
		public ArenaTeam TeamChaos
		{
			get { return Teams != null && Teams.Count > 1 ? Teams[1] : null; }
		}
			...
			foreach (var rec in Arena.Definition.EffectAreas)
			{
				for (int x = rec.X; x < rec.X + rec.Width; x++)
				{
					for (int y = rec.Y; y < rec.Y + rec.Height; y++)
					{
						...
						blocker.MoveToWorld(
							new Point3D(x, y, Arena.Definition.Map.GetAverageZ(x, y)),
		...
		public void DoGateEffect()
		{
			...
			for (int i = 0; i < _EffectZs.Length; i++)
			{
				...
				Effects.SendLocationEffect(
					new Point3D(x, y, z + _EffectZs[i]),
					map,
					0x1A9F,
					Utility.RandomMinMax(0x13, 0x19),
					0x10,
					0x1F,
					0x4
				);
