...
namespace Server.Items
{
	public class UnderworldPuzzleItem : BaseDecayingItem
	{
		...
		public UnderworldPuzzleSolution Solution
		{
			get { return m_Solution; }
		}
		public UnderworldPuzzleSolution CurrentSolution
		{
		...
		private Type[] m_Rewards = new Type[]
		{
			...
			typeof(MouldingBoard),
			typeof(DoughBowl),
			typeof(HornedTotemPole),
			typeof(LargeSquarePillow),
			typeof(LargeDiamondPillow),
			typeof(DustyPillow),
			typeof(StatuePedestal), /*typeof(FlouredBreadBoard),*/
			typeof(LuckyCoin),
		};
	...
	public class UnderworldPuzzleSolution
	{
		...
		public PuzzlePiece Third
		{
			get { return m_Rows[2]; }
			set { m_Rows[2] = value; }
		}
		public PuzzlePiece Fourth
		{
			get { return m_Rows[3]; }
			set { m_Rows[3] = value; }
		}
	...
	public class UnderworldPuzzleGump : Gump
	{
			...
			: base(45, 45)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				if (
					i == row
					&& m_CurrentSolution.Rows[i] != PuzzlePiece.None
					&& m_Item.Attempts < m_Solution.MaxAttempts
				)
				{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 5:
				{
					...
					if (
						movingPiece != m_CurrentSolution.Rows[m_Row]
						|| movingToPiece != m_CurrentSolution.Rows[nextRow]
					)
					{
