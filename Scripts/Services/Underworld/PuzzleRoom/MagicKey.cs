...
namespace Server.Items
{
	public class MagicKey : BaseDecayingItem
	{
				...
				if (m != null && m.Map != Map.Internal)
				{
					if (m_PuzzleRoom.Contains(m.Location))
					{
						...
						Effects.SendLocationParticles(
							EffectItem.Create(loc, m.Map, EffectItem.DefaultDuration),
							0x3728,
							10,
							10,
							2023
					...
					if (pack != null)
					{
						...
						{
							if (
								item is CopperPuzzleKey
								|| item is GoldPuzzleKey
								|| item is MazePuzzleItem
								|| item is MastermindPuzzleItem
							)
								item.Delete();
						}
					}
		...
		private class MagicKeyConfirmGump : Gump
		{
			...
			public MagicKeyConfirmGump(MagicKey key)
				: base(50, 50)
			{
				m_Key = key;
