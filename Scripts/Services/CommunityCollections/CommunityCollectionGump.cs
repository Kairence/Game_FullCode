...
namespace Server.Gumps
{
	public class CommunityCollectionGump : Gump
	{
		...
		public CommunityCollectionGump(
			PlayerMobile from,
			IComunityCollection collection,
			Point3D location,
			Section section
		)
		...
		public void DisplayDonationPage()
		{
			...
			while (offset + next < 330 && m_Index < m_Collection.Donations.Count)
			{
				...
				if (item.Points < 1 && item.Points > 0)
					AddLabel(
						65 + m_Max,
						offset + (int)(height / 2) - 10,
						0x64,
						"1 per " + ((int)Math.Pow(item.Points, -1)).ToString()
					);
				else
		...
		public void DisplayRewardPage()
		{
			...
			while (offset + next < 300 && m_Index < m_Collection.Rewards.Count)
			{
				...
				AddItem(
					55 - item.X + m_Max / 2 - item.Width / 2,
					y,
					item.ItemID,
					points >= item.Points ? item.Hue : 0x3E9
				);
				AddTooltip(item.Tooltip);
				AddLabel(
					65 + m_Max,
					offset + (int)(height / 2) - 10,
					points >= item.Points ? 0x64 : 0x21,
		...
		public override void OnResponse(Server.Network.NetState state, RelayInfo info)
		{
			...
			if (info.ButtonID == (int)Buttons.Rewards)
				...
				info.ButtonID >= 300
				&& m_Collection.Donations != null
				&& info.ButtonID - 300 < m_Collection.Donations.Count
				&& m_Section == Section.Donates
			)
			{
				...
				info.ButtonID >= 200
				&& m_Collection.Rewards != null
				&& info.ButtonID - 200 < m_Collection.Rewards.Count
			...
			)
			{
				...
				info.ButtonID >= 100
				&& m_Item != null
				&& info.ButtonID - 200 < m_Item.Hues.Length
				&& m_Section == Section.Hues
			)
			{
				m_Owner.CloseGump(typeof(ConfirmRewardGump));
				m_Owner.SendGump(
					new ConfirmRewardGump(m_Collection, m_Location, m_Item, m_Item.Hues[info.ButtonID - 100])
				);
			}
		}
			...
			if (pm.Quests != null)
			{
				foreach (BaseQuest q in pm.Quests)
				{
					if (!q.Completed)
					{
						...
						{
							if (
								obj is CollectionsObtainObjective
								&& item.Type == ((CollectionsObtainObjective)obj).Obtain
							)
								return false;
						}
					}
		...
		public static bool CheckType(Item item, Type type, bool checkDerives)
		{
			...
			else
			{
				...
				else if (type == typeof(Crab) && BaseHighseasFish.Crabs.Any(x => x == t))
				{
					...
					type == typeof(Fish)
					&& t != typeof(BaseCrabAndLobster)
					&& !t.IsSubclassOf(typeof(BaseCrabAndLobster))
					&& (
						t.IsSubclassOf(type)
						|| t == typeof(BaseHighseasFish)
						|| t.IsSubclassOf(typeof(BaseHighseasFish))
					)
				)
				{
					return true;
		...
		public static int GetTypes(PlayerMobile pm, CollectionItem colItem)
		{
			...
			bool derives =
				type == typeof(RedScales) || type == typeof(Fish) || type == typeof(Crab) || type == typeof(Lobster);
