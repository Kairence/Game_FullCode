	...
	public class MovementImpl : IMovementImpl
	{
		...
		)
		{
			...
			if (
				landBlocks
				&& canSwim
				&& (TileData.LandTable[landTile.ID & TileData.MaxLandValue].Flags & TileFlag.Wet) != 0
			...
			bool ignoreDoors = (
				m_AlwaysIgnoreDoors || m == null || !m.Alive || m.Body.BodyID == 0x3DB || m.IsDeadBondedPet
			);
		...
		private bool CanMoveOver(Mobile m, Mobile t)
		{
			...
			return (!t.Alive || m == null || !m.Alive || t.IsDeadBondedPet || m.IsDeadBondedPet)
				|| (t.Hidden && t.IsStaff());
		}
		...
		public bool CheckMovement(IPoint3D p, Map map, Point3D loc, Direction d, out int newZ)
		{
			...
			bool checkMobs = (
				p is BaseCreature && !((BaseCreature)p).Controlled && (xForward != m_Goal.X || yForward != m_Goal.Y)
			);
		...
		private void GetStartZ(IPoint3D p, Map map, Point3D loc, List<Item> itemList, out int zLow, out int zTop)
		{
			...
			for (int i = 0; i < staticTiles.Length; ++i)
			{
				...
				if (
					...
					&& (
						(id.Flags & TileFlag.Surface) != 0 || (m != null && m.CanSwim && (id.Flags & TileFlag.Wet) != 0)
					)
					&& loc.Z >= calcTop
				)
				{
