...
namespace Server.Menus.Questions
{
	...
	public class StuckMenu : Gump
	{
		...
		private static readonly StuckMenuEntry[] m_TerMurEntries = new StuckMenuEntry[]
		{
			...
			new StuckMenuEntry(
				1112572,
				new Point3D[] { new Point3D(997, 3869, -42), new Point3D(961, 3921, -42), new Point3D(996, 3962, -42) }
			),
		};
			...
			: base(150, 50)
		{
			...
			StuckMenuEntry[] entries =
				IsTerMur(beheld) ? m_TerMurEntries
				: IsInSecondAgeArea(beheld) ? m_T2AEntries
				: m_Entries;
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			else
			{
				...
				StuckMenuEntry[] entries =
					IsTerMur(m_Mobile) ? m_TerMurEntries
					: IsInSecondAgeArea(m_Mobile) ? m_T2AEntries
					: m_Entries;
		...
		private class TeleportTimer : Timer
		{
			...
			private void MovePetsOfLoggedCharacter(Point3D dest, Map destMap)
			{
				...
				var move = fromMap
					.GetMobilesInRange(fromLoc, 3)
					.Where(m => m is BaseCreature)
					.Cast<BaseCreature>()
					.Where(pet =>
