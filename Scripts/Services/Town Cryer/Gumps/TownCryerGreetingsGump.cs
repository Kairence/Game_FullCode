...
namespace Server.Services.TownCryer
{
	public class TownCryerGreetingsGump : BaseTownCryerGump
	{
		...
		public int Pages
		{
			get { return TownCryerSystem.GreetingsEntries.Count; }
		}
		...
		public override void AddGumpLayout()
		{
			...
			if (Entry.Expires != DateTime.MinValue)
			{
				AddHtmlLocalized(
					...
					20,
					1060658,
					String.Format("{0}\t{1}", "Created", Entry.Created.ToShortDateString()),
					0,
					false,
					false
				);
				AddHtmlLocalized(
					...
					20,
					1060659,
					String.Format("{0}\t{1}", "Expires", Entry.Expires.ToShortDateString()),
					0,
					false,
					false
				);
			...
			AddHtml(
				...
				35,
				20,
				Center(String.Format("{0}/{1}", (Page + 1).ToString(), Pages.ToString())),
				false,
				false
			);
			...
			if (Entry.Link != null)
			{
				if (!string.IsNullOrEmpty(Entry.LinkText))
				{
					AddHtml(
						...
						745,
						40,
						String.Format("<a href=\"{0}\">{1}</a>", Entry.Link, Entry.LinkText),
						false,
						false
					);
				}
				else
				{
					AddHtml(
						...
						745,
						40,
						String.Format("<a href=\"{0}\">{1}</a>", Entry.Link, Entry.Link),
						false,
						false
					);
				}
