	...
	public class TownCryerGump : BaseTownCryerGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddButton(
				355,
				598,
				Category == GumpCategory.EventModerator ? 0x5F4 : 0x5F3,
				0x5F4,
				2,
		...
		private void BuildNewsPage()
		{
			...
			for (int i = start; i < TownCryerSystem.NewsEntries.Count && i < perPage; i++)
			{
				...
				bool doneQuest =
					entry.QuestType != null && QuestHelper.CheckDoneOnce(User, entry.QuestType, Cryer, false);
			...
			if (TownCryerSystem.NewsEntries.Count > perPage)
			{
				...
				AddHtml(
					...
					35,
					20,
					Center(String.Format("{0}/{1}", (Page + 1).ToString(), (Pages + 1).ToString())),
					false,
					false
				);
			}
		...
		private void BuildCityPage()
		{
			...
			AddButton(
				374,
				150,
				City == City.Moonglow ? 0x5E3 : 0x5E2,
				City == City.Moonglow ? 0x5E3 : 0x5E2,
				13,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				418,
				150,
				City == City.NewMagincia ? 0x5DD : 0x5DC,
				City == City.NewMagincia ? 0x5DD : 0x5DC,
				14,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				463,
				150,
				City == City.SkaraBrae ? 0x5DF : 0x5DE,
				City == City.SkaraBrae ? 0x5DF : 0x5DE,
				15,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				555,
				150,
				City == City.Vesper ? 0x5ED : 0x5ED,
				City == City.Vesper ? 0x5ED : 0x5EC,
				17,
				GumpButtonType.Reply,
				0
			);
			...
			AddHtmlLocalized(
				...
				20,
				CenterLoc,
				String.Format("#{0}", TownCryerSystem.GetCityLoc(this.City)),
				0,
				false,
				false
			); // The Latest News from the City of ~1_CITY~

			int y = 300;

			for (
				int i = 0;
				i < TownCryerSystem.CityEntries.Count && i < TownCryerSystem.MaxPerCityGoverrnorEntries;
				i++
			)
			{
		...
		private void BuildGuildPage()
		{
			...
			for (int i = start; i < list.Count && i < perPage; i++)
			{
				...
				AddButton(
					...
					0x5FB,
					0x5FC,
					400 + TownCryerSystem.GuildEntries.IndexOf(entry),
					GumpButtonType.Reply,
					0
				);
				...
				if (
					(guild == entry.Guild && User.GuildRank != null && User.GuildRank.Rank >= 3)
					|| User.AccessLevel >= AccessLevel.GameMaster
				) // Only warlords+
				{
				...
				default:
					{
						...
						else if (id < 300)
						{
							...
							{
								BaseGump.SendGump(
									new TownCryerEventModeratorGump(User, Cryer, TownCryerSystem.ModeratorEntries[id])
								);
							}
						}
						else if (id < 400)
						...
						else if (id < 600)
						{
							...
							{
								BaseGump.SendGump(
									new TownCryerGuildGump(User, Cryer, TownCryerSystem.GuildEntries[id])
								);
							}
						}
						else if (id < 3000)
						{
							if (id < 2500)
							{
								...
								{
									BaseGump.SendGump(
										new CreateEMEntryGump(User, Cryer, TownCryerSystem.ModeratorEntries[id])
									);
								}
							}
							else
						...
						else if (id < 4000)
						{
							...
							if ((city != null && city.Governor == User) || User.AccessLevel >= AccessLevel.GameMaster) // Only Governors
							{
								if (id < 3500)
								{
									...
									{
										BaseGump.SendGump(
											new CreateCityEntryGump(User, Cryer, City, TownCryerSystem.CityEntries[id])
										);
									}
								}
								else
						...
						else if (id < 5000)
						{
							if (id < 4500)
							{
								...
								{
									BaseGump.SendGump(
										new CreateGuildEntryGump(User, Cryer, TownCryerSystem.GuildEntries[id])
									);
								}
							}
							else
