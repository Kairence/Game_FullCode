...
namespace Server.Engines.ShameRevamped
{
	...
	{
		public ShameTeleporter(Point3D dest, Map map)
			: base(dest, map, true) { }

		public ShameTeleporter(Serial serial)
			: base(serial) { }
		...
		public override bool CanTeleport(Mobile m)
		{
			...
			foreach (Item item in eable)
			{
				if (
					...
					&& ((AddonComponent)item).Addon is ShameWall
					&& ((AddonComponent)item).Addon.Visible
				)
				{
					active = true;
					break;
