...
namespace Server.Engines.VoidPool
{
	...
	public class VoidPoolController : Item
	{
		...
		public void SpawnWave()
		{
			...
			for (int i = 0; i < toSpawn; i++)
			{
				...
				for (int j = 0; j < 25; j++)
				{
					int x =
						start.X
						+ Utility.RandomMinMax(start.X - (StartPointVariance / 2), start.X + (StartPointVariance / 2));
					int y =
						start.Y
						+ Utility.RandomMinMax(start.Y - (StartPointVariance / 2), start.Y + (StartPointVariance / 2));
				...
				if (bc != null)
				{
					...
					Timer.DelayCall(
						TimeSpan.FromSeconds((double)i * .75),
						() =>
						{
							if (OnGoing)
							{
			...
			Timer.DelayCall(
				TimeSpan.FromSeconds(toSpawn * .80),
				() =>
				{
		...
		public void EndInvasion()
		{
			...
			VoidPoolStats.OnInvasionEnd(
				this /*CurrentScore, Wave*/
			);
		...
		public void ResetLevel3Spawners(Map map)
		{
			...
			foreach (
				Item item in r.GetEnumeratedItems()
					.Where(i => i is ISpawner && i.X >= 5501 && i.X <= 5627 && i.Y >= 1799 && i.Y <= 1927)
			)
			{
				if (item is XmlSpawner)
				{
		...
		public static Type[][] SpawnTable = new Type[][]
		{
			...
			new Type[]
			{
				typeof(LizardmanWitchdoctor),
				typeof(OrcFootSoldier),
				typeof(RatmanAssassin),
		...
		public VoidPoolController(Serial serial)
			: base(serial) { }
