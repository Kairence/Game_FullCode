...
namespace Server.Items
{
	public class HarvestMap : Item, IUsesRemaining
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool IsMinerMap
		{
			get { return _Resource >= CraftResource.Iron && _Resource <= CraftResource.Valorite; }
		}
		...
		public override void AddNameProperty(ObjectPropertyList list)
		{
			list.Add(
				...
				String.Format(
					"#{0}\t#{1}",
					CraftResources.GetLocalizationNumber(Resource),
					IsMinerMap ? "1152604" : "1152605"
				)
			); // ~1_RES~ ~2_TYPE~ Map
		}
		...
		private string GetSextantLocation()
		{
			...
			if (
				Sextant.Format(
					new Point3D(x, y, TargetMap.GetAverageZ(x, y)),
					TargetMap,
					ref xLong,
					ref yLat,
					ref xMins,
		...
		public static HarvestMap CheckMapOnHarvest(Mobile from, object harvested, HarvestDefinition def)
		{
			...
			if (harvested is IPoint3D && from.Backpack != null)
			{
				...
				foreach (Item item in items)
				{
					...
					if (
						harvestmap != null
						&& harvestmap.TargetMap == map
						&& harvestmap.UsesRemaining > 0
						&& def.GetBank(map, p.X, p.Y)
							== def.GetBank(harvestmap.TargetMap, harvestmap.Target.X, harvestmap.Target.Y)
					)
					{
						return harvestmap;
					}
		...
		public static List<Point2D> LoadLocsFor(Map map, HarvestMap hMap)
		{
			string path = String.Format(
				"Data/HarvestLocs/{0}_{1}.cfg",
				hMap.IsMinerMap ? "MinerLocs" : "LumberLocs",
				map.ToString()
			);
