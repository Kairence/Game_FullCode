...
namespace Server.Items
{
		...
		private class InternalComponent : AddonComponent
		{
			...
			public override void AddNameProperty(ObjectPropertyList list)
			{
				if (Addon != null && Addon.Resource != CraftResource.None)
					list.Add(
						1152600,
						String.Format("#{0}", CraftResources.GetLocalizationNumber(Addon.Resource).ToString())
					); // ~1_RES~ Cauldron of Transmutation
				else
			...
			public override void GetProperties(ObjectPropertyList list)
			{
				...
				list.Add(
					...
					String.Format(
						"#{0}\t#{1}",
						CraftResources.GetLocalizationNumber(Addon.Resource),
						CraftResources.GetLocalizationNumber(res2)
					)
				); // transmutes ~1_SOURCE~ to ~2_DEST~
	...
	public class CauldronOfTransmutationDeed : BaseAddonDeed
	{
		...
		{
			get { return new CauldronOfTransmutation(); }
		}
