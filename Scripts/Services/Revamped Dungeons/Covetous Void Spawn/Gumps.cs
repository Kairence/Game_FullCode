...
namespace Server.Engines.VoidPool
{
	...
	public class VoidPoolGump : Gump
	{
		...
		public VoidPoolGump(VoidPoolController controller, PlayerMobile pm)
			: base(50, 50)
		{
		...
		public void AddGumpLayout()
		{
			...
			else
			{
				...
				if (Controller.NextStart > DateTime.UtcNow)
					AddHtmlLocalized(
						...
						16,
						1152917,
						((int)(Controller.NextStart - DateTime.UtcNow).TotalMinutes).ToString(),
						Orange,
						false,
						false
					); // Starts in ~1_MIN~ min.
			...
			if (stats.BestWave != null)
			{
				...
				AddHtml(
					...
					200,
					16,
					String.Format("<basefont color=#A52A2A>Best Wave: {0}", stats.BestWave.Waves.ToString()),
					false,
					false
				);
			}
		...
		public void AddGumpLayout()
		{
			...
			switch (this.ScoreType)
			{
				...
				case ScoreType.Current:
					...
					loc = 1152535;
					break;
				case ScoreType.BestSingle:
					Score = stats.BestSingle;
					loc = 1152536;
					break;
				case ScoreType.OverallTotal:
					Score = stats.OverallTotal;
					loc = 1152537;
					break;
				case ScoreType.BestWave:
					Score = stats.BestWave.Score;
					loc = "Best Wave Scoreboard";
					break;
			}
			...
			if (this.ScoreType == ScoreType.BestWave)
			{
				AddHtml(
					...
					200,
					16,
					String.Format("<basefont color=#8B0000>Total Waves: {0}", stats.BestWave.Waves.ToString()),
					false,
					false
				);
				AddHtml(
					...
					200,
					16,
					String.Format("<basefont color=#8B0000>Total Score: {0}", stats.BestWave.TotalScore.ToString()),
					false,
					false
				);
			}
