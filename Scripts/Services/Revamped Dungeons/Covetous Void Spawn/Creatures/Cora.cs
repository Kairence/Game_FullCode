...
namespace Server.Mobiles
{
	public class CoraTheSorceress : BaseCreature, IElementalCreature, IAuraCreature
	{
		...
		{
			get { return ElementType.Chaos; }
		}

		public DateTime NextManaDrain { get; set; }

		public TimeSpan ManaDrainInterval
		{
			get { return TimeSpan.FromSeconds(Utility.RandomMinMax(15, 120)); }
		}

		[Constructable]
		public CoraTheSorceress()
			: base(AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.1)
		{
			Body = 0x191;
			Name = "Cora";
			Title = "the sorceress";
		...
		{
			get { return TimeSpan.FromSeconds(Utility.RandomMinMax(30, 60)); }
		}
		public override double TeleportProb
		{
			get { return 1.0; }
		}
		public override bool TeleportsPets
		{
			get { return true; }
		}
		...
		private void DropCrack(MovementPath path)
		{
			...
			for (int i = 0; i < path.Directions.Length; ++i)
			{
				...
				Timer.DelayCall(
					...
					new TimerStateCallback(ManaDrainEffects_Callback),
					new object[] { p, this.Map }
				);
		...
		private class ManaDrainItem : Item
		{
			...
			public ManaDrainItem(int id, Mobile owner)
				: base(id)
			{
				Owner = owner;
			...
			public override bool OnMoveOver(Mobile m)
			{
				if (
					...
					&& m.CanBeHarmful(Owner, false)
				)
				{
					if (
						m is PlayerMobile
						&& Services.TownCryer.TownCryerSystem.UnderMysteriousPotionEffects((PlayerMobile)m, true)
					)
					{
		...
		public override void OnKilledBy(Mobile mob)
		{
			...
			{
				int chance =
					Server.Engines.Despise.DespiseBoss.ArtifactChance
					+ (int)Math.Min(10, ((PlayerMobile)mob).Luck / 180);

				if (chance >= Utility.Random(100))
				{
					Type t = Server.Engines.Despise.DespiseBoss.Artifacts[
						Utility.Random(Server.Engines.Despise.DespiseBoss.Artifacts.Length)
					];
