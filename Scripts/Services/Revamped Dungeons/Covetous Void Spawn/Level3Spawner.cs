...
namespace Server.Engines.VoidPool
{
	...
	public class Level3Spawner : ISpawner
	{
		...
		{
			get { return Spawns == null ? 0 : Spawns.Count; }
		}
		...
		public void LoadSpawns()
		{
			...
			Spawns.Add(
				new SpawnEntry(
					this,
					new Rectangle2D[] { new Rectangle2D(5536, 1876, 8, 8), new Rectangle2D(5552, 1840, 11, 42) }
				)
			);
			Spawns.Add(
				new SpawnEntry(
					this,
					new Rectangle2D[] { new Rectangle2D(5600, 1860, 8, 8), new Rectangle2D(5596, 1872, 15, 12) }
				)
			);
			Spawns.Add(
				new SpawnEntry(
					this,
					new Rectangle2D[] { new Rectangle2D(5608, 1832, 15, 15), new Rectangle2D(5616, 1848, 11, 34) }
				)
			);
		...
		public void RemoveFromSpawner(ISpawnable spawnable)
		{
			...
			if (bc != null)
			{
				...
				if (entry != null)
				{
					...
					{
						Timer.DelayCall<SpawnEntry>(
							TimeSpan.FromMinutes(Utility.RandomMinMax(2, 5)),
							en =>
							{
								Reset(en);
							},
				...
				for (int i = 0; i < MaxCount; i++)
				{
					BaseCreature bc =
						Activator.CreateInstance(
							types[Utility.Random(types.Length)],
