...
namespace Server.Engines.Despise
{
	public class DespiseController : Item
	{
		...
		public static DespiseController Instance
		{
		...
		public Region GoodRegion
		{
			get { return m_GoodRegion; }
		}
		public Region EvilRegion
		{
			get { return m_EvilRegion; }
		}
		public Region LowerRegion
		{
			get { return m_LowerRegion; }
		}
		public Region StartRegion
		{
			get { return m_StartRegion; }
		}
		...
		public List<DespiseCreature> EvilArmy
		{
			get { return m_EvilArmy; }
		}
		public List<DespiseCreature> GoodArmy
		{
			get { return m_GoodArmy; }
		}
		...
		{
			get { return m_SequenceTimer != null || m_CleanupTimer != null; }
		}
		...
		private void BeginTimer()
		{
			...
			m_StartRegion = new DespiseRegion(
				"Despise Start",
				new Rectangle2D[] { new Rectangle2D(5568, 623, 22, 20) }
			);
		}
		...
		public void TryAddToArmy(WispOrb orb)
		{
			if (
				...
				&& orb.Owner != null
				&& m_Sequencing
				&& orb.Alignment == m_SequenceAlignment
				&& !m_ToTransport.Contains(orb.Owner)
			)
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int GoodSpawnerCount
		{
			get { return m_GoodSpawners == null ? 0 : m_GoodSpawners.Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int EvilSpawnerCount
		{
			get { return m_EvilSpawners == null ? 0 : m_EvilSpawners.Count; }
		}
		...
		public bool ResetSpawns
		{
			...
			set
			{
				if (value)
				{
					if (m_GoodSpawners != null)
						m_GoodSpawners.Clear();
					if (m_EvilSpawners != null)
						m_EvilSpawners.Clear();
		...
		private void TransportPlayers()
		{
			...
			foreach (Mobile m in list)
			{
				...
				if (
					...
					|| !orb.Conscripted
					|| m.Region == null
					|| !m.Region.IsPartOf<DespiseRegion>()
				)
		...
		public void BeginSequenceTimer()
		{
			...
			m_SequenceTimer = Timer.DelayCall(
				TimeSpan.FromMinutes(1),
				TimeSpan.FromMinutes(1),
				new TimerCallback(OnSequenceTick)
			);
		...
		{
			EndCleanupTimer();
			m_CleanupTimer = Timer.DelayCall(
				TimeSpan.FromMinutes(5),
				TimeSpan.FromMinutes(5),
				new TimerCallback(EndSequence)
			);
		...
		public static Rectangle2D[] EvilBounds
		{
			get { return m_EvilBounds; }
		}
		private static Rectangle2D[] m_EvilBounds = new Rectangle2D[] { new Rectangle2D(5381, 644, 149, 120) };

		public static Rectangle2D[] GoodBounds
		{
			get { return m_GoodBounds; }
		}
		private static Rectangle2D[] m_GoodBounds = new Rectangle2D[] { new Rectangle2D(5380, 515, 134, 121) };

		public static Rectangle2D[] LowerLevelBounds
		{
			get { return m_LowerLevelBounds; }
		}
		private static Rectangle2D[] m_LowerLevelBounds = new Rectangle2D[] { new Rectangle2D(5379, 771, 247, 250) };
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			if (version == 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (m != null && points > 0)
						Server.Engines.Points.PointsSystem.DespiseCrystals.ConvertFromOldSystem(
							(PlayerMobile)m,
							points
						);
				}
		...
		public void CheckSpawnersVersion3()
		{
			foreach (
				var spawner in World
					...
					.Where(s => s.Name != null && s.Name.ToLower().IndexOf("despiserevamped") >= 0)
			)
			{
				foreach (var obj in spawner.SpawnObjects)
				{
					...
					if (
						Region.Find(spawner.Location, spawner.Map) == m_GoodRegion
