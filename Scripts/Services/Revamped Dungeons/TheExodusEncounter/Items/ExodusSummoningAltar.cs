...
namespace Server.Items
{
	public class ExodusSummoningAlter : BaseDecayingItem
	{
		...
		public class SummoningTarget : Target
		{
			...
			public SummoningTarget(Mobile from, Item deed)
				...
				if (targeted is StaticTarget)
				{
					...
					if (IsValidTile(targ.ItemID) && (from.Map == Map.Felucca || from.Map == Map.Trammel))
					{
						...
						if (alter)
						{
							...
							ExodusTomeAltar.Altar == null
							&& VerLorRegController.Active
							&& VerLorRegController.Mobile != null
							&& CheckExodus()
						)
						{
		...
		{
			return ClockworkExodus.Instances.FirstOrDefault(m =>
					m.Region.IsPartOf("Ver Lor Reg")
					&& ((m.Hits >= m.HitsMax * 0.60 && m.MinHits >= m.HitsMax * 0.60) || (m.Hits >= m.HitsMax * 0.75))
				) != null;
		}
	}
}
