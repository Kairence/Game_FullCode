...
namespace Server.Items
{
	public class ExodusNexus : NexusAddon
	{
		...
		public ExodusNexus(Serial serial)
			: base(serial) { }
		...
		public void OpenGump(Mobile from)
		{
			...
			if (this.User != null)
			{
				...
				if (
					this.User.Deleted
					|| this.User.Map != this.Map
					|| !this.User.InRange(this, 3)
					|| this.User.NetState == null
					|| DateTime.UtcNow - this.LastUse >= m_UseTimeout
				)
				{
					this.User.CloseGump(typeof(NexusGameGump));
				}
		...
		public class DamageTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				this.m_To.LocalOverheadMessage(
					MessageType.Regular,
					0xC9,
					true,
					"* Your body convulses from electric shock *"
				);
				this.m_To.NonlocalOverheadMessage(
					MessageType.Regular,
					0xC9,
					true,
					string.Format("* {0} spasms from electric shock *", this.m_To.Name)
				);
