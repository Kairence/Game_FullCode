...
namespace Server.Engines.Blackthorn
{
	public class BlackthornDungeon : DungeonRegion
	{
		...
		public void OnTick()
		{
			...
			{
				foreach (
					Mobile m in GetEnumeratedMobiles()
		...
		public void MoveLocation(Mobile m)
		{
			...
			for (int x = m.X - 1; x <= m.X + 1; x++)
			{
				for (int y = m.Y - 1; y <= m.Y + 1; y++)
				{
					Effects.SendLocationEffect(
						new Point3D(x, y, m.Z),
						m.Map,
						Utility.RandomList(14120, 4518, 14133),
						16,
						1,
						1166,
						0
			...
			foreach (Mobile mob in eable)
			{
				if (
					mob.Combatant == null
					&& mob is BaseCreature
					&& ((BaseCreature)mob).GetMaster() == null
					&& mob.CanBeHarmful(m)
				)
				{
					if (mob.InLOS(m))
	...
	{
		public static readonly Point3D[] StableLocs = new Point3D[]
		{
			new Point3D(1510, 1543, 25),
			new Point3D(1516, 1542, 25),
			new Point3D(1520, 1542, 25),
			new Point3D(1525, 1542, 25),
		};
