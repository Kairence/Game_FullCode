...
namespace Server.Engines.Blackthorn
{
	...
	public class InvasionController : Item
	{
		...
		public bool BeaconVulnerable
		{
			get { return Beacon != null && (Spawn == null || Spawn.Count == 0); }
		}

		public InvasionController(Map map)
			: base(3796)
		{
			Movable = false;
			Visible = false;
		...
		private Type[][] _SpawnTable =
		{
			...
			{
				typeof(MudElemental),
				typeof(MoltenEarthElemental),
				typeof(DiseasedBloodElemental),
				typeof(GreaterAirElemental),
				typeof(GreaterBloodElemental),
				typeof(GreaterEarthElemental),
				typeof(GreaterWaterElemental),
				typeof(ShameGreaterPoisonElemental),
		...
		public void SpawnWave()
		{
			...
			for (int j = 0; j < 2; j++)
			{
				...
				for (int i = 0; i < count; i++)
				{
					BaseCreature bc =
						Activator.CreateInstance(
							_SpawnTable[(int)this.InvasionType][
								Utility.Random(_SpawnTable[(int)this.InvasionType].Length)
							]
						) as BaseCreature;

					Server.Engines.XmlSpawner2.XmlAttach.AttachTo(
						bc,
						new Server.Engines.XmlSpawner2.XmlData("Notoriety", "red")
					);
				...
				if (
					SpawnMobile(capt, spawnrec)
					|| SpawnMobile(
						capt,
						new Rectangle2D(
							Defs[CurrentInvasion].BeaconLoc.X - 10,
							Defs[CurrentInvasion].BeaconLoc.Y - 10,
							20,
							20
						)
					)
		...
		public static Item CreateItem(Mobile damager)
		{
			Item i = Loot.RandomArmorOrShieldOrWeaponOrJewelry(
				LootPackEntry.IsInTokuno(damager),
				LootPackEntry.IsMondain(damager),
				LootPackEntry.IsStygian(damager)
			);
			...
			if (i != null)
			{
				RunicReforging.GenerateRandomItem(
					i,
					damager,
					Utility.RandomMinMax(700, 800),
					damager is PlayerMobile ? ((PlayerMobile)damager).RealLuck : 0,
					ReforgedPrefix.None,
