...
namespace Server.Items
{
	...
	public class FermentationBarrel : Item
	{
		...
		{
			get { return _BacterialResistance; }
			set
			{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime FermentingEnds
		{
			get { return _FermentingEnds; }
			set
			{
				_FermentingEnds = value;
				InvalidateProperties();
			}
		}
		...
		{
			get { return _BadBatch; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int BottlesRemaining
		{
			get { return _BottlesRemaining; }
			set
			{
				_BottlesRemaining = value;
				InvalidateProperties();
			}
		}
		...
		{
			get { return _BacterialResistance > 0; }
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (from.InRange(this.GetWorldLocation(), 2))
			{
				if (_Fermenting && _FermentingEnds < DateTime.UtcNow)
				{
					...
					int skill = (int)(
						(from.Skills[SkillName.Cooking].Value + from.Skills[SkillName.Alchemy].Value) / 2
					);
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> entries)
		{
			...
			entries.Add(
				new SimpleContextMenuEntry(
					from,
					1157231,
					m => // Begin Fermentation
					{
						if (_Fermenting)
						{
						...
						else
						{
							m.SendGump(
								new ConfirmCallbackGump(
									...
									1157240,
									null,
									confirm: StartFermentation,
									close: CancelFermentation
								)
							); // Are you sure you wish to start the fermentation process?
						}
					},
					3,
					!_Fermented && (HasYeast || _Fruit > 0)
				)
			);
			...
			entries.Add(
				new SimpleContextMenuEntry(
					...
					{
						m.SendGump(
							new ConfirmCallbackGump(m as PlayerMobile, 1157234, 1157235, null, confirm: DoEmpty)
						); // Are you sure you wish to end the fermentation process? All progress and materials will be lost!
					},
					3,
					HasYeast || _Fruit > 0
				)
