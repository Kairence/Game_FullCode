			...
			: base(25, 25)
		{
			...
			if (deed.Material != BulkMaterialType.None)
			{
				AddHtmlLocalized(
					...
					300,
					20,
					SmallBODGump.GetMaterialNumberFor(deed.Material),
					0x7FFF,
					false,
					false
			...
			if (BulkOrderSystem.NewSystemEnabled)
			{
				...
				AddHtmlLocalized(
					...
					20,
					1157301,
					String.Format("{0}\t{1}", points, banked.ToString("0.000000")),
					0x7FFF,
					false,
					false
				); // Worth ~1_POINTS~ turn in points and ~2_POINTS~ bank points.
				y += 24;
				...
				AddHtmlLocalized(
					...
					300,
					20,
					c.PointsMode == PointsMode.Enabled ? 1157302
						: c.PointsMode == PointsMode.Disabled ? 1157303
						: 1157309,
					0x7FFF,
					false,
					false
				); // Banking Points Enabled/Disabled/Automatic
				y += 24;
				...
				if (c != null)
				{
					switch (c.PointsMode)
					{
						case PointsMode.Enabled:
							c.PointsMode = PointsMode.Disabled;
							break;
						case PointsMode.Disabled:
							c.PointsMode = PointsMode.Automatic;
							break;
						case PointsMode.Automatic:
							c.PointsMode = PointsMode.Enabled;
							break;
					}
				}
