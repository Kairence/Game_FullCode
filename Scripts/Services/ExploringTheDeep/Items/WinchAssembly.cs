...
namespace Server.Items
{
	...
	public class WinchAssemblyLever : Item
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (
				m_WinchAssembly.BearingAssembly
				&& m_WinchAssembly.FlyWheel
				&& m_WinchAssembly.WireSpool
				&& m_WinchAssembly.PowerCore
			)
			{
				Timer.DelayCall(
					...
					TimeSpan.FromSeconds(1.0),
					3,
					new TimerStateCallback(m_hatch.DoDownEffect),
					new object[] { m_hatch.Location, 0, from }
				);
	...
	public class Hatch : BaseAddon
	{
		...
		public class InternalTimer : Timer
		{
			...
			public InternalTimer(Hatch hatch)
	...
	public class HatchTile : AddonComponent
	{
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			...
			if (
				Active
				&& m.Player
				&& Utility.InRange(this.Location, m.Location, 0)
				&& !Utility.InRange(this.Location, oldLocation, 0)
			)
			{
