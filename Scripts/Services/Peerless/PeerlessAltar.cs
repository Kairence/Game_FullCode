...
namespace Server.Items
{
	...
	public abstract class PeerlessAltar : Container
	{
		...
		public virtual void KeyStartTimer(Mobile from)
		{
			...
			m_KeyResetTimer = Timer.DelayCall(
		...
		public void Enter(Mobile fighter)
		{
			if (CanEnter(fighter))
			{
				...
				{
					foreach (
						var pet in ((PlayerMobile)fighter)
							.AllFollowers.OfType<BaseCreature>()
							.Where(pet =>
								pet.Alive
								&& pet.InRange(fighter.Location, 5)
								&& !(pet is BaseMount && ((BaseMount)pet).Rider != null)
								&& CanEnter(pet)
							)
					)
					{
		...
		public virtual void StartSlayTimer()
		{
			...
			m_SlayTimer = Timer.DelayCall(
				TimeSpan.FromMinutes(5),
				TimeSpan.FromMinutes(5),
				new TimerCallback(DeadlineCheck)
			);
