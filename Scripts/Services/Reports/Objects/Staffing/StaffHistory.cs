...
namespace Server.Engines.Reports
{
	public class StaffHistory : PersistableObject
	{
		...
		{
			get { return this.m_QueueStats; }
			set { this.m_QueueStats = value; }
		}
		...
		{
			get { return this.m_StaffInfo; }
			set { this.m_StaffInfo = value; }
		}
		...
		public void Render(ObjectCollection objects)
		{
			lock (RenderLock)
			{
				...
				objects.Add(
					this.GraphHourlyPages(this.m_Pages, PageResolution.None, "New pages by hour", "graph_new_pages_hr")
				);
				objects.Add(
					this.GraphHourlyPages(
						this.m_Pages,
				...
				objects.AddRange(
					(PersistableObject[])
						this.ChartTotalPages(staff, TimeSpan.FromDays(1.0), "1 Day", "graph_daily_pages")
				);
				...
				objects.AddRange(
					(PersistableObject[])
						this.ChartTotalPages(staff, TimeSpan.FromDays(7.0), "1 Week", "graph_weekly_pages")
				);
				...
				objects.AddRange(
					(PersistableObject[])
						this.ChartTotalPages(staff, TimeSpan.FromDays(30.0), "1 Month", "graph_monthly_pages")
				);
