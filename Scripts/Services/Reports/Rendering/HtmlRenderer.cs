...
namespace Server.Engines.Reports
{
	public class HtmlRenderer
	{
		...
		public void RenderFull(StringBuilder sb)
		{
			...
			sb.Append("<center>Snapshot taken at ")
				.AppendFormat("{0:d} {0:t}", m_TimeStamp)
		...
		{
			if (obj is Report)
				this.RenderReport(obj as Report, sb);
			else if (obj is BarGraph)
				this.RenderBarGraph(obj as BarGraph, sb);
			else if (obj is PieChart)
				this.RenderPieChart(obj as PieChart, sb);
		}
		...
		private void RenderPieChart(PieChart chart, StringBuilder sb)
		{
			...
			{
				labels[i] = chart.Items[i].Name;
				values[i] = chart.Items[i].Value.ToString();
			}
			...
			using (Bitmap bmp = pieChart.Draw())
			{
				...
				sb.AppendFormat(
					"<table cellpadding=0 cellspacing=0 border=0><tr><td class=\"tbl-border\"><table cellpadding=4 cellspacing=1>"
				);
				sb.AppendFormat("<tr><td colspan=10 align=center class=\"header\">{0}</td></tr>", chart.Name);
				sb.AppendFormat(
					"<tr><td class=\"entry\"><img src=\"{0}\" width={1} height={2}></td></tr></table></td></tr></table>",
					fileName,
					bmp.Width,
					bmp.Height
				);
		...
		private void RenderBarGraph(BarGraph graph, StringBuilder sb)
		{
			...
			{
				labels[i] = graph.Items[i].Name;
				values[i] = graph.Items[i].Value.ToString();
			}
		...
		{
			sb.AppendFormat(
				"<table width=\"{0}\" cellpadding=0 cellspacing=0 border=0><tr><td class=\"tbl-border\"><table width=\"100%\" cellpadding=4 cellspacing=1>",
				report.Width
			);
			...
			if (isNamed)
			{
				...
				foreach (var col in report.Columns)
					sb.AppendFormat(
						"<td class=\"header\" width=\"{0}\" align=\"{1}\">{2}</td>",
						col.Width,
						col.Align,
						col.Name
					);
			...
			foreach (var item in report.Items)
			{
				...
				for (int j = 0; j < item.Values.Count; ++j)
					sb.AppendFormat(
						"<td class=\"entry\" align=\"{0}\">{1}</td>",
						report.Columns[j].Align,
						item.Values[j].Value
					);
				sb.Append("</tr>");
			}
