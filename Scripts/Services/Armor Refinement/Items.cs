...
namespace Server.Items
{
	...
	public class RefinementComponent : Item
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public RefinementSubCraftType SubCraftType
		{
			get { return m_SubCraftType; }
			set { m_SubCraftType = value; }
		}
		...
		public override void AddNameProperty(ObjectPropertyList list)
		{
			list.Add(
				1153966,
				String.Format("#{0}\t#{1}", Labels[(int)m_RefinementType][(int)m_CraftType], GetModLabel())
			); // ~1_OBJTYPE~ ~2_BONUSLEVEL~
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			list.Add(
				1154124,
				String.Format("#{0}", m_RefinementType == RefinementType.Reinforcing ? 1154123 : 1154122)
			); // Bonus Type: ~1_TYPE~
		}
		...
		private Type GetRawMaterial()
		{
			switch (m_CraftType)
			{
				default:
				case RefinementCraftType.Blacksmith:
					return typeof(MalleableAlloy);
				case RefinementCraftType.Tailor:
					return typeof(LeatherBraid);
				case RefinementCraftType.Carpenter:
					return typeof(SolventFlask);
			}
		}
		...
		public void GetSubCraftType()
		{
			...
			{
				case RefinementCraftType.Blacksmith:
					m_SubCraftType = (RefinementSubCraftType)Utility.RandomMinMax(4, 9);
					break;
				case RefinementCraftType.Tailor:
					m_SubCraftType = (RefinementSubCraftType)Utility.RandomMinMax(0, 3);
					break;
				case RefinementCraftType.Carpenter:
					m_SubCraftType = (RefinementSubCraftType)Utility.RandomMinMax(10, 11);
					break;
			}
		}
		...
		{
			return new RefinementComponent(
				(RefinementType)Utility.Random(2),
				(RefinementCraftType)Utility.Random(3),
				GetRandomMod()
			);
		}
		...
		private static ModType GetRandomMod()
		{
			...
			if (ran >= .95)
				return ModType.Invulnerability;
			if (ran >= .80)
				return ModType.Fortification;
			if (ran >= .70)
				return ModType.Hardening;
			if (ran >= .50)
				return ModType.Protection;
	...
	public class RefinementItem : Item
	{
		...
		{
			get { return m_CheckBonus; }
			set { m_CheckBonus = value; }
		}
		...
		public RefinementItem()
			: this(
				RefinementType.Reinforcing,
				RefinementCraftType.Blacksmith,
				RefinementSubCraftType.Ringmail,
				ModType.Defense
			) { }

		public RefinementItem(
			RefinementType type,
			RefinementCraftType craftType,
			RefinementSubCraftType srtype,
			ModType modType
		)
			: base(0)
		{
		...
		public string GetNameArgs()
		{
			return String.Format(
				"#{0}\t#{1}\t#{2}",
				LabelPrefix[(int)m_RefinementType][(int)m_CraftType],
				LabelSuffix[(int)m_CraftType],
				GetModLabel()
			);
		}
		...
		public int[] LabelSuffix = new int[]
		{
			...
			1153975,
			1153974,
			1153976, //Reinforcing and Deflecting
		};
		...
		private int GetModLabel()
		{
			switch ((int)m_ModType)
			{
				default:
				case 0:
					return 1153941; // of Defense
				case 1:
					return 1153944; // of Protection
				case 2:
					return 1153945; // of Hardening
				case 3:
					return 1153946; // of Fortification
				case 4:
					return 1153947; // of Invulnerability
			}
		}
		...
		public static bool CheckForVendor(Mobile from, RefinementItem item)
		{
			...
			{
				case RefinementCraftType.Blacksmith:
					from.SendLocalizedMessage(wtf ? 1154012 : 1154009);
					break;
				case RefinementCraftType.Tailor:
					from.SendLocalizedMessage(wtf ? 1154011 : 1154008);
					break;
				case RefinementCraftType.Carpenter:
					from.SendLocalizedMessage(wtf ? 1154013 : 1154010);
					break;
			}
	...
	public class LeatherBraid : Item
	{
		...
		[Constructable]
		public LeatherBraid(int amount)
			: base(5152)
		{
			Stackable = true;
			Amount = amount;
