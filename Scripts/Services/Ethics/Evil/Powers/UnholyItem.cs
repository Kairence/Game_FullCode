...
namespace Server.Ethics.Evil
{
	public sealed class UnholyItem : Power
	{
		public UnholyItem()
		{
			this.m_Definition = new PowerDefinition(5, "Unholy Item", "Vidda K'balc", "");
		}

		public override void BeginInvoke(Player from)
		{
			from.Mobile.BeginTarget(
				12,
				false,
				Targeting.TargetFlags.None,
				new TargetStateCallback(Power_OnTarget),
				from
			);
		...
		private void Power_OnTarget(Mobile fromMobile, object obj, object state)
		{
			...
			if ((item.SavedFlags & 0x300) != 0)
			{
				from.Mobile.LocalOverheadMessage(
					...
					0x3B2,
					false,
					"That has already beem imbued."
				);
				return;
			}

			bool canImbue =
				(item is Spellbook || item is BaseClothing || item is BaseArmor || item is BaseWeapon)
				&& (item.Name == null);
