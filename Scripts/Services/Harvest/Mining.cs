...
namespace Server.Engines.Harvest
{
	public class Mining : HarvestSystem
	{
		...
		{
			get { return this.m_OreAndStone; }
		}

		public HarvestDefinition Sand
		{
			get { return this.m_Sand; }
		}
		...
		private Mining()
		{
			...
			res = new HarvestResource[] { new HarvestResource(100.0, 70.0, 100.0, 1044631, typeof(Sand)) };
		...
		)
		{
			if (
				...
				&& def == this.m_OreAndStone
				&& 0.1 > Utility.RandomDouble()
				&& HarvestMap.CheckMapOnHarvest(from, harvested, def) == null
			)
			{
