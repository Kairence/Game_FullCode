...
namespace Server.Engines.Harvest
{
	public class Fishing : HarvestSystem
	{
		...
		private Fishing()
		{
			...
			fish.EffectCounts = new int[] { 1, 2, 3 };
			...
			res = new HarvestResource[] { new HarvestResource(00.0, 00.0, 120.0, 1043297, typeof(Fish)) };
			...
			if (Core.ML)
			{
				...
				{
					new BonusHarvestResource(0, 100.0, null, null), //set to same chance as mining ml gems
		...
		private class MutateEntry
		{
			...
			public MutateEntry(double minSkill, double maxSkill, bool deepWater, params Type[] types)
			{
				//m_ReqSkill = reqSkill;
				m_MinSkill = minSkill;
				m_MaxSkill = maxSkill;
		...
		{
			new MutateEntry(80.0, 80.0, true, typeof(SpecialFishingNet)),
			new MutateEntry(0.0, 200.0, false, new Type[1] { null }),
			new MutateEntry(100.0, 80.0, true, typeof(MessageInABottle)),
			new MutateEntry(80.0, 80.0, true, typeof(BigFish)),
			new MutateEntry(
				0.0,
				125.0,
				false,
			...
			new MutateEntry(0.0, 105.0, false, typeof(Boots), typeof(Shoes), typeof(Sandals), typeof(ThighBoots)),
			new MutateEntry(80.0, 80.0, false, typeof(MudPuppy), typeof(RedHerring)),
			new MutateEntry(0.0, 200.0, false, new Type[1] { null }),
		};
		...
		{
			return Items.SpecialFishingNet.ValidateDeepWater(map, p.X, p.Y)
		...
		#region HighSeas
		public static int[] LavaTiles
		{
			get { return m_LavaTiles; }
		}
		...
		public override bool GetHarvestDetails(
			Mobile from,
			Item tool,
			object toHarvest,
			out int tileID,
