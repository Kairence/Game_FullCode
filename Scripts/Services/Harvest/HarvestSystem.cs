...
namespace Server.Engines.Harvest
{
	public abstract class HarvestSystem
	{
		...
		{
			bool wornOut = (
				tool == null || tool.Deleted || (tool is IUsesRemaining && ((IUsesRemaining)tool).UsesRemaining <= 0)
			);
		...
		)
		{
						...
						*/
						...
						if (item.Stackable)
						{
							...
							*/
							...
							if (
								def.Skill is SkillName.Fishing
								&& from.Map == Map.Trammel
								&& Items.SpecialFishingNet.ValidateDeepWater(from.Map, from.Location.X, from.Location.Y)
							)
								HarvestLoop = true;
			...
			if (
				...
				&& (
					tool is BaseHarvestTool
					|| tool is Pickaxe
					|| tool is SturdyPickaxe
					|| tool is GargoylesPickaxe
					|| Siege.SiegeShard
				)
			)
			{
		...
		*/
		...
		public virtual bool CheckHarvestSkill(
			Map map,
			Point3D loc,
			Mobile from,
		...
		{
			return from.Skills[def.Skill].Value >= resource.ReqSkill
	...
	public class FurnitureAttribute : Attribute
	{
		...
		public static bool Check(Item item)
		{
			...
			if (
				...
				&& ((AddonComponent)item).Addon.GetType().IsDefined(typeof(FurnitureAttribute), false)
			)
			{
				return true;
			}
