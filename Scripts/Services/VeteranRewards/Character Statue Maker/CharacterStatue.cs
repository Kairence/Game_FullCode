...
namespace Server.Mobiles
{
	...
	public class CharacterStatue : Mobile, IRewardItem
	{
		...
		public DateTime SculptedOn
		{
			...
			set { this.m_SculptedOn = value; }
		}
		public CharacterStatuePlinth Plinth
		{
			get { return this.m_Plinth; }
			set { this.m_Plinth = value; }
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			{
				if (
					this.m_SculptedBy.ShowFameTitle
					&& (this.m_SculptedBy.Player || this.m_SculptedBy.Body.IsHuman)
					&& this.m_SculptedBy.Fame >= 10000
				)
					list.Add(
						1076202,
						String.Format("{0} {1}", this.m_SculptedBy.Female ? "Lady" : "Lord", this.m_SculptedBy.Name)
					); // Sculpted by ~1_Name~
				else
	...
	public class CharacterStatueDeed : Item, IRewardItem
	{
		...
		{
			get { return this.m_Statue; }
			set { this.m_Statue = value; }
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				TimeSpan time =
					TimeSpan.FromDays(RewardSystem.RewardInterval.TotalDays * 6) - (DateTime.UtcNow - acct.Created);
	...
	public class CharacterStatueTarget : Target
	{
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			...
			if (this.m_Maker.IsChildOf(from.Backpack))
			{
				...
				if (
					targeted is Item
					&& !((Item)targeted).IsLockedDown
					&& !((Item)targeted).IsSecure
