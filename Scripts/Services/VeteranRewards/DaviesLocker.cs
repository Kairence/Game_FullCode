...
namespace Server.Engines.VeteranRewards
{
	public class DaviesLockerAddon : BaseAddon, ISecurable
	{
		...
		{
			get { return new DaviesLockerAddonDeed(m_Entries); }
		}

		private List<DaviesLockerEntry> m_Entries = new List<DaviesLockerEntry>();
		public List<DaviesLockerEntry> Entries
		{
	...
	public class DaviesLockerAddonDeed : BaseAddonDeed, IRewardOption
	{
		...
		{
			get { return new DaviesLockerAddon(m_South, m_Entries); }
		}
	...
	public class TreasureMapEntry : DaviesLockerEntry
	{
		...
		public TreasureMapEntry(GenericReader reader)
			: base(reader)
		{
			int v = reader.ReadInt();
	...
	public class DaviesLockerGump : Gump
	{
			...
			: base(50, 50)
		{
			...
			AddHtmlLocalized(
				...
				20,
				1153560,
				String.Format("{0}\t{1}", m_List.Count, "500"),
				Blue,
				false,
				false
			); // Maps: ~1_NUM~ of ~2_MAX~
			AddHtmlLocalized(
				...
				20,
				1153561,
				String.Format("{0}\t{1}", (page + 1).ToString(), (totalPages).ToString()),
				Blue,
				false,
				false
			); // Page ~1_CUR~ of ~2_MAX~
			...
			for (int i = start; i >= 0 && i < m_List.Count && index < perPage; i++)
			{
				...
				AddHtml(
					...
					100,
					20,
					String.Format("<basefont color=yellow>{0}", entry.Map.ToString()),
					false,
					false
				);
				...
				if (
					(entry is TreasureMapEntry && ((TreasureMapEntry)entry).Decoder == null)
					|| (entry is SOSEntry && !((SOSEntry)entry).Opened)
				)
					...
					AddHtmlLocalized(
						...
						20,
						1060847,
						String.Format("{0}\t{1}", entry.Location.X.ToString(), entry.Location.Y.ToString()),
						Yellow,
						false,
						false
					); // ~1_val~ ~2_val~
		...
		private TreasureMap Construct(TreasureMapEntry entry)
		{
			...
			if (entry.QuestItem)
				map = new HiddenTreasuresTreasureMap(
					entry.Level,
					entry.Map,
					new Point2D(entry.Location.X, entry.Location.Y)
				);
			else
			{
				map = new TreasureMap();
