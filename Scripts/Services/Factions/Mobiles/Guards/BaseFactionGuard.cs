...
namespace Server.Factions
{
	public abstract class BaseFactionGuard : BaseCreature
	{
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (
				...
				&& m.Alive
				&& this.InRange(m, 10)
				&& !this.InRange(oldLocation, 10)
				&& this.InLOS(m)
				&& this.m_Orders.GetReaction(Faction.Find(m)).Type == ReactionType.Warn
			)
			{
				...
				switch (Utility.Random(6))
				{
					...
					case 1:
						warning =
							"It would be wise to leave this area, {0}, lest your head become my commanders' trophy.";
						break;
					case 2:
						warning =
		...
		public override void OnSpeech(SpeechEventArgs e)
		{
			...
			if (!e.Handled && this.InRange(from, ListenRange) && from.Alive)
			{
				...
				else if (DateTime.UtcNow < this.m_OrdersEnd)
				{
					if (
						this.m_Town != null
						&& this.m_Town.IsSheriff(from)
						&& Town.FromRegion(this.Region) == this.m_Town
					)
					{
						this.m_OrdersEnd = DateTime.UtcNow + TimeSpan.FromSeconds(10.0);
						...
						if (understood)
						{
							...
							for (int i = 0; i < factions.Count; ++i)
							{
								...
								if (
									...
									&& Insensitive.Contains(e.Speech, faction.Definition.Keyword)
								)
								{
	...
	public class VirtualMountItem : Item, IMountItem
	{
		...
		public Mobile Rider
		{
			get { return this.m_Rider; }
		}
		public IMount Mount
		{
			get { return this.m_Mount; }
		}
