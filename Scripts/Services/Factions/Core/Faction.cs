...
namespace Server.Factions
{
	...
	public abstract class Faction : IComparable
	{
		...
		{
			get { return this.m_StrongholdRegion; }
			set { this.m_StrongholdRegion = value; }
		}
		...
		{
			get { return this.m_State.Election; }
			set { this.m_State.Election = value; }
		}

		public Mobile Commander
		{
			get { return this.m_State.Commander; }
			set { this.m_State.Commander = value; }
		}

		public int Tithe
		{
			get { return this.m_State.Tithe; }
			set { this.m_State.Tithe = value; }
		}

		public int Silver
		{
			get { return this.m_State.Silver; }
			set { this.m_State.Silver = value; }
		}

		public List<PlayerState> Members
		{
			get { return this.m_State.Members; }
			set { this.m_State.Members = value; }
		}
		...
		{
			get { return this.m_State.FactionMessageReady; }
		}
		...
		public void EndBroadcast(Mobile from, string text)
		{
			...
			this.Broadcast(
				this.Definition.HueBroadcast,
				"{0} [Commander] {1} : {2}",
				from.Name,
				this.Definition.FriendlyName,
				text
			);
		}
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register(
				...
				new CommandEventHandler(FactionElection_OnCommand)
			);
			CommandSystem.Register(
				"FactionCommander",
				AccessLevel.Administrator,
				new CommandEventHandler(FactionCommander_OnCommand)
			);
			CommandSystem.Register(
				"FactionItemReset",
				AccessLevel.Administrator,
				new CommandEventHandler(FactionItemReset_OnCommand)
			);
			CommandSystem.Register(
				"FactionReset",
				AccessLevel.Administrator,
				new CommandEventHandler(FactionReset_OnCommand)
			);
			CommandSystem.Register(
				"FactionTownReset",
				AccessLevel.Administrator,
				new CommandEventHandler(FactionTownReset_OnCommand)
			);
		}
			...
			for (int i = 0; i < sigils.Count; ++i)
			{
				...
				else
				{
					if (sigil.IsBeingCorrupted && (sigil.CorruptionStart + Sigil.CorruptionPeriod) < DateTime.UtcNow)
					{
						...
						sigil.IsPurifying
						&& (sigil.PurificationStart + Sigil.PurificationPeriod) < DateTime.UtcNow
					)
					{
		...
		public static TimeSpan SkillLossPeriod
		{
			get { return Core.SA ? TimeSpan.FromMinutes(5) : TimeSpan.FromMinutes(20.0); }
		}

		private static readonly Dictionary<Mobile, SkillLossContext> m_SkillLoss =
			new Dictionary<Mobile, SkillLossContext>();
		...
		{
			get { return this.m_State.Traps; }
			set { this.m_State.Traps = value; }
		}
		...
		public bool CanHandleInflux(int influx)
		{
			...
			if (
				StabilityFactor > 0
				&& (((this.Members.Count + influx) * 100) / StabilityFactor) > smallest.Members.Count
			)
				return false;
			...
			if (killerState.Faction != victimState.Faction)
			{
				if (victimState.KillPoints <= -6)
				{
					...
					if (
						...
						&& victimEPL != null
						&& victimEPL.Power > 0
						&& victimState.CanGiveSilverTo(killer)
					)
					{
						...
						if (victimState.KillPoints > 0)
						{
							...
							if (silver > 0)
								killer.SendLocalizedMessage(
									1042736,
									String.Format("{0:N0} silver\t{1}", silver, victim.Name)
