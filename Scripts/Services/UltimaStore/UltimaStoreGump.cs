	...
	public class UltimaStoreGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddButton(
				36,
				97,
				Category == StoreCategory.Featured ? 0x9C5F : 0x9C55,
				0x9C5F,
				100,
			...
			AddButton(
				36,
				126,
				Category == StoreCategory.Character ? 0x9C5F : 0x9C55,
				0x9C5F,
				101,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				36,
				155,
				Category == StoreCategory.Equipment ? 0x9C5F : 0x9C55,
				0x9C5F,
				102,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				36,
				184,
				Category == StoreCategory.Decorations ? 0x9C5F : 0x9C55,
				0x9C5F,
				103,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				36,
				213,
				Category == StoreCategory.Mounts ? 0x9C5F : 0x9C55,
				0x9C5F,
				104,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				43,
				360,
				SortBy == SortBy.Name ? 0x9C4F : 0x9C4E,
				SortBy == SortBy.Name ? 0x9C4F : 0x9C4E,
				108,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				43,
				386,
				SortBy == SortBy.PriceLower ? 0x9C4F : 0x9C4E,
				SortBy == SortBy.PriceLower ? 0x9C4F : 0x9C4E,
				109,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				43,
				412,
				SortBy == SortBy.PriceHigher ? 0x9C4F : 0x9C4E,
				SortBy == SortBy.PriceHigher ? 0x9C4F : 0x9C4E,
				110,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				43,
				438,
				SortBy == SortBy.Newest ? 0x9C4F : 0x9C4E,
				SortBy == SortBy.Newest ? 0x9C4F : 0x9C4E,
				111,
				GumpButtonType.Reply,
				0
			...
			AddButton(
				43,
				464,
				SortBy == SortBy.Oldest ? 0x9C4F : 0x9C4E,
				SortBy == SortBy.Oldest ? 0x9C4F : 0x9C4E,
				112,
				GumpButtonType.Reply,
				0
			...
			AddHtmlLocalized(
				...
				25,
				1156593,
				String.Format("@{0}@{1}", UltimaStore.CartCount(User), Configuration.CartCapacity),
				0x7FFF,
				false,
				false
			);
					...
					foreach (var kvp in profile.Cart)
					{
						...
						if (entry.Name[0].Number > 0)
							...
							AddHtml(
								...
								256,
								25,
								Color(C16232(0x6B55), entry.Name[0].String),
								false,
								false
							);
					...
					for (int j = 0; j < entry.Name.Length; j++)
					{
						if (entry.Name[j].Number > 0)
							AddHtmlLocalized(
								...
								25,
								1114513,
								String.Format("#{0}", entry.Name[j].Number.ToString()),
								0x7FFF,
								false,
								false
							);
							...
							AddHtml(
								...
								183,
								25,
								ColorAndCenter("#FFFFFF", entry.Name[j].String),
								false,
								false
							);
					}
		...
		{
			if (
				pm.HasGump(typeof(UltimaStoreGump))
				|| pm.HasGump(typeof(NoFundsGump))
				|| pm.HasGump(typeof(ConfirmPurchaseGump))
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			else if (id < 3000) // Change Amount In Cart
			{
				...
				SendGump(
					new ConfirmCartGump(User, this, entry, Cart != null && Cart.ContainsKey(entry) ? Cart[entry] : 0)
				);
				return;
			}
			...
			for (var i = 0; i < Entry.Name.Length; i++)
			{
				if (Entry.Name[i].Number > 0)
				{
					AddHtmlLocalized(
						...
						20,
						1114513,
						String.Format("#{0}", Entry.Name[i].Number),
						0x6B45,
						false,
						false
					);
				...
				else
				{
					AddHtml(
						...
						400,
						20,
						ColorAndCenter(C16232(0x6B45), Entry.Name[i].String),
						false,
						false
					);
				}
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				Color(
					"#da0000",
					String.Format(
						"This transaction cannot be completed due to insufficient funds available. Visit your shards website for more information on how to obtain {0}.",
