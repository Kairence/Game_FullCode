...
namespace Server.Engines.Astronomy
{
	...
	public static class AstronomySystem
	{
		...
		private static void CreateConstellations(int amount)
		{
			...
			{
				if (
					Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.FiveToEight).Count()
					> Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.NineToEleven).Count()
				)
					next = TimeCoordinate.NineToEleven;
				else if (
					Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.NineToEleven).Count()
					> Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.Midnight).Count()
				)
					next = TimeCoordinate.Midnight;
				else if (
					Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.Midnight).Count()
					> Constellations.Where(c => c.TimeCoordinate == TimeCoordinate.OneToFour).Count()
				)
			...
			for (int i = 0; i < amount; i++)
			{
				...
				switch (next)
				{
					case TimeCoordinate.FiveToEight:
						next = TimeCoordinate.NineToEleven;
						break;
					case TimeCoordinate.NineToEleven:
						next = TimeCoordinate.Midnight;
						break;
					case TimeCoordinate.Midnight:
						next = TimeCoordinate.OneToFour;
						break;
					case TimeCoordinate.OneToFour:
						next = TimeCoordinate.FiveToEight;
						break;
				}
			}
		}
		...
		public static int RandomSkyImage(TimeCoordinate TimeCoordinate)
		{
			switch (TimeCoordinate)
			{
				...
				case TimeCoordinate.FiveToEight:
					return 0x67F;
				case TimeCoordinate.NineToEleven:
					return Utility.RandomMinMax(0x680, 0x682);
				case TimeCoordinate.Midnight:
					return 0x686;
				case TimeCoordinate.OneToFour:
					return Utility.RandomMinMax(0x683, 0x685);
			}
		}
