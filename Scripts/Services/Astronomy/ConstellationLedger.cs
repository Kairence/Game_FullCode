...
namespace Server.Engines.Astronomy
{
	public class ConstellationLedger : Item
	{
		...
		public class ConstellationLedgerGump : BaseGump
		{
			...
			public int Pages
			{
				get { return (int)Math.Ceiling((double)AstronomySystem.DiscoveredConstellations.Count / 20.0); }
			}
			...
			public override void AddGumpLayout()
			{
				...
				AddHtmlLocalized(
					...
					String.Format(
						"{0}\t{1}",
						AstronomySystem.DiscoveredConstellations.Count,
						AstronomySystem.MaxConstellations
					),
					0x0,
					false,
					false
				...
				for (int i = start; i < AstronomySystem.DiscoveredConstellations.Count && i <= start + 20; i++)
				{
					...
					AddHtml(
						...
						112,
						18,
						Color("#0040FF", info.DiscoveredBy != null ? info.DiscoveredBy.Name : "Unknown"),
						false,
						false
					);
					...
					AddHtmlLocalized(
						...
						18,
						1158527,
						String.Format("{0}\t{1}", info.CoordRA, info.CoordDEC),
						0x1F,
						false,
						false
					); // RA ~1_VAL~  DEC ~2_VAL~
