...
namespace Server.Engines.Points
{
	...
	public abstract class PointsSystem
	{
		...
		public virtual bool ShowOnLoyaltyGump
		{
			get { return true; }
		}
		...
		public virtual void SendMessage(PlayerMobile from, double old, double points, bool quest)
		{
			if (quest)
				...
				from.SendLocalizedMessage(
					1115920,
					String.Format("{0}\t{1}", Name.ToString(), ((int)points).ToString())
				); // Your loyalty to ~1_GROUP~ has increased by ~2_AMOUNT~;Original
		}
			...
			else
			{
				...
				if (message)
					from.SendLocalizedMessage(
						1115921,
						String.Format("{0}\t{1}", Name.ToString(), ((int)points).ToString())
					); // Your loyalty to ~1_GROUP~ has decreased by ~2_AMOUNT~;Original
			}
		...
		public TEntry GetPlayerEntry<TEntry>(Mobile mobile, bool create = false)
			where TEntry : PointsEntry
		{
			...
			Persistence.Deserialize(
				...
				reader =>
				{
					...
					for (int i = 0; i < count; i++)
					{
						...
						{
							throw new Exception(
								String.Format("Points System Failed Load: {0} Last Loaded...", loaded.ToString())
							);
						}
					}
				}
