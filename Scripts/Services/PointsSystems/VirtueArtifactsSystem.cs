...
namespace Server.Misc
{
	public class VirtueArtifactsSystem : PointsSystem
	{
		...
		{
			get { return SeasonalEventSystem.IsActive(EventType.VirtueArtifacts); }
		}

		private static Type[] m_VirtueArtifacts = new Type[]
		{
			typeof(KatrinasCrook),
			typeof(JaanasStaff),
			typeof(DragonsEnd),
			typeof(AnkhPendant),
			typeof(SentinelsGuard),
			typeof(LordBlackthornsExemplar),
			typeof(MapOfTheKnownWorld),
		...
		{
			get { return m_VirtueArtifacts; }
		}

		public override PointsType Loyalty
		{
			get { return PointsType.VAS; }
		}
