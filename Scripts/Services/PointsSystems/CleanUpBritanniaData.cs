...
namespace Server.Engines.Points
{
	public class CleanUpBritanniaData : PointsSystem
	{
		...
		{
			get { return PointsType.CleanUpBritannia; }
		}
		...
		public static double GetPoints(Item item)
		{
			...
			else
			{
				...
				if (
					...
					&& points < 100
					&& item is IShipwreckedItem
					&& ((IShipwreckedItem)item).IsShipwreckedItem
				)
				{
					points = 100;
				}
					...
					if (crItem != null && crItem.Resources != null)
					{
						...
						int weight =
							item is BaseWeapon && !((BaseWeapon)item).DImodded
								? Imbuing.GetTotalWeight(item, 12, false, true)
		...
		public bool RemovePointsFromExchange(Mobile m)
		{
			...
			else if (PointsExchange.ContainsKey(a.Username))
			{
				...
				m.SendLocalizedMessage(
					1158453,
					String.Format("{0}\t{1}", points.ToString("N0"), ((int)GetPoints(m)).ToString("N0"))
				); // You have withdrawn ~1_VALUE~ Cleanup Britannia Points.  You now have ~2_VALUE~ points.
				return true;
			}
	...
	public class AppraiseforCleanupTarget : Target
	{
		...
		public AppraiseforCleanupTarget(Mobile from)
