...
namespace Server.Items
{
	...
	public class Clippers : BaseTool
	{
		...
		public virtual PlantHue PlantHue
		{
			get { return PlantHue.None; }
		}
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (pm.ToggleCutClippings)
				{
					...
					from.AddToBackpack(
						new PlantClippings { Hue = ((PlantItem)targeted).Hue, PlantHue = plant.PlantHue }
					);
					plant.Delete();
				...
				{
					from.PlaySound(0x248);
					from.AddToBackpack(new DryReeds { Hue = ((PlantItem)targeted).Hue, PlantHue = plant.PlantHue });
					plant.Delete();
					m_Item.ConsumeUse(from);
				}
				...
				{
					if (
						plant.PlantType == PlantType.HedgeTall
						|| plant.PlantType == PlantType.HedgeShort
						|| plant.PlantType == PlantType.JuniperBush
					)
					{
	...
	public class TopiaryGump : Gump
	{
		...
		public TopiaryGump(PlantItem plant, Clippers clippers)
			: base(0, 0)
		{
			m_plant = plant;
			m_clippers = clippers;
