...
namespace Server.Items
{
	public class NaturalDye : Item, IPigmentHue
	{
		...
		public override void AddNameProperty(ObjectPropertyList list)
		{
			...
			if (Amount > 1)
				list.Add(
					PlantPigmentHueInfo.IsBright(m_Hue) ? 1113277 : 1113276,
					"{0}\t{1}",
					Amount,
					"#" + hueInfo.Name
				); // ~1_COLOR~ Softened Reeds
			else
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (item != null)
				{
					bool valid = (
						...
						|| item is BaseJewel
						|| item is BaseStatuette
						|| item is BaseWeapon
						|| item is Runebook
						|| item is Spellbook
						|| item is DecorativePlant
						|| item is ShoulderParrot
						|| item.IsArtifact
						|| BasePigmentsOfTokuno.IsValidItem(item)
					);
					...
					if (!valid && item is BaseArmor)
					{
						...
						if (
							(CraftResourceType.Leather == restype || CraftResourceType.Metal == restype)
