...
namespace Server.Regions
{
	public class SeaMarketRegion : BaseRegion
	{
		...
		public Dictionary<BaseBoat, DateTime> BoatTable
		{
			get { return m_BoatTable; }
		}
		...
		private static Rectangle2D[] m_Bounds = new Rectangle2D[] { new Rectangle2D(4529, 2296, 45, 112) };
		...
		{
			if (
				m_PirateBlabTable.ContainsKey(from) && m_PirateBlabTable[from] > DateTime.UtcNow
				|| BountyQuestSpawner.Bounties.Count <= 0
			)
				return;
			...
			if (bounties.Count > 0)
			{
				...
				if (bounty != null)
				{
					...
					combine = String.Format(
						"{0}\t{1}",
						capt.PirateName > -1 ? String.Format("#{0}", capt.PirateName) : capt.Name,
						locArgs
					);
		...
		public static void CheckBabble(Region r)
		{
			...
			foreach (
				var player in r.GetEnumeratedMobiles()
					.Where(p =>
						p is PlayerMobile && p.Alive /*&&
		...
		public void OnTick()
		{
			...
			{
				if (
					!m_BoatTable.ContainsKey(boat)
					&& !boat.IsMoving
					&& boat.Owner != null
					&& boat.Owner.AccessLevel == AccessLevel.Player
				)
					AddToTable(boat);
			}
		...
		public void AddToTable(BaseBoat boat)
		{
			...
			if (boat.Owner != null && boat.Owner.NetState != null)
				boat.Owner.SendMessage(
					"You can only dock your boat here for {0} minutes.",
					(int)KickDuration.TotalMinutes
				);
		}
		...
		public static void StartTimers_Callback()
		{
			...
			m_BlabTimer = Timer.DelayCall(
				TimeSpan.FromSeconds(1),
				TimeSpan.FromSeconds(1),
				new TimerCallback(CheckBlab_Callback)
			);
		...
		public static void GetBoatInfo_OnCommand(CommandEventArgs e)
		{
			...
			for (int i = 0; i < boats.Count; i++)
			{
				...
				e.Mobile.SendMessage(
					"Boat Name: {0}; Boat Owner: {1}; Expires: {2}",
					boat.ShipName,
					boat.Owner,
					times[i]
				);
