...
namespace Server.Regions
{
	public class CorgulRegion : BaseRegion
	{
		...
		public override bool OnBeginSpellCast(Mobile m, ISpell s)
		{
			...
			{
				if (
					s is Server.Spells.Sixth.MarkSpell
					|| s is Server.Spells.Fourth.RecallSpell
					|| s is Server.Spells.Seventh.GateTravelSpell
		...
		public void RemovePlayers(bool message)
		{
			...
			foreach (Mobile m in list)
			{
				...
				if (
					m is PlayerMobile
					|| (m is BaseCreature && ((BaseCreature)m).Controlled || ((BaseCreature)m).Summoned)
				)
				{
