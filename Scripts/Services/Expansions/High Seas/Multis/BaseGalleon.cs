...
namespace Server.Multis
{
	...
	public abstract class BaseGalleon : BaseBoat
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public ShipWheel Wheel
		{
			get
			{
				return _Wheel ?? (_Wheel = Fixtures.FirstOrDefault(f => f.GetType() == typeof(ShipWheel)) as ShipWheel);
			}
		}
		...
		public GalleonHold GalleonHold
		{
			...
			{
				return _Hold
					?? (_Hold = Fixtures.FirstOrDefault(f => f.GetType() == typeof(GalleonHold)) as GalleonHold);
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public BindingPole Pole
		{
			get
			{
				return _Pole
					?? (_Pole = Fixtures.FirstOrDefault(f => f.GetType() == typeof(BindingPole)) as BindingPole);
			}
		}
		...
		public override bool IsClassicBoat
		{
			get { return false; }
		}

		public virtual int MaxCannons
		{
			get { return 0; }
		}
		public virtual int WheelDistance
		{
			get { return 0; }
		}
		public virtual int CaptiveOffset
		{
			get { return 0; }
		}
		public virtual int MaxAddons
		{
			get { return 0; }
		}
		public virtual double CannonDamageMod
		{
			get { return 1.0; }
		}
		...
		public BaseGalleon(Direction direction)
		...
		private void AddFixtures(bool fromConstruct)
		{
			...
			foreach (
				var mte in mcl.List.Where(e =>
					(TileFlag)e.m_Flags == TileFlag.None || (TileFlag)e.m_Flags == TileFlag.Generic
				)
			)
			{
			...
			if (_InternalCannon != null)
			{
				foreach (KeyValuePair<Item, Item> kvp in _InternalCannon)
				{
					Point3D p = new Point3D(
						kvp.Value.X,
						kvp.Value.Y,
						kvp.Value.Z + TileData.ItemTable[kvp.Value.ItemID & TileData.MaxItemValue].CalcHeight
					);
		...
		public override bool CanMoveOver(IEntity entity)
		{
			if (
				entity.Z <= Z
				&& entity is Item
				&& !((Item)entity).ItemData.Impassable
				&& ((Item)entity).ItemData.Height < ZSurface / 2
			)
				return true;
		...
		public override bool HasAccess(Mobile from)
		{
			if (
				Owner == null
				|| (Scuttled && IsEnemy(from)) /* || (Owner is BaseCreature && !Owner.Alive)*/
			)
				return true;
		...
		public override bool IsComponentItem(IEntity entity)
		{
			...
			if (Addons != null)
			{
				if (item is BaseAddon && Addons.ContainsKey(item))
				{
					...
					&& ((AddonComponent)item).Addon != null
					&& Addons.ContainsKey(((AddonComponent)item).Addon)
				)
				{
					return true;
				}
		...
		public override DryDockResult CheckDryDock(Mobile from, Mobile dockmaster)
		{
			...
			if (
				dockmaster != null
				&& pack != null
				&& pack.GetAmount(typeof(Gold)) < DockMaster.DryDockAmount
				&& Banker.GetBalance(from) < DockMaster.DryDockAmount
			)
		...
		public override void SetFacingComponents(
			Direction newDirection,
			Direction oldDirection,
			bool ignoreLastDirection
		)
		{
			...
			{
				foreach (
					var fixture in Fixtures.Where(f =>
						f.X - X == mte.m_OffsetX && f.Y - Y == mte.m_OffsetY && f.Z - Z == mte.m_OffsetZ
					)
				)
				{
		...
		public void UpdateCannonID(Item cannon)
		{
			...
			int type =
				cannon is PumpkinCannon ? 3
				: cannon is Blundercannon ? 2
				: cannon is LightShipCannon || cannon is Culverin ? 0
				: 1;
		...
		public static int[][] CannonIDs
		{
			get { return m_CannonIDs; }
		}
		...
		public void PaintComponents()
		{
			foreach (
				var fixture in Fixtures.Where(f =>
					f.GetType() != typeof(MooringLine) && f.GetType() != typeof(ShipWheel)
				)
			)
			{
				fixture.Hue = Hue;
		...
		public Point3D GetRecallOffset()
		{
			switch (Facing)
			{
				default:
				case Direction.North:
					return new Point3D(X, Y + RuneOffset, GalleonPilot.Z);
				case Direction.South:
					return new Point3D(X, Y - RuneOffset, GalleonPilot.Z);
				case Direction.East:
					return new Point3D(X + RuneOffset, Y, GalleonPilot.Z);
				case Direction.West:
					return new Point3D(X - RuneOffset, Y, GalleonPilot.Z);
			}
		}
		...
		#region Addons
		public static int[] ShipAddonTiles
		{
			get { return m_ShipAddonTiles; }
		}
	...
	public class SecurityEntry
	{
		...
		public Dictionary<Mobile, SecurityLevel> Manifest
		{
		...
		public bool IsPublic
		{
			get { return DefaultPublicAccess != SecurityLevel.Denied; }
		}
		...
		public bool IsInParty(Mobile from)
		{
			...
			if (fromParty == ownerParty)
			{
				switch (PartyAccess)
				{
					case PartyAccess.Never:
						return false;
					case PartyAccess.LeaderOnly:
						return ownerParty.Leader == Galleon.Owner;
					case PartyAccess.MemberOnly:
						return true;
				}
			}
			return false;
