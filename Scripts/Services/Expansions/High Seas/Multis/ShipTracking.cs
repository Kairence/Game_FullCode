...
namespace Server.Multis
{
	public class ShipTrackingContext
	{
		...
		public List<BoatTrackingArrow> Arrows
		{
			get { return m_Arrows; }
		}
	...
	public class BoatTrackingArrow : QuestArrow
	{
		...
		public BoatTrackingArrow(Mobile from, Item boat, int range)
			: base(from, boat)
		{
			m_Boat = boat;
			m_From = from;
		...
		public static void StartTracking(Mobile from)
		{
			...
			foreach (Item item in eable)
			{
				...
				if (
					...
					&& ((item is BaseBoat && (BaseBoat)item != fromBoat) || item is PlunderBeaconAddon)
				)
					targets.Add(item);
			}
	...
	public class BoatTrackingTimer : Timer
	{
		...
		public BoatTrackingTimer(Mobile from, Item target, int range, QuestArrow arrow)
