	...
	public class GalleonPilot : BaseCreature
	{
			...
			if (Core.SE && this.Map == Map.Tokuno)
			{
				...
				else
				{
					switch (Utility.Random(2))
					{
						case 0:
						{
							...
							SetWearable(new JinBaori(GetRandomHue()));
							break;
						}
						case 1:
							SetWearable(new HakamaShita(GetRandomHue()));
							break;
					}
					switch (Utility.Random(2))
					{
						case 0:
							SetWearable(new Hakama(GetRandomHue()));
							break;
						case 1:
							SetWearable(new TattsukeHakama(GetRandomHue()));
							break;
					}
				}
				switch (Utility.Random(2))
				{
					case 0:
						...
						break;
					case 1:
						SetWearable(new Waraji(GetShoeHue()));
						break;
				}
			}
			else
				...
				if (m_Pilot != null && m_Pilot.Galleon != null)
				{
					...
					else if (g.IsUnderEmergencyRepairs())
					{
						...
						m_From.SendLocalizedMessage(
							1116592,
							left != TimeSpan.Zero ? left.TotalMinutes.ToString() : "0"
						); // Your ship is underway with emergency repairs holding for an estimated ~1_TIME~ more minutes.
					}
						...
						m_From.SendLocalizedMessage(
							...
							string.Format(
								"{0}\t{1}",
								BaseBoat.EmergencyRepairClothCost.ToString(),
								BaseBoat.EmergencyRepairWoodCost
							)
						); //You need a minimum of ~1_CLOTH~ yards of cloth and ~2_WOOD~ pieces of lumber to effect emergency repairs.
				}
			}
		}
		...
		private class ResetSecuritySettings : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				{
					m_From.SendGump(
						new BasicConfirmGump<BaseGalleon>(
							new TextDefinition(1116618),
							(m, boat) => // Are you sure you wish to clear your ship's access list?
							{
								boat.SecurityEntry.Manifest.Keys.Where(x => !boat.IsOwner(x))
									.ToList()
									.ForEach(y =>
									{
