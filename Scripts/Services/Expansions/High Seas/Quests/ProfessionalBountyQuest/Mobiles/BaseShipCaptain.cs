...
namespace Server.Mobiles
{
	public class BaseShipCaptain : BaseCreature
	{
		...
		{
			get { return m_OnCourse; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime NextCannonShot
		{
			get { return m_NextCannonShot; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime NextMoveCheck
		{
			get { return m_NextMoveCheck; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime NextCrewCheck
		{
			get { return m_NextCrewCheck; }
			set { m_NextCrewCheck = value; }
		}
		...
		{
			get { return m_Blockade; }
			set { m_Blockade = value; }
		}
		...
		{
			get { return TimeSpan.FromSeconds(Math.Min(20, 20.0 - ((double)m_Crew.Count * 2.5))); }
		}
		...
		public void ShootCannons(Mobile focus, bool shootAtBoat)
		{
			...
			foreach (var cannon in cannons.OfType<IShipCannon>())
			{
				if (cannon != null)
				{
					...
					cannon.Shoot(this);
					m_ShootTable[cannon] =
						DateTime.UtcNow + ShootFrequency + TimeSpan.FromSeconds(Utility.RandomMinMax(0, 3));
				}
			}
		}
		...
		public void CheckCrew()
		{
			...
			List<Mobile> crew = new List<Mobile>(
				m_Crew.Where(m => m != null && m.Alive && m.Map != null && m.Map != Map.Internal)
			);
			...
			foreach (var crewman in crew)
			{
				if (!m_Galleon.Contains(crewman))
				{
					crewman.MoveToWorld(
						new Point3D(
							m_Galleon.X + Utility.RandomList(-1, 1),
							m_Galleon.Y + Utility.RandomList(-1, 0, 1),
							m_Galleon.ZSurface
						),
						this.Map
					);
