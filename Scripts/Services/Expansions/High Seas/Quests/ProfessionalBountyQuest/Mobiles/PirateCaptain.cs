...
namespace Server.Mobiles
{
	public class PirateCaptain : BaseShipCaptain
	{
		...
		public int PirateName
		{
			get { return m_PirateName; }
		}
		public int Adjective
		{
			get { return m_Adjective; }
		}
		public int Noun
		{
			get { return m_Noun; }
		...
		public void OnBound(ProfessionalBountyQuest quest)
		{
			...
			Timer.DelayCall(
				...
				new TimerStateCallback(MoveCaptainToShip),
				new object[] { x, y, pole }
			);
		}
		...
		public override bool OnBeforeDeath()
		{
			...
			for (int i = 0; i < rights.Count; i++)
			{
				...
				if (
					mob is PlayerMobile
					&& mob.NetState != null
					&& QuestHelper.GetQuest((PlayerMobile)mob, typeof(ProfessionalBountyQuest)) != null
				)
			...
			if (m_IsCaught && m_Quest != null)
			{
				...
				{
					if (
						m_Quest.Objectives[i] is BountyQuestObjective
						&& ((BountyQuestObjective)m_Quest.Objectives[i]).CapturedCaptain == this
					)
					{
