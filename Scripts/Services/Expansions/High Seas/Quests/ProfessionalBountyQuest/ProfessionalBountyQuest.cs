...
namespace Server.Engines.Quests
{
	public class ProfessionalBountyQuest : BaseQuest
	{
		...
		public BindingRope Rope
		{
			get { return m_Rope; }
		}
		public Mobile Captain
		{
			get { return m_Captain; }
			set { m_Captain = value; }
		}
		...
		public void AddPole()
		{
			...
			m_Pole.MoveToWorld(
				new Point3D(m_Galleon.X + xOffset, m_Galleon.Y + yOffset, m_Galleon.ZSurface),
				m_Galleon.Map
			);
				...
				if (mob.NetState != null || mob == Owner)
				{
					...
					if (captured)
					{
						...
						if (reward != null)
						{
							...
							if (
								reward is HeavyFlameCannonball
								|| reward is LightFlameCannonball
								|| reward is HeavyFrostCannonball
								|| reward is LightFrostCannonball
							)
					...
					foreach (Mobile mobile in m_Helpers)
					{
						if (mobile != mob && mobile.NetState != null)
							mobile.SendLocalizedMessage(
								1149837,
								String.Format("{0}\t{1}\t{2}", eachAward, mob.Name, Owner.Name)
							); //~1_val~ gold is for ~2_val~, I can't find them so I'm giving this to Captain ~3_val~.
					}
