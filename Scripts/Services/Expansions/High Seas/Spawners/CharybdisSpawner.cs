...
namespace Server.Mobiles
{
	...
	public class CharydbisSpawner
	{
		...
		public Charydbis Charydbis
		{
			get { return m_Charydbis; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Rectangle2D CurrentLocation
		{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Rectangle2D LastLocation
		{
		...
		public bool TrySpawnCharybdis(Mobile from)
		{
			...
			else if (m_IsSummoned)
			{
				if (m_Charydbis != null && m_Charydbis.Alive)
				{
					...
					from.SendMessage(
						String.Format("The location you seek is: {0} in {1}", GetSextantLocation(pnt), m_Map)
					);
				}
		...
		public void SpawnCharydbis(Mobile from, Point3D pnt, Map map, BaseBoat boat)
		{
			...
			Timer.DelayCall(
				...
				new TimerStateCallback(DoTeleportEffect),
				new object[] { pnt, map }
			);
			Timer.DelayCall(
				TimeSpan.FromSeconds(12),
				new TimerStateCallback(DoDelayedSpawn),
				new object[] { from, pnt, map, boat }
			);
		}
			...
			if (m_Charydbis != null && m_Charydbis.Alive)
			{
				...
				foreach (Mobile mob in eable)
				{
					if (mob is PlayerMobile)
						mob.SendMessage(
							"Charydbis sinks to the depths of the ocean from which it came from...You have taken too long!"
						);
				}
				eable.Free();
