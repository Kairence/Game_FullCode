...
namespace Server.Engines.Quests
{
	public class BountyQuestSpawner : Item
	{
		...
		public static Dictionary<Mobile, int> Bounties
		{
		...
		public static Dictionary<SpawnZone, SpawnDefinition> Zones
		{
			get { return m_Zones; }
		}
		...
		public int aFelMoonglowCount
		{
			get { return m_ActiveZones[SpawnZone.FelMoonglow].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTramMoonglowCount
		{
			get { return m_ActiveZones[SpawnZone.TramMoonglow].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTramJhelomCount
		{
			get { return m_ActiveZones[SpawnZone.TramJhelom].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aFelJhelomCount
		{
			get { return m_ActiveZones[SpawnZone.FelJhelom].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTokunoPirateCount
		{
			get { return m_ActiveZones[SpawnZone.TokunoPirate].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTramMerch1Count
		{
			get { return m_ActiveZones[SpawnZone.TramMerch1].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aFelMerch1Count
		{
			get { return m_ActiveZones[SpawnZone.FelMerch1].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTramMerch2Count
		{
			get { return m_ActiveZones[SpawnZone.TramMerch2].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aFelMerch2Count
		{
			get { return m_ActiveZones[SpawnZone.FelMerch2].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int aTokunoMerchCount
		{
			get { return m_ActiveZones[SpawnZone.TokunoMerch].Count; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int MaxTram
		{
			get { return m_MaxTram; }
			set { m_MaxTram = value; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int MaxTokuno
		{
			get { return m_MaxTokuno; }
			set { m_MaxTokuno = value; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int bPirateCount
		{
			get { return m_Bounties.Count; }
		}
		...
		public void SpawnPirateAndGalleon(SpawnZone zone, Map map)
		{
			...
			if (map != Map.Internal && map != null)
			{
				...
				for (int i = 0; i < crewCount; i++)
				{
					...
					crew.MoveToWorld(
						new Point3D(
							gal.X + Utility.RandomList(-1, 1),
							gal.Y + Utility.RandomList(-1, 0, 1),
							gal.ZSurface
						),
						map
					);
			...
			if (hold != null)
			{
				...
				if (0.50 > Utility.RandomDouble())
				{
					switch (Utility.Random(4))
					{
						...
						case 1:
						case 2:
							hold.DropItem(new IronWire(Utility.RandomMinMax(1, 5)));
							break;
						case 3:
						case 4:
						case 5:
							hold.DropItem(new CopperWire(Utility.RandomMinMax(1, 5)));
							break;
						case 6:
						case 7:
							hold.DropItem(new SilverWire(Utility.RandomMinMax(1, 5)));
							break;
						case 8:
							hold.DropItem(new GoldWire(Utility.RandomMinMax(1, 5)));
							break;
					}
				}
				...
				{
					case 0:
						hold.DropItem(new HeavyCannonball(Utility.RandomMinMax(5, 10)));
						break;
					case 1:
						hold.DropItem(new LightCannonball(Utility.RandomMinMax(5, 10)));
						break;
					case 2:
						hold.DropItem(new HeavyGrapeshot(Utility.RandomMinMax(5, 10)));
						break;
					case 3:
						hold.DropItem(new LightGrapeshot(Utility.RandomMinMax(5, 10)));
						break;
				}
		...
		public BountyQuestSpawner(Serial serial)
			: base(serial) { }
	...
	public class SpawnDefinition
	{
		...
		public Rectangle2D SpawnRegion
		{
			get { return m_SpawnRegion; }
		}
		public Point2D[][] Waypoints
		{
		...
		{
			new SpawnDefinition(
				new Rectangle2D(1500, 3600, 180, 400),
				m_PirateTramFelCoursesJhelom,
				SpawnZone.TramJhelom,
				Map.Trammel
			);
			new SpawnDefinition(
				new Rectangle2D(1500, 3600, 180, 400),
			...
			new SpawnDefinition(
				new Rectangle2D(4570, 630, 400, 100),
				m_PirateTramFelCoursesMoonglow,
				SpawnZone.TramMoonglow,
				Map.Trammel
			);
			new SpawnDefinition(
				new Rectangle2D(4570, 630, 400, 100),
				m_PirateTramFelCoursesMoonglow,
				SpawnZone.FelMoonglow,
				Map.Felucca
			);
			new SpawnDefinition(
				new Rectangle2D(1022, 1182, 350, 200),
				m_PirateTokunoCourses,
				SpawnZone.TokunoPirate,
				Map.Tokuno
			);

			new SpawnDefinition(
				new Rectangle2D(1780, 1650, 300, 200),
				m_MerchantTramFelCourses1,
				SpawnZone.TramMerch1,
				Map.Trammel
			);
			new SpawnDefinition(
				new Rectangle2D(1780, 1650, 300, 200),
			...
			new SpawnDefinition(
				new Rectangle2D(3780, 2300, 100, 200),
				m_MerchantTramFelCourses2,
				SpawnZone.TramMerch2,
				Map.Trammel
			);
			new SpawnDefinition(
				new Rectangle2D(3780, 2300, 100, 200),
			...
			new SpawnDefinition(
				new Rectangle2D(425, 1335, 160, 80),
				m_MerchantTokunoCourses,
				SpawnZone.TokunoMerch,
				Map.Tokuno
			);
		}

		public static Point2D[][] PirateTramFelCoursesJhelom
		{
			get { return m_PirateTramFelCoursesJhelom; }
		}
		private static Point2D[][] m_PirateTramFelCoursesJhelom = new Point2D[][]
		{
			new Point2D[]
			{
				new Point2D(1598, 3861),
				new Point2D(1520, 3470),
				new Point2D(1418, 3314),
				new Point2D(1159, 3277),
				new Point2D(1320, 3508),
				new Point2D(1527, 3584),
			},
			new Point2D[]
			{
				new Point2D(1947, 3536),
				new Point2D(2023, 4016),
				new Point2D(1795, 3855),
				new Point2D(1613, 3887),
			},
			new Point2D[]
			{
				new Point2D(2135, 4070),
				new Point2D(2802, 4070),
				new Point2D(2620, 3761),
				new Point2D(1725, 3794),
			},
			new Point2D[]
			{
				new Point2D(2154, 3775),
				new Point2D(2378, 3652),
				new Point2D(2388, 3812),
				new Point2D(1696, 3797),
			},
			new Point2D[]
			{
				new Point2D(1599, 3933),
				new Point2D(1299, 3953),
				new Point2D(971, 3799),
				new Point2D(813, 3326),
				new Point2D(1247, 3296),
				new Point2D(1655, 3890),
			},
			new Point2D[] { new Point2D(1694, 3735), new Point2D(1960, 3678), new Point2D(1808, 3966) },
			new Point2D[]
			{
				new Point2D(1799, 4070),
				new Point2D(1810, 3727),
				new Point2D(2150, 3727),
				new Point2D(1691, 3916),
			},
		};

		public static Point2D[][] PirateTramFelCoursesMoonglow
		{
			get { return m_PirateTramFelCoursesMoonglow; }
		}
		private static Point2D[][] m_PirateTramFelCoursesMoonglow = new Point2D[][]
		{
			new Point2D[]
			{
				new Point2D(4415, 792),
				new Point2D(3927, 900),
				new Point2D(4069, 1630),
				new Point2D(4799, 1670),
				new Point2D(4861, 1061),
				new Point2D(4533, 589),
			},
			new Point2D[]
			{
				new Point2D(4265, 145),
				new Point2D(5015, 153),
				new Point2D(5001, 669),
				new Point2D(4950, 720),
				new Point2D(4573, 663),
			},
			new Point2D[]
			{
				new Point2D(4265, 145),
				new Point2D(3709, 107),
				new Point2D(3703, 773),
				new Point2D(4131, 913),
				new Point2D(4533, 713),
			},
			new Point2D[] { new Point2D(5043, 155), new Point2D(4447, 231), new Point2D(4531, 609) },
		};

		public static Point2D[][] PirateTokunoCourses
		{
			get { return m_PirateTokunoCourses; }
		}
		private static Point2D[][] m_PirateTokunoCourses = new Point2D[][]
		{
			new Point2D[]
			{
				new Point2D(1324, 1178),
				new Point2D(1358, 1334),
				new Point2D(1032, 1358),
				new Point2D(1070, 1240),
			},
			new Point2D[]
			{
				new Point2D(1370, 1074),
				new Point2D(1422, 962),
				new Point2D(1416, 620),
				new Point2D(1422, 1310),
			},
			new Point2D[]
			{
				new Point2D(1032, 1104),
				new Point2D(982, 1078),
				new Point2D(942, 914),
				new Point2D(942, 1086),
				new Point2D(982, 1078),
				new Point2D(1134, 1202),
			},
			new Point2D[]
			{
				new Point2D(1320, 1378),
				new Point2D(1050, 1204),
				new Point2D(1356, 1088),
				new Point2D(1244, 1300),
			},
		};

		public static Point2D[][] MerchantTokunoCourses
		{
			get { return m_MerchantTokunoCourses; }
		}
		...
		public static Point2D[][] MerchantTramFelCourses1
		{
			get { return m_MerchantTramFelCourses1; }
		}
		private static Point2D[][] m_MerchantTramFelCourses1 = new Point2D[][]
		{
			new Point2D[]
			{
				new Point2D(2420, 1708),
				new Point2D(2420, 1400),
				new Point2D(3516, 1400),
				new Point2D(3516, 1696),
				new Point2D(2500, 1696),
			},
			new Point2D[]
			{
				new Point2D(2420, 1708),
				new Point2D(2420, 2760),
				new Point2D(3164, 2760),
				new Point2D(3164, 1696),
				new Point2D(2500, 1696),
			},
		};

		public static Point2D[][] MerchantTramFelCourses2
		{
			get { return m_MerchantTramFelCourses2; }
		}
		private static Point2D[][] m_MerchantTramFelCourses2 = new Point2D[][]
		{
			new Point2D[]
			{
				new Point2D(4129, 2367),
				new Point2D(4129, 1891),
				new Point2D(4773, 1891),
				new Point2D(4773, 2639),
				new Point2D(4129, 2639),
				new Point2D(4129, 2351),
			},
			new Point2D[]
			{
				new Point2D(4000, 2415),
				new Point2D(4000, 3515),
				new Point2D(3141, 3515),
				new Point2D(3141, 3043),
				new Point2D(4093, 3043),
				new Point2D(4093, 2371),
			},
		};
	}
}
