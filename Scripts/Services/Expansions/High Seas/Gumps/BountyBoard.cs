...
namespace Server.Items
{
	...
	{
		[Constructable]
		public ProfessionalBountyBoard()
			: base(7774)
		{
			Movable = false;
		}
		...
		public ProfessionalBountyBoard(Serial serial)
			: base(serial) { }
		...
		public override void AddGumpLayout()
		{
			...
			for (int i = Index; i < mobs.Count; i++)
			{
				...
				if (User.NetState != null && User.NetState.IsEnhancedClient && VendorSearch.StringList != null)
				{
					...
					args = String.Format(
						"{0} {1} {2}",
						strList.GetString(capt.Adjective),
						strList.GetString(capt.Noun),
						capt.PirateName > 0 ? strList.GetString(capt.PirateName) : capt.Name
					);
