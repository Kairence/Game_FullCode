...
namespace Server.Items
{
	public class BoatPaint : Item
	{
		...
		public BoatPaint(object hue)
			: this((int)hue) { }

		[Constructable]
		public BoatPaint(int hue)
			: base(4011)
		{
			Hue = hue;
			m_Permanent = false;
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(Mobile from, BoatPaint paint)
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				if (targeted is IPoint3D)
				{
					...
					if (galleon.GetSecurityLevel(from) < SecurityLevel.Captain)
						...
						galleon.Contains(pnt) /*&& boat.X == pnt.X && boat.Y == pnt.Y*/
					)
					{
	...
	public class PermanentBoatPaint : BoatPaint
	{
		...
		[Constructable]
		public PermanentBoatPaint(int hue)
			: base(hue)
		{
			Permanent = true;
		}
		...
		public PermanentBoatPaint(Serial serial)
			: base(serial) { }
