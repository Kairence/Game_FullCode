...
namespace Server.Items
{
	public class CorgulAltar : Container
	{
		...
		public static Rectangle2D[] WarpLocations
		{
			get { return m_WarpLocations; }
		}

		private static Rectangle2D m_BoatKickLocation = new Rectangle2D(2400, 2500, 500, 500);
		public static Rectangle2D BoatKickLocation
		{
			get { return m_BoatKickLocation; }
		}

		private static Rectangle2D m_LandKickLocation = new Rectangle2D(2125, 3090, 25, 30);
		public static Rectangle2D LandKickLocation
		{
			get { return m_LandKickLocation; }
		}

		private static Rectangle2D m_CorgulBounds = new Rectangle2D(6337, 1156, m_RegionSize, m_RegionSize);

		public static Rectangle2D CorgulBounds
		{
			get { return m_CorgulBounds; }
		}
		#endregion
		...
		public bool Active
		{
			...
			set
			{
				...
				PublicOverheadMessage(
					...
					25,
					false,
					String.Format("Corgul Altar for {0} has been {1}", this.Map, m_Active ? "activated" : "deactivated")
				);
			}
		}

		public CorgulWarpRegion WarpRegion
		{
			get { return m_WarpRegion; }
		}
		public CorgulRegion BossRegion
		{
			get { return m_BossRegion; }
		}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(CorgulAltar altar)
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				if (
					targeted is Item
					&& ((Item)targeted).IsChildOf(from.Backpack)
					&& m_Altar.IsKey((Item)targeted, from)
				)
