...
namespace Server.Items
{
	public class Bait : Item
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (IsChildOf(from.Backpack))
			{
				...
				else
				{
					...
					from.SendMessage(
						"Target the fishing pole or lobster trap that you would like to apply the bait to."
					);
				}
			}
		}
		...
		public void TryBeginTarget(Mobile from, int amount)
		{
			if (amount < 0)
				amount = 1;
			if (amount > m_UsesRemaining)
				amount = m_UsesRemaining;
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (targeted is LobsterTrap)
				{
					...
					targeted is Bait
					&& ((Bait)targeted).IsChildOf(from.Backpack)
					&& ((Bait)targeted).BaitType == m_Bait.BaitType
				)
				{
