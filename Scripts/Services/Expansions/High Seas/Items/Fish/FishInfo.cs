...
namespace Server.Items
{
	public class FishInfo
	{
		...
		{
			//Rare Fish; 0 - 15
			m_FishInfos.Add(new FishInfo(2506, typeof(AutumnDragonfish), 1116090, "Despise", false, RareChance, 105.4)); //Confirmed
			...
			m_FishInfos.Add(
				new FishInfo(1167, typeof(CrystalFish), 1116092, "Prism of Light", false, RareChance, 105.4)
			); //Confirmed
			...
			m_FishInfos.Add(new FishInfo(1257, typeof(GiantKoi), 1116088, "Diceit", true, RareChance, 95.8)); //Confirmed
			m_FishInfos.Add(
				new FishInfo(
					2579 /*1287*/
					,
					typeof(GreatBarracuda),
					1116100,
					"Wrong",
					true,
					RareChance,
			...
			m_FishInfos.Add(new FishInfo(2959, typeof(HolyMackerel), 1116087, "Covetous", false, RareChance, 102.9)); //Confirmed
			m_FishInfos.Add(new FishInfo(2075, typeof(LavaFish), 1116096, "Fire", false, RareChance, 110.0)); //Confirmed
			m_FishInfos.Add(
				new FishInfo(
					2075 /*1152*/
					,
			...
			m_FishInfos.Add(new FishInfo(2558, typeof(StoneCrab), 1116365, "Trammel", true, RareChance, 103.1)); //Confirmed
			m_FishInfos.Add(new FishInfo(43, typeof(SummerDragonfish), 1116091, "Destard", false, RareChance, 105.2)); //Confirmed
			m_FishInfos.Add(
				new FishInfo(1154, typeof(UnicornFish), 1116086, "Twisted Weald", false, RareChance, 110.0)
			); //Confirmed
			m_FishInfos.Add(
				new FishInfo(2515, typeof(YellowtailBarracuda), 1116098, "Trammel", true, RareChance, 81.9)
			); //Confirmed

			//Legendary Fish ; 16-34
			m_FishInfos.Add(
				new FishInfo(2406, typeof(AbyssalDragonfish), 1116118, "Destard", false, LegendaryChance, 120.0)
			); //Confirmed
			m_FishInfos.Add(new FishInfo(2500, typeof(BlackMarlin), 1116099, "Wrong", true, LegendaryChance, 110.1)); //Confirmed
			...
			m_FishInfos.Add(
				new FishInfo(1158, typeof(DungeonPike), 1116107, "Terathan Keep", false, LegendaryChance, 105.1)
			); //Confirmed
			m_FishInfos.Add(
				new FishInfo(2576, typeof(GiantSamuraiFish), 1116103, "Diceit", true, LegendaryChance, 110.1)
			); //Confirmed
			m_FishInfos.Add(new FishInfo(1281, typeof(GoldenTuna), 1116102, "Orc Cave", true, LegendaryChance, 105.0)); //Confirmed
			m_FishInfos.Add(new FishInfo(2604, typeof(Kingfish), 1116085, "Trammel", true, LegendaryChance, 92.2)); //Confirmed
			m_FishInfos.Add(
				new FishInfo(1463, typeof(LanternFish), 1116106, "Prism of Light", false, LegendaryChance, 105.1)
			); //Confirmed
			m_FishInfos.Add(
				new FishInfo(1283, typeof(RainbowFish), 1116108, "Twisted Weald", false, LegendaryChance, 105.1)
			);
			m_FishInfos.Add(
				new FishInfo(2076, typeof(SeekerFish), 1116109, "Labyrinth", false, LegendaryChance, 105.1)
			);
			m_FishInfos.Add(
				new FishInfo(0, typeof(SpringDragonfish), 1116104, "Ilshenar", false, LegendaryChance, 120.0)
			); //Confirmed
			m_FishInfos.Add(new FishInfo(1837, typeof(StoneFish), 1116110, "Trammel", true, LegendaryChance, 115.1)); //Confirmed
			m_FishInfos.Add(new FishInfo(2600, typeof(TunnelCrab), 1116372, "Trammel", false, LegendaryChance, 115.1)); //Confirmed
			m_FishInfos.Add(new FishInfo(2069, typeof(VoidCrab), 1116368, "TerMur", false, LegendaryChance, 120.0)); //Confirmed
			m_FishInfos.Add(new FishInfo(2071, typeof(VoidLobster), 1116369, "Trammel", false, LegendaryChance, 120.0));
			m_FishInfos.Add(
				new FishInfo(2499, typeof(WinterDragonfish), 1116105, "Ice", false, LegendaryChance, 120.0)
			); //Confirmed
			m_FishInfos.Add(new FishInfo(2551, typeof(ZombieFish), 1116101, "Covetous", false, LegendaryChance, 115.1)); //Confirmed

			//Cannot fish up but used for bait
			m_FishInfos.Add(
				new FishInfo(1170, typeof(Charydbis), 1150208, "cannotfishup", true, LegendaryChance, 120.0)
			);
		...
		public bool RequiresDeepWater
		{
			get { return m_RequiresDeepWater; }
		}
		public double BaseChance
		{
			get { return m_BaseChance; }
		}
		...
		public static Type GetSpecialItem(Mobile from, Item harvestItem, Point3D pnt, double bump, bool rareOnly)
		{
			...
			for (int i = 0; i < infos.Count; i++)
			{
				...
				if (info.Location is string)
				{
					...
					if (
						loc.ToLower() == "t2a"
						&& Server.Spells.SpellHelper.IsAnyT2A(map, fromLoc)
						&& info.Roll(from, baitStr, bump)
					)
						item = info.Type;

					if (
						loc.ToLower() == "trammelandfelucca"
					...
					if (
						loc.ToLower() == "fire island"
						&& (map == Map.Felucca || map == Map.Trammel)
						&& (
							from.X > 4559 && from.X < 4636 && from.Y > 3548 && from.Y < 3627
							|| from.X > 4465 && from.X < 4493 && from.Y > 4479 && from.Y < 3746
						)
						&& info.Roll(from, baitStr, bump)
					)
						item = info.Type;
		...
		public static bool IsDungeon(Point3D pnt, Map map, Region region)
		{
			return region.IsPartOf<DungeonRegion>()
				|| IsMondainDungeon(region)
				|| Server.Spells.SpellHelper.IsTrammelWind(map, pnt)
				|| Server.Spells.SpellHelper.IsFeluccaWind(map, pnt);
		}
		...
		public static Type[] SOSArtifacts
		{
			get { return m_SOSArtifacts; }
		}
