...
namespace Server.Items
{
	...
	public abstract class BaseShipCannon : Container, IShipCannon
	{
		...
		public bool Empty
		{
			get { return !CanLight && Items.Count == 0; }
		}
			...
			if (Loaded == CannonAction.Finish)
			{
				switch (AmmoType)
				{
					...
					case AmmunitionType.FrostCannonball:
						item = new FrostCannonball();
						break;
					case AmmunitionType.FlameCannonball:
						item = new FlameCannonball();
						break;
				}
		...
		private Type[] _Types = { typeof(Cannonball), typeof(Grapeshot), typeof(PowderCharge), typeof(FuseCord) };
		...
		public void DoAreaMessage(int cliloc, int range, Mobile from)
		{
			...
			Galleon
				.GetEntitiesOnBoard()
				.OfType<PlayerMobile>()
				.Where(x => x != from && Galleon.GetSecurityLevel(x) > SecurityLevel.Denied)
				.ToList()
				.ForEach(y =>
				{
					if (from != null)
					...
					case AmmunitionType.FlameCannonball:
						{
							...
							for (int i = -lateralOffset; i <= lateralOffset; i++)
							{
								...
								damageables.AddRange(
									FindDamageables(shooter, newPoint, map, false, false, false, true, true)
								);
							}
							...
							if (list.Count > 0)
							{
								...
								if (toHit is Mobile)
								{
									Timer.DelayCall(
										...
										new object[] { (Mobile)toHit, newPoint, ammo, shooter }
									);
									hit = true;
								}
								else if (toHit is BaseBoat)
								{
									Timer.DelayCall(
										...
										new object[] { (BaseBoat)toHit, newPoint, ammo, shooter }
									);
									hit = true;
								}
								else if (toHit is DamageableItem)
								{
									Timer.DelayCall(
										delay,
										new TimerStateCallback(OnDamageableItemHit),
										new object[] { (DamageableItem)toHit, newPoint, ammo, shooter }
									);
									hit = true;
								}
							}
					...
					case AmmunitionType.Grapeshot:
						{
							...
							for (int i = -lateralOffset; i <= lateralOffset; i++)
							{
								...
								damageables.AddRange(
									FindDamageables(shooter, newPoint, map, true, true, false, true, true)
								);
							}
		...
		public void DoShootEffects()
		{
			...
			Effects.SendPacket(
				p,
				map,
				new GraphicalEffect(EffectType.FixedXYZ, Serial.Zero, Serial.Zero, 0x3728, p, p, 14, 14, true, true)
			);
				...
				if (ammoInfo.AmmoType == AmmunitionType.Grapeshot)
				{
					for (int count = 15; count > 0; count--)
					{
						...
						Point3D loc = new Point3D(
							hit.X + Utility.RandomMinMax(0, 4),
							hit.Y + Utility.RandomMinMax(0, 4),
							hit.Z
						);
						Effects.SendPacket(
							loc,
		...
		public virtual void OnDamageableItemHit(object obj)
		{
			...
			Effects.SendLocationEffect(
				new Point3D(toHit.X, toHit.Y, toHit.Z + 5),
				toHit.Map,
		...
		public List<IDamageable> FindDamageables(
			Mobile shooter,
			Point3D pnt,
			Map map,
			bool player,
		...
		public BaseShipCannon(Serial serial)
			: base(serial) { }
		...
		public class ShipCannonGump : BaseGump
		{
			...
			public override void AddGumpLayout()
			{
				...
				AddHtmlLocalized(
					...
					115,
					18,
					Cannon.Charged == CannonAction.Finish ? 1149631 : 1149632,
					Cannon.Charged == CannonAction.Finish ? 0x1FE7 : 0x7CE7,
					false,
					false
				); // Charged / Not Charged
				AddHtmlLocalized(
					...
					18,
					1114057,
					Cannon.Loaded == CannonAction.Finish ? AmmoInfo.GetAmmoName(Cannon).ToString() : "#1149636",
					Cannon.Loaded == CannonAction.Finish ? 0x1FE7 : 0x7CE7,
					false,
					false
				); // Cannonball / Not Loaded
				AddHtmlLocalized(
					115,
					89,
					115,
					18,
					Cannon.Primed == CannonAction.Finish ? 1149640 : 1149639,
					Cannon.Primed == CannonAction.Finish ? 0x1FE7 : 0x7CE7,
					false,
					false
				); // Primed / No Fuse
				...
				if (Cannon.Actions.ContainsKey(User))
				{
					...
					for (int i = list.Count - 1; i >= 0; i--)
					{
						AddHtmlLocalized(
							...
							230,
							18,
							Cannon.Actions[User][i],
							actual == list.Count - 1 ? 0x7FE7 : 0x3DEF,
							false,
							false
						);
						actual++;
	...
	public class Culverin : BaseShipCannon
	{
		...
		{
			get { return new CulverinDeed(); }
		}
		public override CannonPower Power
		{
			get { return CannonPower.Light; }
		}

		public Culverin(BaseGalleon g)
			: base(g) { }

		public Culverin(Serial serial)
			: base(serial) { }
	...
	public class Carronade : BaseShipCannon
	{
		...
		public Carronade(BaseGalleon g)
			: base(g) { }

		public Carronade(Serial serial)
			: base(serial) { }
	...
	public class Blundercannon : BaseShipCannon
	{
		...
		public override ShipCannonDeed GetDeed
		{
			get { return new BlundercannonDeed(); }
		}
		public override CannonPower Power
		{
		...
		public Blundercannon(BaseGalleon g)
			: base(g) { }

		public Blundercannon(Serial serial)
			: base(serial) { }
