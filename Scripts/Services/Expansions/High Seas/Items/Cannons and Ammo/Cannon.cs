...
namespace Server.Items
{
	...
	public abstract class BaseCannon : Item, IShipCannon
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Type LoadedAmmo
		{
			get { return m_LoadedAmmo; }
			set { m_LoadedAmmo = value; }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public DamageLevel DamageState
		{
			get { return m_DamageState; }
			set { m_DamageState = value; }
		}
		...
		public virtual ShipCannonDeed GetDeed
		{
			get { return null; }
		}
		public virtual bool HitMultipleMobs
		{
			get { return false; }
		}
		...
		public bool CanLight
		{
			get
			{
				return m_Cleaned && m_Charged && m_Primed && m_AmmoType != AmmunitionType.Empty && m_LoadedAmmo != null;
			}
		}
		...
		{
			get { return ((double)m_Hits / (double)MaxHits) * 100.0; }
		}
			...
			else if (m_Galleon.GetSecurityLevel(from) >= SecurityLevel.Crewman)
			{
				...
				switch (m_DamageState)
				{
					case DamageLevel.Slightly:
						from.SendLocalizedMessage(1116606);
						break; //The cannon is lightly damaged and needs some minor repair.
					case DamageLevel.Moderately:
						from.SendLocalizedMessage(1116607);
						break; //The cannon is moderately damaged and needs some repairs. This cannon is safe to fire.
					case DamageLevel.Severely:
						from.SendLocalizedMessage(1116608);
						break; //The cannon is severely damaged. It needs major repairs before it is safe to fire.
				}
			}
			else
					...
					case AmmunitionType.Cannonball:
						{
							...
							for (int i = -lateralOffset; i <= lateralOffset; i++)
							{
								if (xOffset == 0)
									newPoint = new Point3D(
										pnt.X + (xOffset + i),
										pnt.Y + (yOffset * currentRange),
										pnt.Z
									);
								else
									newPoint = new Point3D(
										pnt.X + (xOffset * currentRange),
										pnt.Y + (yOffset + i),
										pnt.Z
									);
							...
							if (list.Count > 0)
							{
								...
								if (toHit is Mobile)
								{
									Timer.DelayCall(
										delay,
										new TimerStateCallback(OnMobileHit),
										new object[] { mobs, newPoint, ammo, shooter }
									);
									hit = true;
								}
								else if (toHit is BaseGalleon)
								{
									Timer.DelayCall(
										delay,
										new TimerStateCallback(OnShipHit),
										new object[] { (BaseGalleon)toHit, newPoint, ammo, shooter }
									);
									hit = true;
								}
							}
					...
					case AmmunitionType.Grapeshot:
						{
							...
							if (mobiles.Count > 0)
							{
								Timer.DelayCall(
									delay,
									new TimerStateCallback(OnMobileHit),
									new object[] { mobiles, newPoint, ammo, shooter }
								);
								hit = true;
							}
						}
				...
				else
				{
					...
					foreach (
						var mob in target
							.GetMobilesOnBoard()
							.OfType<PlayerMobile>()
							.Where(pm => shooter.CanBeHarmful(pm, false))
					)
					{
		...
		public virtual void OnMobileHit(object obj)
		{
			...
			if (!info.SingleTarget)
			{
				foreach (Mobile mob in mobsToHit)
				{
					...
					if (
						toHit is BaseSeaChampion
						&& info.AmmoType != AmmunitionType.Empty
						&& info.AmmoType == AmmunitionType.Cannonball
					)
						damage *= 100;
		...
		public List<Mobile> FindMobiles(
			Mobile shooter,
			Point3D pnt,
			Map map,
			bool player,
		...
		public void TryRepairCannon(Mobile from)
		{
			...
			from.SendLocalizedMessage(
				1116605,
				String.Format("{0}\t{1}", ((int)temp).ToString(), ((int)percRepaired).ToString())
			); //You make repairs to the cannon using ~1_METAL~ ingots. The cannon is now ~2_DMGPCT~% repaired.
		}
		...
		public void DoLoad(Mobile from, Item ammo)
		{
			...
			int cliloc =
				ammo is ICannonAmmo && ((ICannonAmmo)ammo).AmmoType == AmmunitionType.Cannonball ? 1116036 : 1116037;
		...
		public void Load(object state)
		{
			...
			{
				cliloc =
					ammo is ICannonAmmo && ((ICannonAmmo)ammo).AmmoType == AmmunitionType.Cannonball
						? 1116057
						: 1116058;
		...
		public Dictionary<Mobile, List<int>> Actions
		{
		...
		private class CleanContext : ContextMenuEntry
		{
			...
			public CleanContext(BaseCannon cannon, Mobile from)
				: base(1149626, 3)
			{
				m_From = from;
				m_Cannon = cannon;
		...
		private class ChargeContext : ContextMenuEntry
		{
			...
			public ChargeContext(BaseCannon cannon, Mobile from)
				: base(1149630, 3)
			{
				m_From = from;
				m_Cannon = cannon;
		...
		private class PrimeContext : ContextMenuEntry
		{
			...
			public PrimeContext(BaseCannon cannon, Mobile from)
				: base(1149637, 3)
			{
				m_From = from;
				m_Cannon = cannon;
			...
			public override void OnClick()
			{
				if (m_Cannon != null)
				{
					if (
						...
						|| m_Cannon.Primed
						|| m_Cannon.Charged
						|| m_Cannon.AmmoType != AmmunitionType.Empty
					)
		...
		private class LoadCannonTarget : Target
		{
			...
			public LoadCannonTarget(BaseCannon cannon)
	...
	public class LightShipCannon : BaseCannon
	{
		...
		public override ShipCannonDeed GetDeed
		{
			get { return new LightShipCannonDeed(); }
		}
		...
		public override Type[] LoadTypes
		{
			get
			{
				return new Type[]
				{
					...
					typeof(LightGrapeshot),
					typeof(LightFlameCannonball),
					typeof(LightFrostCannonball),
				};
			}
		}

		public LightShipCannon(BaseGalleon g)
			: base(g) { }
	...
	public class HeavyShipCannon : BaseCannon
	{
		...
		public override Type[] LoadTypes
		{
			get
			{
				return new Type[]
				{
					typeof(HeavyCannonball),
					typeof(HeavyGrapeshot),
					typeof(HeavyFrostCannonball),
					typeof(HeavyFlameCannonball),
				};
			}
		}

		public HeavyShipCannon(BaseGalleon g)
			: base(g) { }
