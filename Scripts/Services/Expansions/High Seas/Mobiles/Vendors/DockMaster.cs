...
namespace Server.Mobiles
{
	public class DockMaster : BaseVendor
	{
		...
		private class RetrieveHoldEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if (
					...
					&& Banker.GetBalance(m_From) < DryDockAmount
				)
				{
					m_DockMaster.PrivateOverheadMessage(
						MessageType.Regular,
						m_DockMaster.SpeechHue,
						1116506,
						DockMaster.DryDockAmount.ToString(),
						m_From.NetState
					); //The price is ~1_price~ and I will accept nothing less!
					return;
				}
		...
		public BaseBoat GetBoatInRegion(Mobile from)
		{
			...
			{
				IPooledEnumerable eable = this.Map.GetItemsInBounds(
					new Rectangle2D(rec.Start.X, rec.Start.Y, rec.Width, rec.Height)
				);
