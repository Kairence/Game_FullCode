...
namespace Server.Mobiles
{
	public class Osiredon : BaseSeaChampion
	{
		...
		public override bool CanDamageBoats
		{
			get { return true; }
		}
		public override bool TaintedLifeAura
		{
			get { return true; }
		}
		public override TimeSpan BoatDamageCooldown
		{
			get { return TimeSpan.FromSeconds(120); }
		}
		public override int MinBoatDamage
		{
			get { return 3; }
		}
		public override int MaxBoatDamage
		{
			get { return 8; }
		}
		...
		{
			get { return new Type[] { typeof(MiniSoulForgeDeed) }; }
		}
		public override Type[] DecorativeList
		{
			get { return new Type[] { typeof(EnchantedBladeDeed), typeof(EnchantedVortexDeed) }; }
		}

		public override bool NoGoodies
		{
			get { return true; }
		}
		...
		public override void OnActionCombat()
		{
			...
			if (
				combatant == null
				|| combatant.Deleted
				|| combatant.Map != this.Map
				|| !this.InRange(combatant, 12)
				|| !this.CanBeHarmful(combatant)
				|| !this.InLOS(combatant)
			)
				return;
			...
			if (DateTime.UtcNow >= this.m_NextWaterBall)
			{
				...
				{
					(combatant as PlayerMobile).SetMountBlock(
		...
		public void DoAreaExplosion()
		{
			...
			{
				if (
					!CanBeHarmful(mob, false)
					|| mob == this
					|| (mob is BaseCreature && ((BaseCreature)mob).GetMaster() == this)
				)
					continue;
				if (mob.Player)
					toExplode.Add(mob);
				if (
					...
					&& (
						...
						|| ((BaseCreature)mob).Summoned
						|| ((BaseCreature)mob).Team != this.Team
					)
				)
					toExplode.Add(mob);
			}
