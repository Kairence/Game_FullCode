...
namespace Server.Engines.MyrmidexInvasion
{
	public class BattleFlag : Item
	{
					...
					Timer.DelayCall(
						...
						() =>
						{
							m.SendLocalizedMessage(
								...
								String.Format(
									"{0}\t{1}\t{2}",
									(BattleSpawner.WaveCount - count).ToString(),
									BattleSpawner.WaveCount.ToString(),
									wave.ToString()
								)
							); // Myrmidex have lost ~1_VAL~ of ~2_VAL~ from wave ~3_VAL~ of their front line.
						}
					);
				}
		...
		{
			get { return BattleSpawner != null && NextSpawn < DateTime.UtcNow; }
		}
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (m is BaseCreature && NextSpawn < DateTime.UtcNow)
			{
				...
				{
					if (
						bc is BritannianInfantry
						|| (bc is BaseEodonTribesman && ((BaseEodonTribesman)bc).TribeType != EodonTribe.Barrab)
					)
					{
