...
namespace Server.Engines.MyrmidexInvasion
{
	public class AllegianceIdol : Item
	{
		...
		[Constructable]
		public AllegianceIdol(Allegiance allegiance)
			: base(allegiance == Allegiance.Myrmidex ? 9730 : 17099)
		{
					...
					else
					{
						from.SendGump(
							new ConfirmCallbackGump(
								...
								(int)_AllegianceType,
								String.Format(
									"Your current allegiance is with the {0}.  Select yes to pledge your allegiance to the {1}.",
									entry.Allegiance == Allegiance.Tribes ? "Eodonians" : "Myrmidex",
									_AllegianceType == Allegiance.Tribes ? "Eodonians" : "Myrmidex"
								),
								entry,
								confirm: (m, state) =>
								{
		...
		public AllegianceIdol(Serial serial)
			: base(serial) { }
