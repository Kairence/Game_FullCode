...
namespace Server.Engines.MyrmidexInvasion
{
	...
	public class MyrmidexInvasionSystem
	{
		...
		{
			return (IsAlliedWithMyrmidex(a) && IsAlliedWithMyrmidex(b))
				|| (IsAlliedWithEodonTribes(a) && IsAlliedWithEodonTribes(b));
		}

		public static bool AreEnemies(Mobile a, Mobile b)
		{
			if (
				(IsAlliedWithEodonTribes(a) && !IsAlliedWithMyrmidex(b))
				|| (IsAlliedWithEodonTribes(b) && !IsAlliedWithMyrmidex(a))
		...
		public static bool IsAlliedWithMyrmidex(Mobile m)
		{
			if (m is BaseCreature)
			{
				...
				return m is MyrmidexLarvae
					|| m is MyrmidexWarrior
					|| m is MyrmidexQueen
					|| m is MyrmidexDrone
					|| (m is BaseEodonTribesman && ((BaseEodonTribesman)m).TribeType == EodonTribe.Barrab);
			}
		...
		public static bool IsAlliedWithEodonTribes(Mobile m)
		{
			if (m is BaseCreature)
			{
				...
				return (m is BaseEodonTribesman && ((BaseEodonTribesman)m).TribeType != EodonTribe.Barrab)
					|| m is BritannianInfantry;
			}
					...
					e.Mobile.BeginTarget(
						...
						(from, targeted) =>
						{
							if (targeted is PlayerMobile)
							{
								...
								if (entry != null)
								{
									((PlayerMobile)targeted).SendGump(
										new PropertiesGump((PlayerMobile)targeted, entry)
									);
								}
								else
