...
namespace Server.Engines.MyrmidexInvasion
{
	public class BattleSpawner : Item
	{
		...
		public static void Initialize()
		{
			if (Instance != null)
			{
				Instance.BattleRegion =
					Region.Regions.FirstOrDefault(r => r.GetType() == typeof(BattleRegion)) as BattleRegion;
		...
		public void CheckPlayers()
		{
			if (HasPlayers())
			{
				...
				if (
					LastMyrmidexWave + TimeSpan.FromSeconds(WaveDuration) < DateTime.UtcNow
					&& MyrmidexTeam.Count < MaxWaves
				)
		...
		private void Reset()
		{
			...
			foreach (
				BaseCreature bc in list.Where(b => b.Alive && !b.Controlled && !b.Summoned && b.GetMaster() == null)
			)
			{
				bc.Kill();
			}
		...
		public void SpawnWave(Allegiance allegiance)
		{
			...
			else
			{
				...
				for (int i = 0; i < WaveCount; i++)
				{
					...
					if (type.IsSubclassOf(typeof(BaseEodonTribesman)))
					{
						EodonTribe tribe = Utility.RandomList<EodonTribe>(
							EodonTribe.Jukari,
							EodonTribe.Kurak,
							EodonTribe.Barako,
							EodonTribe.Urali,
		...
		public IEnumerable<PlayerMobile> GetPlayers(Allegiance allegiance)
		{
			...
			return BattleRegion
				.GetEnumeratedMobiles()
				.OfType<PlayerMobile>()
				.Where(p => MyrmidexInvasionSystem.IsAlliedWith(p, allegiance));
		}
		...
		public bool HasPlayers(bool ignorestaff = true)
		{
			...
			return BattleRegion
				.GetEnumeratedMobiles()
				.Any(m => m is PlayerMobile && (!ignorestaff || m.AccessLevel == AccessLevel.Player));
		}
		...
		public void RegisterDeath(BaseCreature bc)
		{
			...
			foreach (
				var ds in rights.Where(ds =>
					ds.m_Mobile is PlayerMobile && ds.m_HasRight && MyrmidexInvasionSystem.AreEnemies(ds.m_Mobile, bc)
				)
			)
			{
		...
		private static Point2D[][][] _NavPoints1 =
		{
			...
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(853, 1785),
					new Point2D(853, 1800),
					new Point2D(853, 1815),
					new Point2D(855, 1830),
					new Point2D(855, 1845),
					new Point2D(855, 1860),
					new Point2D(855, 1875), /*shares with next*/
					new Point2D(860, 1883),
					new Point2D(872, 1887),
					new Point2D(891, 1887),
				},
				new Point2D[]
				{
					new Point2D(903, 1875),
					new Point2D(903, 1860),
					new Point2D(903, 1845),
					new Point2D(903, 1830),
					new Point2D(903, 1815), /*shares with next*/
					new Point2D(903, 1807),
					new Point2D(905, 1797),
					new Point2D(906, 1787),
				},
			},
			//Lane 2
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(859, 1785),
					new Point2D(859, 1800),
					new Point2D(859, 1815),
					new Point2D(859, 1830),
					new Point2D(859, 1845),
					new Point2D(859, 1860),
					new Point2D(859, 1875), /*shares with prev*/
					new Point2D(860, 1883),
					new Point2D(872, 1887),
					new Point2D(891, 1887),
				},
				new Point2D[]
				{
					new Point2D(909, 1875),
					new Point2D(909, 1860),
					new Point2D(909, 1845),
					new Point2D(909, 1830),
					new Point2D(909, 1815), /*shares with prev*/
					new Point2D(909, 1807),
					new Point2D(910, 1797),
					new Point2D(912, 1787),
				},
			},
			//Lance 3
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(865, 1785),
					new Point2D(865, 1800),
					new Point2D(865, 1815),
					new Point2D(865, 1830),
					new Point2D(865, 1845),
					new Point2D(865, 1860),
					new Point2D(865, 1875), /*shares with next*/
					new Point2D(872, 1880),
					new Point2D(879, 1887),
					new Point2D(891, 1887),
					new Point2D(903, 1885),
				},
				new Point2D[]
				{
					new Point2D(915, 1875),
					new Point2D(915, 1860),
					new Point2D(915, 1845),
					new Point2D(915, 1830),
					new Point2D(915, 1815), /*shares with next*/
					new Point2D(915, 1807),
					new Point2D(915, 1797),
					new Point2D(915, 1787),
				},
			},
			// Lane 4
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(871, 1785),
					new Point2D(871, 1800),
					new Point2D(871, 1815),
					new Point2D(871, 1830),
					new Point2D(871, 1845),
					new Point2D(871, 1860),
					new Point2D(871, 1875), /*shares with prev*/
					new Point2D(872, 1880),
					new Point2D(879, 1887),
					new Point2D(891, 1887),
					new Point2D(903, 1885),
					new Point2D(912, 1882),
				},
				new Point2D[]
				{
					new Point2D(921, 1875),
					new Point2D(921, 1860),
					new Point2D(921, 1845),
					new Point2D(921, 1830),
					new Point2D(921, 1815), /*shares with prev*/
					new Point2D(921, 1807),
					new Point2D(921, 1797),
					new Point2D(921, 1787),
				},
			},
		};
		...
		private static Point2D[][][] _NavPoints2 =
		{
			...
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(906, 1877),
					new Point2D(904, 1860),
					new Point2D(903, 1845),
					new Point2D(903, 1830),
					new Point2D(903, 1815), /*shares with next*/
					new Point2D(909, 1807),
					new Point2D(921, 1807),
					new Point2D(935, 1801),
					new Point2D(950, 1797),
					new Point2D(959, 1798),
					new Point2D(973, 1802),
				},
				new Point2D[]
				{
					new Point2D(973, 1815),
					new Point2D(973, 1830),
					new Point2D(973, 1845),
					new Point2D(973, 1860),
					new Point2D(973, 1875),
				},
			},
			//Lane 2
			new Point2D[][]
			{
				new Point2D[]
				{
					...
					new Point2D(959, 1798),
					new Point2D(973, 1802),
					new Point2D(979, 1805),
				},
				new Point2D[]
				{
					new Point2D(979, 1820),
					new Point2D(979, 1820),
					new Point2D(979, 1835),
					new Point2D(979, 1850),
					new Point2D(979, 1865),
					new Point2D(979, 1880),
				},
			},
			//Lance 3
			new Point2D[][]
			{
				new Point2D[]
				{
					...
					new Point2D(973, 1802),
					new Point2D(979, 1805),
					new Point2D(985, 1809),
				},
				new Point2D[]
				{
					new Point2D(985, 1825),
					new Point2D(985, 1840),
					new Point2D(985, 1855),
					new Point2D(985, 1870),
					new Point2D(985, 1885),
				},
			},
			// Lane 4
			new Point2D[][]
			{
				new Point2D[]
				{
					new Point2D(919, 1877),
					new Point2D(920, 1860),
					new Point2D(921, 1845),
					new Point2D(921, 1830),
					new Point2D(921, 1815), /*shares with prev*/
					new Point2D(921, 1807),
					...
					new Point2D(979, 1805),
					new Point2D(985, 1809),
					new Point2D(991, 1813),
				},
				new Point2D[]
				{
					new Point2D(991, 1830),
					new Point2D(991, 1845),
					new Point2D(991, 1860),
					new Point2D(991, 1875),
					new Point2D(991, 1885),
				},
			},
		};
		...
		private void AssignFlags()
		{
			if (BattleRegion != null)
			{
				BattleFlag flag = BattleRegion
					.GetEnumeratedItems()
					.OfType<BattleFlag>()
					.FirstOrDefault(i => i != null && i.ItemID == 17099);
				if (flag != null)
				{
					TribalFlag = flag;
				...
				flag = BattleRegion
					.GetEnumeratedItems()
					.OfType<BattleFlag>()
					.FirstOrDefault(i => i != null && i.ItemID == 1068);
				if (flag != null)
				{
					MyrmidexFlag = flag;
					flag.BattleSpawner = this;
