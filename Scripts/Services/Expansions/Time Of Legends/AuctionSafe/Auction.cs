...
namespace Server.Engines.Auction
{
	...
	public class Auction : IDisposable
	{
		...
		{
			get { return StartTime + TimeSpan.FromMinutes(Duration); }
		}
		...
		public bool InClaimPeriod
		{
			get { return HasBegun && ClaimPeriod != DateTime.MinValue; }
		}
		...
		public int CurrentGoldBid
		{
			get
			{
				return (int)(
					CurrentBid >= Account.CurrencyThreshold
						? CurrentBid - (CurrentPlatBid * Account.CurrencyThreshold)
						: CurrentBid
				);
			}
		}
		public int CurrentPlatBid
		{
			get { return (int)(CurrentBid >= Account.CurrencyThreshold ? CurrentBid / Account.CurrencyThreshold : 0); }
		}

		public int BuyoutGold
		{
			get
			{
				return (int)(
					Buyout >= Account.CurrencyThreshold ? Buyout - (BuyoutPlat * Account.CurrencyThreshold) : Buyout
				);
			}
		}
		public int BuyoutPlat
		{
			get { return (int)(Buyout >= Account.CurrencyThreshold ? Buyout / Account.CurrencyThreshold : 0); }
		}
		...
		public bool AuctionItemOnDisplay()
		{
			...
			return !AuctionItem.Movable
				&& AuctionItem.X == Safe.X
				&& AuctionItem.Y == Safe.Y
				&& AuctionItem.Z == Safe.Z + 7;
		}
		...
		public bool TryPlaceBid(Mobile m, long bidTotal)
		{
			...
			else
			{
				...
				if (HighestBid != null)
				{
					...
					var message = new NewMaginciaMessage(
						null,
						new TextDefinition(1156427),
						String.Format(
							"{0}\t{1}\t{2}",
							name,
		...
		public void EndAuction()
		{
			if (HighestBid != null && HighestBid.Mobile != null)
			{
				...
				NewMaginciaMessage message = new NewMaginciaMessage(
					null,
					new TextDefinition(1156426),
				...
				message = new NewMaginciaMessage(
					null,
					new TextDefinition(1156428),
					TimeSpan.FromHours(24),
					String.Format(
						"{0}\t{1}\t{2}",
						name,
		...
		public void HouseCollapse()
		{
			...
			if (Bids != null)
			{
				Bids.ForEach(bid =>
				{
					...
					var mes = new NewMaginciaMessage(
						null,
						new TextDefinition(1156454),
						String.Format(
							"{0}\t{1}\t{2}",
							CurrentPlatBid.ToString("N0", CultureInfo.GetCultureInfo("en-US")),
							CurrentGoldBid.ToString("N0", CultureInfo.GetCultureInfo("en-US")),
				...
				case 1:
					...
					for (int i = 0; i < count; i++)
					{
						...
						if (m != null)
						{
							...
							if (
								entry.CurrentBid > 0
								&& (HighestBid == null || entry.CurrentBid > HighestBid.CurrentBid)
							)
								HighestBid = entry;
						}
					}
