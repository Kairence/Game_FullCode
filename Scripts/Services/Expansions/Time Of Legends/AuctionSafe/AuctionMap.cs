		...
		public class ConfirmTeleportGump : BaseGump
		{
			...
			public override void AddGumpLayout()
			{
				...
				AddHtmlLocalized(
					...
					String.Format(
						"@{0}@{1}@{2}",
						AuctionMap.TeleportCost.ToString(),
						AuctionMap.HouseName,
						AuctionMap.DeleteDelayMinutes.ToString()
					),
					0xFFFF,
					false,
					false
				); // Please select 'Accept' if you would like to pay ~1_cost~ gold to teleport to auction house ~2_name~. For this price you will also be able to teleport back to this location within the next ~3_minutes~ minutes.
				...
				switch (info.ButtonID)
				{
					...
					case 1:
					{
						...
						else if (!AuctionMap.CheckItem())
						{
							...
							AuctionMap.SetLocation != Point3D.Zero
							&& (
								!Utility.InRange(AuctionMap.SetLocation, User.Location, 100)
								|| AuctionMap.SetMap != User.Map
							)
						)
						{
