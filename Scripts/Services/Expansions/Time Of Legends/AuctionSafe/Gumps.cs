	...
	public class BaseAuctionGump : Gump
	{
		...
		public virtual void AddGumpLayout()
		{
			...
			AddHtml(
				...
				175,
				18,
				Color(HGray, acct != null ? acct.TotalGold.ToString("N0", CultureInfo.GetCultureInfo("en-US")) : "0"),
				false,
				false
			);
			...
			AddHtml(
				...
				175,
				18,
				Color(HGray, acct != null ? acct.TotalPlat.ToString("N0", CultureInfo.GetCultureInfo("en-US")) : "0"),
				false,
				false
			);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			switch (info.ButtonID)
			{
				...
				case 100:
					Refresh();
					User.SendGump(new AuctionInfoGump(User));
					break;
				case 101:
					Refresh();
					User.SendGump(new BidHistoryGump(User, Auction));
					break;
			}
						...
						if (Auction.AuctionItem != null)
						{
							if (Auction.AuctionItem.LabelNumber != 0)
							{
								from.SendLocalizedMessage(
									1152339,
									String.Format("#{0}", Auction.AuctionItem.LabelNumber)
								); // A reward of ~1_ITEM~ has been placed in your backpack.
							}
							else
							{
	...
	public class AuctionBidGump : BaseAuctionGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				175,
				18,
				Color(HGray, Auction.CurrentPlatBid.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
			...
			AddHtml(
				...
				175,
				18,
				Color(HGray, Auction.CurrentGoldBid.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
				false,
				false
			);
			...
			if (
				Auction.Buyout > 0
				&& (Auction.HighestBid == null || Auction.HighestBid != null && Auction.HighestBid.Mobile != User)
			)
			{
				...
				AddHtml(
					...
					175,
					18,
					Color(HGray, Auction.BuyoutPlat.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
					false,
					false
				);
				...
				AddHtml(
					...
					175,
					18,
					Color(HGray, Auction.BuyoutGold.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
					false,
					false
				);
	...
	public class BidHistoryGump : Gump
	{
			...
			: base(100, 200)
		{
			...
			for (int i = Auction.BidHistory.Count - 1; i >= 0; i--)
			{
				...
				long gold =
					bid >= Account.CurrencyThreshold
						? bid - ((bid / Account.CurrencyThreshold) * Account.CurrencyThreshold)
						: bid;
				...
				AddHtml(
					...
					Color(
						HGray,
						String.Format("{0}*****", h.Mobile != null ? h.Mobile.Name.Trim()[0].ToString() : "?")
					),
					false,
					false
				);
				AddHtml(
					...
					117,
					22,
					Color(HGray, plat.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
					false,
					false
				);
				AddHtml(
					...
					117,
					22,
					Color(HGray, gold.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
					false,
					false
				);
				AddHtml(
