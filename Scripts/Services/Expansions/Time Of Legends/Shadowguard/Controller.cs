...
namespace Server.Engines.Shadowguard
{
	...
	[DeleteConfirm(
		"Are you sure you want to delete this? Deleting this will delete any saved encounter data your players have."
	)]
	public class ShadowguardController : Item
	{
		public static readonly TimeSpan ReadyDuration = TimeSpan.FromSeconds(
			Config.Get("Shadowguard.ReadyDuration", 30)
		);
			...
			if (encounter == EncounterType.Roof)
			{
				if (p != null)
				{
					foreach (PartyMemberInfo info in p.Members)
					{
						if (
							Table == null
							|| !Table.ContainsKey(info.Mobile)
							|| (Table[info.Mobile] & EncounterType.Required) != EncounterType.Required
						)
						{
					...
					Table == null
					|| !Table.ContainsKey(m)
					|| (Table[m] & EncounterType.Required) != EncounterType.Required
				)
				{
		...
		{
			new Point3D(96, 2016, -20),
			new Point3D(224, 2016, -20),
			new Point3D(352, 2016, -20),
			new Point3D(480, 2016, -20),
			new Point3D(160, 2080, -20),
			new Point3D(288, 2080, -20),
			new Point3D(416, 2080, -20),
			new Point3D(96, 2144, -20),
			new Point3D(224, 2144, -20),
			new Point3D(352, 2144, -20),
			new Point3D(160, 2208, -20),
			new Point3D(288, 2208, -20),
			new Point3D(416, 2208, -20),
			new Point3D(64, 2336, 0),
			new Point3D(160, 2336, 0),
			new Point3D(64, 2432, 0),
			new Point3D(160, 2432, 0),
		};
			...
			: base(100, 50)
		{
			...
			for (int i = 0; i < _Encounters.Length; i++)
			{
				...
				AddHtmlLocalized(
					...
					200,
					16,
					ShadowguardController.GetLocalization(encounter),
					hue,
					false,
					false
				);
	...
	public class ShadowguardRegion : BaseRegion
	{
		...
		public override void OnSpeech(SpeechEventArgs args)
		{
			...
			if (
				...
				&& args.Speech != null
				&& args.Speech.ToLower().Trim() == "getprops"
			)
			{
				if (Instance.Encounter != null)
