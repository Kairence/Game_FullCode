...
namespace Server.Engines.Shadowguard
{
	public class ShadowguardBoss : BaseCreature
	{
		...
		public virtual Type[] SummonTypes
		{
			get { return null; }
		}
		public virtual Type[] ArtifactDrops
		{
			get { return _ArtifactTypes; }
		}

		public virtual bool CanSummon
		{
			get { return Hits <= HitsMax - (HitsMax / 4); }
		}

		private Type[] _ArtifactTypes = new Type[]
		{
			typeof(AnonsBoots),
			typeof(AnonsSpellbook),
			typeof(BalakaisShamanStaff),
			typeof(EnchantressCameo),
			typeof(GrugorsShield),
			typeof(WamapsBoneEarrings),
			typeof(HalawasHuntingBow),
			typeof(HawkwindsRobe),
			typeof(JumusSacredHide),
			typeof(JuonarsGrimoire),
			typeof(LereisHuntingSpear),
			typeof(UnstableTimeRift),
			typeof(MinaxsSandles),
			typeof(MocapotlsObsidianSword),
			typeof(OzymandiasObi),
			typeof(ShantysWaders),
			typeof(TotemOfTheTribe),
			typeof(BalakaisShamanStaffGargoyle),
		};

		public ShadowguardBoss(AIType ai)
			: base(ai, FightMode.Closest, 10, 1, .15, .3)
		{
		...
		public ShadowguardBoss(Serial serial)
			: base(serial) { }
		...
		public virtual void Summon()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				BaseCreature spawn =
					Activator.CreateInstance(SummonTypes[Utility.Random(SummonTypes.Length)]) as BaseCreature;
	...
	{
		public override Type[] SummonTypes
		{
			get { return _SummonTypes; }
		}
		...
		{
			get { return _LastChange + TimeSpan.FromSeconds(Utility.RandomMinMax(75, 90)) < DateTime.UtcNow; }
		}
		...
		private void SetHighResistance(ResistanceType type)
		{
			...
			SetResistance(
				ResistanceType.Fire,
				type == ResistanceType.Fire ? 80 : 50,
				type == ResistanceType.Fire ? 90 : 60
			);
			SetResistance(
				ResistanceType.Cold,
				type == ResistanceType.Cold ? 80 : 50,
				type == ResistanceType.Cold ? 90 : 60
			);
			SetResistance(
				ResistanceType.Poison,
				type == ResistanceType.Poison ? 80 : 50,
				type == ResistanceType.Poison ? 90 : 60
			);
			SetResistance(
				ResistanceType.Energy,
				type == ResistanceType.Energy ? 80 : 50,
				type == ResistanceType.Energy ? 90 : 60
			);
		}
	...
	public class Juonar : ShadowguardBoss
	{
		...
		private Type[] _SummonTypes = new Type[]
		{
		...
		public override bool CanDiscord
		{
			get { return true; }
		}
		public override bool PlayInstrumentSound
		{
			get { return false; }
		}
		...
		public override void OnThink()
		{
			...
			if (Combatant is Mobile)
			{
				...
				if (
					InRange(m.Location, 10)
					&& !InRange(m.Location, 2)
					&& m.Alive
	...
	public class Virtuebane : ShadowguardBoss
	{
		...
		public void DoNuke(Point3D p)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				Server.Misc.Geometry.Circle2D(
					...
					{
						Effects.SendLocationParticles(
							EffectItem.Create(pnt, map, EffectItem.DefaultDuration),
							0x3709,
							10,
							30,
							5052
							...
							Server.Misc.Geometry.Circle2D(
								...
								(pnt, map) =>
								{
									Effects.SendLocationEffect(
										...
										14,
										10,
										Utility.RandomMinMax(2497, 2499),
										2
									);
								}
							);
		...
		public void DoDismount(Mobile m)
		{
			...
			Timer.DelayCall(
				TimeSpan.FromMilliseconds(250 * range),
				() =>
				{
		...
		public Virtuebane(Serial serial)
			: base(serial) { }
	...
	public class Ozymandias : ShadowguardBoss
	{
		...
		public override void OnThink()
		{
			...
			if ((wep is Fists || wep is BaseRanged) && InRange(Combatant.Location, 1) && 0.1 > Utility.RandomDouble())
			{
				...
				(wep is Fists || !(wep is BaseRanged))
				&& !InRange(Combatant.Location, 1)
		...
		public Ozymandias(Serial serial)
			: base(serial) { }
