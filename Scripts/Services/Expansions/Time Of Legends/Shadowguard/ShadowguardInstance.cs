...
namespace Server.Engines.Shadowguard
{
	...
	public class ShadowguardInstance
	{
		...
		public ShadowguardInstance(ShadowguardController controller, Point3D center, Rectangle2D bounds, int index)
		{
			...
			Region = new ShadowguardRegion(
				bounds,
				index >= 13 ? String.Format("Roof {0}", (index - 12).ToString()) : index.ToString(),
				this
			);
			Region.Register();
		}
		...
		private ShadowguardEncounter ConstructEncounter(EncounterType type)
		{
			switch (type)
			{
				...
				case EncounterType.Bar:
					return new BarEncounter(this);
				case EncounterType.Orchard:
					return new OrchardEncounter(this);
				case EncounterType.Armory:
					return new ArmoryEncounter(this);
				case EncounterType.Fountain:
					return new FountainEncounter(this);
				case EncounterType.Belfry:
					return new BelfryEncounter(this);
				case EncounterType.Roof:
					return new RoofEncounter(this);
			}
		}
		...
		private Type[] DeleteList = { typeof(ShadowguardCanal) };
		...
		public static void Initialize()
		{
			...
			ColUtility.ForEach(
				ShadowguardController.Instance.Addons.Where(addon => addon.Map != Map.Internal),
				addon =>
				{
