...
namespace Server.Engines.Shadowguard
{
		...
		public override void ClearItems()
		{
			if (Bottles != null)
			{
				List<ShadowguardBottleOfLiquor> list = new List<ShadowguardBottleOfLiquor>(
					Bottles.Where(b => b != null && !b.Deleted)
				);
	...
	public class OrchardEncounter : ShadowguardEncounter
	{
		...
		public override Type AddonType
		{
			get { return typeof(OrchardAddon); }
		}
	...
	public class ArmoryEncounter : ShadowguardEncounter
	{
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			if (Armor != null)
				Armor.ForEach(a => writer.Write(a));

			writer.Write(DestroyedArmor == null ? 0 : DestroyedArmor.Count);
			if (DestroyedArmor != null)
				DestroyedArmor.ForEach(a => writer.Write(a));

			writer.Write(Spawn == null ? 0 : Spawn.Count);
			if (Spawn != null)
				Spawn.ForEach(a => writer.Write(a));
	...
	public class FountainEncounter : ShadowguardEncounter
	{
		...
		public override Type AddonType
		{
			get { return typeof(ShadowguardFountainAddon); }
		}
		...
		public override void Setup()
		{
			...
			Timer.DelayCall(
				ShadowguardController.ReadyDuration + TimeSpan.FromSeconds(30),
				() =>
				{
					for (int i = 0; i < toSpawn; i++)
						SpawnRandomElemental();
		...
		public class FlowChecker
		{
			...
			{
				get { return _Spigot != null && _Drain != null; }
			}
				...
				if (item is ShadowguardCanal)
				{
					...
					if (
						(
							southFacing
							&& (
								canal.Flow == Flow.NorthSouth
								|| canal.Flow == Flow.SouthEastCorner
								|| canal.Flow == Flow.SouthWestCorner
							)
						)
						|| (
							!southFacing
							&& (
								canal.Flow == Flow.EastWest
								|| canal.Flow == Flow.SouthEastCorner
								|| canal.Flow == Flow.NorthEastCorner
							)
						)
					)
					{
				...
				if (one is ShadowguardCanal && two is ShadowguardDrain)
				{
					...
					switch (d)
					{
						case Direction.North:
							canConnect =
								flow == Flow.NorthSouth || flow == Flow.SouthEastCorner || flow == Flow.SouthWestCorner;
							break;
						case Direction.East:
							canConnect =
								flow == Flow.EastWest || flow == Flow.NorthWestCorner || flow == Flow.SouthWestCorner;
							break;
						case Direction.South:
							canConnect =
								flow == Flow.NorthSouth || flow == Flow.NorthEastCorner || flow == Flow.NorthWestCorner;
							break;
						case Direction.West:
							canConnect =
								flow == Flow.EastWest || flow == Flow.NorthEastCorner || flow == Flow.SouthEastCorner;
							break;
					}
			...
			private int[] _Offsets = new int[] { 0, -1, 1, 0, 0, 1, -1, 0 };
			...
			public void Serialize(GenericWriter writer)
			{
				...
				if (_Checked != null)
					_Checked.ForEach(c => writer.Write(c));
			}
		}
	...
	public class BelfryEncounter : ShadowguardEncounter
	{
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			if (Drakes != null)
				Drakes.ForEach(d => writer.Write(d));

			writer.Write(Bells == null ? 0 : Bells.Count);
			if (Bells != null)
				Bells.ForEach(b => writer.Write(b));
		}
	...
	public class RoofEncounter : ShadowguardEncounter
	{
		...
		public override TimeSpan EncounterDuration
		{
			get { return TimeSpan.MaxValue; }
		}
		public override TimeSpan ResetDuration
		{
