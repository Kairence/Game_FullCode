...
namespace Server.Engines.Shadowguard
{
	public class ShadowguardBottleOfLiquor : BaseDecayingItem
	{
		...
		[Constructable]
		public ShadowguardBottleOfLiquor(BarEncounter encounter)
			: base(0x99B)
		{
			Encounter = encounter;
		}
		...
		public ShadowguardBottleOfLiquor(Serial serial)
			: base(serial) { }
	...
	public class ShadowguardApple : BaseDecayingItem
	{
		...
		public ShadowguardApple(Serial serial)
			: base(serial) { }
	...
	public class ShadowguardCypress : Item
	{
		...
		public bool IsOppositeVirtue(VirtueType type)
		{
			switch (type)
			{
				...
				case VirtueType.Honesty:
					return this.VirtueType == VirtueType.Deceit;
				case VirtueType.Compassion:
					return this.VirtueType == VirtueType.Despise;
				case VirtueType.Valor:
					return this.VirtueType == VirtueType.Destard;
				case VirtueType.Justice:
					return this.VirtueType == VirtueType.Wrong;
				case VirtueType.Sacrafice:
					return this.VirtueType == VirtueType.Covetous;
				case VirtueType.Honor:
					return this.VirtueType == VirtueType.Shame;
				case VirtueType.Spirituality:
					return this.VirtueType == VirtueType.Hythloth;
				case VirtueType.Humility:
					return this.VirtueType == VirtueType.Pride;
				case VirtueType.Deceit:
					return this.VirtueType == VirtueType.Honesty;
				case VirtueType.Despise:
					return this.VirtueType == VirtueType.Compassion;
				case VirtueType.Destard:
					return this.VirtueType == VirtueType.Valor;
				case VirtueType.Wrong:
					return this.VirtueType == VirtueType.Justice;
				case VirtueType.Covetous:
					return this.VirtueType == VirtueType.Sacrafice;
				case VirtueType.Shame:
					return this.VirtueType == VirtueType.Honor;
				case VirtueType.Hythloth:
					return this.VirtueType == VirtueType.Spirituality;
				case VirtueType.Pride:
					return this.VirtueType == VirtueType.Humility;
			}
		}
		...
		public ShadowguardCypress(Serial serial)
			: base(serial) { }
	...
	public class Phylactery : BaseDecayingItem
	{
		...
		public override int LabelNumber
		{
			get { return _Purified ? 1156221 : 1156220; }
		} // Purified Phylactery : Corrupt Phylactery
					...
					(from, targeted) =>
					{
						if (targeted is PurifyingFlames)
						{
							...
							else if (!Purified)
							{
								...
								Effects.SendLocationParticles(
									EffectItem.Create(flames.Location, flames.Map, EffectItem.DefaultDuration),
									0x3709,
									10,
									30,
									5052
							...
							else
							{
								...
								Timer.DelayCall(
									...
									() =>
									{
										...
										ArmoryEncounter encounter =
											ShadowguardController.GetEncounter(p, Map.TerMur) as ArmoryEncounter;
												...
												Server.Misc.Geometry.Circle2D(
													...
													(pnt, mob) =>
													{
														...
														Effects.SendLocationEffect(
															pnt,
															mob,
															Utility.RandomBool() ? 14000 : 14013,
															20,
															2018,
															0
														);
		...
		public Phylactery(Serial serial)
			: base(serial) { }
	...
	public class CursedSuitOfArmor : Item
	{
		...
		public CursedSuitOfArmor(ShadowguardEncounter encounter)
			: base(0x151A)
		{
			Encounter = encounter;
			Movable = false;
		...
		public CursedSuitOfArmor(Serial serial)
			: base(serial) { }
	...
	public class PurifyingFlames : Item
	{
		...
		public PurifyingFlames(Serial serial)
			: base(serial) { }
	...
	public class ShadowguardCanal : BaseDecayingItem, IChopable
	{
		...
		[Constructable]
		public ShadowguardCanal()
			: base(Utility.RandomList(39911, 39915, 39919, 39924, 39928, 39932))
		{
			InvalidateID();
			Hue = 2500;
		}
		...
		[Constructable]
		public ShadowguardCanal(Flow flow)
			: base(0)
		{
			Flow = flow;
			Hue = 2500;
		...
		private void InvalideIDfromFlow()
		{
			switch (_Flow)
			{
				case Flow.NorthSouth:
					ItemID = 39911;
					break;
				case Flow.SouthEastCorner:
					ItemID = 39915;
					break;
				case Flow.SouthWestCorner:
					ItemID = 39919;
					break;
				case Flow.EastWest:
					ItemID = 39924;
					break;
				case Flow.NorthEastCorner:
					ItemID = 39928;
					break;
				case Flow.NorthWestCorner:
					ItemID = 39932;
					break;
			}
		}
		...
		public bool Connects(ShadowguardCanal next)
		{
			...
			switch (d)
			{
				case Direction.North:
					switch (_Flow)
					{
						case Flow.EastWest:
							return false; //
						case Flow.NorthSouth:
							return f == Flow.NorthSouth || f == Flow.NorthEastCorner || f == Flow.NorthWestCorner; //
						case Flow.NorthWestCorner:
							return false; //
						case Flow.NorthEastCorner:
							return false; //
				...
				case Direction.South:
					switch (_Flow)
					{
						case Flow.EastWest:
							return false; //
						case Flow.NorthSouth:
							return f == Flow.NorthSouth || f == Flow.SouthEastCorner || f == Flow.SouthWestCorner; //
						case Flow.NorthWestCorner:
							return f == Flow.NorthSouth || f == Flow.SouthEastCorner || f == Flow.SouthWestCorner; //
						case Flow.NorthEastCorner:
							return f == Flow.NorthSouth || f == Flow.SouthEastCorner || f == Flow.SouthWestCorner; //
				...
				case Direction.East:
					...
					{
						case Flow.EastWest:
							return f == Flow.EastWest || f == Flow.NorthEastCorner || f == Flow.SouthEastCorner; //
						case Flow.NorthSouth:
							return false; //
						case Flow.NorthWestCorner:
							return f == Flow.EastWest || f == Flow.NorthEastCorner || f == Flow.SouthEastCorner; //
				...
				case Direction.West:
					...
					{
						case Flow.EastWest:
							return f == Flow.EastWest || f == Flow.NorthWestCorner || f == Flow.SouthWestCorner; //
						case Flow.NorthSouth:
							return false; //
						case Flow.NorthWestCorner:
							return false; //
		...
		public ShadowguardCanal(Serial serial)
			: base(serial) { }
	...
	public class ShadowguardSpigot : Item
	{
		...
		public ShadowguardSpigot(int id)
			: base(id)
		{
			Movable = false;
		}
		...
		public override void OnDoubleClick(Mobile m)
		{
			FountainEncounter encounter =
				ShadowguardController.GetEncounter(this.Location, this.Map) as FountainEncounter;
		...
		public ShadowguardSpigot(Serial serial)
			: base(serial) { }
	...
	public class ShadowguardDrain : Item
	{
		...
		public ShadowguardDrain(Serial serial)
			: base(serial) { }
	...
	public class MagicDrakeWing : BaseDecayingItem
	{
		...
		public MagicDrakeWing(Serial serial)
			: base(serial) { }
	...
	public class FeedingBell : BaseAddon
	{
		...
		public FeedingBell(Serial serial)
			: base(serial) { }
	...
	public class WitheringBones : Container
	{
		...
		{
			get { return 1156214; }
		} // The Withered Bones of an Adventurer
