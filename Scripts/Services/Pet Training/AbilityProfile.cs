...
namespace Server.Mobiles
{
	...
	public class AbilityProfile
	{
		...
		public bool CanChooseSpecialAbility(SpecialAbility[] list)
		{
			...
			if (
				...
				&& (
					SpecialAbilities == null
					|| SpecialAbilities.Length == 0
					|| SpecialAbilities.All(a => a.NaturalAbility)
				)
			)
				return true;

			return !HasSpecialMagicalAbility()
				&& (SpecialAbilities == null || SpecialAbilities.Where(a => !a.NaturalAbility).Count() == 0)
				&& AbilityCount() < 3;
		}
		...
		public bool CanChooseWeaponAbility()
		{
			...
			return !HasSpecialMagicalAbility()
				&& (WeaponAbilities == null || WeaponAbilities.Length < 2)
				&& AbilityCount() < 3;
		}
		...
		public bool IsNaturalAbility(object o)
		{
			...
			if (o is SpecialAbility)
			{
				return SpecialAbilities != null
					&& !Advancements.Any(s => s is SpecialAbility && (SpecialAbility)s == (SpecialAbility)o);
			}

			if (o is WeaponAbility)
			{
				return WeaponAbilities != null
					&& !Advancements.Any(s => s is WeaponAbility && (WeaponAbility)s == (WeaponAbility)o);
			}
		...
		public AbilityProfile(BaseCreature bc, GenericReader reader)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					case 1:
						Advancements.Add((MagicalAbility)reader.ReadInt());
						break;
					case 2:
						Advancements.Add(SpecialAbility.Abilities[reader.ReadInt()]);
						break;
					case 3:
						Advancements.Add(AreaEffect.Effects[reader.ReadInt()]);
						break;
					case 4:
						Advancements.Add(WeaponAbility.Abilities[reader.ReadInt()]);
						break;
					case 5:
						Advancements.Add((SkillName)reader.ReadInt());
						break;
				}
			}
		}
