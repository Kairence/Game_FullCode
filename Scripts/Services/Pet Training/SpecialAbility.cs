...
namespace Server.Mobiles
{
	public abstract class SpecialAbility
	{
		...
		public virtual double TriggerChance
		{
			get { return 0.1; }
		}
		public virtual bool RequiresCombatant
		{
			get { return true; }
		}
		public virtual TimeSpan CooldownDuration
		{
			get { return TimeSpan.FromSeconds(30); }
		}

		public virtual MagicalAbility RequiredSchool
		{
			get { return MagicalAbility.None; }
		}
		public virtual bool NaturalAbility
		{
			get { return false; }
		}

		public virtual bool TriggerOnGotMeleeDamage
		{
			get { return false; }
		}
		public virtual bool TriggerOnDoMeleeDamage
		{
			get { return false; }
		}
		public virtual bool TriggerOnGotSpellDamage
		{
			get { return false; }
		}
		public virtual bool TriggerOnDoSpellDamage
		{
			get { return false; }
		}
		public virtual bool TriggerOnThink
		{
			get { return false; }
		}
		public virtual bool TriggerOnApproach
		{
			get { return false; }
		}
		...
		public static void CheckCombatTrigger(Mobile attacker, Mobile defender, ref int damage, DamageType type)
		{
			...
			if (attacker is BaseCreature && !((BaseCreature)attacker).Summoned)
			{
				...
				if (profile != null)
				{
					...
					var abilties = profile
						.EnumerateSpecialAbilities()
						.Where(m =>
							(type == DamageType.Melee && m.TriggerOnDoMeleeDamage)
							|| (type >= DamageType.Spell && m.TriggerOnDoSpellDamage) && !m.IsInCooldown(attacker)
						)
						.ToArray();
			...
			if (defender is BaseCreature && !((BaseCreature)defender).Summoned)
			{
				...
				if (profile != null)
				{
					...
					var abilties = profile
						.EnumerateSpecialAbilities()
						.Where(m =>
							(type == DamageType.Melee && m.TriggerOnGotMeleeDamage)
							|| (type >= DamageType.Spell && m.TriggerOnGotSpellDamage) && !m.IsInCooldown(defender)
						)
						.ToArray();
		...
		public static bool CheckThinkTrigger(BaseCreature bc)
		{
			...
			if (combatant is Mobile)
			{
				if (profile != null)
				{
					...
					var abilties = profile
						.EnumerateSpecialAbilities()
						.Where(m => m.TriggerOnThink && !m.IsInCooldown(bc))
						.ToArray();
			...
			else
			{
				...
				var abilties = profile
					.EnumerateSpecialAbilities()
					.Where(m => m.TriggerOnThink && !m.IsInCooldown(bc) && !m.RequiresCombatant)
					.ToArray();
		...
		public virtual bool Validate(BaseCreature attacker, Mobile defender)
		{
			...
			return defender != null
				&& defender.Alive
				&& !defender.Deleted
				&& !defender.IsDeadBondedPet
				&& attacker.Alive
				&& !attacker.IsDeadBondedPet
				&& defender.InRange(attacker.Location, MaxRange)
				&& defender.Map == attacker.Map
				&& attacker.InLOS(defender)
				&& !attacker.BardPacified
				&& attacker.CanBeHarmful(defender);
		}
	...
	public class FlurryForce : SpecialAbility
	{
		...
		public override bool NaturalAbility
		{
			get { return true; }
		}
		public override int ManaCost
	...
	public class DragonBreath : SpecialAbility
	{
		...
		public override bool TriggerOnThink
		{
			get { return true; }
		}
		public override int ManaCost
		...
		public override void AddToCooldown(BaseCreature m)
		{
			if (CooldownDuration != TimeSpan.MinValue)
			{
				...
				Timer.DelayCall<Mobile>(
					TimeSpan.FromSeconds(Utility.RandomMinMax(def.MinDelay, def.MaxDelay)),
					RemoveFromCooldown,
					m
				);
			}
		...
		public virtual void DoBreath(BaseCreature creature, Mobile target)
		{
			...
			{
				var list = Server
					.Spells.SpellHelper.AcquireIndirectTargets(creature, target, creature.Map, 5)
					.OfType<Mobile>()
					.Where(m => m.InRange(creature.Location, MaxRange))
					.ToList();
				...
				for (int i = 0; i < 5; i++)
				{
					...
					list.Remove(m);
					Timer.DelayCall(
						TimeSpan.FromSeconds(def.EffectDelay),
						new TimerStateCallback<BaseCreature, Mobile, DragonBreathDefinition>(BreathEffect_Callback),
						creature,
						m,
						def
					);
		...
		public void BreathEffect_Callback(BaseCreature creature, Mobile target, DragonBreathDefinition def)
		{
			...
			Timer.DelayCall(
				TimeSpan.FromSeconds(def.DamageDelay),
				new TimerStateCallback<BaseCreature, Mobile, DragonBreathDefinition>(BreathDamage_Callback),
				creature,
				target,
				def
			);
		...
		public int BreathComputeDamage(BaseCreature creature, DragonBreathDefinition def)
		{
			...
			if (creature.Combatant != null && creature.Combatant is PlayerMobile)
			{
				PlayerMobile pm = creature.Combatant as PlayerMobile;
				double parryBonus = 0;
				if (pm.TimerList[69] != 0)
					parryBonus += pm.PotionDefense * 0.1;

				if (pm.ParryTime < DateTime.Now && pm.Stam >= 1)
				{
					BaseShield target_shield = pm.FindItemOnLayer(Layer.TwoHanded) as BaseShield;
					if (target_shield != null)
					{
						double parrychance = pm.Skills.Parry.Value * 0.01;
						bool stamcheck = true;
						if (Utility.RandomDouble() < parrychance)
						{
							parryBonus += pm.Skills.Parry.Value * 0.2;
							...
							pm.FixedParticles(0x376A, 1, 30, 0x251E, 5, 3, EffectLayer.Waist);
						}
					}
				}
				damage = (int)(damage * (100 - parryBonus));
				damage /= 100;
			}
		...
		public class DragonBreathDefinition
		{
			...
			public static List<DragonBreathDefinition> Definitions { get; private set; } =
				new List<DragonBreathDefinition>();

			public static void Initialize()
			{
				Definitions.Add(
					new DragonBreathDefinition(
						Core.AOS ? 0.16 : 0.05,
						1.0,
						1.3,
						1.0,
				...
				Definitions.Add(
					new DragonBreathDefinition(
						...
						12,
						false,
						new Type[] { typeof(DeepSeaSerpent) }
					)
				);
				// Kraken
				Definitions.Add(
	...
	public class HowlOfCacophony : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			...
			BuffInfo.AddBuff(
				defender,
				new BuffInfo(BuffIcon.HowlOfCacophony, 1153793, 1153820, TimeSpan.FromSeconds(30), defender, "60\t5\t5")
			);
		}
	...
	{
		public override bool TriggerOnGotMeleeDamage
		{
			get { return true; }
		}
		public override bool TriggerOnDoMeleeDamage
	...
	public class Repel : SpecialAbility
	{
		...
		public override bool TriggerOnGotSpellDamage
		{
			get { return true; }
		}
		public override int ManaCost
	...
	public class VenomousBite : SpecialAbility
	{
		...
		public override MagicalAbility RequiredSchool
		{
			get { return MagicalAbility.Poisoning; }
		}
	...
	public class RuneCorruption : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			...
			BuffInfo.AddBuff(
				defender,
				new BuffInfo(
					BuffIcon.RuneBeetleCorruption,
					1153796,
					1153823,
					TimeSpan.FromSeconds(5.0),
					defender,
					String.Format("{0}\t{1}\t{2}\t{3}\t{4}", phy, cold, poison, energy, fire)
				)
			);
	...
	public class LifeLeech : SpecialAbility
	{
		...
		public override TimeSpan CooldownDuration
		{
			get { return TimeSpan.FromSeconds(Utility.Random(10, 20)); }
		}
		public override bool TriggerOnDoMeleeDamage
		{
			get { return true; }
	...
	public class Anemia : SpecialAbility
	{
		...
		{
			if (
				!_Table.ContainsKey(attacker)
				&& creature.InRange(attacker, 1)
				&& 0.25 > Utility.RandomDouble()
				&& !FountainOfFortune.UnderProtection(attacker)
			)
			{
				...
				int str = creature.Int / 10;
				int dex = creature.Int / 10;
				...
				BuffInfo.AddBuff(
					attacker,
					new BuffInfo(
						BuffIcon.BloodwormAnemia,
						1153797,
						1153824,
						String.Format("{0}\t{1}\t{2}", str, dex, Int)
					)
				);
		...
		private class ExpireTimer : Timer
		{
			...
			{
				m_Victim = m;
				_Expires = DateTime.Now + TimeSpan.FromMinutes(1);
			}

			protected override void OnTick()
			{
				if (_Expires < DateTime.Now || m_Victim.Deleted || !m_Victim.Alive || m_Victim.IsDeadBondedPet)
				{
				...
				{
					m_Victim.Stam = 0; //m_Victim.Stam < 2 ? 0 : Utility.RandomMinMax(2, 5);
					m_Victim.Mana = 0;
				}
			}
		}
	...
	public class BloodDisease : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			...
			if (
				!_Table.ContainsKey(defender)
				&& creature.InRange(defender, 1)
				&& 0.25 > Utility.RandomDouble()
				&& !FountainOfFortune.UnderProtection(defender)
			)
			{
	...
	public class Heal : SpecialAbility
	{
		...
		public override double TriggerChance
		{
			get { return 1.0; }
		}
		public override bool RequiresCombatant
		{
			get { return false; }
		}
		public override int ManaCost
	...
	{
		public override bool TriggerOnApproach
		{
			get { return true; }
		}
		public override int MaxRange
		...
		public override TimeSpan CooldownDuration
		{
			get { return TimeSpan.FromMinutes(Utility.RandomMinMax(1, 3)); }
		}
		...
		public override void DoEffects(BaseCreature creature, Mobile target, ref int damage)
		{
			...
			BuffInfo.AddBuff(
				target,
				new BuffInfo(BuffIcon.TrueFear, 1153791, 1153827, TimeSpan.FromSeconds(seconds), target)
			);
	...
	public class LifeDrain : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			foreach (Mobile m in AreaEffect.FindValidTargets(creature, 2).OfType<Mobile>())
			{
				...
				*/
				...
				AOS.Damage(m, creature, toDrain, 0, 0, 0, 0, 0, 100, 0);
		...
		{
			int amount = Utility.RandomMinMax(100, 400);

			if (bc is Semidar || bc is Lilith)
			{
				amount *= 3;
			}
