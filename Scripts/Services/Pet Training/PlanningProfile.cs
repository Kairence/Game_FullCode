			...
			if (tp is MagicalAbility && (MagicalAbility)tp <= MagicalAbility.WrestlingMastery)
			{
				...
				foreach (var en in Entries)
				{
					if (trainingPoint.Requirements != null && trainingPoint.Requirements.Length > 0)
					{
						...
						{
							if (
								(req.Requirement is WeaponAbility && en.TrainPoint is WeaponAbility)
		...
		public PlanningProfile(BaseCreature bc, GenericReader reader)
		{
			...
			for (int i = 0; i < count; i++)
			{
				switch (reader.ReadInt())
				{
					...
					case 1:
						Entries.Add(
							new PlanningEntry((MagicalAbility)reader.ReadInt(), reader.ReadInt(), reader.ReadInt())
						);
						break;
					case 2:
						Entries.Add(
							new PlanningEntry(
								SpecialAbility.Abilities[reader.ReadInt()],
								reader.ReadInt(),
								reader.ReadInt()
							)
						);
					...
					case 3:
						Entries.Add(
							new PlanningEntry(AreaEffect.Effects[reader.ReadInt()], reader.ReadInt(), reader.ReadInt())
						);
						break;
					case 4:
						Entries.Add(
							new PlanningEntry(
								WeaponAbility.Abilities[reader.ReadInt()],
								reader.ReadInt(),
								reader.ReadInt()
							)
						);
						break;
					case 5:
						Entries.Add(new PlanningEntry((PetStat)reader.ReadInt(), reader.ReadInt(), reader.ReadInt()));
						break;
					case 6:
						Entries.Add(
							new PlanningEntry((ResistanceType)reader.ReadInt(), reader.ReadInt(), reader.ReadInt())
						);
						break;
					case 7:
						Entries.Add(new PlanningEntry((SkillName)reader.ReadInt(), reader.ReadInt(), reader.ReadInt()));
						break;
				}
			}
		}
