...
namespace Server.Mobiles
{
	public class NewAnimalLoreGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				160,
				18,
				(!Creature.Controlled || Creature.Loyalty == 0) ? 1061643 : 1049595 + (Creature.Loyalty / 10),
				_Label,
				false,
				false
			);
			...
			AddHtml(180, 92, 75, 18, FormatStat(RegenRates.Mobile_HitsRegenRate(Creature)), false, false);

			AddHtmlLocalized(53, 110, 160, 18, 1079411, _Label, false, false); // Stamina Regeneration
			AddHtml(180, 110, 75, 18, FormatStat(RegenRates.Mobile_StamRegenRate(Creature)), false, false);

			AddHtmlLocalized(53, 128, 160, 18, 1079410, _Label, false, false); // Mana Regeneration
			AddHtml(180, 128, 75, 18, FormatStat(RegenRates.Mobile_ManaRegenRate(Creature)), false, false);
			...
			if (Creature.Tamable)
			{
				...
				AddHtmlLocalized(
					...
					18,
					1157600,
					Creature.CurrentTameSkill.ToString("0.0"),
					_Label,
					false,
					false
				);
							...
							if (loc[0] != null)
							{
								if (loc[0].Number > 0)
								{
									AddHtmlLocalized(
										...
										18,
										loc[0],
										C32216(skill ? 0x008000 : 0xFF4500),
										false,
										false
									);
									...
									if (skill)
									{
										AddHtml(
											...
											18,
											String.Format(
												"<div align=right>{0:F1}</div>",
												Creature.Skills[(SkillName)profile.Advancements[i]].Cap
											),
											false,
											false
										);
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			switch (id)
			{
				...
				case 2: // pet training options
					if (
						User.HasGump(typeof(PetTrainingConfirmationGump))
	...
	public class PetTrainingProgressGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			List<BaseCreature> pets = new List<BaseCreature>(
				User.AllFollowers.OfType<BaseCreature>()
					.Where(p => p.TrainingProfile != null && p.TrainingProfile.HasBegunTraining && p.Map == User.Map)
			);
			...
			for (int i = 0; i < pets.Count; i++)
			{
				...
				AddHtml(
					...
					50,
					18,
					NewAnimalLoreGump.FormatDouble(progress, false, true),
					false,
					false
				);
			}
	...
	public class PetTrainingConfirmGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				16,
				CenterLoc,
				String.Format("#{0}", _Title.ToString()),
				0xF424E5,
				false,
				false
			);
	...
	public class PetTrainingOptionsGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			switch (Category)
			{
				...
				case 1:
					...
					break;
				case 2:
					BuildMagicSkillCapsPage();
					break;
				case 3:
					BuildCombatSkillCapsPage();
					break;
				case 4:
					BuildMagicalAbilitiesPage();
					break;
				case 5:
					BuildSpecialAbilitiesPage();
					break;
				case 6:
					BuildSpecialMovesPage();
					break;
				case 7:
					BuildAreaEffectsPage();
					break;
			}
		}
		...
		private void BuildMagicSkillCapsPage()
		{
			...
			{
				if (
					(!PetTrainingHelper.CommonSkill(Creature, skill) && Creature.Skills[skill].Base <= 0)
					|| Creature.Skills[skill].Cap >= 120
				)
					continue;
		...
		private void BuildMagicalAbilitiesPage()
		{
			...
			for (int i = 0; i < PetTrainingHelper.MagicalAbilities.Length; i++)
			{
				...
				if (
					(abil == MagicalAbility.Chivalry && master != null && master.Karma < 0)
				...
				if (
					(Definition.MagicalAbilities & abil) == 0
					|| AbilityProfile.HasAbility(abil)
		...
		private void BuildSpecialAbilitiesPage()
		{
			...
			for (int i = 0; i < SpecialAbility.Abilities.Length; i++)
			{
				...
				if (
					AbilityProfile.HasAbility(abil)
					|| (AbilityProfile.HasSpecialMagicalAbility() && !AbilityProfile.IsRuleBreaker(abil))
				)
					continue;

				if (
					Definition.HasSpecialAbility(abil)
					|| (AbilityProfile.HasAbility(MagicalAbility.Poisoning) && abil is VenomousBite)
				)
				{
					var tp = PetTrainingHelper.GetTrainingPoint(abil);
		...
		private void BuildAreaEffectsPage()
		{
			...
			for (int i = 0; i < AreaEffect.Effects.Length; i++)
			{
				...
				if (
					Definition.HasAreaEffect(abil)
					|| (AbilityProfile.HasAbility(MagicalAbility.Poisoning) && abil is PoisonBreath)
				)
				{
					var tp = PetTrainingHelper.GetTrainingPoint(abil);
		...
		private bool HasAvailable(object o)
		{
			...
			if (o is SpecialAbility[])
			{
				...
				if (
					...
					&& !AbilityProfile.HasAbility(SpecialAbility.VenomousBite)
				)
				{
					return true;
				}
			...
			else if (o is AreaEffect[])
			{
				...
				if (
					AbilityProfile.HasAbility(MagicalAbility.Poisoning)
					&& !AbilityProfile.HasAbility(AreaEffect.PoisonBreath)
				)
				{
					return true;
				}
	...
	public class PetTrainingConfirmationGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddButton(
				...
				0x9CC8,
				0x9CC7,
				profile.TrainingMode == TrainingMode.Regular ? 8 : 9,
			...
			AddHtml(
				...
				126,
				20,
				Center(profile.TrainingMode == TrainingMode.Regular ? "Train Pet" : "Add To Plan"),
				false,
				false
			);
			...
			if (StartValue > 0)
			{
				...
				if (TrainingPoint.TrainPoint is PetStat && (PetStat)TrainingPoint.TrainPoint <= PetStat.Mana)
				{
					...
					double currentTotal =
						stat <= PetStat.Int
							? PetTrainingHelper.GetTotalStatWeight(Creature)
							: PetTrainingHelper.GetTotalAttributeWeight(Creature);
			...
			else if (TrainingPoint.TrainPoint is SkillName)
			{
				...
				AddHtmlLocalized(
					...
					60,
					cliloc,
					String.Format("#{0}", TrainingPoint.Name.Number),
					0,
					false,
					false
				);
			...
			AddLabel(
				...
				245,
				0,
				String.Format("{0}/{1}", ((int)(Value * weight)).ToString(), (max * weight).ToString())
			);
			...
			switch (info.ButtonID)
			{
				...
				case 8: // train pet
					...
					else if (!Creature.InRange(User.Location, 12))
					{
						...
						Server.Spells.SpellHelper.CheckCombat(User)
						|| Server.Spells.SpellHelper.CheckCombat(Creature)
					...
					else if (cost > profile.TrainingPoints)
					{
						...
						TrainingPoint.TrainPoint is SkillName
						&& !CheckPowerScroll((SkillName)TrainingPoint.TrainPoint, Value, ref scroll)
					)
					{
					...
					else if (PetTrainingHelper.CanControl(User, Creature, profile))
					{
						BaseGump.SendGump(
							new PetTrainingConfirmGump(
								User,
								1157502,
								TrainingPoint.TrainPoint is MagicalAbility ? 1157566 : 1157503,
								() =>
								{
									if (PetTrainingHelper.ApplyTrainingPoint(Creature, TrainingPoint, Value))
									{
										...
										if (TrainingPoint.TrainPoint is SkillName)
										{
											...
											Effects.SendMovingParticles(
												new Entity(
													Server.Serial.Zero,
													new Point3D(Creature.X - 6, Creature.Y - 6, Creature.Z + 15),
													Creature.Map
												),
												Creature,
												0x36D4,
											...
											Effects.SendMovingParticles(
												new Entity(
													Server.Serial.Zero,
													new Point3D(Creature.X - 4, Creature.Y - 6, Creature.Z + 15),
													Creature.Map
												),
												Creature,
												0x36D4,
											...
											Effects.SendMovingParticles(
												new Entity(
													Server.Serial.Zero,
													new Point3D(Creature.X - 6, Creature.Y - 4, Creature.Z + 15),
													Creature.Map
												),
												Creature,
												0x36D4,
				...
				case 9:
					...
					{
						PetTrainingHelper
							.GetPlanningProfile(Creature, true)
							.AddToPlan(
								TrainingPoint.TrainPoint,
								Value,
								PetTrainingHelper.GetTotalCost(TrainingPoint, Creature, Value, StartValue)
							);
		...
		private bool CheckPowerScroll(SkillName name, int value, ref Item scroll)
		{
			...
			scroll = User
				.Backpack.Items.OfType<PowerScroll>()
				.FirstOrDefault(ps => ps.Skill == name && ps.Value == 100 + (value / 10));
	...
	public class PetTrainingPlanningGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				126,
				20,
				Center(profile.TrainingMode == TrainingMode.Planning ? "DISABLE" : "ENABLE"),
				false,
				false
			);
			...
			for (int i = 0; i < plan.Entries.Count; i++)
			{
				...
				AddHtmlLocalized(
					...
					200,
					18,
					PetTrainingHelper.GetCategoryLocalization(entry.TrainPoint),
					false,
					false
				);
