...
namespace Server.Engines.CannedEvil
{
	public class ChampionSkullPlatform : BaseAddon
	{
		...
		public ChampionSkullPlatform()
		{
			...
			this.AddComponent(
				this.m_Enlightenment = new ChampionSkullBrazier(this, ChampionSkullType.Enlightenment),
				1,
				-1,
				5
			);
		...
		{
			if (
				this.Validate(this.m_Power)
				&& this.Validate(this.m_Enlightenment)
				&& this.Validate(this.m_Venom)
				&& this.Validate(this.m_Pain)
				&& this.Validate(this.m_Greed)
				&& this.Validate(this.m_Death)
			)
			{
