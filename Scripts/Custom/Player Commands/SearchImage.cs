...
namespace Server.Custom
{
		...
		public static void Initialize()
		{
			...
			if (ImagesLoaded)
			{
				CommandSystem.Register(
					...
					new CommandEventHandler(SearchImage_OnCommand)
				);
				CommandSystem.Register(
					"GetImage",
					AccessLevel.GameMaster,
				...
				CommandSystem.Register(
					"BrowseImage",
					AccessLevel.GameMaster,
					new CommandEventHandler(BrowseImages_OnCommand)
				);
				CommandSystem.Register(
					"BrowseImages",
					AccessLevel.GameMaster,
	...
	public class BrowseImageNamesGump : Gump
	{
			...
			: base(140, 70)
		{
			...
			{
				if (list.Count - index < 32)
					num = list.Count - index;
				...
				if (index > 1)
					AddButton(120, 5, 0x1519, 0x1519, 3, GumpButtonType.Reply, 0); // Previous Page
				if (index + 32 < list.Count)
					AddButton(120, 680, 0x151A, 0x151A, 4, GumpButtonType.Reply, 0); // Next Page
			}
		}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			else if (x >= 20)
			{
				...
				m.SendGump(new BrowseImageNamesGump(m_Index - 32)); //Previous Page
			else if (x == 4)
				m.SendGump(new BrowseImageNamesGump(m_Index + 32)); //Next Page
		}
	}
	...
	public class SearchImageGump : Gump
	{
			...
			: base(0, 54)
		{
			...
			if (m_Search != "")
			{
				...
				if (list != null && list.Count > 0)
				{
					...
					for (int q = index; q < index + num; q++)
					{
						...
						catch
						{
							...
							AddHtml(
								...
								60,
								120,
								string.Format("Unable to show Image ID {0}.", list[q].ToString()),
								false,
								false
							);
						}
					...
					AddHtml(20, 92, 260, 40, "No results found for that search.", false, false);
			}
			else
				AddHtml(20, 92, 260, 40, "Please enter a search string in the box.", false, false);
		...
		private IEnumerable<int> GetList(string search)
		{
			...
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((TileData.ItemTable[x].Flags & TileFlag.Foliage) != 0)
						list.Add(x);
			}
			else if (search == "LightSource")
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((TileData.ItemTable[x].Flags & TileFlag.LightSource) != 0)
						list.Add(x);
			}
			else if (search == "Containers")
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((TileData.ItemTable[x].Flags & TileFlag.Container) != 0)
						list.Add(x);
			}
			else if (search == "Weapons")
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((TileData.ItemTable[x].Flags & TileFlag.Weapon) != 0)
						list.Add(x);
			}
			else if (search == "Armor")
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((TileData.ItemTable[x].Flags & TileFlag.Armor) != 0)
						list.Add(x);
			}
			else if (search == "Consumables")
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
					if ((int)TileData.ItemTable[x].Flags == 18432)
						list.Add(x);
			}
			//TODO: These need to be refined/replaced with better lists, but they are a start...
			else if (search == "Chairs")
			{
				list.AddRange(GetList("chair"));
				list.AddRange(GetList("stool"));
			}
			else if (search == "Storage")
			{
				list.AddRange(GetList("crate"));
				list.AddRange(GetList("box"));
			}
			else if (search == "Tables")
			{
				list.AddRange(GetList("table"));
			}
			else if (search == "Beds")
			{
				list.AddRange(GetList("bed"));
			}
			else if (search == "Shelves")
			{
				list.AddRange(GetList("shelf"));
			}
			else if (search == "Dressers")
			{
				list.AddRange(GetList("dresser"));
			}
			else if (search == "Trees")
			{
				list.AddRange(GetList("tree"));
			}
			else if (search == "Plants")
			{
				list.AddRange(GetList("plant"));
			}
			else if (search == "Rocks")
			{
				list.AddRange(GetList("rock"));
			}
			else if (search == "Animals")
			{
				list.AddRange(GetList("cat"));
				list.AddRange(GetList("dog"));
				list.AddRange(GetList("bird"));
				list.AddRange(GetList("sheep"));
				list.AddRange(GetList("pig"));
				list.AddRange(GetList("cow"));
				list.AddRange(GetList("deer"));
				list.AddRange(GetList("bear"));
			}
			else if (search == "Monsters")
			{
				list.AddRange(GetList("rat"));
				list.AddRange(GetList("lizard"));
				list.AddRange(GetList("orc"));
			}
			else if (search == "Water")
			{
				list.AddRange(GetList("water"));
			}
			else if (search == "Utensils")
			{
				list.AddRange(GetList("knife"));
				list.AddRange(GetList("fork"));
				list.AddRange(GetList("spoon"));
			}
			else if (search == "Tools")
			{
				list.AddRange(GetList("shovel"));
				list.AddRange(GetList("pick"));
				list.AddRange(GetList("tool"));
				list.AddRange(GetList("hammer"));
				list.AddRange(GetList("froe"));
				list.AddRange(GetList("saw"));
			}
			else if (search == "Bottles")
			{
				list.AddRange(GetList("bottle"));
			}
			else if (search == "Bags")
			{
				list.AddRange(GetList("bag"));
				list.AddRange(GetList("backpack"));
			}
			else if (search == "Banners")
			{
				list.AddRange(GetList("banner"));
				list.AddRange(GetList("flag"));
			}
			else if (search == "Signs")
			{
				list.AddRange(GetList("sign"));
			}
			else if (search == "Stones")
			{
				list.AddRange(GetList("stone"));
			}
			else if (search == "Debris")
			{
				list.AddRange(GetList("debris"));
			}
			else if (search == "Symbols")
			{
				list.AddRange(GetList("symbol"));
			}
			//TODO: These need to be refined/replaced with better lists, but they are a start...
			...
			else
			{
				for (int x = 0; x < TileData.ItemTable.Length; x++)
				{
					try
					{
						if (Insensitive.Contains(TileData.ItemTable[x].Name, search))
							list.Add(x);
					}
					catch { }
				}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			else if (x == 5 && tr2 != null)
			{
				...
				m.SendGump(new SearchImageGump(m_Index - 10, m_Search, m_ShowImage)); //Previous Page
			else if (x == 4)
				m.SendGump(new SearchImageGump(m_Index + 10, m_Search, m_ShowImage)); //Next Page
			else if (x == 11)
				m.SendGump(new SearchImageGump(0, "WoodWalls"));
			else if (x == 12)
				m.SendGump(new SearchImageGump(0, "StoneWalls"));
			else if (x == 13)
				m.SendGump(new SearchImageGump(0, "MarbleWalls"));
			else if (x == 14)
				m.SendGump(new SearchImageGump(0, "PlasterWalls"));
			else if (x == 15)
				m.SendGump(new SearchImageGump(0, "ElvenWalls"));
			else if (x == 16)
				m.SendGump(new SearchImageGump(0, "OtherWalls"));
			else if (x == 21)
				m.SendGump(new SearchImageGump(0, "SecretDoors"));
			else if (x == 22)
				m.SendGump(new SearchImageGump(0, "Gates"));
			else if (x == 23)
				m.SendGump(new SearchImageGump(0, "NormalDoors"));
			else if (x == 24)
				m.SendGump(new SearchImageGump(0, "ElvenDoors"));
			else if (x == 25)
				m.SendGump(new SearchImageGump(0, "CrystalDoors"));
			else if (x == 26)
				m.SendGump(new SearchImageGump(0, "SamuraiDoors"));
			else if (x == 31)
				m.SendGump(new SearchImageGump(0, "PavedFloors"));
			else if (x == 32)
				m.SendGump(new SearchImageGump(0, "TileFloors"));
			else if (x == 33)
				m.SendGump(new SearchImageGump(0, "WoodFloors"));
			else if (x == 34)
				m.SendGump(new SearchImageGump(0, "RusticFloors"));
			else if (x == 35)
				m.SendGump(new SearchImageGump(0, "TatamiFloors"));
			else if (x == 36)
				m.SendGump(new SearchImageGump(0, "OtherFloors"));
			else if (x == 41)
				m.SendGump(new SearchImageGump(0, "RusticRoofs"));
			else if (x == 42)
				m.SendGump(new SearchImageGump(0, "CeramicRoofs"));
			else if (x == 43)
				m.SendGump(new SearchImageGump(0, "HardRoofs"));
			else if (x == 44)
				m.SendGump(new SearchImageGump(0, "WoodenRoofs"));
			else if (x == 45)
				m.SendGump(new SearchImageGump(0, "ElvenRoofs"));
			else if (x == 46)
				m.SendGump(new SearchImageGump(0, "NewAgeRoofs"));
			else if (x == 61)
				m.SendGump(new SearchImageGump(0, "MiscArches"));
			else if (x == 62)
				m.SendGump(new SearchImageGump(0, "VariousMisc"));
			else if (x == 63)
				m.SendGump(new SearchImageGump(0, "RoundedWalls"));
			else if (x == 64)
				m.SendGump(new SearchImageGump(0, "Battlements"));
			else if (x == 65)
				m.SendGump(new SearchImageGump(0, "MiscRoofs"));
			else if (x == 66)
				m.SendGump(new SearchImageGump(0, "MiscFences"));
			else if (x == 51)
				m.SendGump(new SearchImageGump(0, "Stairs"));
			else if (x == 71)
				m.SendGump(new SearchImageGump(0, "Chairs"));
			else if (x == 72)
				m.SendGump(new SearchImageGump(0, "Storage"));
			else if (x == 73)
				m.SendGump(new SearchImageGump(0, "Tables"));
			else if (x == 74)
				m.SendGump(new SearchImageGump(0, "Beds"));
			else if (x == 75)
				m.SendGump(new SearchImageGump(0, "Shelves"));
			else if (x == 76)
				m.SendGump(new SearchImageGump(0, "Dressers"));
			else if (x == 81)
				m.SendGump(new SearchImageGump(0, "Trees"));
			else if (x == 82)
				m.SendGump(new SearchImageGump(0, "Plants"));
			else if (x == 83)
				m.SendGump(new SearchImageGump(0, "Rocks"));
			else if (x == 84)
				m.SendGump(new SearchImageGump(0, "Animals"));
			else if (x == 85)
				m.SendGump(new SearchImageGump(0, "Monsters"));
			else if (x == 86)
				m.SendGump(new SearchImageGump(0, "Water"));
			else if (x == 91)
				m.SendGump(new SearchImageGump(0, "LightSource"));
			else if (x == 92)
				m.SendGump(new SearchImageGump(0, "Weapons"));
			else if (x == 93)
				m.SendGump(new SearchImageGump(0, "Armor"));
			else if (x == 94)
				m.SendGump(new SearchImageGump(0, "Tools"));
			else if (x == 95)
				m.SendGump(new SearchImageGump(0, "Bottles"));
			else if (x == 96)
				m.SendGump(new SearchImageGump(0, "Containers"));
			else if (x == 101)
				m.SendGump(new SearchImageGump(0, "Banners"));
			else if (x == 102)
				m.SendGump(new SearchImageGump(0, "Signs"));
			else if (x == 103)
				m.SendGump(new SearchImageGump(0, "Stones"));
			else if (x == 104)
				m.SendGump(new SearchImageGump(0, "Consumables"));
			else if (x == 105)
				m.SendGump(new SearchImageGump(0, "Debris"));
			else if (x == 106)
				m.SendGump(new SearchImageGump(0, "Symbols"));
			else
				m.CloseGump(typeof(SearchImageGump));
		}
		...
		private class CreateItemTarget : Target
		{
			...
			public CreateItemTarget(int itemID, string search, int index, bool showImage)
