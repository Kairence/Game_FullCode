...
namespace Server.Misc
{
	public static class GoldDistributor
	{
		...
		{
			if (bc == null || rights == null || rights.Count == 0)
				return;
		...
		private static HashSet<Mobile> GetValidKillers(List<DamageStore> rights)
		{
			...
			{
				if (!ds.m_HasRight || ds.m_Mobile == null)
					continue;
		...
		private static void DetermineTotalRewards(BaseCreature bc, HashSet<Mobile> killers)
		{
			...
			// 등급 보너스 (Grade 시스템 연동)
			if (bc.Grade >= 6)
				totalGoldPool = (int)(totalGoldPool * 1.5);
			if (bc.Boss)
				totalGoldPool *= 2;
			...
			int dropCount = 0;
			if (bc.Boss)
				dropCount = Utility.RandomMinMax(7, 12);
			else if (bc.Grade == 7)
				dropCount = Utility.RandomMinMax(4, 8);
			else if (bc.Grade == 6)
				dropCount = Utility.RandomMinMax(2, 5);
			else if (bc.Grade >= 2)
				dropCount = Utility.RandomMinMax(1, 3);
			else
				dropCount = Utility.RandomMinMax(0, 1);
			...
			foreach (Mobile m in killers)
			{
				...
				// 1. 실버 포인트 지급
				double silverBonus =
					1000 + pm.SilverPoint[2] * 50 + AosAttributes.GetValue(pm, AosAttribute.LowerAmmoCost);
				int get_silverpoint = (int)(bc.Fame * 0.33 * silverBonus / 1000);
				pm.Getsilverpoint(get_silverpoint);

				// 2. 명성 및 카르마 지급
				if (shareFame > 0)
					Titles.AwardFame(pm, shareFame, true);
				if (shareKarma != 0)
					Titles.AwardKarma(pm, shareKarma, true);
		...
		{
			if (pm == null || bc == null || dropCount <= 0)
				return;
			...
			for (int d = 0; d < dropCount; d++)
			{
				...
				{
					// 장비 생성 (카테고리 대응)
					if (entry.ItemType == typeof(BaseArmor))
						droppedItem = Loot.RandomArmor();
					else if (entry.ItemType == typeof(BaseWeapon))
						droppedItem = Loot.RandomWeapon();
					else if (entry.ItemType == typeof(BaseJewel))
						droppedItem = Loot.RandomJewelry();
					else
