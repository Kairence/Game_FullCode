...
namespace Server.Misc
{
    ...
    {
        // [확률 테이블] Key: CreatureBalancer의 Grade 번호
        // Value: { 일반, 희귀, 영웅, 서사, 전설, 신화 } 확률 (백분율 기준)
        private static Dictionary<int, double[]> GradeProbabilities = new Dictionary<int, double[]>
        {
            // 일반 (Grade 1): 일반 60%, 희귀 20%, 영웅 10%, 서사 6%, 전설 3%, 신화 1%
            { 1, new double[] { 60.0, 20.0, 10.0, 6.0, 3.0, 1.0 } },

            // 레어 (Grade 2): 일반 10%, 희귀 40%, 영웅 25%, 서사 15%, 전설 7.5%, 신화 2.5%
            { 2, new double[] { 10.0, 40.0, 25.0, 15.0, 7.5, 2.5 } },

            // 엘리트 (Grade 6): 일반 5%, 희귀 15%, 영웅 20%, 서사 20%, 전설 15%, 신화 5%
            { 6, new double[] { 5.0, 15.0, 20.0, 20.0, 15.0, 5.0 } },

            // 치프 (Grade 7): 일반 0%, 희귀 10%, 영웅 20%, 서사 40%, 전설 20%, 신화 10%
            { 7, new double[] { 0.0, 10.0, 20.0, 40.0, 20.0, 10.0 } },

            // 보스 (Grade 8): 일반 0%, 희귀 20%, 영웅 20%, 서사 20%, 전설 20%, 신화 20%
            { 8, new double[] { 0.0, 20.0, 20.0, 20.0, 20.0, 20.0 } }
        };
        ...
        public static void LoadSkillPool()
        {
            ...
            // 패시브 위력 (0:일반, 1:희귀, 2:영웅, 3:서사, 4:전설, 5:신화)
		...
		{
			if (bc == null || bc.PassiveSkills == null) return;
			if (bc.PassiveSkills[0] > 0) return;

			// [슬롯 결정 로직]
			int maxSlots = 1; 
			double slotDice = Utility.RandomDouble();
			if (slotDice < 0.05) maxSlots = 3;      // 5% 확률로 3개
			else if (slotDice < 0.25) maxSlots = 2; // 25% 확률로 2개
			else maxSlots = 1;                     // 나머지 100% (최소 1개 확정)

			if (SkillPool.Count == 0) LoadSkillPool();

			for (int i = 0; i < maxSlots; i++)
			{
				// bc.Grade가 0이면 1(일반)로 취급
		...
		{
			// 만약 해당 등급에 스킬이 없으면 하위 등급에서라도 찾아서 채워줍니다 (무조건 1개 보장)
			for (int g = skillGrade; g >= 0; g--)
			{
				if (SkillPool.ContainsKey(g))
				{
					var availableSkills = SkillPool[g].Where(s => s.CanApply(bc)).ToList();
					var finalCandidates = availableSkills.Where(s => !IsAlreadyAssigned(bc, s.ID)).ToList();

					if (finalCandidates.Count > 0)
					{
						var skill = finalCandidates[Utility.Random(finalCandidates.Count)];
						skill.Apply(bc);

						int currentCount = bc.PassiveSkills[0];
						int index = 1 + (currentCount * 2);
		...
		// AnimalPassiveSkillHandler 클래스 내부에 추가
		...
		private static void AddGradePool(int gradeIdx, int hpVal, int mainVal, int resistVal, int skillVal, int hungerVal)
		{
			...
			// [0-1] 스탯/공격력: 그대로 배율(곱연산) 유지
			list.Add(new PassiveEffect($"최대 체력 +{hpVal}%", 0, hpVal, bc => true, 
				bc => { bc.HitsMaxSeed = (int)(bc.HitsMaxSeed * (1 + hpVal / 100.0)); bc.Hits = bc.HitsMax; }));
			
			list.Add(new PassiveEffect($"공격력 +{mainVal}%", 1, mainVal, bc => true, 
				bc => { 
					bc.DamageMin = (int)(bc.DamageMin * (1 + mainVal / 100.0)); 
					bc.DamageMax = (int)(bc.DamageMax * (1 + mainVal / 100.0)); 
				}));
			...
			// [10-15] 스킬: 요청하신 대로 단순 합연산 적용 (Base 수치에 직접 가산)
			// 예: skillVal이 20이면 스킬 0.0에서 20.0으로 상승
