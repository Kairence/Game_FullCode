...
namespace Server.Misc
{
	...
	public partial class AnimalPassiveSkillHandler
	{
		...
		{
			if (bc == null || bc.PassiveSkills == null || bc.PassiveSkills[0] > 0)
				return;
		...
		private static void ApplyRandomSkill(BaseCreature bc, int skillGrade)
		{
			...
			for (int g = skillGrade; g >= 0; g--)
			{
				...
				{
					var availableSkills = SkillPool[g]
						.Where(s => s.CanApply(bc) && !IsAlreadyAssigned(bc, s.ID))
						.ToList();
		...
		private static bool IsAlreadyAssigned(BaseCreature bc, int skillID)
		{
			for (int i = 0; i < bc.PassiveSkills[0]; i++)
				if (bc.PassiveSkills[1 + (i * 2)] == skillID)
					return true;
			return false;
		}
		...
		public static string GetPassiveName(int id)
		{
			switch ((PassiveID)id)
			{
				case PassiveID.MaxHits:
					return "최대 체력";
				case PassiveID.Damage:
					return "공격력";
				case PassiveID.PhysicalResist:
					return "물리 저항";
				case PassiveID.FireResist:
					return "화염 저항";
				case PassiveID.ColdResist:
					return "냉기 저항";
				case PassiveID.PoisonResist:
					return "독 저항";
				case PassiveID.EnergyResist:
					return "에너지 저항";
				case PassiveID.Str:
					return "힘";
				case PassiveID.Dex:
				...
				case PassiveID.Int:
					return "지능";
				case PassiveID.Poisoning:
					return "독 스킬";
				case PassiveID.Magery:
					return "마법학";
				case PassiveID.MagicResist:
					return "주문 저항";
				case PassiveID.Tactics:
					return "전술";
				case PassiveID.Anatomy:
					return "해부학";
				case PassiveID.Healing:
					return "회복술";
				case PassiveID.Harvesting:
					return "자원 채취";
				case PassiveID.Hunger:
					return "배고픔 감소";
				default:
					return "미지의 능력";
			}
		...
		private static void AddGradePool(
			int gradeIdx,
			int hpVal,
			int mainVal,
			int resistVal,
		...
		)
		{
			...
			list.Add(
				new PassiveEffect(
					...
					bc =>
					{
						bc.HitsMaxSeed = (int)(bc.HitsMaxSeed * (1 + hpVal / 100.0));
						bc.Hits = bc.HitsMax;
					}
				)
			);
			...
			list.Add(
				new PassiveEffect(
					...
					resistVal,
					bc => true,
					bc => bc.PhysicalResistanceSeed += resistVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					resistVal,
					bc => true,
					bc => bc.FireResistSeed += resistVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					resistVal,
					bc => true,
					bc => bc.ColdResistSeed += resistVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					resistVal,
					bc => true,
					bc => bc.PoisonResistSeed += resistVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					resistVal,
					bc => true,
					bc => bc.EnergyResistSeed += resistVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					bc =>
					{
						bc.RawStr = (int)(bc.RawStr * (1 + mainVal / 100.0));
						bc.Hits = bc.HitsMax;
					}
				)
			);
			list.Add(
				new PassiveEffect(
					...
					bc =>
					{
						bc.RawDex = (int)(bc.RawDex * (1 + mainVal / 100.0));
						bc.Stam = bc.StamMax;
					}
				)
			);
			list.Add(
				new PassiveEffect(
					...
					bc =>
					{
						bc.RawInt = (int)(bc.RawInt * (1 + mainVal / 100.0));
						bc.Mana = bc.ManaMax;
					}
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.Poisoning].Base += skillVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.Magery].Base += skillVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.MagicResist].Base += skillVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.Tactics].Base += skillVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.Anatomy].Base += skillVal
				)
			);
			list.Add(
				new PassiveEffect(
					...
					skillVal,
					bc => true,
					bc => bc.Skills[SkillName.Healing].Base += skillVal
				)
			);
