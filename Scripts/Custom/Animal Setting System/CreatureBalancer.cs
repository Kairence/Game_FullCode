...
namespace Server.Misc
{
	public static class CreatureBalancer
	{
		...
		{
			if (bc == null || bc.Deleted || bc.Grade > 0)
				return;
			...
			{
				case 1:
					bc.Loyalty = Utility.RandomMinMax(0, 100);
					break; // 0% ~ 10% 보너스
				case 2:
					bc.Loyalty = Utility.RandomMinMax(200, 350);
					break; // 20% ~ 35% 보너스
				case 6:
					bc.Loyalty = Utility.RandomMinMax(500, 750);
					break; // 50% ~ 75% 보너스
				case 7:
					bc.Loyalty = Utility.RandomMinMax(800, 900);
					break; // 80% ~ 90% 보너스
				case 8:
					bc.Loyalty = 0;
					break; // Boss: 100% 보너스 (2배)
				default:
					bc.Loyalty = 0;
					break;
			}
			...
			// 기본 스탯 배율 설정
			double sM = loyaltyScalar,
				dM = loyaltyScalar,
				iM = loyaltyScalar;
			double hM = loyaltyScalar,
				staM = loyaltyScalar,
				manM = loyaltyScalar;
		...
		private static void ApplyAppearance(BaseCreature bc)
		{
			...
			int grade = bc.Grade;
			if (grade == 2 && !bc.Name.StartsWith("Rare "))
			{
				bc.Name = "Rare " + bc.Name;
				bc.Fame += 500;
			}
			else if (grade == 6 && !bc.Name.StartsWith("Elite "))
			{
				bc.Name = "Elite " + bc.Name;
				bc.Hue = 1272;
				bc.Fame += 1500;
			}
			else if (grade == 7 && !bc.Name.StartsWith("Chief "))
			{
				bc.Name = "Chief " + bc.Name;
				bc.Hue = 1157;
				bc.Fame += 3000;
			}
			else if (grade == 8 && !bc.Name.StartsWith("Boss "))
			{
				bc.Name = "Boss " + bc.Name;
			}
		...
		private static void ApplySlayerBonus(
			BaseCreature bc,
			ref double sM,
			ref double dM,
			ref double iM,
		...
		{
			if (IsSlayer(bc, SlayerName.Silver))
			{
				hM *= 1.1;
			}
			else if (IsSlayer(bc, SlayerName.ElementalBan))
			{
				iM *= 1.15;
				manM *= 1.15;
			}
			else if (IsSlayer(bc, SlayerName.Exorcism))
			{
				sM *= 1.1;
				hM *= 1.1;
			}
			else if (IsSlayer(bc, SlayerName.ArachnidDoom))
			{
				dM *= 1.1;
				staM *= 1.1;
			}
			else if (IsSlayer(bc, SlayerName.ReptilianDeath))
			{
				hM *= 1.2;
				sM *= 1.15;
			}
			else if (IsSlayer(bc, SlayerName.Fey))
			{
				dM *= 1.2;
				iM *= 1.2;
			}
