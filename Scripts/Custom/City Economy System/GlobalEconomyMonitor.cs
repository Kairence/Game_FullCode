...
namespace Server.Misc
{
    public static class GlobalEconomyMonitor
    {
        ...
        private static void OnSave(WorldSaveEventArgs e)
        {
            ...
            // ==============================================================================
            // [기획 예정: 아크메이지(Archmage) 스타일 아마겟돈(Armageddon) 코어 시스템]
            // TODO: 정해진 월(Month) 주기가 아닌, '세계 점수(World Score)' 기반의 동적 멸망 및 번영 시스템
            // 
            // [1. 멸망의 인과율 (아마겟돈 페이즈)]
            //  - 경제 붕괴 -> 모험가 이탈 -> 던전 방치(Heat 하락) -> 보스 및 몬스터 과증식
            //  - 전체 보스 수와 타운 빈곤도가 '임계치(Critical Mass)'를 넘어서는 순간 멸망(Wipe) 확정.
            //  - ※ 주의: 유저들의 어뷰징(일부러 파멸시켜 파밍)을 막기 위해 멸망을 막아내는 '구원(디펜스)'은 없음.
            //  - 발동 즉시 몬스터 폭주 이벤트 후 모든 데이터 강제 초기화(아마겟돈).
            //
            // [2. 번영의 인과율 (대륙 해금 및 폐쇄 페이즈)]
            //  - 경제가 무너지지 않고 '번영(Prosperity)' 수치가 누적 기준치를 달성할 때마다 다음 대륙 개방.
            //  - [대륙 해금 순서] Trammel ➔ Ilshenar ➔ Malas ➔ Tokuno ➔ Ter Mur ➔ Felucca
            //
            //  - [대륙 폐쇄 및 주택 고립 시스템 (Isolation)]
            //    * 번영 점수가 하락하여 대륙이 다시 닫힐 경우, 공공 문게이트 및 리콜(진입)이 전면 차단됨.
            //    * 해당 대륙에 집을 지어둔 유저는 집 내부(House Region)에서만 이동 가능하며, 집 밖으로 한 발자국도 나갈 수 없음.
            //    * 이를 극복하기 위해 유저들은 미리 닫힘을 대비하여 안전한 대륙과 연결되는 '하우스 텔레포터'를 설치하는 메타 형성.
            // ==============================================================================
