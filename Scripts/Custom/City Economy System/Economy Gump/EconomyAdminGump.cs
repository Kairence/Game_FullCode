...
namespace Server.Misc
{
    public class EconomyAdminGump : Gump
    {
        ...
        private void DrawTownList()
        {
            ...
            // =========================================================================
            AddImageTiled(20, 75, 760, 55, 2624); // 대시보드 배경(반투명 박스)
            ...
            // 두 번째 줄: 엔진 스위치 및 보정값 입력
            bool engineOn = GlobalEconomyMonitor.EnableEconomyEngine;
            AddLabel(30, 105, 2100, "엔진 개입:");
            AddButton(100, 105, 4005, 4007, 40, GumpButtonType.Reply, 0); // 스위치 토글 (ID: 40)
            AddLabel(135, 107, engineOn ? 68 : 33, engineOn ? "ON" : "OFF");

            AddLabel(260, 105, 2100, "보정값(+/-):");
            AddImageTiled(345, 105, 120, 20, 3004); // 입력창 배경
            AddTextEntry(347, 105, 116, 20, 0, 0, GlobalEconomyMonitor.TownWealthOffset.ToString()); // ID: 0
            AddButton(470, 105, 4011, 4012, 41, GumpButtonType.Reply, 0); // 적용 버튼 (ID: 41)
            // =========================================================================

            // --- 2. 마을 리스트 헤더 ---
            int y = 140; // 대시보드 삽입으로 전체 Y축 30px 하강
            ...
            if (m_TownID == 0)
            {
                switch (id)
                {
                    ...
                    case 41:
                        if (info.GetTextEntry(0) is TextRelay text)
                        {
                            if (long.TryParse(text.Text, out long newOffset))
                            {
                                ...
                                GlobalEconomyMonitor.GenerateUserReport(); // 적용 즉시 GII 최신화
                    ...
                    case 14: 
                        ...
                        GlobalEconomyMonitor.GenerateUserReport(); // 동기화 시 글로벌 리포트도 갱신
