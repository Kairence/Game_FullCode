...
namespace Server.Misc
{
    public static class GlobalEconomyReport
    {
        ...
        public static void GenerateMasterReport(List<TownEconomy> allTowns)
        {
            ...
            foreach (var town in allTowns)
            {
                foreach (var res in town.Warehouse)
                {
                    ...
                    {
                        // 🌟 [수정] 객체 참조(res.Key, res.Value) 대신, 명확한 이름(res.Key.Name)과 숫자(res.Value.Stock) 출력으로 버그 해결
                        sb.AppendLine($"| {town.TownName} | {res.Key.Name} | {res.Value.Stock} | [위험] Critical |");
                    }
                }
            }
        ...
        {
            // 🌟 [수정] 이모지 별점 제거, 텍스트 등급으로 직관적 표시
            double avg = town.Citizens.Average(c => c.Satisfaction);
            if (avg > 80) return "[최고] Very High";
            if (avg > 60) return "[좋음] High";
            if (avg > 40) return "[보통] Normal";
            return "[나쁨] Low";
        }
    }
}