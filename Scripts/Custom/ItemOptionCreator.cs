...
namespace Server.Misc
{
	public class ItemOptionCreator
	{
				...
				if (equip.SuffixOption[1] > 0)
				{
					...
					if (equip.Resource == CraftResource.Bronze || Utility.RandomDouble() < 0.001)
					{
						...
						if (artifact != null)
						{
							...
							artifactSave.ItemPower = (ItemPower)
								Enum.ToObject(typeof(ItemPower), equip.SuffixOption[1] + 3);
		...
		*/
		...
		//옵션 선택
		public static int[] RankValue = { 10, 35, 50, 65, 85, 30, 45, 60, 80, 100 };
				...
				if (equip.SuffixOption[0] == 0)
				{
					...
					if (selectLine > -1)
					{
						...
						for (int i = 0; i < equip.SuffixOption[0]; ++i)
						{
							...
							equip.SuffixOption[11 + i] = OptionValueSelect(
								equip.SuffixOption[1],
								equip.PrefixOption[11 + i]
							); //옵션 값
