...
namespace Server.Items
{
	...
	public class WorldOmniporter : Item
	{
		...
		public enum OptFlags
		{
			...
			///-- When 'true' transport is allowed to the 'Felucca' region
			Trammel = 0x00000002,

			///-- When 'true' transport is allowed to the 'Trammel' region
			Ilshenar = 0x00000004,

			///-- When 'true' transport is allowed to the 'Ilshenar' region
			Malas = 0x00000008,

			///-- When 'true' transport is allowed to the 'Malas' region
			Tokuno = 0x00000010,

			///-- When 'true' transport is allowed to the 'Tokuno' region
			TerMur = 0x00000020,

			///-- When 'true' transport is allowed to the 'TerMur' region
			...
			///-- When 'true' transport is allowed to the 'Custom' region
			UseGlobal = 0x09000800,

			///-- When 'true' changes made to this Omniporter affect all others, and vice versa
			PublicMoongates = 0x09000010,

			///-- When 'true' transport is allowed to the 'PublicMoongates' region
			IlshenarShrines = 0x09000040,

			///-- When 'true' transport is allowed to the 'IlshenarShrines' region
			TramDungeons = 0x09000002,

			///-- When 'true' transport is allowed to the 'TramDungeons' region
			FelDungeons = 0x09000008,

			///-- When 'true' transport is allowed to the 'FelDungeons' region
			AllowReds = 0x09000200,

			///-- When 'true' transport is allowed by Reds ( 5+ Murdercount )
			...
			WO_PetsMustPay =
				0x09800000 //##### -- % -- When 'true' owners must pay for pets also
			,
		}
		...
		public int LocalUses
		{
			get { return m_LocalUses; }
			set { m_LocalUses = value < 0 ? 0 : value; }
		}
		...
		public int EntryUses(Mobile m)
		{
			...
			catch
			{
				...
				try
				{
					GlobalMobileUse.Add("0", new UseEntry("None", 0, DateTime.UtcNow));
				}
				catch
				{
					Console.WriteLine("Exception caught Adding '0' entry to GlobalMobileUse.");
				}
			}
			return uses;
		}
		...
		public void RaiseUses(Mobile m, int raise)
		{
			...
			try
			{
				uses = EntryUses(m);
				if (uses < 1)
					RegisterUse(m, new UseEntry(m.Name, raise, DateTime.UtcNow));
				else
					RegisterUse(m, new UseEntry(m.Name, uses + raise, DateTime.UtcNow));
			}
			catch (Exception e)
			{
		...
		public void RegisterUse(Mobile from, UseEntry entry)
		{
			...
			{
				Console.WriteLine("Not a PlayerMobile using the Omniporter?");
				return;
			}
			try
			{
		...
		public int SkillsCostBonus
		{
			...
			{
				if (UseGlobal)
					m_GlobalSkillsCostBonus = value;
				else
					m_SkillsCostBonus = value;
			}
		}
		...
		public int KarmaDiscountBonus
		{
			...
			{
				if (UseGlobal)
					m_GlobalKarmaDiscountBonus = value;
				else
					m_KarmaDiscountBonus = value;
			}
		}
		...
		public int GetPrice(Mobile m, int num, bool pets)
		{
			...
			if (WO_AllPayToUse || (WO_DelayAfterUse && WO_PayBeforeDelay && NextUseTime(m) > DateTime.UtcNow))
			{
				...
				if (!WO_AllPayToUse)
					price *= (delay + (next < 0 ? 0 : next)) / delay;
				if (price < 0.0)
					price = 0.0;
				if (pets && WO_PetsMustPay)
					price *= num;
		...
		public string NextUseMessage(Mobile m, int num, bool pets)
		{
			...
			string pay = "";
			string reason = "";
			if (pets && !WO_PetsMayTravel)
				reason = " because your pets might try to follow you";
			if (CanUse(m, pets))
			{
				...
				if (MustPay(m, pets))
				{
					if (!CanDelay(m))
						...
						delay = String.Format(" or pay {0} Gold now", GetPrice(m, num, pets));
				}
				if (MustWait(m))
					pay = String.Format(
						"You must wait {0} minute{1} to use {2} Omniporter{3}.{4}",
						minutes,
						minutes == 1 ? "" : "s",
						UseGlobal ? "any" : "this",
						delay,
		...
		{
			if (WO_DelayAfterUse && !WO_AllPayToUse)
				return true;
			return false;
		}
		...
		{
			if (WO_DelayAfterUse && !WO_PayBeforeDelay && NextUseTime(m) > DateTime.UtcNow)
				return true;
			return false;
		}
		...
		{
			if (pets && WO_PetsMayTravel && WO_PetsMustPay)
				return true;
		...
		public bool CanUse(Mobile from, bool dungeon, bool felucca)
		{
			...
			if (from is PlayerMobile)
				...
				return false;
			if (!AllowReds && m.Kills > 4)
				return false;
			if (WO_OnlyAllowYoung && !m.Young)
				return false;
			if (WO_FeluccaNoYoung && felucca && m.Young)
				return false;
			if (WO_DungeonsNoYoung && dungeon && m.Young)
				return false;
			return true;
		}
		...
		public static void Initialize()
		{
			...
			GlobalEntries.Add(
				...
				new OmniEntry[]
				{
					...
					new OmniEntry("코베투스 입구", new Point3D(2498, 921, 0), Map.Trammel),
					//new OmniEntry("코베투스 2층", new Point3D(2387, 837, 0), Map.Trammel ),
					//new OmniEntry("코베투스 3층", new Point3D(2545, 856, 0), Map.Trammel ),
					new OmniEntry("데스파이즈 입구", new Point3D(1301, 1080, 0), Map.Trammel),
					new OmniEntry("오크 던전 입구", new Point3D(1021, 1431, 0), Map.Trammel),
					new OmniEntry("디싯 입구", new Point3D(4111, 434, 5), Map.Trammel),
					//new OmniEntry("디싯 3층", new Point3D(5137, 649, 5), Map.Trammel ),
					new OmniEntry("쉐임 입구", new Point3D(511, 1565, 0), Map.Trammel),
					//new OmniEntry("쉐임 3층", new Point3D(5507, 187, 0), Map.Trammel )
					/*
			...
			GlobalEntries.Add(
				...
				new OmniEntry[]
				{
					new OmniEntry("코브", new Point3D(2359, 1181, 10), Map.Trammel, true),
					new OmniEntry("브리튼", new Point3D(1336, 1997, 5), Map.Trammel, true),
					new OmniEntry("부케니어스 덴", new Point3D(2844, 2249, 0), Map.Trammel, true),
					new OmniEntry("젤롬", new Point3D(1499, 3771, 5), Map.Trammel, true),
					new OmniEntry("마진시아", new Point3D(3563, 2139, 34), Map.Trammel, true),
					new OmniEntry("미녹", new Point3D(2701, 692, 5), Map.Trammel, true),
					new OmniEntry("문글로우", new Point3D(4467, 1283, 5), Map.Trammel, true),
					new OmniEntry("누젤롬", new Point3D(3517, 1209, 0), Map.Trammel, true),
					new OmniEntry("뉴헤븐", new Point3D(3450, 2677, 25), Map.Trammel, true),
					new OmniEntry("써펜트 홀드", new Point3D(2889, 3542, 10), Map.Trammel, true),
					new OmniEntry("스카라 브레", new Point3D(643, 2067, 5), Map.Trammel, true),
					new OmniEntry("트린식", new Point3D(1828, 2948, -20), Map.Trammel, true),
					new OmniEntry("베스퍼", new Point3D(2715, 1041, 0), Map.Trammel, true),
					new OmniEntry("유", new Point3D(771, 752, 5), Map.Trammel, true),
					/*
			...
			GlobalEntries.Add(
				"Custom",
				new OmniEntry[] //add locations to the custom map here
				{
					new OmniEntry("브리튼 머더러 캠프", new Point3D(885, 1682, 0), Map.Trammel),
					new OmniEntry("유 언데드 사원", new Point3D(961, 789, 0), Map.Trammel),
					/*
					new OmniEntry("House Gates", new Point3D(616, 2113, 0), Map.Trammel ),
					new OmniEntry("Hue Room", new Point3D(5379, 1094, 0), Map.Trammel ),
					new OmniEntry("Mook Town", new Point3D(1069, 1443, -90), Map.Trammel ),
					new OmniEntry("Taming Forest", new Point3D(902, 912, 0), Map.Trammel ),
					new OmniEntry("Gate Room", new Point3D(6079, 451, -22), Map.Trammel ),
					new OmniEntry("Champ Spawn Abyss", new Point3D(5256, 3325, 4), Map.Trammel ),
		...
		public static int GenerateWorldOmniporters()
		{
			...
			if (GetOptFlag(m_GlobalFlags, OptFlags.PublicMoongates))
				gen += GenerateEntry("Public Moongates");
		...
		public bool Trammel
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Trammel); }
			set { SetOptFlag(OptFlags.Trammel, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool TramDungeons
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.TramDungeons); }
			set { SetOptFlag(OptFlags.TramDungeons, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool Felucca
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Felucca); }
			set { SetOptFlag(OptFlags.Felucca, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool FelDungeons
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.FelDungeons); }
			set { SetOptFlag(OptFlags.FelDungeons, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool PublicMoongates
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.PublicMoongates); }
			set { SetOptFlag(OptFlags.PublicMoongates, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool Ilshenar
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Ilshenar); }
			set { SetOptFlag(OptFlags.Ilshenar, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool IlshenarShrines
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.IlshenarShrines); }
			set { SetOptFlag(OptFlags.IlshenarShrines, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool Malas
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Malas); }
			set { SetOptFlag(OptFlags.Malas, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool Tokuno
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Tokuno); }
			set { SetOptFlag(OptFlags.Tokuno, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool TerMur
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.TerMur); }
			set { SetOptFlag(OptFlags.TerMur, value); }
		}
		...
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.AllowReds); }
			set { SetOptFlag(OptFlags.AllowReds, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool Custom
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.Custom); }
			set { SetOptFlag(OptFlags.Custom, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool UseGlobal
		{
			get { return GetOptFlag(m_Flags, OptFlags.UseGlobal); }
			set { SetOptFlag(ref m_Flags, OptFlags.UseGlobal, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_OnlyAllowYoung
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_OnlyAllowYoung); }
			set { SetOptFlag(OptFlags.WO_OnlyAllowYoung, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_FeluccaNoYoung
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_FeluccaNoYoung); }
			set { SetOptFlag(OptFlags.WO_FeluccaNoYoung, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_DungeonsNoYoung
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_DungeonsNoYoung); }
			set { SetOptFlag(OptFlags.WO_DungeonsNoYoung, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_AllPayToUse
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_AllPayToUse); }
			set { SetOptFlag(OptFlags.WO_AllPayToUse, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_SkillsCost
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_SkillsCost); }
			set { SetOptFlag(OptFlags.WO_SkillsCost, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_KarmaDiscount
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_KarmaDiscount); }
			set { SetOptFlag(OptFlags.WO_KarmaDiscount, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_DelayAfterUse
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_DelayAfterUse); }
			set { SetOptFlag(OptFlags.WO_DelayAfterUse, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_PayBeforeDelay
		{
			get
			{
				if (!WO_DelayAfterUse)
					return false;
				return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_PayBeforeDelay);
			}
			set { SetOptFlag(OptFlags.WO_PayBeforeDelay, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_PetsMayTravel
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_PetsMayTravel); }
			set { SetOptFlag(OptFlags.WO_PetsMayTravel, value); }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public bool WO_PetsMustPay
		{
			get { return GetOptFlag((UseGlobal ? m_GlobalFlags : m_Flags), OptFlags.WO_PetsMustPay); }
			set { SetOptFlag(OptFlags.WO_PetsMustPay, value); }
		}
		...
		[Constructable]
		public WorldOmniporter(bool moongate)
			: this((int)m_GlobalFlags, moongate) { }

		[Constructable]
		public WorldOmniporter(int flags)
			: this(flags, false) { }

		[Constructable]
		//public WorldOmniporter( int flags, bool moongate ) : base( moongate? 0xF6C : 7107 )  //Original Daat99 MoonGate Animation
		public WorldOmniporter(int flags, bool moongate)
			: base(3948) //MoonGate Animation
		{
			{
				this.Movable = false;
		...
		public WorldOmniporter(Serial serial)
			: base(serial) { }
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				...
				case 1:
				{
					...
					{
						Console.WriteLine("Exception caught Deserializing GlobalMobileUse.");
					}
	...
	public class WorldOmniporterGump : Gump
	{
		...
		public WorldOmniporterGump(Mobile from, WorldOmniporter WO, int page)
			: base(100, 100)
		{
			...
			AddHtml(75, 40, 400, 25, "<basefont color=#FFFFFF>이동할 곳을 고르세요</basefont>", false, false);
			AddHtml(165, 12, 200, 25, "<basefont color=#FFFFFF>카이렌스 문게이트 시스템</basefont>", false, false);
			...
			*/
			if (WO.PublicMoongates && (WO.Felucca || (WO.Trammel && m_Reds)))
			{
				...
				AddPageButton("<basefont color=#FFFFFF>문게이트</basefont>", null, p++, 5);
			}

			if (WO.TramDungeons && m_Reds && WO.CanUse(m, true, false))
			{
				GenerateMapListing(2);
				AddPageButton("트라멜 던전", Map.Trammel, p++, 2);
			}
			...
			*/
			...
			{
				GenerateMapListing(11);
				AddPageButton("<basefont color=#FFFFFF>기타 지역</basefont>", null, p++, 11);
			}
		}
		...
		private void GenerateMapListing(int page)
		{
			...
			{
				PlayerMobile pm = m as PlayerMobile;
				if ( pm.MoongateTime > DateTime.Now && pm.DeathCheck <= 0 && name != "Public Moongates" )
				{
					m.SendMessage("오늘은 더 이상 이동할 수 없습니다.");
					return;
				}
			}
			*/
			...
			for (int i = 0, l = 0; i < oe.Length; i++)
			{
				...
				if (
					(
						(gates || name == "Felucca")
						&& entry.Map == Map.Felucca
						&& (!m_WO.Felucca || !m_WO.CanUse(m, false, true))
					)
				)
					continue;
				...
				else
				{
					...
					if (m_Staff)
						AddHtml(243, 70 + (l * 20), 150, 20, label, false, false); //Staff Location Display Name
					else
						AddHtml(273, 70 + (l * 20), 150, 20, label, false, false); //Player Location Display Name
					if (m_Staff)
						AddLabel(365, 70 + (l * 20), 1152, entry.ToCount.ToString()); // Staff - Player Location Usage Count To
					if (m_Staff)
						AddLabel(405, 70 + (l * 20), 1152, entry.FromCount.ToString()); // Staff - Player Location Usage Count From
					if (m_Staff)
						AddButton(208, 70 + (l * 20), 4015, 4016, (i + offset), GumpButtonType.Reply, 0); //Staff Location Buttons
					else
						AddButton(238, 70 + (l * 20), 4015, 4016, (i + offset), GumpButtonType.Reply, 0); //Player Location Buttons
					l++;
				}
			}
		}
			...
			if (m_Entry == null)
				...
				from.SendGump(
					new ConfirmGump(
						...
						300,
						300,
						new ConfirmGumpCallback(PaymentConfirm_Callback)
					)
				);
			else
				from.SendMessage(message); // You may not use the Omniporter...
		...
		public void DoTravel( Mobile from )
		{
			...
			if( from is PlayerMobile )
			{
				...
				BaseRegion MoongateCheck = Region.Find(m_Entry.Destination, m_Entry.Map) as BaseRegion;
				if( !MoongateCheck.IsPartOf( "Moongates" ) )
				{
					if ( pm.MoongateTime < DateTime.Now )
						pm.MoongateTime = DateTime.Now + TimeSpan.FromHours(6.0);
					else if ( pm.DeathCheck >= 1 )
						pm.DeathCheck--;
					else
						check = false;
				}
			...
			{
				from.SendMessage("당신은 일일 귀환 횟수가 모자랍니다.");
				return;
			}
		...
		*/
		public void DoTravel(Mobile from)
		{
			...
			if( from is PlayerMobile )
			{
				...
				PlayerMobile pm = from as PlayerMobile;
				BaseRegion MoongateCheck = Region.Find(m_Entry.Destination, m_Entry.Map) as BaseRegion;
				if( MoongateCheck == null || !MoongateCheck.IsPartOf( "Moongates" ) )
				{
					if ( pm.MoongateTime < DateTime.Now )
						pm.MoongateTime = DateTime.Now + TimeSpan.FromHours(6.0);
					else if ( pm.DeathCheck >= 1 )
	...
	public class OmniEntry
	{
		...
		public int ToCount
		{
			get { return m_ToCount; }
			set { m_ToCount = value; }
		}
		public int FromCount
		{
			get { return m_FromCount; }
			set { m_FromCount = value; }
		}

		private bool m_Moongate;
		public bool Moongate
		{
			get { return m_Moongate; }
			set { m_Moongate = value; }
		}

		public OmniEntry(string name, Point3D p, Map map)
			: this(name, p, map, false) { }
	...
	public class WorldOmniGenCommand
	{
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(WorldOmniDel_OnCommand)
			);
			CommandSystem.Register(
				"WorldOmniGen",
				AccessLevel.Administrator,
				new CommandEventHandler(WorldOmniGen_OnCommand)
			);
		}
		...
		public static void WorldOmniGen_OnCommand(CommandEventArgs e)
		{
			...
			World.Broadcast(
				0x35,
				true,
				"Finished generating {0} world Omniporters.",
				WorldOmniporter.GenerateWorldOmniporters()
			);
		}
