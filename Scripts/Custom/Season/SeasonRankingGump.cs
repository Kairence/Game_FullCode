...
namespace Server.Misc
{
	...
	public class SeasonRankingGump : Gump
	{
		...
		public SeasonRankingGump(PlayerMobile from, RankingType type, int index)
			: base(150, 150)
		{
			m_Type = type;
			m_Index = index;
			...
			AddHtml(
				...
				410,
				25,
				$"<BASEFONT SIZE=5 COLOR=#FFD700><CENTER>{titleName} RANKING (TOP 10)</CENTER></BASEFONT>",
				false,
				false
			);
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (i < rankings.Count)
				{
					...
					AddLabel(
						...
						y + 5,
						color,
						type == RankingType.Skill ? data.Value.ToString("F1") : data.Value.ToString("N0")
					);
				}
				else
				{
			...
			AddHtml(
				...
				390,
				25,
				$"<BASEFONT SIZE=4 COLOR=#FFFFFF><CENTER>{myInfo}</CENTER></BASEFONT>",
				false,
				false
			);
		...
		private string GetResourceName(int index)
		{
			if (index >= SeasonCount.MetalStart && index < SeasonCount.MetalStart + 7)
				return new string[] { "철", "구리", "청동", "금", "아가파이트", "베라이트", "벨러라이트" }[
					index - SeasonCount.MetalStart
				];
			...
			if (index >= SeasonCount.LeatherStart && index < SeasonCount.LeatherStart + 7)
				return new string[]
				{
					...
					"뿔 가죽",
					"미늘 가죽",
				}[index - SeasonCount.LeatherStart];

			if (index >= SeasonCount.FishStart && index < SeasonCount.FishStart + 7)
				return new string[] { "송어", "배스", "은어", "붕어", "메기", "대구", "농어" }[
					index - SeasonCount.FishStart
				];
		...
		private List<RankEntry> GetRankData(RankingType type, int index)
		{
			...
			foreach (Mobile m in World.Mobiles.Values)
			{
				if (m is PlayerMobile pm && pm.Young)
				{
					...
					if (val > 0)
						list.Add(new RankEntry(pm.Name, val));
				}
			}
		...
		public class RankEntry
		{
			...
			public RankEntry(string n, double v)
			{
				Name = n;
				Value = v;
			}
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			if (info.ButtonID == 1)
			{
				...
				{
					case RankingType.Skill:
						sender.Mobile.SendGump(new SkillAchievementGump(sender.Mobile));
						break;
					case RankingType.Resource:
						sender.Mobile.SendGump(new ResourceAchievementGump(sender.Mobile));
						break;
					case RankingType.Monster:
						sender.Mobile.SendGump(new MonsterDropHandlerGump(sender.Mobile, 0));
						break;
				}
			}
		}
