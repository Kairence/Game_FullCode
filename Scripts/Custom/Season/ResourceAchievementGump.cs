...
namespace Server.Misc
{
	...
	{
		public ResourceAchievementGump(Mobile from)
			: base(50, 50)
		{
			Account acct = from.Account as Account;
			if (acct == null || acct.Point == null)
				return;
			...
			AddHtml(
				...
				1200,
				30,
				"<BASEFONT SIZE=6 COLOR=#FFFF00><CENTER>시즌 업적: 전 자원 채취 현황</CENTER></BASEFONT>",
				false,
				false
			);
			...
			RenderResourceColumn(
				acct,
				new string[] { "철", "구리", "청동", "금", "아가파이트", "베라이트", "벨러라이트" },
				new int[] { 0x19B9, 0x19B9, 0x19B9, 0x19B9, 0x19B9, 0x19B9, 0x19B9 },
				new int[] { 0x000, 0x96D, 0x972, 0x8A5, 0x979, 0x89F, 0x8AB },
				40,
				85,
				SeasonCount.MetalStart,
				true,
				1000
			);
			...
			RenderResourceColumn(
				acct,
				new string[] { "나무", "떡갈 나무", "물푸레 나무", "주목 나무", "심재 나무", "피 나무", "서리 나무" },
				new int[] { 0x1BDD, 0x1BDD, 0x1BDD, 0x1BDD, 0x1BDD, 0x1BDD, 0x1BDD },
				new int[] { 0x000, 0x7DA, 0x4A7, 0x4A8, 0x4A9, 0x4AA, 0x47F },
				620,
				85,
				SeasonCount.WoodStart,
				false,
				1100
			);
			...
			RenderResourceColumn(
				acct,
				new string[] { "가죽", "질긴 가죽", "거친 가죽", "경화 가죽", "가시 가죽", "뿔 가죽", "미늘 가죽" },
				new int[] { 0x1079, 0x1079, 0x1079, 0x1079, 0x1079, 0x1079, 0x1079 },
				new int[] { 0x000, 0x283, 0x227, 0x1C1, 0x8AC, 0x845, 0x851 },
				40,
				435,
				SeasonCount.LeatherStart,
				true,
				1200
			);
			...
			RenderResourceColumn(
				acct,
				new string[] { "송어", "배스", "은어", "붕어", "메기", "대구", "농어" },
				new int[] { 2508, 2509, 2510, 2511, 17606, 17159, 17155 },
				new int[] { 0, 0, 0, 0, 0, 0, 0 },
				620,
				435,
				SeasonCount.FishStart,
				true,
				1300
			);
			...
			AddHtml(
				...
				1160,
				20,
				"<BASEFONT COLOR=#ffff00><CENTER>항목 우측의 랭킹 버튼을 클릭하여 실시간 순위를 확인하세요! (계정 공유)</CENTER></BASEFONT>",
				false,
				false
			);
		}
		...
		private void RenderCategoryHeader(Account acct, int idx, string title, int x, int y)
		{
			...
			AddHtml(
				...
				540,
				20,
				$"<BASEFONT SIZE=4 COLOR=#ffffff><CENTER>{title} : {current:N0} / 1,000,000,000 ({pct:F1}%)</CENTER></BASEFONT>",
				false,
				false
			);
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (bid >= 1000 && bid < 1400)
			{
				...
				if (typeOffset == 1000)
					actualIndex = SeasonCount.MetalStart + indexInGroup;
				else if (typeOffset == 1100)
					actualIndex = SeasonCount.WoodStart + indexInGroup;
				else if (typeOffset == 1200)
					actualIndex = SeasonCount.LeatherStart + indexInGroup;
				else if (typeOffset == 1300)
					actualIndex = SeasonCount.FishStart + indexInGroup;
