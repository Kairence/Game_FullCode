...
namespace Server.Misc
{
    public class SeasonCount
    {
        ...
        private static void Update(Mobile from, Account acct, int idx, int amount)
        {
            ...
            bool isTotal = (idx % 10 == 1 && idx <= 51) || idx == 400 || 
                           (idx % 10 == 1 && idx >= 401 && idx <= 451) || idx == 800;

            int limit = isTotal ? TotalMaxGoal : MaxGoal;

            if (acct.Point[idx] >= limit) return;
            ...
            if ((acct.Point[idx] += amount) >= limit)
            {
                ...
                if (idx >= 1 && idx <= 400)
                {
                    from.SendMessage(0x42, isTotal ? $"[시즌 대업적] {idx}번 카테고리 달성!" : $"[시즌 업적] {idx}번 항목 달성!");
                }
            }
        }
    }
