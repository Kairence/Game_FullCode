...
namespace Server.Misc
{
	...
	{
		public SkillAchievementGump(Mobile from)
			: base(30, 50)
		{
			...
			string title =
				(pm.Young && pm.seasonSkillBonus > 0)
			...
			if (pm.Young && pm.seasonSkillBonus > 0)
				AddHtml(
					...
					1210,
					20,
					"<BASEFONT COLOR=#00FF00><CENTER>왼쪽 화살표: 50.0 즉시 상승(기회 차감) | 오른쪽 버튼: 실시간 스킬 랭킹 확인</CENTER></BASEFONT>",
					false,
					false
				);
			else
				AddHtml(
					...
					1210,
					20,
					"<BASEFONT COLOR=#00FF00><CENTER>시즌이 종료되기 전까지 랭킹 1위를 목표로 스킬을 연마하세요!</CENTER></BASEFONT>",
					false,
					false
				);
		}
			...
			if (buttonID >= 100 && buttonID < 158)
			{
				...
				if (pm.Young && pm.seasonSkillBonus > 0)
				{
					if (pm.Skills[skillIndex].Base < 50.0)
					{
						...
						pm.SendMessage(
							0x42,
							$"{SkillInfo.Table[skillIndex].Name} 스킬 숙련도가 50.0으로 보정되었습니다."
						);
					}
