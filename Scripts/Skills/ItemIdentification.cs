...
public class ItemIdentification
{
	...
	public static void Initialize() => SkillInfo.Table[(int)SkillName.ItemID].Callback = OnUse;
	...
	private class InternalTarget : Target
	{
		public InternalTarget()
			: base(8, false, TargetFlags.None) => AllowNonlocal = true;
		...
		protected override void OnTarget(Mobile from, object o)
		{
			...
			if (o is Item item && item.RootParent == from && item is IEquipOption equip)
			{
				...
				if (from.Skills.ItemID.Value < GradeSkillTable[grade])
				{
					from.SendMessage(
						0x22,
						$"{GradeNames[grade]} 등급 장비를 감정하려면 스킬이 {GradeSkillTable[grade]} 이상 필요합니다."
					);
					from.EndAction(typeof(ItemIdentification));
					return;
				}
		...
		: Timer(TimeSpan.FromSeconds(2.0), TimeSpan.FromSeconds(2.0))
	{
		...
		{
			if (
				from is not { Alive: true }
				|| item is not { Deleted: false }
				|| from.Location != _location
				|| item.RootParent != from
			)
			{
	...
	{
		if (from == null || item == null || equip == null)
			return;
