			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (targeted is BaseCreature)
				{
					...
					if (
						...
						|| from.Skills[SkillName.AnimalLore].Value >= c.MinTameSkill
					)
					{
						SendGump(from, c);
					}
			...
			else
			{
				for (int i = 0; i < count; i++)
				{
					...
					AddHtml(
						...
						120,
						18,
						String.Format("<BASEFONT COLOR={0}>{1}</BASEFONT>", colorCode, name),
						false,
						false
					);
					AddHtml(
						...
						75,
						18,
						String.Format("<div align=right><BASEFONT COLOR={0}>{1}</BASEFONT></div>", colorCode, valStr),
						false,
						false
					);
				}
