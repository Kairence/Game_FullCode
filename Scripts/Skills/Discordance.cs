		...
		private static void ProcessDiscordance(DiscordanceInfo info)
		{
			...
			else
			{
				...
				if (
					!targ.Alive
					|| targ.Deleted
					|| targ.IsDeadBondedPet
					|| !from.Alive
					|| from.Hidden
					|| targ.Hidden
					|| from.IsDeadBondedPet
				)
				{
					ends = true;
				}
			...
			protected override void OnTarget(Mobile from, object target)
			{
				...
				else if (target is Mobile)
				{
					...
					else if (m_Table.ContainsKey(targ)) //Already discorded
					{
						...
						!targ.Player
						|| (from is BaseCreature && ((BaseCreature)from).CanDiscord)
					...
					)
					{
						...
						else if (from.CheckTargetSkill(SkillName.Discordance, target, diff - 25.0, diff + 25.0))
						{
							...
							{
								info = new DiscordanceInfo(
									from,
									targ,
									0,
									null,
									true,
									from.Skills.CurrentMastery == SkillName.Discordance ? 6 : 4
								);
								from.DoHarmful(targ);
							}
							else
							{
								...
								if (Core.AOS)
								{
									...
									for (int i = 0; i < targ.Skills.Length; ++i)
									{
										...
										{
											mods.Add(
												new DefaultSkillMod((SkillName)i, true, targ.Skills[i].Value * scalar)
											);
										}
									}
								}
										...
										if (targ.Skills[i].Value > 0)
										{
											mods.Add(
												new DefaultSkillMod(
													(SkillName)i,
													true,
													Math.Max(100, targ.Skills[i].Value) * scalar
												)
											);
										}
									}
