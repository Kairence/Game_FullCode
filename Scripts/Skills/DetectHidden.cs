		...
		public class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile src, object targ)
			{
				...
				if (range > 0)
				{
					...
					foreach (Mobile trg in inRange)
					{
						if (trg.Hidden && src != trg)
						{
							...
							if (
								...
								&& (ss >= ts || houseCheck)
								&& Utility.RandomDouble() > shadow
							)
							{
								if (
									(trg is ShadowKnight && (trg.X != p.X || trg.Y != p.Y))
					...
					{
						if (
							item is LibraryBookcase
							&& Server.Engines.Khaldun.GoingGumshoeQuest3.CheckBookcase(src, item)
						)
						{
							foundAnyone = true;
						}
