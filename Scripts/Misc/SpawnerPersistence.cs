...
namespace Server
{
	public class SpawnerPersistence
	{
		...
		public static bool SpawnsConverted
		{
			get { return _SpawnsConverted; }
		}
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register(
				...
				{
					string str =
						"By selecting OK, you will wipe all XmlSpawners that were placed via World Load, and will replace "
					...
					e.Mobile.SendGump(
						new WarningGump(
							...
							(from, ok, state) =>
							{
								...
								{
									from.SendMessage(
										"Stand by while spawners are converted. This may take a few minutes..."
									);
			...
			CommandSystem.Register(
				...
				{
					string str =
						"By selecting OK, you will wipe all XmlSpawners that were left over from conversion to "
					...
					e.Mobile.SendGump(
						new WarningGump(
							...
							(from, ok, state) =>
							{
								...
								{
									from.SendMessage(
										"Stand by while spawners are converted back to XmlSpawners. This may take a few minutes..."
									);
		...
		public static void GenerateTramKhaldun()
		{
			...
			if (region != null)
			{
				...
				ToConsole(
					String.Format(
						"Copied {0} khaldun spawners, {1} teleporters and placed in trammel!",
						spawners,
						teleporters
					)
				);
		...
		public static void CheckQuestQuesters()
		{
			...
			ToConsole(
				String.Format(
					"Quester Re-assignment: {0} quests re-assigned quester type. Some quester types may still be null. These quests will need to be quit.",
					count.ToString()
				),
		...
		private static void CheckSmartSpawn(Type check, bool subclasses)
		{
			...
			ToConsole(
				String.Format(
					"Smart Spawn Removal: {0} spawners [type {1}] smart spawning disabled.",
					count.ToString(),
					check.Name
				)
			);
		...
		public static void Delete(string current)
		{
			...
			ToConsole(
				String.Format(
					"Spawner Deletion: deleted {0} spawners containing -{1}-.",
					toDelete.Count.ToString(),
					current
				)
			);
		...
		public static void Replace(string current, string replace, string check)
		{
			...
			ToConsole(
				String.Format(
					"Spawn Replacement: {0} spawners replaced [{1} replaced with {2}].",
					count.ToString(),
					current,
					replace
				)
		...
		public static void Replace(string current, string replace, string name, string check)
		{
			...
			foreach (
				var spawner in World
					.Items.Values.OfType<ISpawner>()
					.Where(s =>
						s is Item && ((Item)s).Name != null && ((Item)s).Name.ToLower().IndexOf(name.ToLower()) >= 0
					)
			)
			{
				if (Replace(spawner, current, replace, check))
			...
			ToConsole(
				String.Format(
					"Spawn Replacement: {0} spawners named {1} replaced [{2} replaced with {3}].",
					count.ToString(),
					name,
					current,
					replace
		...
		public static void Remove(string toRemove, Func<XmlSpawner, bool> predicate = null)
		{
			...
			ToConsole(
				String.Format(
					"Spawn Removal: {0} spawn lines removed containing -{1}-. [{2} deleted].",
					count.ToString(),
					toRemove,
					deleted
				)
		...
		/// <param name="exempt">Condition to prevent action being performed on spawner</param>
		/// <param name="action">action to be performed, setup in calling method</param>
		public static void ActionOnSpawner(
			Type typeCheck,
			string lineCheck,
			string nameCheck,
			string exempt,
			Action<ISpawner> action,
			bool inherits = false
		)
		{
			int count = 0;
				...
				List<ISpawner> list = World
					...
					.Where(s =>
						...
						|| (
							s is Item
							&& ((Item)s).Name != null
							&& ((Item)s).Name.ToLower().IndexOf(nameCheck.ToLower()) >= 0
						)
					)
					.ToList();
			...
			ToConsole(
				String.Format(
					"Spawner Action: Performed action to {0} spawners{1}",
					count.ToString(),
					lineCheck != null ? " containing " + lineCheck + "."
						: typeCheck != null ? " containing " + typeCheck.Name + "."
						: "."
				)
			);
		}

		public static bool ActionOnSpawner(
			ISpawner spawner,
			Type typeCheck,
			string lineCheck,
			string exempt,
		...
		)
		{
			...
			foreach (var str in list)
			{
				...
				else
				{
					...
					if (
						(lookFor == null || spawnObject.IndexOf(lookFor) >= 0)
						&& (exempt == null || spawnObject.IndexOf(exempt.ToLower()) <= 0)
					)
					{
						if (action != null)
							action(spawner);
		...
		private static string[] GetSpawnList(ISpawner spawner)
		{
			...
			if (
				spawner is XmlSpawner
				&& ((XmlSpawner)spawner).SpawnObjects != null
				&& ((XmlSpawner)spawner).SpawnObjects.Length > 0
			)
			{
				...
				spawner is Spawner
				&& ((Spawner)spawner).SpawnObjects != null
				&& ((Spawner)spawner).SpawnObjects.Count > 0
			)
			{
		...
		public static void LoadFromXmlSpawner(string location, Map map, string prefix = null)
		{
			...
			ToConsole(
				String.Format(
					"Created {0} spawners from {1} with -{2}- prefix.",
					processedspawners,
					location,
					SpawnerPrefix == string.Empty ? "NO" : SpawnerPrefix
				)
			);
		}
		...
		public static void ConvertXmlToSpawners()
		{
			...
			if (System.IO.Directory.Exists(filename) == true)
			{
				...
				ToConsole(
					String.Format("Found {0} Xmlspawner files for conversion.", files.Count),
					files != null && files.Count > 0 ? ConsoleColor.Green : ConsoleColor.Red
				);
				...
				if (files != null && files.Count > 0)
				{
							...
							if (ds.Tables["Points"] != null && ds.Tables["Points"].Rows.Count > 0)
							{
								foreach (DataRow dr in ds.Tables["Points"].Rows)
								{
									...
									if (!convert)
									{
										...
										{
											loc = new Point3D(
												int.Parse((string)dr["CentreX"]),
												int.Parse((string)dr["CentreY"]),
												int.Parse((string)dr["CentreZ"])
											);
					...
					if (converted > 0)
						ToConsole(
							String.Format("Converted {0} XmlSpawners to standard spawners.", converted),
							ConsoleColor.Green
						);
					...
					if (failed > 0)
						ToConsole(
							String.Format(
								"Failed to convert {0} XmlSpawners to standard spawners. {1} kept due to XmlSpawner Functionality",
								failed,
								keep
							),
							ConsoleColor.Red
		...
		{
			XmlSpawner spawner = World
				.Items.Values.OfType<XmlSpawner>()
				.FirstOrDefault(s => s.Location == p && s.Map == map);
		...
		public static void RevertToXmlSpawners(Mobile from)
		{
			...
			if (System.IO.Directory.Exists(filename) == true)
			{
				...
				ToConsole(
					String.Format("Found {0} Xmlspawner files for removal.", files == null ? 0 : files.Count),
					files != null && files.Count > 0 ? ConsoleColor.Green : ConsoleColor.Red
				);
				...
				if (files != null && files.Count > 0)
				{
					...
					ToConsole(
						String.Format(
							"Deleted {0} XmlSpawners [{2} no id] in {3} seconds.",
							deletedxml,
							nospawner,
							((Core.TickCount - start) / 1000).ToString()
						),
						ConsoleColor.Cyan
					);
				}
		...
		public static void RemoveSpawnsFromXmlFile(string directory, string filename)
		{
			if (System.IO.Directory.Exists(directory) == true)
			{
				...
				ToConsole(
					String.Format(
						"Found {0} Xmlspawner files for removal.",
				...
				if (files != null && files.Count > 0)
				{
					...
					ToConsole(
						String.Format("Deleted {0} XmlSpawners from {1}/{2}.xml.", deletedxml, directory, filename),
						ConsoleColor.Cyan
					);
				}
				else
		...
		public static void WipeSpawnersFromFile()
		{
			...
			if (System.IO.Directory.Exists(filename) == true)
			{
				...
				if (files != null && files.Count > 0)
				{
								...
								foreach (DataRow dr in ds.Tables["Points"].Rows)
								{
									...
									else
									{
										...
										if (!deleted)
										{
											...
											ToConsole(
												String.Format("Failed to Delete: {0} in {1}", (string)dr["Name"], file)
											);
										}
									}
								}
					...
					ToConsole(
						String.Format(
							"Deleted {0} XmlSpawners [{1} failed] in {2} seconds.",
							deletedxml,
							nodelelete,
							((Core.TickCount - start) / 1000).ToString()
						),
