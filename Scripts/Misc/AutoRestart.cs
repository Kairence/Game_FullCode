	...
	public class AutoRestart : Timer
	{
		...
		public static void Initialize()
		{
			...
			if (Enabled)
			{
				...
				Utility.WriteConsoleColor(
					...
					"[Auto Restart] Configured for {0}:{1}:00, every {2} hours!",
					RestartTime.Hour,
					RestartTime.Minute,
					Frequency
				);
				Utility.WriteConsoleColor(
					ConsoleColor.Magenta,
					"[Auto Restart] Next Shard Restart: {0}",
		...
		protected override void OnTick()
		{
			...
			if (WarningDelay > TimeSpan.Zero && !DoneWarning && RestartTime - WarningDelay < DateTime.Now)
			{
				World.Broadcast(
					0x22,
					true,
					"The server will be going down in about {0} minute{1}.",
					WarningDelay.TotalMinutes.ToString(),
					WarningDelay.TotalMinutes == 1 ? "" : "s"
				);
		...
		private static void TimedShutdown(bool restart)
		{
			World.Broadcast(
				...
				true,
				String.Format(
					"The server will be going down in about {0} seconds!",
					RestartDelay.TotalSeconds.ToString()
				)
			);
			Timer.DelayCall(
				RestartDelay,
