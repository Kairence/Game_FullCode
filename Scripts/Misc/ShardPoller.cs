...
namespace Server.Misc
{
	public class ShardPoller : Item
	{
		...
		public string Title
		{
			...
			set { this.m_Title = ShardPollPrompt.UrlToHref(value); }
		}
	...
	public class ShardPollOption
	{
		...
		{
			get { return this.m_LineBreaks; }
		}
		public int Votes
		{
			get { return this.m_Voters.Length; }
		}
		public IPAddress[] Voters
		{
			get { return this.m_Voters; }
			set { this.m_Voters = value; }
		}
			...
			: base(50, 50)
		{
			...
			if (editing)
			{
				this.AddHtml(
					...
					294,
					20,
					this.Color(String.Format("{0} total", totalVotes), LabelColor32),
					false,
					false
				);
			...
			if (this.m_Polls != null && this.m_Polls.Count > 0)
			{
				...
				if (poller != null)
					Timer.DelayCall(
						...
						new object[] { this.m_From, this.m_Polls }
					);
			}
				...
				if (this.m_Editing)
				{
					...
					else
					{
						...
						this.m_From.SendGump(
							new ShardPollGump(this.m_From, this.m_Poller, this.m_Editing, this.m_Polls)
						);
					}
				}
				else
	...
	{
		private static readonly Regex m_UrlRegex = new Regex(
			@"\[url(?:=(.*?))?\](.*?)\[/url\]",
