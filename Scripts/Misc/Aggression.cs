...
namespace Server.Misc
{
	public class Aggression
	{
		...
		public static TimeSpan CombatHeatDelay
		{
			get { return Delay; }
		}
		...
		public static void EventSink_AggressiveAction(AggressiveActionEventArgs e)
		{
			...
			BuffInfo.AddBuff(
				aggressor,
				new BuffInfo(BuffIcon.HeatOfBattleStatus, 1153801, 1153827, Delay, aggressor, true)
			);
			BuffInfo.AddBuff(
				aggressed,
				new BuffInfo(BuffIcon.HeatOfBattleStatus, 1153801, 1153827, Delay, aggressed, true)
			);
		}
		...
		public static bool CheckHasAggression(Mobile m, bool aggressedOnly = false)
		{
			...
			for (int i = 0; i < list.Count; ++i)
			{
				...
				if (
					(defender is PlayerMobile || (defender is BaseCreature && !((BaseCreature)defender).IsMonster))
			...
			for (int i = 0; i < list.Count; ++i)
			{
				...
				if (
					(attacker is PlayerMobile || (attacker is BaseCreature && !((BaseCreature)attacker).IsMonster))
