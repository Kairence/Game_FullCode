...
namespace Server
{
    ...
    public class AOS
    {
		...
		public static int Damage(IDamageable damageable, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int chaos, int direct, bool keepAlive, DamageType type = DamageType.Melee, int aggro = 100)
		{
			...
			// [ì»¤ìŠ¤?€] ì¶”ì (Reflexes) ?¤í‚¬: ?°ë?ì§€ ê°ì†Œ ë°?ìµœì†Œ??ë¡œì§
			if (m != null)
			{
				...
				if (!hasMetalArmor)
				{
					...
					// 100?¤í‚¬ ë³´ë„ˆ?? ìµœì¢… ?¼ê²© 50 ê°ì†Œ (ê¸°ë³¸ ê°ì†Œ???”í•´ì§?
					int flatReduction = (int)(tracking / 10); // ê¸°ë³¸ ?¼ê²© ê°ì†Œ (10??1ë¡??°ì •)
					...
					// ?ê±°ë¦?ë§ˆë²• ?¬í•¨) ?°ë?ì§€ ìµœì†Œ???•ë¥ 
					if (type == DamageType.Ranged || type == DamageType.Spell)
					{
						double minimizeChance = tracking * 0.001; // ?¤í‚¬??0.1%
						if (tracking >= 100.0) minimizeChance += 0.05; // 100?¤í‚¬ ??5% ì¶”ê?

						if (Utility.RandomDouble() < minimizeChance)
						{
							totalDamage = 1; // ?°ë?ì§€ ìµœì†Œ??
							m.SendMessage("ì¶”ì (Reflexes) ?¤í‚¬ë¡??¸í•´ ?ê±°ë¦??¼í•´ë¥?ìµœì†Œ?”í–ˆ?µë‹ˆ??");
						}
					}

					// 150?¤í‚¬ ë³´ë„ˆ?? ì¹˜ëª…?€ ?¼í•´ ë³´ì • ë¡œì§?€ ì¹˜ëª…?€ ê³„ì‚°ë¶€ê°€ ë³„ë„ë¡?ì¡´ì¬??ê²½ìš° ê·¸ê³³???‘ì„±. ?¬ê¸°?œëŠ” ìµœì¢… ?°ë?ì§€ë¡?ë³´ì •.
					// (ì¹˜ëª…?€ ê³„ìˆ˜ë¥?ì§ì ‘ ?????†ìœ¼ë¯€ë¡? ?¥í›„ ì¹˜ëª…?€ ?°ë?ì§€ ?°ì‚°ë¶€??ì¶”ê? ê¶Œì¥)
				}
			}
			...
			// ?ŒŸ bc, bm?€ ?„ì—???´ë? ? ì–¸?ˆìœ¼ë¯€ë¡??¤ì‹œ ? ì–¸?˜ì? ?ŠìŒ
			...
			if (m != null)
			{
				...
				// ?ŒŸ ê²°íˆ¬ ë¡œì§ (pm ì¤‘ë³µ ? ì–¸ ë°©ì?ë¥??„í•´ ìºìŠ¤???¬ìš©)
		...
		// 1. ê³µí†µ ë¡œì§??ì²˜ë¦¬??ë¡œì»¬ ?¨ìˆ˜ ?•ì˜ (ë©”ì„œ??ìµœìƒ?¨ì´???¬ìš© ì§ì „??? ì–¸)
        ...
        public static void DoLeech(int damageGiven, Mobile from, Mobile target)
        {
            ...
            if (totalManaLeech > 0)
            {
                // ?ë????„ì¬ ë§ˆë‚˜ë¥?ì´ˆê³¼?˜ì—¬ ëºì–´?????†ìŒ
                totalManaLeech = Math.Min(totalManaLeech, target.Mana);
                
                if (totalManaLeech > 0)
                {
                    target.Mana -= totalManaLeech; // ?ë? ë§ˆë‚˜ ê³ ê°ˆ
                    from.Mana += totalManaLeech;
                    playedSound = true;
                }
            }

            // ê¸°ë ¥ ì²˜ë¦¬ (ë¹„ë? ?¡ìˆ˜ + ê³ ì • ?ë“)
            ...
            if (totalStamLeech > 0)
            {
                // ?ë????„ì¬ ê¸°ë ¥??ì´ˆê³¼?˜ì—¬ ëºì–´?????†ìŒ
                totalStamLeech = Math.Min(totalStamLeech, target.Stam);
                
                if (totalStamLeech > 0)
                {
                    target.Stam -= totalStamLeech; // ?ë? ê¸°ë ¥ ê³ ê°ˆ
		...
		public static int GetStatus(Mobile from, int index)
		{
			...
			switch (index)
			{
				// --- ìµœë? ?€?? ¥ (Max Resistances) ---
				// ?íƒœì°½ì˜ ?°ì¸¡ ë¶„ëª¨ë¥??´ë‹¹ (GetMaxResistance ?´ë??ì„œ ?•ìˆ˜ë¡?ë°˜í™˜??
				...
				case 18: // ë¬¼ë¦¬ ë°˜ì‚¬ (ë¬´ê¸° ê³µê²© ë°˜ì‚¬: 60ë²?
					return Math.Min(30000, ItemOptionCreator.GetAttributeValue(from, 60) / VS); 
				case 19: // ?¬ì…˜ ê°•í™” (ì¹˜ìœ ??ì¦ê?%: 58ë²ˆìœ¼ë¡??€ì²´í•˜ê±°ë‚˜ 0)
					return Math.Min(30000, ItemOptionCreator.GetAttributeValue(from, 58) / VS); 

				// --- ê¸°ë³¸ ?¤íƒ¯ ---
				case 20: // ??(ê¸°ë³¸?¤íƒ¯ + ë³´ë„ˆ??0ë²? + ëª¨ë“ ?¤íƒ¯(3ë²?)
					return from.Str; 
				case 21: // ë¯¼ì²© (ê¸°ë³¸?¤íƒ¯ + ë³´ë„ˆ??1ë²? + ëª¨ë“ ?¤íƒ¯(3ë²?)
					return from.Dex; 
				case 22: // ì§€??(ê¸°ë³¸?¤íƒ¯ + ë³´ë„ˆ??2ë²? + ëª¨ë“ ?¤íƒ¯(3ë²?)
					return from.Int; 
				...
				// --- ?ì› ìµœë?ì¹?---
				// from.HitsMax ?´ë??ì„œ ?´ë? (ê¸°ë³¸ + ì²´ë ¥ ì¦ê?(5ë²? + ëª¨ë“  ?ì› ì¦ê?(8ë²?)???©ì‚°?˜ì–´ ?•ìˆ˜ë¡??˜ì˜µ?ˆë‹¤.
    ...
    {
        RegenHits = 0x00000001, //ì²´ë ¥ ?¬ìƒ
        RegenStam = 0x00000002, //ê¸°ë ¥ ?¬ìƒ
        RegenMana = 0x00000004, //ë§ˆë‚˜ ?¬ìƒ
        DefendChance = 0x00000008, //ë°©ì–´??ì¦ê?
        AttackChance = 0x00000010, //ëª…ì¤‘ë¥?ì¦ê?
        BonusStr = 0x00000020, //??ì¦ê?
        BonusDex = 0x00000040, //ë¯¼ì²© ì¦ê?
        BonusInt = 0x00000080, //ì§€??ì¦ê?
        BonusHits = 0x00000100, //ì²´ë ¥ ì¦ê?
        BonusStam = 0x00000200, //ê¸°ë ¥ ì¦ê?
        BonusMana = 0x00000400, //ë§ˆë‚˜ ì¦ê?
        WeaponDamage = 0x00000800, //?¼í•´ ì¦ê?%
        WeaponSpeed = 0x00001000, //ê³µê²© ?ë„ ì¦ê?%
        SpellDamage = 0x00002000, //ì£¼ë¬¸ ?¼í•´ ì¦ê?%
        CastRecovery = 0x00004000, //ì£¼ë¬¸ ì¹˜ëª…?€ ?•ë¥  ì¦ê?
        CastSpeed = 0x00008000, //ì£¼ë¬¸ ?ë„ ì¦ê?%
        LowerManaCost = 0x00010000, //?œì‘ ê²½í—˜ì¹?ì¦ê?%
        LowerRegCost = 0x00020000, //ì±„ì§‘ ê²½í—˜ì¹?ì¦ê?%
        ReflectPhysical = 0x00040000, //ë¬¼ë¦¬?°ë?ì§€ë°˜ì‚¬
        EnhancePotions = 0x00080000, //ì¹˜ìœ ??ì¦ê?%
        Luck = 0x00100000, 			 //??ì¦ê?
        SpellChanneling = 0x00200000, //ë§ˆë²• ì¹˜ëª…?€ ?¼í•´ ì¦ê?
        NightSight = 0x00400000,		//ê¸ˆí™” ?ë“ ì¦ê?
        IncreasedKarmaLoss = 0x00800000, //ì¹´ë¥´ë§ˆê°?Œì¦ê°€
        Brittle = 0x01000000,			 //ë¬¼ë¦¬ ì¹˜ëª…?€ ?¼í•´ ì¦ê?
        LowerAmmoCost = 0x02000000,		 //?„íˆ¬ ê²½í—˜ì¹?ì¦ê?
        BalancedWeapon = 0x04000000,	 //ë¬¼ë¦¬ ?¼í•´ ì¦ê?%
		WeaponDamageBonus = 0x08000000,	 //ë¬´ê¸° ?°ë?ì§€ì¦ê?+
		SpellDamageBonus = 0x10000000,   //ë§ˆë²• ?°ë?ì§€ì¦ê?+
		HealBonus = 0x20000000,          //ì¹˜ìœ ??ì¦ê?
		WeaponCritical = 0x40000000, //ë¬¼ë¦¬ ì¹˜ëª…?€ ?•ë¥  ì¦ê?
    }
		...
		*/
        public static int GetValue(Mobile m, AosAttribute attribute)
        {
            ...
            else if (attribute == AosAttribute.WeaponSpeed)
            {
				...
				// [ì¶”ê?] Enemy of One ?¤ì˜¤??ì²´í¬: ê³µê²© ?ë„ +10
				if (Server.Spells.Chivalry.EnemyOfOneSpell.UnderAura(m))
					value += 100000;
            }
    ...
    {
        LowerStatReq = 0x00000001,  		//?¥ë¹„ ?”êµ¬ì¹?ê°ì†Œ
        SelfRepair = 0x00000002,			//?ê? ?˜ë¦¬
        HitLeechHits = 0x00000004,			//ì²´ë ¥ ?¡ìˆ˜
        HitLeechStam = 0x00000008,			//ê¸°ë ¥ ?¡ìˆ˜
        HitLeechMana = 0x00000010,			//ë§ˆë‚˜ ?¡ìˆ˜
        HitLowerAttack = 0x00000020,		//ê³µê²©??ê°ì†Œ
        HitLowerDefend = 0x00000040,		//ë°©ì–´??ê°ì†Œ
        HitMagicArrow = 0x00000080,			//ë§¤ì§ ?”ì‚´ ë°œë™
        HitHarm = 0x00000100,				//??ë°œë™
        HitFireball = 0x00000200, 			//?Œì´?´ë³¼ ë°œë™
        HitLightning = 0x00000400,			//?¼ì´?¸ë‹ ë°œë™
        HitDispel = 0x00000800,				//?„ë” ë°œë™
        HitColdArea = 0x00001000,			//ê´‘ì—­ ?‰ê¸° ë²”ìœ„ ì¦ê?%	
        HitFireArea = 0x00002000,			//ê´‘ì—­ ?”ì—¼ ë²”ìœ„ ì¦ê?%
        HitPoisonArea = 0x00004000,			//ê´‘ì—­ ??ë²”ìœ„ ì¦ê?%
        HitEnergyArea = 0x00008000,			//ê´‘ì—­ ?ë„ˆì§€ ë²”ìœ„ ì¦ê?%
        HitPhysicalArea = 0x00010000,		//ê´‘ì—­ ë¬¼ë¦¬ ë²”ìœ„ ì¦ê?%
        ...
        UseBestSkill = 0x00400000,			//ëª¨ë“  ?¼í•´%
        MageWeapon = 0x00800000,			//ëª¨ë“  ?ë„%
        DurabilityBonus = 0x01000000,
        BloodDrinker = 0x02000000,			//?¼ê²© ??ë¬¼ë¦¬ ì¹˜ëª… ?•ë¥  ê°ì†Œ
        BattleLust = 0x04000000,			//?¼ê²© ??ë¬¼ë¦¬ ì¹˜ëª… ?¼í•´ ê°ì†Œ
        HitCurse = 0x08000000,				//
        HitFatigue = 0x10000000,			//?¼ê²© ??ë§ˆë²• ì¹˜ëª… ?•ë¥  ê°ì†Œ
        HitManaDrain = 0x20000000,			//?¼ê²© ??ë§ˆë²• ì¹˜ëª… ?¼í•´ ê°ì†Œ
        SplinteringWeapon = 0x40000000,		//?œì‹± ë¬´ê¸° ?¤í‚¬ +1
    ...
    public enum ExtendedWeaponAttribute
    {
        ...
        HitSwarm        = 0x00000002, //???€??„±%
        HitSparks       = 0x00000004, //?¨ì • ?Œí”¼%
        Bane            = 0x00000008, //???€??„±
        MysticWeapon    = 0x00000010, //
        AssassinHoned   = 0x00000020, //?´ê·¸ë¡?ê°ì†Œ
        Focus           = 0x00000040, //ë¶•ë? ?¬ìš© ?????Œë³µ
        HitExplosion    = 0x00000080, //?°ì†Œ ?°ë?ì§€ ì¦ê?
        Freezing		= 0x00000100, //?™ìƒ ?°ë?ì§€ ì¦ê?
		InfectionBonus	= 0x00000200,  //?¸íŒ©???°ë?ì§€ ì¦ê?
		LightningBonus	= 0x00000400,  //?¼ì´?¸ë‹(4?¨í´, 7?¨í´) ?°ë?ì§€ ì¦ê?
		ChaosDamage		= 0x00000800,  //?¼ëˆ ?°ë?ì§€% ì¦ê?
		DirectDamage	= 0x00001000,  //? ì„± ?°ë?ì§€% ì¦ê?
		ChaosPlus		= 0x00002000,  //?¼ëˆ ?°ë?ì§€ ì¦ê?
		DirectPlus		= 0x00004000,  //? ì„± ?°ë?ì§€ ì¦ê?
		AggroPoint		= 0x00008000,  //?´ê·¸ë¡?
		AggroPointBonus	= 0x00010000,  //?´ê·¸ë¡?
		SPMAllBonus		= 0x00020000,  //ëª¨ë“  ?¹ìˆ˜ê¸?ì¦ê?
		SPMFirstBonus	= 0x00040000,  //?¹ìˆ˜ê¸?ì²«ë²ˆì§?ì¦ê?
		SPMSecondBonus	= 0x00080000,  //?¹ìˆ˜ê¸??ë²ˆì§?ì¦ê?
		SPMSwordBonus	= 0x00100000,  //ê²€ ?¹ìˆ˜ê¸?ì¦ê?
		SPMMaceBonus	= 0x00200000,  //?”ê¸° ?¹ìˆ˜ê¸?ì¦ê?
		SPMFancingBonus	= 0x00400000,  //?œì‹± ?¹ìˆ˜ê¸?ì¦ê?
		SPMBowBonus		= 0x00800000,  //???ê¶ ?¹ìˆ˜ê¸?ì¦ê?
		SPMWrestling	= 0x01000000,  //ë§¨ì† ?¹ìˆ˜ê¸?ì¦ê?
		BaseWeaponDamage= 0x02000000,  //ë¬´ê¸° ?¼í•´
		BaseSpellDamage	= 0x04000000,  //ë§ˆë²• ?¼í•´
		BaseAllDamage	= 0x08000000   //?„ì²´ ?¼í•´
    }
    ...
    public enum AosArmorAttribute
    {
        ...
        SoulCharge = 0x00000020,	//?Œë³µ??
		PierceResist = 0x00000040, //ê´€???€?? ¥
		ShockResist = 0x00000080, //ì¶©ê²© ?€?? ¥
		BleedResist = 0x00000100, //ì¶œí˜ˆ ?€?? ¥
		WeaponDefense = 0x00000200, //ë°©ì–´??
		MagicDefense = 0x00000400, //ë§ˆë²• ë°©ì–´??
		StunDefense = 0x00000800, //?¤í„´ ?œê°„ ê°ì†Œ
		ShieldRecovery = 0x00001000, //ë°©íŒ¨ ë°©ì–´ ?•ë¥ 
		AllDefenseBonus = 0x00002000, //?„ì²´ ?¼ê²© ?°ë?ì§€ ê°ì†Œ
		ElementalResist = 0x00004000, //?ì†Œ ?€?? ¥%
		AllResist = 0x00008000, //ëª¨ë“  ?€?? ¥%
		DefenseStam = 0x00010000, //ë°©ì–´??ê¸°ë ¥ ?Œëª¨ ê°ì†Œ
		MagicAllBonus = 0x00020000, //ëª¨ë“  ë§ˆë²• ?¤í‚¬ ì¦ê? 
		MagicOneCircleBonus = 0x00040000, //1?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicTwoCircleBonus = 0x00080000, //2?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicThreeCircleBonus = 0x00100000, //3?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicFourCircleBonus = 0x00200000, //4?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicFiveCircleBonus = 0x00400000, //5?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicSixCircleBonus = 0x00800000, //6?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicSevenCircleBonus = 0x01000000, //7?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicEightCircleBonus = 0x02000000, //8?¨í´ ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicNecromancyBonus = 0x04000000, //ê°•ë ¹??ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicElementalismBonus = 0x08000000, //?ì†Œ??ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicMysticismBonus = 0x10000000, //? ë¹„??ë§ˆë²• ?¤í‚¬ ì¦ê?
		MagicChivalryBonus = 0x20000000 //ê¸°ì‚¬??ë§ˆë²• ?¤í‚¬ ì¦ê?
        #endregion
    }
    ...
    {
        EaterFire = 0x00000001, //ë¶??¼í•´ ì¦ê?
        EaterCold = 0x00000002, //?‰ê¸° ?¼í•´ ì¦ê?
        EaterPoison = 0x00000004, //???¼í•´ ì¦ê?
        EaterEnergy = 0x00000008, //?ë„ˆì§€ ?¼í•´ ì¦ê?
        EaterKinetic = 0x00000010, //ì¶©ê²© ?¼í•´ ì¦ê?
        EaterDamage = 0x00000020, //ë¬¼ë¦¬ ?¼í•´ ì¦ê?
        ResonanceFire = 0x00000040, //ë¶??¼í•´ ì¦ê?%
        ResonanceCold = 0x00000080, //?‰ê¸° ?¼í•´ ì¦ê?%
        ResonancePoison = 0x00000100, //???¼í•´ ì¦ê?%
        ResonanceEnergy = 0x00000200, //?ë„ˆì§€ ?¼í•´ ì¦ê?%
        ResonanceKinetic = 0x00000400, //ì¶©ê²© ?¼í•´ ì¦ê?%
        /*Soul Charge is wrong. 
         * Do not use these types. 
         * Use AosArmorAttribute type only.
         * Fill these in with any new attributes.*/
        SoulChargeFire = 0x00000800, //?°ì†Œ ?€?? ¥%
        SoulChargeCold = 0x00001000, //?™ìƒ ?€?? ¥%
        SoulChargePoison = 0x00002000, //ë¶€???€?? ¥%
        SoulChargeEnergy = 0x00004000, //ê°ì „ ?€?? ¥%
        SoulChargeKinetic = 0x00008000, //?Œë³µ??ì¦ê?%
        CastingFocus = 0x00010000, //?œì „ ?¤íŒ¨ ?•ë¥  ê°ì†Œ
		EaterPierce = 0x00020000, //ê´€???¼í•´ ì¦ê?
        ResonancePierce = 0x00040000, //ê´€???¼í•´ ì¦ê?%
		EaterBleed = 0x00080000, //ì¶œí˜ˆ ?¼í•´ ì¦ê?
        ResonanceBleed = 0x00100000, //ì¶œí˜ˆ ?¼í•´ ì¦ê?%
		HumanoidDamage = 0x00200000, //?ì¥ë¥??¼í•´ ì¦ê?%
		UndeadDamage = 0x00400000, //?¸ë°???¼í•´ ì¦ê?%
		ElementalDamage = 0x00800000, //?•ë ¹ ?¼í•´ ì¦ê?%
		AbyssDamage = 0x01000000, //?…ë§ˆ ?¼í•´ ì¦ê?%
		ArachnidDamage = 0x02000000, //ê±°ë?ë¥??¼í•´ ì¦ê?%
		ReptilianDamage = 0x04000000, //?Œì¶©ë¥??¼í•´ ì¦ê?%
		FeyDamage = 0x08000000 //?”ì • ?¼í•´ ì¦ê?%
    }
    ...
    public sealed class SAAbsorptionAttributes : BaseAttributes
    {
        ...
        public static int GetValue(Mobile m, SAAbsorptionAttribute attribute)
        {
			...
			if( m is PlayerMobile )
			{
				...
				{
					// BaseWeapon.cs??GetDamageScalar ?ëŠ” ? ì‚¬??ë³´ë„ˆ???©ì‚° ë©”ì„œ??
