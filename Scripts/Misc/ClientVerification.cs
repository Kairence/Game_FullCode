...
namespace Server.Misc
{
	public class ClientVerification
	{
		...
		public static bool AllowEC = Config.Get("Client.AllowEC", false);
		...
		{
			get { return m_RequiredEC; }
			set { m_RequiredEC = value; }
		}
		...
		public static void Initialize()
		{
			...
			if (Required != null)
			{
				...
				Console.WriteLine(
					"Restricting classic client version to {0}. Action to be taken: {1}",
					Required,
					m_OldClientResponse
				);
				Utility.PopColor();
			...
			if (RequiredEC != null)
			{
				Utility.PushColor(ConsoleColor.White);
				Console.WriteLine(
					"Restricting enhanced client version to {0}. Action to be taken: {1}",
					RequiredEC,
					"Kick"
				);
				Utility.PopColor();
					...
					if (required != null && version < required)
					{
						Timer.DelayCall(
							...
							() =>
							{
								...
								{
									state.Mobile.SendMessage(
										"This server requires your enhanced client version be at least {0}. You will be disconnected in 5 seconds.",
										required
									);
									...
									Timer.DelayCall(
										...
										() =>
										{
											Utility.PushColor(ConsoleColor.DarkRed);
											Console.WriteLine(
												"Client: {0}: Disconnecting, bad enhanced client version.",
												state
											);
											Utility.PopColor();
