...
namespace Server
{
	...
	public enum BuffIcon : short
	{
		...
		Honored,
		ManaPhase,
		FanDancerFanFire, //디버프
		Rage,
		Webbing, //디버프
		MedusaStone, //디버프
		TrueFear, //Dragon Slasher Fear
		AuraOfNausea, //디버프
		HowlOfCacophony, //디버프
		GazeDespair, //
		HiryuPhysicalResistance, //디버프
		RuneBeetleCorruption,
		BloodwormAnemia,
		RotwormBloodDisease,
		SkillUseDelay,
		FactionStatLoss, //스텟 하락
		HeatOfBattleStatus, //디버프
		CriminalStatus,
		ArmorPierce,
		SplinteringEffect,
		SwingSpeedDebuff,
		...
		UnknownStar, //스펠 마스터리
		FeintDebuff,
		CaddelliteInfused, //사용 안함
		PotionGloriousFortune, //포션
	...
	public sealed class AddBuffPacket : Packet
	{
		public AddBuffPacket(Mobile m, BuffInfo info)
			: this(
				...
				info.Args,
				info.NoTimer ? TimeSpan.Zero
					: (info.TimeStart != DateTime.MinValue) ? ((info.TimeStart + info.TimeLength) - DateTime.UtcNow)
					: TimeSpan.Zero
			) { }
