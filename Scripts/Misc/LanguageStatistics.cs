	...
	**/
	public class LanguageStatistics
	{
		...
		struct InternationalCode
		{
			...
			{
				get { return this.m_Language_LocalName; }
			}
			public string Country_LocalName
			{
				get { return this.m_Country_LocalName; }
			}
			...
			public string GetName()
			{
				...
				if (this.m_HasLocalInfo)
				{
					s = String.Format(
						"{0}‎ - {1}",
						DefaultLocalNames ? this.m_Language_LocalName : this.m_Language,
						DefaultLocalNames ? this.m_Country_LocalName : this.m_Country
					);

					if (ShowAlternatives)
						s += String.Format(
							"‎ 【{0}‎ - {1}‎】",
							DefaultLocalNames ? this.m_Language : this.m_Language_LocalName,
							DefaultLocalNames ? this.m_Country : this.m_Country_LocalName
						);
				}
				else
				{
