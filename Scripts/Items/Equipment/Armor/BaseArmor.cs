...
namespace Server.Items
{
			...
			IEquipOption
	{
		...
		*/
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedPhysical
		{
			get { return m_RefinedPhysical; }
			set
			{
				m_RefinedPhysical = value;
				InvalidateProperties();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedFire
		{
			get { return m_RefinedFire; }
			set
			{
				m_RefinedFire = value;
				InvalidateProperties();
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedCold
		{
			get { return m_RefinedCold; }
			set
			{
				m_RefinedCold = value;
				InvalidateProperties();
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedPoison
		{
			get { return m_RefinedPoison; }
			set
			{
				m_RefinedPoison = value;
				InvalidateProperties();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedEnergy
		{
			get { return m_RefinedEnergy; }
			set
			{
				m_RefinedEnergy = value;
				InvalidateProperties();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int RefinedDefenseChance
		{
			get { return -(m_RefinedPhysical + m_RefinedFire + m_RefinedCold + m_RefinedPoison + m_RefinedEnergy); }
		}
		...
		{
			return from
				.Items.OfType<BaseArmor>()
				.Any(armor =>
					armor.m_RefinedPhysical > 0
		...
		public override void AddResistanceProperties(ObjectPropertyList list)
		{
			if (PhysicalResistance != 0 || m_RefinedPhysical != 0)
			{
				if (m_RefinedPhysical != 0)
					list.Add(
						1153735,
						String.Format("{0}\t{1}\t{2}", PhysicalResistance.ToString(), "", m_RefinedPhysical.ToString())
					); // physical resist ~1_val~% / ~2_symb~~3_val~% Max
				else
			...
			if (FireResistance != 0 || m_RefinedFire != 0)
			{
				if (m_RefinedFire != 0)
					list.Add(
						1153737,
						String.Format("{0}\t{1}\t{2}", FireResistance.ToString(), "", m_RefinedFire.ToString())
					); // physical resist ~1_val~% / ~2_symb~~3_val~% Max
				else
			...
			if (ColdResistance != 0 || m_RefinedCold != 0)
			{
				if (m_RefinedCold != 0)
					list.Add(
						1153739,
						String.Format("{0}\t{1}\t{2}", ColdResistance.ToString(), "", m_RefinedCold.ToString())
					); // physical resist ~1_val~% / ~2_symb~~3_val~% Max
				else
			...
			if (PoisonResistance != 0 || m_RefinedPoison != 0)
			{
				if (m_RefinedPoison != 0)
					list.Add(
						1153736,
						String.Format("{0}\t{1}\t{2}", PoisonResistance.ToString(), "", m_RefinedPoison.ToString())
					); // physical resist ~1_val~% / ~2_symb~~3_val~% Max
				else
			...
			if (EnergyResistance != 0 || m_RefinedEnergy != 0)
			{
				if (m_RefinedEnergy != 0)
					list.Add(
						1153738,
						String.Format("{0}\t{1}\t{2}", EnergyResistance.ToString(), "", m_RefinedEnergy.ToString())
					); // physical resist ~1_val~% / ~2_symb~~3_val~% Max
				else
		...
		public static int GetInherentLowerManaCost(Mobile from)
		{
			...
			foreach (BaseArmor armor in from.Items.OfType<BaseArmor>())
			{
				if (
					...
					|| armor.MaterialType == ArmorMaterialType.Wood
					|| armor is BaseShield
				)
					continue;
		...
		{
			get { return m_StrReq == -1 ? AosStrReq : 1000; }
			set
			{
				m_StrReq = value;
				InvalidateProperties();
		...
		{
			get { return m_DexReq == -1 ? AosDexReq : 1000; }
			set
			{
				m_DexReq = value;
				InvalidateProperties();
		...
		{
			get { return m_IntReq == -1 ? AosIntReq : 1000; }
			set
			{
				m_IntReq = value;
				InvalidateProperties();
		...
		*/
		...
		public override int FireResistance
		{
			get
			{
				return BaseFireResistance
					...
					+ m_AosArmorAttributes.ElementalResist / 100
					+ m_AosArmorAttributes.AllResist / 10000;
			}
		}
		...
		public override int EnergyResistance
		{
			get
			{
				return BaseEnergyResistance
					...
					+ m_AosArmorAttributes.ElementalResist / 1000
					+ m_AosArmorAttributes.AllResist / 10000;
			}
		}
				...
				if (PrefixOption[0] == 100)
				{
					...
					for (int i = 0; i < 5; ++i)
					{
						...
						list.Add(
							...
							(((double)SuffixOption[i + 31]) * Misc.Util.PercentCalc(PrefixOption[i + 31])).ToString()
						);
					}
				}
					...
					for (int i = 0; i < 7; ++i)
					{
						if (PrefixOption[3 + i] > 0)
						{
							list.Add(
								...
								"{0}\t{1}",
								PrefixOption[i + 3],
								(((double)SuffixOption[i + 3]) * 0.01).ToString()
							);
						}
					}
				}
		...
		public int SetResistBonus(ResistanceType resist)
		{
			if (SetHelper.ResistsBonusPerPiece(this))
			{
				...
				{
					case ResistanceType.Physical:
						return m_SetEquipped ? PhysicalResistance + m_SetPhysicalBonus : PhysicalResistance;
					case ResistanceType.Fire:
						return m_SetEquipped ? FireResistance + m_SetFireBonus : FireResistance;
					case ResistanceType.Cold:
						return m_SetEquipped ? ColdResistance + m_SetColdBonus : ColdResistance;
					case ResistanceType.Poison:
						return m_SetEquipped ? PoisonResistance + m_SetPoisonBonus : PoisonResistance;
					case ResistanceType.Energy:
						return m_SetEquipped ? EnergyResistance + m_SetEnergyBonus : EnergyResistance;
				}
			}
			else
			{
				switch (resist)
				{
					case ResistanceType.Physical:
						return m_SetEquipped
							? LastEquipped
								? (PhysicalResistance * Pieces) + m_SetPhysicalBonus
								: 0
							: PhysicalResistance;
					case ResistanceType.Fire:
						return m_SetEquipped
							? LastEquipped
								? (FireResistance * Pieces) + m_SetFireBonus
								: 0
							: FireResistance;
					case ResistanceType.Cold:
						return m_SetEquipped
							? LastEquipped
								? (ColdResistance * Pieces) + m_SetColdBonus
								: 0
							: ColdResistance;
					case ResistanceType.Poison:
						return m_SetEquipped
							? LastEquipped
								? (PoisonResistance * Pieces) + m_SetPoisonBonus
								: 0
							: PoisonResistance;
					case ResistanceType.Energy:
						return m_SetEquipped
							? LastEquipped
								? (EnergyResistance * Pieces) + m_SetEnergyBonus
								: 0
							: EnergyResistance;
				}
			}
