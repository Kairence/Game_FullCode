...
namespace Server.Items
{
	public class MythicCharacterToken : Item, IPromotionalToken
	{
		...
		public TextDefinition ItemName
		{
			get { return 1152353; }
		...
		{
			get { return typeof(MythicCharacterToken.InternalGump); }
		}
		...
		public class InternalGump : BaseGump
		{
			...
			{
				get { return C32216(0x32CD32); }
			}
			public static int LightGreen
			{
				get { return C32216(0x90EE90); }
			}
			public static int Yellow
			{
				get { return C32216(0xFFE4C4); }
			}
			public static int Beige
			{
				get { return C32216(0xF5F5DC); }
			}
			public static int Gray
			{
				get { return C32216(0x696969); }
			}
			...
			public override void AddGumpLayout()
			{
				...
				else
				{
					...
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Magic),
						Width / 3,
						65,
						SkillCat.Magic,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Magic]
					);
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Bard),
						Width / 3,
						345,
						SkillCat.Bard,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Bard]
					);
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Combat),
						(Width / 3) * 2,
						65,
						SkillCat.Combat,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Combat]
					);
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Wilderness),
						(Width / 3) * 2,
						305,
						SkillCat.Wilderness,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Wilderness]
					);
					...
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.TradeSkills),
						Width / 3,
						65,
						SkillCat.TradeSkills,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.TradeSkills]
					);
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Miscellaneous),
						Width / 3,
						285,
						SkillCat.Miscellaneous,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Miscellaneous]
					);
					BuildSkillCategory(
						BaseSpecialScrollBook.GetCategoryLocalization(SkillCat.Thievery),
						(Width / 3) * 2,
						150,
						SkillCat.Thievery,
						ScrollOfAlacrityBook._SkillInfo[SkillCat.Thievery]
					);
				}
			}
			...
			private void BuildSkillCategory(int titleLoc, int x, int y, SkillCat cat, List<SkillName> skills)
			{
				...
				for (int i = 0; i < skills.Count; i++)
				{
					...
					AddHtmlLocalized(
						...
						Width / 3,
						20,
						User.Skills[skills[i]].Info.Localization,
						hue,
						false,
						false
					);
					...
					case 2501: // Continue
						...
						else
						{
							Effects.SendLocationParticles(
								EffectItem.Create(User.Location, User.Map, EffectItem.DefaultDuration),
								0,
								0,
								0,
								0,
							...
							Effects.SendMovingParticles(
								new Entity(
									Server.Serial.Zero,
									new Point3D(User.X - 6, User.Y - 6, User.Z + 15),
									User.Map
								),
								User,
								0x36D4,
							...
							Effects.SendMovingParticles(
								new Entity(
									Server.Serial.Zero,
									new Point3D(User.X - 4, User.Y - 6, User.Z + 15),
									User.Map
								),
								User,
								0x36D4,
							...
							Effects.SendMovingParticles(
								new Entity(
									Server.Serial.Zero,
									new Point3D(User.X - 6, User.Y - 4, User.Z + 15),
									User.Map
								),
								User,
								0x36D4,
