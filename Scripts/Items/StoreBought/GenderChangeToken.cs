...
namespace Server.Items
{
	public class GenderChangeToken : Item, IPromotionalToken
	{
		...
		{
			get { return typeof(GenderChangeConfirmGump); }
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (!IsChildOf(from.Backpack))
			{
				...
				from.IsBodyMod
				|| from.HueMod > 0
				|| !from.CanBeginAction(typeof(Server.Spells.Fifth.IncognitoSpell))
			)
			{
		...
		public void OnChangeHairstyle(Mobile from, bool facialHair, int itemID)
		{
			...
			else
			{
				...
				{
					from.SendGump(
						new ChangeHairstyleGump(
							!from.Female,
							from,
							null,
							0,
							true,
							from.Race == Race.Gargoyle
								? ChangeHairstyleEntry.BeardEntriesGargoyle
								: ChangeHairstyleEntry.BeardEntries,
							this
						)
					);
				}
		...
		public override void OnResponse(RelayInfo info)
		{
			if (info.ButtonID == 1 && Token.IsChildOf(User.Backpack))
			{
				if (
					User.IsBodyMod
					|| User.HueMod > 0
					|| !User.CanBeginAction(typeof(Server.Spells.Fifth.IncognitoSpell))
				)
				{
				...
				{
					User.SendGump(
						new ChangeHairstyleGump(!User.Female, User, null, 0, false, GetHairstyleEntries(User), Token)
					);
				}
			}
		}
