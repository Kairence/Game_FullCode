...
namespace Server.Items
{
	public class WoodworkersBench : BaseAddon
	{
		...
		{
			get { return new WoodworkersBenchDeed(); }
		}
		...
		public override void OnComponentUsed(AddonComponent c, Mobile m)
		{
			...
			else if (HasBonus(m))
			{
				...
				m.SendLocalizedMessage(
					1071522,
					String.Format("{0}\t{1}", tsRem.Minutes.ToString(), tsRem.Seconds.ToString())
				); // You already have the bench's buff. Time remaining of the buff: ~1_VAL~ min ~2_VAL~ sec
			}
			else
			{
	...
	public class WoodworkersBenchDeed : BaseAddonDeed, IRewardOption
	{
		...
		{
			get { return new WoodworkersBench(m_East); }
		}
