...
namespace Server.Items
{
	public class UndertakersStaff : GnarledStaff
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool SummonAll
		{
			get { return _SummonAll; }
			set
			{
				_SummonAll = value;
				InvalidateProperties();
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> entries)
		{
			...
			{
				var entry1 = new SimpleContextMenuEntry(
					from,
					1071507,
					m =>
					{
				...
				var entry2 = new SimpleContextMenuEntry(
					from,
					1071508,
					m =>
					{
		...
		public void TryEndSummon(Mobile m, List<Corpse> corpses)
		{
			...
			if (_SummonAll)
			{
				...
				foreach (var c in copy)
				{
					...
					if (
						c.Killer is PlayerMobile
						&& c.Killer != m
						&& c.TimeOfDeath + TimeSpan.FromSeconds(180) > DateTime.UtcNow
					)
					{
						remove = true;
						notEnoughTime = true;
