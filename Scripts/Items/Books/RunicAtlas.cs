...
namespace Server.Items
{
	...
	public class RunicAtlas : Runebook
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (
				...
				&& (from.InRange(GetWorldLocation(), 2) || from.AccessLevel >= AccessLevel.Counselor)
			)
			{
		...
		public override int OnCraft(
			...
			bool makersMark,
			Mobile from,
			Engines.Craft.CraftSystem craftSystem,
			Type typeRes,
			ITool tool,
			Engines.Craft.CraftItem craftItem,
			int resHue
		)
		{
			if (makersMark)
			...
			{
				MaxCharges = 10000; //Utility.RandomList(80, 90, 100);
			}
			else
			{
				MaxCharges = 8000;
		...
		public RunicAtlas(Serial serial)
			: base(serial) { }
	...
	public class RunicAtlasGump : BaseGump
	{
		...
		public int Selected
		{
			get { return Atlas == null ? -1 : Atlas.Selected; }
		}
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				182,
				18,
				string.Format("<center>{0}</center>", entry != null ? entry.Description : "Empty"),
				false,
				false
			);
