	...
	public class SpecialScrollBookGump : BaseGump
	{
		...
		public virtual void BuildCategoriesPage()
		{
			...
			foreach (var kvp in Book.SkillInfo)
			{
				AddHtmlLocalized(
					...
					100,
					20,
					BaseSpecialScrollBook.GetCategoryLocalization(kvp.Key),
					false,
					false
				);
		...
		public virtual void BuildSkillsPage()
		{
			AddHtmlLocalized(
				...
				20,
				CenterLoc,
				String.Format("#{0}", BaseSpecialScrollBook.GetCategoryLocalization(Category)),
				0,
				false,
				false
			); // Power Scrolls
		...
		public virtual void BuildSkillPage()
		{
			AddHtmlLocalized(
				...
				20,
				CenterLoc,
				String.Format("#{0}", SkillInfo.Table[(int)Skill].Localization),
				0,
				false,
				false
			);
		...
		{
			return Book.Items.FirstOrDefault(i => i is SpecialScroll && skills.Contains(((SpecialScroll)i).Skill))
				!= null;
		}
