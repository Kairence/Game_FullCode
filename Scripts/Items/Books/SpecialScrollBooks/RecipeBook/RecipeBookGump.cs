	...
	public class RecipeBookGump : Gump
	{
		...
		public bool CheckFilter(RecipeScrollDefinition recipe)
		{
			...
			else if (f.Skill == 3 && recipe.Skill != RecipeSkillName.Fletching)
			{
				...
				f.Skill == 4
				&& recipe.Skill != RecipeSkillName.Carpentry
				&& recipe.Skill != RecipeSkillName.Masonry
			)
			{
				return false;
			}
		...
		private class SetPricePrompt : Prompt
		{
			...
			public SetPricePrompt(
				RecipeBook book,
				RecipeScrollDefinition recipe,
				int page,
				List<RecipeScrollDefinition> list
			)
			{
				m_Book = book;
				m_Recipe = recipe;
			...
			: base(12, 24)
		{
			...
			for (int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i)
			{
				...
				AddHtmlLocalized(
					...
					130,
					32,
					Recipe.Recipes[recipe.RecipeID].TextDefinition.Number,
					"#103221",
					0xFFFFFF,
					false,
					false
