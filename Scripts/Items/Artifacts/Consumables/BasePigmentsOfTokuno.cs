...
namespace Server.Items
{
	public abstract class BasePigmentsOfTokuno : Item, IUsesRemaining
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (IsAccessibleTo(from) && from.InRange(GetWorldLocation(), 3))
			{
				...
				from.BeginTarget(
					...
					new TargetStateCallback(InternalCallback),
					this
				);
			}
			else
		...
		private void InternalCallback(Mobile from, object targeted, object state)
		{
			...
			if (
				pigment.Deleted
				|| pigment.UsesRemaining <= 0
				|| !from.InRange(pigment.GetWorldLocation(), 3)
			...
			if (i == null)
				...
				i is MetalPigmentsOfTokuno
				|| i is LesserPigmentsOfTokuno
				|| i is PigmentsOfTokuno
				|| i is CompassionPigment
			)
		...
		public static bool IsValidItem(Item i)
		{
			...
			if (
				i is BaseAddonDeed
				&& ((BaseAddonDeed)i).UseCraftResource
				&& !((BaseAddonDeed)i).IsReDeed
				&& ((BaseAddonDeed)i).Resource != CraftResource.None
			)
				return true;
