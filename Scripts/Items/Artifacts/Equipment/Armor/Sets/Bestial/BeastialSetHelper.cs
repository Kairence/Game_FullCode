...
namespace Server
{
	public class BestialSetHelper
	{
		...
		public static void OnDamage(Mobile victim, Mobile attacker, ref int damage)
		{
			...
			if ( /*victim != attacker && */
				equipped > 0
				&& victim.Hits - damage < (victim.HitsMax / 2)
			)
			{
		...
		public static void OnAdded(Mobile m, Item item)
		{
			if (
				_Table != null
				&& _Table.ContainsKey(m)
				&& _Table[m].Running
				&& item is ISetItem
				&& ((ISetItem)item).SetID == SetItem.Bestial
			)
			{
		...
		{
			foreach (
				Item i in m.Items.Where(item =>
					item is ISetItem && ((ISetItem)item).SetID == SetItem.Bestial && item.Hue != hue
				)
			)
			{
				i.Hue = hue;
		...
		public class BerserkTimer : Timer
		{
			...
			public int Level
			{
				get { return Math.Min(5, Math.Max(1, _DamageTaken / 50)); }
			}

			public BerserkTimer(Mobile m)
