...
namespace Server
{
	...
	public static class SetHelper
	{
		public static void GetSetProperties(ObjectPropertyList list, ISetItem setItem)
		{
			...
			if (setItem.SetSkillBonuses.Skill_1_Value != 0)
				list.Add(
					1072502,
					"{0}\t{1}",
					"#" + (1044060 + (int)setItem.SetSkillBonuses.Skill_1_Name),
					setItem.SetSkillBonuses.Skill_1_Value
				); // ~1_skill~ ~2_val~ (total)
		}
		...
		{
			if (
				item.SetPhysicalBonus == 0
				&& item.SetFireBonus == 0
				&& item.SetColdBonus == 0
				&& item.SetPoisonBonus == 0
				&& item.SetEnergyBonus == 0
			)
				return true;
		...
		public static int GetSetTotalResist(Mobile m, ResistanceType resist)
		{
			...
			foreach (
				Item item in m.Items.Where(i => i is ISetItem && ((ISetItem)i).IsSetItem && ((ISetItem)i).SetEquipped)
			)
			{
