...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
		...
		{
			get { return GetFlag(CorpseFlag.LootCriminal); }
			set { SetFlag(CorpseFlag.LootCriminal, value); }
		}
		...
		{
			get { return GetFlag(CorpseFlag.Murderer); }
		...
		public virtual void Open(Mobile from, bool checkSelfLoot)
		{
			if (from.IsStaff() || from.InRange(GetWorldLocation(), 2))
			{
				...
				if (player != null)
				{
					...
					{
						GetDaemonBoneObjective obj =
							qs.FindObjective(typeof(GetDaemonBoneObjective)) as GetDaemonBoneObjective;

						if (
							obj != null
							&& obj.CorpseWithBone == this
					...
					{
						VanquishDaemonObjective obj =
							qs.FindObjective(typeof(VanquishDaemonObjective)) as VanquishDaemonObjective;
		...
		public override void OnAosSingleClick(Mobile from)
		{
			...
			{
				from.Send(
					new MessageLocalized(Serial, ItemID, MessageType.Label, hue, 3, opl.Header, Name, opl.HeaderArgs)
				);
			}
		}
