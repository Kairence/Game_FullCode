...
namespace Server.Items
{
	...
	public class WallSafeDeed : Item
	{
							...
							if (house != null && BaseHouse.FindHouseAt(m) == house && house.IsCoOwner(m))
							{
								...
								if (northWall && westWall)
								{
									switch (from.Direction & Direction.Mask)
									{
										...
										default:
											from.SendMessage("Turn to face the wall on which to place the safe.");
											return;
									}
								}
		...
		public WallSafeDeed(Serial serial)
			: base(serial) { }
	...
	public class WallSafeGump : Gump
	{
		...
		public void AddGumpLayout()
		{
			...
			AddHtmlLocalized(
				...
				16,
				1155859,
				Safe.HoldAmount.ToString("N0", CultureInfo.GetCultureInfo("en-US")),
				0xFFFF,
				false,
				false
			); // Gold Deposited: ~1_AMOUNT~
			AddHtmlLocalized(
				20,
				65,
				380,
				16,
				1155864,
				String.Format("{0}\t{1}", User.Name, secureAmount.ToString("N0", CultureInfo.GetCultureInfo("en-US"))),
				0xFFFF,
				false,
				false
			); // ~1_NAME~'s Secure Account: ~2_AMOUNT~
							...
							if (text != null && !String.IsNullOrEmpty(text))
							{
								...
								else if (acct != null)
								{
									...
									if (v > left)
									{
										...
										Safe.AddHistory(
											String.Format(
												"<basefont color=green>{0} +{1}",
												User.Name,
												left.ToString("N0", CultureInfo.GetCultureInfo("en-US"))
											)
										);
									}
									else
									{
										...
										Safe.AddHistory(
											String.Format(
												"<basefont color=green>{0} +{1}",
												User.Name,
												v.ToString("N0", CultureInfo.GetCultureInfo("en-US"))
											)
										);
									}
