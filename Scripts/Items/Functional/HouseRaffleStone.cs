...
namespace Server.Items
{
	...
	public class HouseRaffleStone : Item
	{
		...
		{
			get { return this.m_Deed; }
			set { this.m_Deed = value; }
		}
		...
		{
			get { return this.m_ExpireAction; }
			set { this.m_ExpireAction = value; }
		}

		[CommandProperty(AccessLevel.GameMaster, AccessLevel.Seer)]
		public int TicketPrice
		{
			get { return this.m_TicketPrice; }
			set
			{
		...
		{
			return (
				this.m_Bounds.Start != Point2D.Zero
				&& this.m_Bounds.End != Point2D.Zero
				&& this.m_Facet != null
				&& this.m_Facet != Map.Internal
			);
		}
			...
			switch (this.m_State)
			{
				...
				case HouseRaffleState.Completed:
				{
					this.LabelTo(
						from,
						1060658,
						String.Format("Winner\t{0}", (this.m_Winner == null) ? "Unknown" : this.m_Winner.Name)
					); // ~1_val~: ~2_val~
					break;
				}
			}
		...
		public void Purchase_Callback(Mobile from, bool okay, object state)
		{
			if (
				...
				|| !from.CheckAlive()
				|| this.HasEntered(from)
				|| this.IsAtIPLimit(from)
			)
				return;
			...
			if (okay)
			{
				...
				if (
					...
					|| (from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Gold), this.m_TicketPrice))
					|| Mobiles.Banker.Withdraw(from, m_TicketPrice, true)
				)
				{
