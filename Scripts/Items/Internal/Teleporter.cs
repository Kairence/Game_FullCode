...
namespace Server.Items
{
	public class Teleporter : Item
	{
		...
		public virtual bool CanTeleport(Mobile m)
		{
			...
			if (
				(m_CombatCheck || m.Region is DungeonRegion || m.Region.Name == "Ancient Lair")
				&& SpellHelper.CheckCombat(m)
			)
			{
	...
	public class ConditionTeleporter : Teleporter
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int ClilocNumber { get; set; }
		...
		public override bool CanTeleport(Mobile m)
		{
			...
			if (
				GetFlag(ConditionFlag.DenyFollowers)
			...
			if (pack != null)
			{
				...
				if (
					GetFlag(ConditionFlag.DenyPackEthereals)
					&& (
	...
	public class ClickTeleporter : Teleporter
	{
		...
		{
			if (!from.InRange(Location, Range) || !from.InLOS(this) || !from.CanSee(this))
			{
