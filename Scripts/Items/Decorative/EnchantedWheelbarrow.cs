...
namespace Server.Items
{
	public class EnchantedWheelbarrow : Item, IFlipable, ISecurable
	{
		...
		{
			typeof(DecorativePlant),
			typeof(DecorativePlantWhiteFlowers),
			typeof(DecorativePlantVines),
			typeof(DecorativePlantFlax),
			typeof(DecorativePlantPoppies),
			typeof(DecorativePlantLilypad),
		};
		...
		public override void OnDoubleClick(Mobile m)
		{
			...
			else
			{
				if (CheckAccessible(m, this))
				{
					...
					{
						Item i =
							Activator.CreateInstance(DecorativePlants[Utility.Random(DecorativePlants.Length)]) as Item;
