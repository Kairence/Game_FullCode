	...
	public abstract class BaseLight : Item, ICraftable, IResource, IQuality, ISecurable
	{
		...
		public CraftResource Resource
		{
			...
			set
			{
				_Resource = value;
				_Resource = value;
				Hue = CraftResources.GetHue(this._Resource);
		...
		{
			get { return this.m_BurntOut; }
			set { this.m_BurntOut = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public bool Protected
		{
			get { return this.m_Protected; }
			set { this.m_Protected = value; }
		}
		...
		public override void AddNameProperty(ObjectPropertyList list)
		{
			if (_Resource > CraftResource.Iron)
			{
				list.Add(
					1053099,
					"#{0}\t{1}",
					CraftResources.GetLocalizationNumber(_Resource),
					String.Format("#{0}", LabelNumber.ToString())
