...
namespace Server.Items
{
	...
	public class GorgonLense : Item, ICraftable
	{
		...
		public LenseType LenseType
		{
			get { return m_LenseType; }
			set
			{
				m_LenseType = value;
		...
		{
			return dropped is GorgonLense
				&& ((GorgonLense)dropped).LenseType == m_LenseType
		...
		public void OnTarget(Mobile from, object targeted)
		{
			if (targeted is Item)
			{
				...
				else if (targeted is BaseArmor)
				{
					...
					if (
						armor.Layer == Layer.Neck
						|| armor.Layer == Layer.Helm
						|| armor is BaseShield
						|| (armor.RequiredRace == Race.Gargoyle && armor.Layer == Layer.Earrings)
					)
					{
		...
		private class InternalTarget : Server.Targeting.Target
		{
			...
			public InternalTarget(GorgonLense lense)
		...
		)
		{
			...
			{
				default:
					m_LenseType = LenseType.Limited;
					break;
				case 2223:
					m_LenseType = LenseType.Regular;
					break;
				case 1266:
					m_LenseType = LenseType.Enhanced;
					break;
			}
	...
	public class GorgonLenseWarningGump : BaseConfirmGump
	{
		...
		public override void Confirm(Mobile from)
		{
			if (!m_Lense.IsChildOf(from.Backpack) || !m_Item.IsChildOf(from.Backpack))
			{
				...
				m_Item is BaseShield
				|| m_Item.Layer == Layer.Neck
				|| m_Item.Layer == Layer.Earrings
				|| m_Item.Layer == Layer.Helm
			)
			{
