...
namespace Server.Items
{
	public abstract class BaseRewardTitleToken : Item
	{
		...
		public BaseRewardTitleToken(int id)
			: base(id)
		{
		...
		private class InternalGump : Gump
		{
			...
			public InternalGump(PlayerMobile pm, BaseRewardTitleToken token)
				: base(50, 50)
			{
				Token = token;
				User = pm;
				...
				Token.Titles.ForEach(tuple =>
				{
					...
					if (textdef.Number > 0)
						...
						AddHtml(
							...
							240,
							20,
							String.Format("<basefond color=#FFFFFF>{0}", textdef.String),
							false,
							false
						);
		...
		public BaseRewardTitleToken(Serial serial)
			: base(serial) { }
