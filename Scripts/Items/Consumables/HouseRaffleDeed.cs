...
namespace Server.Items
{
	public class HouseRaffleDeed : Item
	{
		...
		{
			get { return this.m_PlotLocation; }
			set
			{
		...
		{
			get { return (this.m_Stone == null || this.m_Stone.Deleted || this.m_Stone.IsExpired); }
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (this.ValidLocation())
			{
				list.Add(
					1060658,
					"location\t{0}",
					HouseRaffleStone.FormatLocation(this.m_PlotLocation, this.m_Facet, false)
				); // ~1_val~: ~2_val~
					...
					int daysLeft = (int)
						Math.Ceiling(
							(
								deed.Stone.Started
								+ deed.Stone.Duration
								+ HouseRaffleStone.ExpirationTime
								- DateTime.UtcNow
							).TotalDays
						);
