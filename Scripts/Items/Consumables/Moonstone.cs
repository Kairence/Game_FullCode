...
namespace Server.Items
{
	...
	public class Moonstone : Item
	{
			...
			: base(0xF8B)
		{
			Weight = 1.0;
			m_Type = type;
			if (m_Type == MoonstoneType.Despise)
				Name = "데스파이즈 히든 아이템";
		}
		...
		*/
		...
		public void OnTarget(Mobile from, object obj)
		{
			...
			{
				Item item = (Item)obj;
				if (item is UserMoongate)
				{
					UserMoongate mg = item as UserMoongate;
					mg.Target = new Point3D(5412, 859, 45);
					mg.TargetMap = Map.Trammel;
					Delete();
					return;
				}
				else
				{
					from.SendMessage("이 아이템은 여기에 사용하는 것이 아닌 것 같습니다...");
				}
			}
		}
		...
		private class SettleTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else if (m_Count >= 10)
				{
					...
					{
						if (
							!Region.Find(m_Location, m_Map).IsDefault || !Region.Find(m_Location, m_TargetMap).IsDefault
						)
						{
