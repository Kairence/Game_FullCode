...
namespace Server.Items
{
	public class EndlessDecanter : Pitcher
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Point3D LinkLocation
		{
			get { return m_LinkLocation; }
			set { m_LinkLocation = value; }
		}
		...
		public EndlessDecanter(Serial serial)
			: base(serial) { }
		...
		private class LinkEntry : ContextMenuEntry
		{
			...
			{
				if (
					this.m_Decanter.Deleted
					|| !this.m_Decanter.Movable
					|| !this.m_From.CheckAlive()
					|| !this.m_Decanter.CheckItemUse(this.m_From)
				)
					return;
