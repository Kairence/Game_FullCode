...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		public static double LootChance = Config.Get("TreasureMaps.LootChance", .00);
		...
		private static Rectangle2D[] m_FelTramWrap = new Rectangle2D[] { new Rectangle2D(0, 0, 5119, 4095) };
		...
		public static Point2D[] Locations
		{
			get { return m_Locations; }
		}
		public static Point2D[] HavenLocations
		{
			get { return m_Locations; }
		}
		...
		public static bool ValidateLocation(int x, int y, Map map)
		{
			...
			{
				if (
					reg.IsPartOf<TownRegion>()
					|| reg.IsPartOf<DungeonRegion>()
			...
			else
			{
				if (m_Level == 6)
				{
					LabelTo(
						from,
						1041522,
						String.Format("#{0}\t \t#{1}", 1063452, Facet == Map.Felucca ? 1041502 : 1041503)
					);
				}
				else
				{
					LabelTo(
						from,
						1041522,
						String.Format("#{0}\t \t#{1}", 1041510 + m_Level, Facet == Map.Felucca ? 1041502 : 1041503)
					);
				}
			}
		}
