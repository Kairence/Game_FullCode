			...
			*/
				...
				: base(50, 50)
			{
				...
				for (int i = 1; i < RacialData.Length; i++)
				{
					this.AddHtmlLocalized(
						this.LayoutArray[i][2],
						this.LayoutArray[i][3],
						(i == 1) ? 125 : 80,
						(i == 1) ? 70 : 35,
						(this.m_From.Female) ? RacialData[i][0] : RacialData[i][1],
						false,
						false
					);
					if (this.LayoutArray[i][4] != 0)
					{
						...
						this.AddImage(
							this.LayoutArray[i][4],
							this.LayoutArray[i][5],
							(this.m_From.Female) ? RacialData[i][4] : RacialData[i][5]
						);
					}
					this.AddButton(
						this.LayoutArray[i][6],
						this.LayoutArray[i][7],
						0xFA5,
						0xFA7,
						i,
			...
			public override void OnResponse(NetState sender, RelayInfo info)
			{
				...
				if (this.m_From is PlayerMobile)
				{
					...
					this.m_From.HairItemID =
						(this.m_From.Female) ? RacialData[info.ButtonID][2] : RacialData[info.ButtonID][3];
					this.m_Deed.Delete();
				}
			}
		}
