		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is Item)
				{
					...
					{
						if (
							((BaseWeapon)item).Attributes.Brittle > 0
					...
					{
						if (
							((BaseArmor)item).Attributes.Brittle > 0
					...
					{
						if (
							((BaseClothing)item).Attributes.Brittle > 0
					...
					{
						if (
							((BaseJewel)item).Attributes.Brittle > 0
					...
					if (targeted is IDurability)
					{
						...
						if (
							(item.IsChildOf(from.Backpack) || (Core.ML && item.Parent == from))
