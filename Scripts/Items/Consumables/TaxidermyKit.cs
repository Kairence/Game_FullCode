...
namespace Server.Items
{
	...
	public class TaxidermyKit : Item
	{
		...
		public static TrophyInfo[] TrophyInfos
		{
			get { return m_Table; }
		}
		...
		private class CorpseTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (
					!(targeted is Corpse)
					&& !(targeted is BigFish)
					&& !(targeted is BaseHighseasFish)
					&& !(targeted is HuntingPermit)
				)
				{
				...
				else if (targeted is HuntingPermit)
				{
					...
					else if (from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Board), 10))
					{
						...
						{
							Server.Engines.HuntsmasterChallenge.HuntingTrophyInfo info = Server
								.Engines
								.HuntsmasterChallenge
								.HuntingTrophyInfo
								.Infos[index];
							...
							if (info != null)
							{
								...
								if (info.Complex)
									from.AddToBackpack(
										new HuntTrophyAddonDeed(
											name,
											index,
											lic.KillEntry.Measurement,
											lic.KillEntry.DateKilled.ToShortDateString(),
											lic.KillEntry.Location
										)
									);
								else
									from.AddToBackpack(
									...
									else if (targeted is RareFish)
									{
										...
										from.AddToBackpack(
											new FishTrophyDeed(
												...
												hunter,
												dateCaught,
												m_Table[i].DeedNumber,
												m_Table[i].AddonNumber,
												m_Table[i].NorthID
											)
										);
	...
	public class TrophyAddon : Item, IAddon
	{
		...
		public TrophyAddon(Mobile from, int itemID, int westID, int northID, int deedNumber, int addonNumber)
			: this(from, itemID, westID, northID, deedNumber, addonNumber, null, 0, DateTime.MinValue) { }
