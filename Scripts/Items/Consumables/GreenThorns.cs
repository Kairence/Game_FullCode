...
namespace Server.Items
{
	...
	public abstract class GreenThornsEffect : Timer
	{
		private static readonly TilesAndEffect[] m_Table = new TilesAndEffect[]
		{
			...
			new TilesAndEffect(new int[] { 0x9, 0x15, 0x150, 0x15C }, typeof(FurrowsGreenThornsEffect)),
			new TilesAndEffect(
				new int[]
				{
					0x9C4,
			...
			foreach (TilesAndEffect taep in m_Table)
			{
				...
				if (contains)
				{
					GreenThornsEffect effect = (GreenThornsEffect)
						Activator.CreateInstance(taep.Effect, new object[] { land.Location, from.Map, from });
					return effect;
				}
			}
