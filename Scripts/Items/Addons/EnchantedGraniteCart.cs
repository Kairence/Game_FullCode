...
namespace Server.Engines.VeteranRewards
{
	public class EnchantedGraniteCartComponent : LocalizedAddonComponent, IDyable
	{
		...
		{
			get { return ((EnchantedGraniteCartAddon)Addon).RewardCount; }
		}
	...
	public class EnchantedGraniteCartAddon : BaseAddon, ISecurable
	{
		...
		public override BaseAddonDeed Deed
		{
			get
			{
				EnchantedGraniteCartAddonDeed deed = new EnchantedGraniteCartAddonDeed { IsRewardItem = IsRewardItem };
		...
		public void OnTick()
		{
			if (NextUse < DateTime.Now && RewardCount <= 100)
			{
				RewardCount += 1;
			...
			if (from.InRange(c.Location, 3))
			{
				...
				if (house != null)
				{
					if (CheckAccessible(from, this))
					{
						...
						{
							if (
								Activator.CreateInstance(GraniteType[Utility.Random(GraniteType.Length)])
								is Item granite
							)
							{
								granite.Amount = 1;
