...
namespace Server.Items
{
	public class Aquarium : BaseAddonContainer
	{
		...
		{
			get { return this.m_LiveCreatures; }
		}
		...
		public bool IsFull
		{
			get { return (this.Items.Count >= this.MaxItems); }
		}
		...
		{
			get { return this.m_VacationLeft; }
			set
			{
		...
		{
			get { return (this.m_Food.State == (int)FoodState.Full && this.m_Water.State == (int)WaterState.Strong); }
		}
		...
		{
			get { return this.m_RewardAvailable; }
			set
			{
		...
		public virtual void Evaluate()
		{
			...
			else if (this.m_EvaluateDay)
			{
				...
				if (
					(
						this.m_Food.Added < this.m_Food.Maintain
						&& this.m_Food.State != (int)FoodState.Overfed
						&& this.m_Food.State != (int)FoodState.Dead
				...
				if (
					(
						this.m_Food.Added >= this.m_Food.Improve
						&& this.m_Food.State != (int)FoodState.Full
						&& this.m_Food.State != (int)FoodState.Overfed
		...
		public virtual void UpdateFoodState()
		{
			if (this.m_Food.Added < this.m_Food.Maintain)
				...
				this.m_Food.State =
					(this.m_Food.State >= (int)FoodState.Overfed) ? (int)FoodState.Overfed : this.m_Food.State + 1;
		...
		public virtual void UpdateWaterState()
		{
			if (this.m_Water.Added < this.m_Water.Maintain)
				...
				this.m_Water.State =
					(this.m_Water.State >= (int)WaterState.Strong) ? (int)WaterState.Strong : this.m_Water.State + 1;
		...
		private static readonly int[] m_FishHues = new int[] { 0x1C2, 0x1C3, 0x2A3, 0x47E, 0x51D };
		...
		private class ViewEventEntry : ContextMenuEntry
		{
			...
			{
				if (
					this.m_Aquarium.Deleted
					|| !this.m_Aquarium.HasAccess(this.Owner.From)
					|| this.m_Aquarium.Events.Count == 0
				)
					return;
