...
namespace Server.Items
{
	...
	public class EnormousVenusFlytrapAddon : BaseAddonContainer
	{
		...
		{
			get { return new EnormousVenusFlytrapAddonDeed(Hue); }
		}
					...
					if (checkbagpoint > 0 && m_Cleanup.Find(x => x.serials == list[i].Serial) == null)
					{
						m_Cleanup.Add(
							new CleanupArray
							{
								mobiles = from,
								items = list[i],
								points = checkbagpoint,
								serials = list[i].Serial,
							}
						);
		...
		public void ConfirmCleanupItem(Item item)
		{
			if (item is BaseContainer)
			{
				...
				m_Cleanup
					.Where(r => list.Select(k => k.Serial).Contains(r.items.Serial))
					.ToList()
					.ForEach(k => k.confirm = true);
			}
			else
			{
	...
	public class EnormousVenusFlytrapAddonDeed : BaseAddonContainerDeed
	{
		...
		{
			get { return new EnormousVenusFlytrapAddon(Hue); }
		}
