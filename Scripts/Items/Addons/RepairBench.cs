	...
	public class RepairBenchAddon : BaseAddon, ISecurable
	{
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			for (int x = 0; x < toolcount; x++)
			{
				...
				Tools.Add(
					new RepairBenchDefinition(GetInfo(skill).System, skill, GetInfo(skill).Cliloc, skillvalue, charge)
				);
			}
		}
	}
	...
	public class RepairBenchDeed : BaseAddonDeed, IRewardItem, IRewardOption
	{
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (Tools != null)
			{
				...
				); // Tinkering: ~1_CHARGES~<br>Blacksmithing: ~2_CHARGES~<br>Carpentry: ~3_CHARGES~<br>Tailoring: ~4_CHARGES~<br>Fletching: ~5_CHARGES~<br>Masonry: ~6_CHARGES~<br>Glassblowing: ~7_CHARGES~
			}
		}
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			for (int x = 0; x < toolcount; x++)
			{
				...
				Tools.Add(
					new RepairBenchDefinition(
						RepairBenchAddon.GetInfo(skill).System,
						skill,
						RepairBenchAddon.GetInfo(skill).Cliloc,
						skillvalue,
						charge
					)
				);
	...
	public class ConfirmRemoveGump : Gump
	{
			...
			: base(340, 340)
		{
			...
			AddHtmlLocalized(
				...
				80,
				1158874,
				String.Format("#{0}", addon.Tools.Find(x => x.Skill == skill).Cliloc),
				0x7FFF,
				false,
				false
			); // Are you sure you wish to remove all the ~1_SKILL~ charges from the bench? This action will delete all existing charges and will not refund any deeds.
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is RepairDeed)
				{
					...
					if (m_Addon.Tools.Any(x => x.Skill == deed.RepairSkill && x.Charges >= 500))
					{
						...
						m_Addon.Tools.Any(x =>
							x.Skill == deed.RepairSkill && x.Charges != 0 && x.SkillValue != deed.SkillLevel
						)
					)
					{
					...
					for (int i = c.Items.Count - 1; i >= 0; --i)
					{
						if (i < c.Items.Count && c.Items[i] is RepairDeed)
						{
							...
							if (m_Addon.Tools.Any(x => x.Skill == deed.RepairSkill && x.Charges >= 500))
							{
								...
								m_Addon.Tools.Any(x => x.Skill == deed.RepairSkill && x.SkillValue == deed.SkillLevel)
							)
							{
								var tool = m_Addon.Tools.Find(x => x.Skill == deed.RepairSkill);
