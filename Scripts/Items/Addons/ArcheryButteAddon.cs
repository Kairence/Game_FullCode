...
namespace Server.Items
{
	...
	{
		private static readonly TimeSpan UseDelay = TimeSpan.FromSeconds(3.0);
		...
		{
			if (
				(from.Weapon is Boomerang || from.Weapon is Cyclone || from.Weapon is BaseThrown)
				&& from.InRange(GetWorldLocation(), 1)
			)
				Fire(from);
		...
		*/
		public void Fire(Mobile from)
		{
			...
			if (house != null && house.IsOwner(from))
				skillbonus += 20;
			else if (from.Skills[ranged.Skill].Base >= 100)
			{
			...
			*/
			score *= skillbonus;
			score /= 100;
			from.CheckSkill(ranged.Skill, score);
	...
	public class ArcheryButteDeed : BaseAddonDeed, IRewardOption
	{
		...
		{
			get { return new ArcheryButteAddon(Facing); }
		}
