    ...
    public class TrashBarrel : BaseTrash, IChopable
    {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
			...
			if( from is PlayerMobile )
			{
				...
				int point = Misc.Util.Price(pm, dropped);
				if( point > 0 )
				{
					//골드 반환
					Gold newgold = new Gold( Misc.Util.Price(pm, dropped) );
					pm.AddToBackpack( newgold );
					from.SendMessage("당신은 활동 포인트로 {0}골드를 획득합니다.", Misc.Util.Price(pm, dropped));
					pm.ActionPoint += point;
					dropped.Delete();
					return false;
				}
			}
        ...
        public override bool OnDragDropInto(Mobile from, Item item, Point3D p)
        {
			...
			{
				PlayerMobile pm = from as PlayerMobile;
				int point = Misc.Util.Price(pm, item);
				if( point > 0 )
				{
					//골드 반환
					Gold newgold = new Gold( Misc.Util.Price(pm, item) );
					pm.AddToBackpack( newgold );
					from.SendMessage("당신은 활동 포인트로 {0}골드를 획득합니다.", Misc.Util.Price(pm, item));
					pm.ActionPoint += point;
					item.Delete();
					return false;
				}
