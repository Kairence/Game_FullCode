...
namespace Server.Engines.Mahjong
{
	public class MahjongGame : Item, ISecurable
	{
			...
			: base(0xFAA)
		{
			...
			this.BuildWalls();
			this.m_DealerIndicator = new MahjongDealerIndicator(
				this,
				new Point2D(300, 300),
		...
		{
			get { return this.m_DealerIndicator; }
		}
		public MahjongWallBreakIndicator WallBreakIndicator
		{
			get { return this.m_WallBreakIndicator; }
		}
		public MahjongDices Dices
		{
			get { return this.m_Dices; }
		}
		...
		{
			get { return this.m_SpectatorVision; }
			set
			{
		...
		)
		{
			for (int i = 0; i < 17; i++)
			{
				...
				this.m_Tiles[index + i] = new MahjongTile(
					this,
					index + i,
					typeGenerator.Next(),
					position,
		...
		private class ResetGameEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if (
					from.CheckAlive()
					&& !this.m_Game.Deleted
					&& this.m_Game.IsAccessibleTo(from)
					&& this.m_Game.Players.GetInGameMobiles(true, false).Count == 0
				)
