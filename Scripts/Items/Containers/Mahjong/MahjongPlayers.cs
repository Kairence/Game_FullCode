...
namespace Server.Engines.Mahjong
{
	public class MahjongPlayers
	{
		...
		public int Seats
		{
			get { return this.m_Players.Length; }
		}
		public Mobile Dealer
		{
			get { return this.m_Players[this.m_DealerPosition]; }
		}
		public int DealerPosition
		{
			get { return this.m_DealerPosition; }
		}
		...
		public void CheckPlayers()
		{
				...
				if (player != null)
				{
					...
					else if (this.m_InGame[i])
					{
						if (player.NetState == null)
						{
							...
							!this.m_Game.IsAccessibleTo(player)
							|| player.Map != this.m_Game.Map
							|| !player.InRange(this.m_Game.GetWorldLocation(), 5)
						)
						{
							this.m_InGame[i] = false;
			...
			for (int i = 0; i < this.m_Spectators.Count; )
			{
				...
				if (mobile.NetState == null || mobile.Deleted)
				{
					...
					!this.m_Game.IsAccessibleTo(mobile)
					|| mobile.Map != this.m_Game.Map
					|| !mobile.InRange(this.m_Game.GetWorldLocation(), 5)
				)
				{
					this.m_Spectators.RemoveAt(i);
