...
namespace Server.Items
{
	public abstract class BaseContainer : Container, IEngravable
	{
		...
		public override int DefaultMaxItems
		{
			...
			{
				int items = 125;
				//if( ItemQuality.Quality == ItemQuality.Exceptional )
				{
					items *= 125;
					items /= 100;
				}
		...
		public override bool TryDropItem(Mobile from, Item dropped, bool sendFullMessage)
		{
			...
			{
				if (
					dropped is VendorRentalContract
					|| (
						dropped is Container
		...
		public override bool OnDragDropInto(Mobile from, Item item, Point3D p)
		{
			...
			if (house != null && house.IsLockedDown(this))
			{
				if (
					...
					|| (item is Container && ((Container)item).FindItemByType(typeof(VendorRentalContract)) != null)
				)
				{
					from.SendLocalizedMessage(1062492); // You cannot place a rental contract in a locked down container.
					return false;
	...
	public class SuperStrongBackpack : Backpack //Used on Pack animals
	{
		[Constructable]
		public SuperStrongBackpack()
		{
			Layer = Layer.Backpack;
			Weight = 13.0;
		}

		public SuperStrongBackpack(Serial serial)
			: base(serial) { }
