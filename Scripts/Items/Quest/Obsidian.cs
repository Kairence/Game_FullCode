...
namespace Server.Engines.Quests.Collector
{
	public class Obsidian : Item
	{
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
			...
			if (
				from.Alive
				&& this.m_Quantity >= m_Partial
				&& this.m_Quantity < m_Completed
				&& this.IsChildOf(from.Backpack)
			)
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (this.m_Quantity < m_Completed)
			{
				if (!this.IsChildOf(from.Backpack))
					from.Send(
						new MessageLocalized(this.Serial, this.ItemID, MessageType.Regular, 0x2C, 3, 500309, "", "")
					); // Nothing Happens.
				else
		...
		private class DisassembleEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if (
					!this.m_Obsidian.Deleted
					&& this.m_Obsidian.Quantity >= Obsidian.m_Partial
					&& this.m_Obsidian.Quantity < Obsidian.m_Completed
					&& this.m_Obsidian.IsChildOf(from.Backpack)
					&& from.CheckAlive()
				)
				{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (
					this.m_Obsidian.IsChildOf(from.Backpack)
					&& targ.IsChildOf(from.Backpack)
					&& targ is Obsidian
					&& targ != this.m_Obsidian
				)
				{
				...
				from.Send(
					new MessageLocalized(
						this.m_Obsidian.Serial,
						this.m_Obsidian.ItemID,
						MessageType.Regular,
						0x2C,
						3,
						500309,
