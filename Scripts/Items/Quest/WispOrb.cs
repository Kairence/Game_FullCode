...
namespace Server.Engines.Despise
{
	...
	public class WispOrb : Item
	{
		...
		public WispOrb(Mobile owner, Alignment alignment)
			: base(8448)
		{
			m_Owner = owner;
		...
		public bool CheckOwnerAlignment()
		{
			if (
				m_Owner == null
				|| (m_Owner.Karma > 0 && m_Alignment != Alignment.Good)
				|| (m_Owner.Karma < 0 && m_Alignment != Alignment.Evil)
			)
			{
				if (m_Owner != null)
		...
		private class ConscriptEntry : ContextMenuEntry
		{
			...
			public ConscriptEntry(Mobile from, WispOrb orb)
				: base(1153285, -1) // Conscript
			{
				m_From = from;
				m_Orb = orb;
			...
			public override void OnClick()
			{
				if (
					m_Orb.Pet != null
					&& m_Orb.IsChildOf(m_From.Backpack)
					&& !m_Orb.Conscripted
					&& m_Orb.Pet.Alignment == m_Orb.Alignment
				)
				{
		...
		private class ReleaseEntry : ContextMenuEntry
		{
			...
			public ReleaseEntry(Mobile from, WispOrb orb)
				: base(1153284, -1) // Release
			{
				m_From = from;
				m_Orb = orb;

				if (
					m_Orb.Pet == null /* || !m_Orb.Conscripted*/
				)
					Flags |= Server.Network.CMEFlags.Disabled;
			}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(WispOrb orb)
		...
		private object GetAnchorName()
		{
			...
			if (m_Anchor is StaticTarget)
				return String.Format(
					"{0} {1}",
					((StaticTarget)m_Anchor).Name,
					((StaticTarget)m_Anchor).Location.ToString()
				);

			if (m_Anchor is LandTarget)
				return String.Format(
					"{0} {1}",
					((LandTarget)m_Anchor).Name,
					((LandTarget)m_Anchor).Location.ToString()
				);
		...
		{
			return (int)m_Pet.GetDistanceToSqrt(GetAnchorLocation()) > m_Pet.GetLeashLength() + 1
				&& m_Pet.ControlOrder == OrderType.Follow;
		}
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (reader.ReadInt())
			{
				case 0:
					break;
				case 1:
					m_Anchor = (IEntity)reader.ReadMobile();
					break;
				case 2:
					m_Anchor = (IEntity)reader.ReadItem();
					break;
			}
		...
		public static List<WispOrb> Orbs
		{
			get { return m_Orbs; }
		}
	}
