...
namespace Server.Items
{
	public class GreaterBraceletOfBinding : BraceletOfBinding
	{
		...
		public bool IsFull
		{
			get { return Friends.FirstOrDefault(entry => entry == null) != null; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public override int MaxRecharges
		{
			get { return -1; }
		}
		...
		{
			get { return "greater bracelet of binding"; }
		}
			...
			public override void AddGumpLayout()
			{
				...
				if (Choose)
				{
					AddHtmlLocalized(
						...
						CenterLoc,
						"#1151796",
						Server.Engines.Quests.BaseQuestGump.C32216(0x0000CD),
						false,
						false
					); // **Choose slot to bind**
				}
				else
				{
					...
					AddHtmlLocalized(
						...
						20,
						1017337,
						Bracelet.Charges.ToString(),
						Server.Engines.Quests.BaseQuestGump.C32216(0x0000CD),
						false,
						false
					); // Teleport Charges: ~1_val~
				}
							...
							User.BeginTarget(
								...
								(from, targeted) =>
								{
									if (targeted is PlayerMobile)
									{
										...
										BraceletOfBinding pmBrac =
											pm.FindItemOnLayer(Layer.Bracelet) as BraceletOfBinding;
										...
										else if (pmBrac == null)
										{
											...
											pm.HasGump(typeof(ConfirmBindGump))
											|| (
												pmBrac is GreaterBraceletOfBinding
												&& ((GreaterBraceletOfBinding)pmBrac).Pending != null
											)
										)
										{
											...
											(
												pmBrac is GreaterBraceletOfBinding
												&& ((GreaterBraceletOfBinding)pmBrac).IsFull
											) || (!(pmBrac is GreaterBraceletOfBinding) && pmBrac.Bound != null)
										)
										{
				...
				else
				{
					...
					if (id >= 0 && id < Bracelet.Friends.Length && Bracelet.Friends[id] != null)
					{
						BaseGump.SendGump(
							new ConfirmBindGump(
								User,
								Bracelet.Friends[id].Mobile,
								id,
								Bracelet.Friends[id].Bracelet as GreaterBraceletOfBinding,
								true
							)
						);
					}
		...
		public class ConfirmBindGump : BaseGump
		{
			...
			public ConfirmBindGump(
				PlayerMobile user,
				PlayerMobile from,
				int index,
				GreaterBraceletOfBinding bracelet,
				bool remove
			)
				: base(user, 200, 200)
			{
			...
			public override void OnResponse(RelayInfo info)
			{
				switch (info.ButtonID)
				{
					case 1:
						...
						{
							GreaterBraceletOfBinding bracelet =
								User.FindItemOnLayer(Layer.Bracelet) as GreaterBraceletOfBinding;
						...
						else
						{
							...
							if (brac != null)
							{
								...
								if (brac is GreaterBraceletOfBinding && !((GreaterBraceletOfBinding)brac).IsBound(From))
								{
									...
									BaseGump.SendGump(
										new GreaterBraceletOfBindingGump(User, (GreaterBraceletOfBinding)brac, entry)
									);
								}
								else
								{
