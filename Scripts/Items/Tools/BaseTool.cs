    ...
    public abstract class BaseTool : Item, ITool, IResource, IQuality
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
			if( from is PlayerMobile )
			{
				...
				if( overWeight > 0 )
				{
					from.SendLocalizedMessage(1042025);
					pm.Loop = false;
					return;
				}
			}			
        ...
        public int OnCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, ITool tool, CraftItem craftItem, int resHue)
        {
			...
			int resourceBonus = (int)Resource;
			
			if( resourceBonus >= 301 )
				resourceBonus -= 300;//(int)Resource.RegularWood;
			else if( resourceBonus >= 101 )
				resourceBonus -= 100;
			else if( resourceBonus >= 4 )
				resourceBonus -= 2;
			...
			m_UsesRemaining = 500 + (resourceBonus * 250);
