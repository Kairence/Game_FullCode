	...
	public class DyeTub : Item, ISecurable
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (from.InRange(GetWorldLocation(), 1))
			{
				...
				from.Target = new InternalTarget(this, m_Count);
			}
			else
			{
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			list.Add(1060584, m_Count.ToString()); // uses remaining: ~1_val~
		}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(DyeTub tub, int count)
									...
									if (!house.IsCoOwner(from))
										...
										|| (!house.IsLockedDown(item) && !house.IsSecure(item))
											&& (
												...
												|| !house.Addons.ContainsKey(((AddonComponent)item).Addon)
											)
									)
					...
					else if (m_Tub.AllowLeather)
					{
						if (
							(
								item is BaseArmor
								&& (
									((BaseArmor)item).MaterialType == ArmorMaterialType.Leather
									|| ((BaseArmor)item).MaterialType == ArmorMaterialType.Studded
								)
							)
							|| (
								item is BaseClothing
									&& (((BaseClothing)item).DefaultResource == CraftResource.RegularLeather)
								|| item is WoodlandBelt
								|| item is BarbedWhip
								|| item is BladedWhip
								|| item is SpikedWhip
							)
						...
						(
							item is BaseArmor
							&& (
								((BaseArmor)item).MaterialType == ArmorMaterialType.Chainmail
								|| ((BaseArmor)item).MaterialType == ArmorMaterialType.Ringmail
								|| ((BaseArmor)item).MaterialType == ArmorMaterialType.Plate
							)
						) && m_Tub.AllowMetal
					)
					{
