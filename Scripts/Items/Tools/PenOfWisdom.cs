...
namespace Server.Items
{
	...
	public class PenOfWisdom : Item, IUsesRemaining
	{
		...
		private class SourceTarget : Target
		{
			...
			public SourceTarget(PenOfWisdom pen)
		...
		private class CopyTarget : Target
		{
			...
			public CopyTarget(PenOfWisdom pen, Runebook book)
				: base(12, false, TargetFlags.None)
			{
				Pen = pen;
				SourceBook = book;
	...
	public class PenOfWisdomGump : Gump
	{
		...
		public PenOfWisdomGump(
			Mobile from,
			PenOfWisdom pen,
			Runebook sourcebook,
			Runebook copybook,
			List<RunebookEntry> list
		)
			: base(50, 50)
		{
			...
			AddHtmlLocalized(
				...
				false,
				false
			); // <CENTER>Pen of Wisdom<br>(Mark Scrolls: ~1_VAL~, Runes: ~2_VAL~ | Selected: ~3_VAL~, Blank: ~4_VAL~)</CENTER>
			...
			for (int i = 0; i < entrycount; i++)
			{
				...
				if (yy < 8)
				{
					AddButton(
						15,
						110 + (yy * 25),
						Checked.Contains(SourceBook.Entries[i]) ? 211 : 210,
						Checked.Contains(SourceBook.Entries[i]) ? 210 : 211,
						i,
					...
					AddLabelCropped(
						...
						115,
						17,
						RunebookGump.GetMapHue(SourceBook.Entries[i].Map),
			...
			switch (info.ButtonID)
			{
				...
				case 22: // OK
				{
					...
					else if (Blank < Checked.Count)
					{
						...
						!SourceBook.IsChildOf(from.Backpack) && SourceBook.Movable
						|| !CopyBook.IsChildOf(from.Backpack)
					)
					{
						from.SendLocalizedMessage(1115329); // Runebooks you wish to copy must be in your backpack.
					}
