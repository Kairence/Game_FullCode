				...
				: base(150, 50)
			{
				...
				if (!InHouse(from))
				{
					AddButton(
						40,
						36,
						(decorator.Command == DecorateCommand.GetHue ? 2154 : 2152),
						2154,
						4,
				...
				else
				{
					AddButton(
						40,
						36,
						(decorator.Command == DecorateCommand.Turn ? 2154 : 2152),
						2154,
						1,
						GumpButtonType.Reply,
						0
					...
					AddButton(
						40,
						86,
						(decorator.Command == DecorateCommand.Up ? 2154 : 2152),
						2154,
						2,
						GumpButtonType.Reply,
						0
					...
					AddButton(
						40,
						136,
						(decorator.Command == DecorateCommand.Down ? 2154 : 2152),
						2154,
						3,
						GumpButtonType.Reply,
						0
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (targeted is Item && CheckUse(m_Decorator, from))
				{
					...
					else if (item is AddonComponent || item is AddonContainerComponent || item is BaseAddonContainer)
					{
						...
						{
							FlipableAddonAttribute[] attributes = (FlipableAddonAttribute[])
								addon.GetType().GetCustomAttributes(typeof(FlipableAddonAttribute), false);
			...
			private static void Turn(Item item, Mobile from)
			{
				...
				if (item is AddonComponent || item is AddonContainerComponent || item is BaseAddonContainer)
				{
					...
					FlipableAddonAttribute[] aAttributes = (FlipableAddonAttribute[])
						addon.GetType().GetCustomAttributes(typeof(FlipableAddonAttribute), false);
				...
				FlipableAttribute[] attributes = (FlipableAttribute[])
					item.GetType().GetCustomAttributes(typeof(FlipableAttribute), false);
