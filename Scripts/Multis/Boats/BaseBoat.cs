...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti, IMount
	{
		...
		public virtual int ZSurface
		{
			get { return 0; }
		}
		public virtual int RuneOffset
		{
			get { return 0; }
		}
		...
		{
			get { return m_Hits / (double)MaxHits * 100.0; }
		}
		...
		#region High Seas
		public virtual bool IsClassicBoat
		{
			get { return true; }
		}
		public virtual bool IsRowBoat
		{
			get { return false; }
		}
		public virtual double TurnDelay
		{
			get { return 0.5; }
		}
		...
		public virtual double ScuttleLevel
		{
			get { return 25.0; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public virtual bool Scuttled
		{
			get { return !IsUnderEmergencyRepairs() && Durability < ScuttleLevel; }
		}

		public virtual TimeSpan BoatDecayDelay
		{
			get { return TimeSpan.FromDays(13); }
		}
		public virtual bool CanLinkToLighthouse
		{
			get { return true; }
		}
		...
		public override void OnLocationChange(Point3D old)
		{
			if (TillerMan != null)
			{
				if (TillerMan is Mobile && (Math.Abs(X - old.X) > 1 || Math.Abs(Y - old.Y) > 1))
					((Mobile)TillerMan).Location = new Point3D(
						X + (((Mobile)TillerMan).X - old.X),
						Y + (((Mobile)TillerMan).Y - old.Y),
						Z + (((Mobile)TillerMan).Z - old.Z)
					);
				else if (TillerMan is Item)
					((Item)TillerMan).Location = new Point3D(
						X + (((Item)TillerMan).X - old.X),
						Y + (((Item)TillerMan).Y - old.Y),
						Z + (((Item)TillerMan).Z - old.Z)
					);
			}
		...
		*/
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int FastInterval { get; set; } = 100; //250;

		[CommandProperty(AccessLevel.GameMaster)]
		public int NormalInterval { get; set; } = 100; //500;

		[CommandProperty(AccessLevel.GameMaster)]
		public int SlowInterval { get; set; } = 100; //1000;

		public TimeSpan FastInt
		{
			get { return TimeSpan.FromMilliseconds(FastInterval); }
		}
		public TimeSpan NormalInt
		{
			get { return TimeSpan.FromMilliseconds(NormalInterval); }
		}
		public TimeSpan SlowInt
		{
			get { return TimeSpan.FromMilliseconds(SlowInterval); }
		}

		public TimeSpan FastDriftInterval
		{
			get { return NormalInt; }
		}
		public TimeSpan SlowDriftInterval
		{
			get { return SlowInt; }
		}
		...
		public virtual DryDockResult CheckDryDock(Mobile from, Mobile dockmaster)
		{
			...
			{
				if (
					(SPlank == null || !Key.ContainsKey(pack, SPlank.KeyValue))
		...
		private Type[] WoodTypes = new Type[]
		{
			typeof(Board),
			typeof(OakBoard),
			typeof(AshBoard),
		...
		public void TryRepairs(Mobile from)
		{
			...
			for (int i = 0; i < WoodTypes.Length; i++)
			{
				...
				if (pack != null)
					wood += pack.GetAmount(type);
				if (hold != null)
					wood += hold.GetAmount(type);
				if (secure != null)
					wood += secure.GetAmount(type);
			}
			...
			for (int i = 0; i < ClothTypes.Length; i++)
			{
				...
				if (pack != null)
					cloth += pack.GetAmount(type);
				if (hold != null)
					cloth += hold.GetAmount(type);
				if (secure != null)
					cloth += secure.GetAmount(type);
			}
			...
			if (wood < minWood || cloth < minCloth)
			{
				from.SendLocalizedMessage(
					1116593,
					string.Format("{0}\t{1}", ((int)minCloth).ToString(), ((int)minWood).ToString())
				); //You need a minimum of ~1_CLOTH~ yards of cloth and ~2_WOOD~ pieces of lumber to effect repairs to this ship.
				return;
			}
			...
			string args = string.Format(
				"{0}\t{1}\t{2}",
				((int)clothTemp).ToString(),
				((int)woodTemp).ToString(),
		...
		public bool StartCourse(bool single, bool message)
		{
			...
			else if (Map != BoatCourse.Map)
			{
				...
				|| NextNavPoint < 0
				|| NextNavPoint >= BoatCourse.Waypoints.Count
			)
			{
				if (message && TillerMan != null)
					TillerManSay(1042551); // I don't see that navpoint, sir.
		...
		public bool CanFit(Point3D p, Map map, int itemID)
		{
			...
			for (int x = 0; x < newComponents.Width; ++x)
			{
				for (int y = 0; y < newComponents.Height; ++y)
				{
					...
					if (
						...
						|| Contains(tx, ty)
						|| IsExcludedTile(newComponents.Tiles[x][y])
					)
						continue;
			...
			foreach (IEntity e in eable)
			{
				...
				if (e is Item)
				{
					...
					if (
						CheckItem(itemID, item, p)
						|| CanMoveOver(item)
						|| item.Z < p.Z
						|| ExemptOverheadComponent(p, itemID, item.X, item.Y, item.Z + item.ItemData.Height)
					)
						continue;
				}
				else if (e is Mobile)
		...
		public static bool HasBoat(Mobile from)
		{
			...
			return Boats.Any(boat =>
				boat.Owner == from && !boat.Deleted && boat.Map != Map.Internal && !boat.IsRowBoat
			);
		}
		...
		public bool Move(Direction dir, int speed, int clientSpeed, bool message)
		{
			...
			{
				if (!CanFit(new Point3D(X + (i * rx), Y + (i * ry), Z - 3), Map, ItemID))
				{
					if (i == 1)
					{
						if (message && TillerMan != null)
			...
			for (int i = 0; i < wrap.Length; ++i)
			{
				...
				if (rect.Contains(new Point2D(X, Y)) && !rect.Contains(new Point2D(newX, newY)))
				{
					...
					{
						if (!CanFit(new Point3D(newX + (j * rx), newY + (j * ry), Z - 3), Map, ItemID))
						{
							if (message && TillerMan != null)
								TillerManSay(501424); // Ar, we've stopped sir.
		...
		{
			foreach (
				var ent in GetEntitiesOnBoard().Where(e => !IsComponentItem(e) && !CanMoveOver(e) && e != TillerMan)
			)
			{
			...
			foreach (IEntity o in eable)
			{
				if (o is Item item)
				{
					...
					if (
						...
						&& item.Visible
						&& item.Z >= Z
						&& !(item is TillerMan || item is Hold || item is Plank || item is RudderHandle)
					)
						toMove.Add(o);
				}
		...
		public virtual void SetFacingComponents(
			Direction newDirection,
			Direction oldDirection,
			bool ignoreLastFacing
		) { }
		...
		public virtual bool IsComponentItem(IEntity item)
		{
			...
			if (
				item == this
				|| (TillerMan is Item && item == (Item)TillerMan)
				|| item == SPlank
				|| item == PPlank
				|| item == Hold
			)
