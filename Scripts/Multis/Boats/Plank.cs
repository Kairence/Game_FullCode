...
namespace Server.Items
{
	...
	public class Plank : Item, ILockable
	{
		...
		public bool IsOpen
		{
			get { return ItemID == 0x3ED5 || ItemID == 0x3ED4 || ItemID == 0x3E84 || ItemID == 0x3E89; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public bool Starboard
		{
			get { return Side == PlankSide.Starboard; }
		}
		...
		public override bool OnMoveOver(Mobile from)
		{
			if (IsOpen)
			{
				...
				if (
					(from.Player && (from.Direction & Direction.Running) != 0) || (Boat != null && !Boat.Contains(from))
				)
					return true;
				...
				for (int i = 1; i <= 6; ++i)
				{
					...
					for (int j = -8; j <= 8; ++j)
					{
						...
						if (
							...
							&& !Spells.SpellHelper.CheckMulti(new Point3D(x, y, z), map)
							&& !Region.Find(new Point3D(x, y, z), map).IsPartOf(typeof(Factions.StrongholdRegion))
						)
						{
							if (i == 1 && j >= -2 && j <= 2)
								return true;
		...
		public class PlanksContext : ContextMenuEntry
		{
			...
			public PlanksContext(Mobile from, Plank plank)
				: base(6132, 10)
			{
				m_Plank = plank;
				m_From = from;
