...
namespace Server.Items
{
	...
	{
		public virtual bool UseCustomHousePlots
		{
			get { return Server.Misc.TestCenter.Enabled; }
		}
	...
	public class HousePlacementCategoryGump : Gump
	{
		...
		public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				{
					m_From.SendGump(
						new HousePlacementListGump(m_Tool, m_From, HousePlacementEntry.TwoStoryFoundations)
					);
					break;
				}
				...
				{
					m_From.SendGump(
						new HousePlacementListGump(m_Tool, m_From, HousePlacementEntry.ThreeStoryFoundations)
					);
					break;
				}
	...
	public class HousePlacementListGump : Gump
	{
		...
		public HousePlacementListGump(
			HousePlacementTool tool,
			Mobile from,
			...
			: base(50, 50)
		{
			...
			AddLabel(
				...
				370,
				LabelHue,
				Banker.GetBalance(from).ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US"))
			);
			...
			for (int i = 0; i < entries.Length; ++i)
			{
				...
				AddLabel(
					...
					y,
					LabelHue,
					entry.Cost.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US"))
				);
			}
		}
	...
	public class HousePlacementEntry
	{
		...
		{
			new HousePlacementEntry(typeof(SmallOldHouse), 1011303, 580, 290, 667, 333, 14, 36750, 0, 4, 0, 0x0064),
			new HousePlacementEntry(typeof(SmallOldHouse), 1011304, 580, 290, 667, 333, 14, 36750, 0, 4, 0, 0x0066),
			new HousePlacementEntry(typeof(SmallOldHouse), 1011305, 580, 290, 667, 333, 14, 36500, 0, 4, 0, 0x0068),
			new HousePlacementEntry(typeof(SmallOldHouse), 1011306, 580, 290, 667, 333, 14, 35000, 0, 4, 0, 0x006A),
			new HousePlacementEntry(typeof(SmallOldHouse), 1011307, 580, 290, 667, 333, 14, 36500, 0, 4, 0, 0x006C),
			new HousePlacementEntry(typeof(SmallOldHouse), 1011308, 580, 290, 667, 333, 14, 36500, 0, 4, 0, 0x006E),
			new HousePlacementEntry(typeof(SmallShop), 1011321, 800, 400, 920, 460, 18, 50250, -1, 4, 0, 0x00A0),
			new HousePlacementEntry(typeof(SmallShop), 1011322, 800, 400, 920, 460, 18, 52250, 0, 4, 0, 0x00A2),
			new HousePlacementEntry(typeof(SmallTower), 1011317, 1100, 550, 1265, 632, 24, 73250, 3, 4, 0, 0x0098),
			new HousePlacementEntry(typeof(TwoStoryVilla), 1011319, 1100, 550, 1265, 632, 24, 113500, 3, 6, 0, 0x009E),
			new HousePlacementEntry(typeof(SandStonePatio), 1011320, 1100, 550, 1265, 632, 24, 76250, -1, 4, 0, 0x009C),
			new HousePlacementEntry(typeof(LogCabin), 1011318, 1100, 550, 1265, 632, 24, 81250, 1, 6, 0, 0x009A),
			new HousePlacementEntry(typeof(GuildHouse), 1011309, 2119, 1059, 2437, 1218, 28, 131250, -1, 7, 0, 0x0074),
		...
		private static readonly HousePlacementEntry[] m_HousesEJ =
		{
			...
			new HousePlacementEntry(typeof(TwoStoryVilla), 1011319, 1370, 685, 1576, 788, 28, 113500, 3, 6, 0, 0x009E),
		...
		private static readonly HousePlacementEntry[] m_CustomHouseContest = new HousePlacementEntry[]
		{
			new HousePlacementEntry(
				...
				0,
				0x147C
			), // 23x23 3-Story Customizable Keep
			new HousePlacementEntry(
				typeof(HouseFoundation),
				1158539,
				4076,
				...
				0,
				0x147D
			), // 32x32 3-Story Customizable Castle
		};
		...
		{
			get { return m_CustomHouseContest; }
		}
	...
	public class HouseSwapGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddLabel(
				...
				370,
				LabelHue,
				Banker.GetBalance(m_From).ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US"))
			);
					...
					if (e != null)
					{
						...
						if (newHouse != null)
						{
							...
							newHouse.MoveToWorld(
								new Point3D(
									m_House.X + m_House.ConvertOffsetX,
									m_House.Y + m_House.ConvertOffsetY,
									m_House.Z + m_House.ConvertOffsetZ
								),
								m_House.Map
							);
							m_House.Delete();
