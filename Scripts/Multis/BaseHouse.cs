...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        {
            foreach (var vendor in Server.Mobiles.RetailVendor.RetailVendors.Where(v => v.Map == this.Map && this.Region.Contains(v.Location)).ToList())
            {
                vendor.FireVendor(this);
            }
        ...
        public Dictionary<IEntity, Mobile> GetHouseEntities()
        {
            ...
            foreach (var mobile in Server.Mobiles.RetailVendor.RetailVendors.Where(v => v.Map == this.Map && this.Region.Contains(v.Location)))
            {
                if (mobile.Map != Map.Internal)
                {
                    mobile.Say(68, "집 공사 중이시군요! 잠시 쉬고 오겠습니다~");
                    list[mobile] = Owner;
                }
            }
        ...
        public void Kick(Mobile from, Mobile targ)
        {
            ...
            {
                Server.Misc.WorkshopTicket.ProcessRefund(this, targ);
        ...
        public virtual int MaxYardExtensions
        {
            get
            {
                // ���� �Ը�(��ٿ� ��ġ)�� ���� �ִ� ���� Ȯ��(3x3) ���� Ƚ���� �����մϴ�.
                // ��ȹ�� �°� ��ġ�� �����ϼ���! (��: ��/ŵ�� 6��, 18x18�� 4��, �������� 1��)
                if (MaxLockDowns >= 2000) return 6; // ĳ��, ŵ, 18x18 �� ���� ��
                if (MaxLockDowns >= 1000) return 3; // ���� ��
                return 1;                           // ���� ��
            }
        }
        ...
        public int GetCurrentYardExtensions()
        {
            ...
            if (LockDowns != null)
            {
                // ��ٿ�� �����۵��� ��ĵ�Ͽ� Ȯ�� ���� ��ٴ�(ExAddOnTile �� 0x31F4)�� ������ ���ϴ�.
                foreach (Item item in LockDowns.Keys)
                {
                    // ExAddOnTile �̶�� �̸��� Ŭ�������� �Ǻ� (���ӽ����̽� ����)
        ...
        public static bool CheckAccountHouseLimit(Mobile m, bool message = true)
        {
            if (AtAccountHouseLimit(m))
            {
                if (message)
                {
                    ...
                    {
                        m.SendMessage("����� ����{0}ä�� ���� �����ϰ� �ֽ��ϴ�!", BaseHouse.AccountHouseLimit.ToString());
                    }
                }
