...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		{
			get { return GlobalBonusStorageScalar; }
		}
		...
		{
			typeof(Engines.Plants.SeedBox),
			typeof(GardenShedAddon),
		...
		{
			typeof(TenthAnniversarySculpture),
			typeof(RewardBrazier),
		...
		private Type[] _AccessibleToFriends = { typeof(PotionKeg) };
		...
		{
			BaseDoor northDoor = MakeDoor(
				wood,
				invert ? DoorFacing.SouthCCW
					: altopen ? DoorFacing.NorthCCW
					: DoorFacing.NorthCW
			);
			BaseDoor southDoor = MakeDoor(
				wood,
				invert ? DoorFacing.NorthCW
					: altopen ? DoorFacing.SouthCW
					: DoorFacing.SouthCCW
			);
		...
		public void SetLockdown(Mobile m, Item i, bool locked)
		{
			...
			Timer.DelayCall(
				() => i.PrivateOverheadMessage(MessageType.Regular, 0, locked ? 501721 : 501657, m.NetState)
			); // locked down! : [no longer locked down]
		...
		public bool ReleaseSecure(Mobile m, Item item)
		{
			...
			{
				if (
					(IsOwner(m) || info.Owner == m) /*&& HasSecureAccess(m, info.Level)*/
				)
				{
					item.IsLockedDown = false;
					item.IsSecure = false;
		...
		private void CheckUnregisteredAddons()
		{
			...
			foreach (
				var item in Region
					.GetEnumeratedItems()
					.Where(i => i is AddonComponent && ((AddonComponent)i).Addon == null)
			)
			{
				item.Delete();
			}
		...
		public static bool CheckAccountHouseLimit(Mobile m, bool message = true)
		{
			if (AtAccountHouseLimit(m))
			{
				if (message)
				{
					...
					{
						m.SendMessage(
							"당신은 벌써{0}채의 집을 소유하고 있습니다!",
							BaseHouse.AccountHouseLimit.ToString()
						);
					}
				}
