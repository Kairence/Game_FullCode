...
namespace Server.Multis
{
	public class HouseTeleporterTile : HouseTeleporter, IFlipable
	{
		...
		{
			get { return ItemID == 0x574A || ItemID == 0xA1CB || ItemID == 0xA1CC || ItemID == 0x40BB; }
		}
		...
		{
			get { return Link == null ? 1114916 : 1113917; }
		} // house teleporter (unlinked) -or- House Teleporter
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
			if (from.InRange(Location, 2) || IsChildOf(from.Backpack))
			{
				...
				list.Add(new WriteLocation(from, this));
			}

			base.GetContextMenuEntries(from, list);
		}

		private class WriteLocation : ContextMenuEntry
		{
			private readonly Mobile Mobile;
			private readonly HouseTeleporterTile Item;

			public WriteLocation(Mobile mobile, HouseTeleporterTile item)
				: base(6313, 2)
			{
				Mobile = mobile;
				Item = item;
			...
			public override void OnClick()
			{
				...
				Mobile.Prompt = new RenamePrompt(Item);
		...
		private class RenamePrompt : Prompt
		{
			...
			private readonly HouseTeleporterTile m_Rune;

			public RenamePrompt(HouseTeleporterTile rune)
			{
				m_Rune = rune;
			}
				...
				if (targeted is Item)
				{
					...
					if (item is GateTravelScroll)
					{
						...
						else
						{
							...
							if (scroll.Amount <= MaxCharges - Item.Charges)
							{
								Item.Charges += scroll.Amount; //Math.Min(MaxCharges, Item.Charges + (scroll.Amount * 5));
								scroll.Delete();
							}
							else
							{
								scroll.Amount -= MaxCharges - Item.Charges;
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			{
				if (Map == Map.Tokuno)
					list.Add(1063260, m_Description); // ~1_val~ (Tokuno Islands)[(House)]
				else if (Map == Map.Malas)
					list.Add(1062454, m_Description); // ~1_val~ (Malas)[(House)]
				else if (Map == Map.Felucca)
					list.Add(1062452, m_Description); // ~1_val~ (Felucca)[(House)]
				else if (Map == Map.Trammel)
					list.Add(1062453, m_Description); // ~1_val~ (Trammel)[(House)]
				else if (Map == Map.TerMur)
					list.Add(1113206, m_Description); // ~1_val~ (Ter Mur)(House)
			}
		}
		...
		public bool CheckTravel(Mobile from, Point3D dest, Map destMap)
		{
			...
			else if (destMap == Map.Felucca && from is PlayerMobile && ((PlayerMobile)from).Young)
			{
				...
				&& from.Murderer
				&& destMap.Rules != MapRules.FeluccaRules
				&& !Siege.SiegeShard
			)
			{
		...
		public HouseTeleporterTile(Serial serial)
			: base(serial) { }
