...
namespace Server.Mobiles
{
	...
	public class DreadHorn : BasePeerless
	{
		...
		{
			var toTele = SpellHelper
				.AcquireIndirectTargets(this, Location, Map, StrikingRange)
				.OfType<PlayerMobile>()
				.ToList();
		...
		public void HoofStomp()
		{
			...
			foreach (
				Mobile m in SpellHelper.AcquireIndirectTargets(this, Location, Map, StrikingRange).OfType<Mobile>()
			)
			{
				if (m.GetStatMod("DreadHornStr") == null)
				{
					...
					m.AddStatMod(
						new StatMod(
							StatType.Str,
							"DreadHornStr",
							m.Str < Math.Abs(malas) ? m.Str / 2 : malas,
					...
					m.AddStatMod(
						new StatMod(
							StatType.Dex,
							"DreadHornDex",
							m.Dex < Math.Abs(malas) ? m.Dex / 2 : malas,
							TimeSpan.FromSeconds(60)
						)
					);
					m.AddStatMod(
						new StatMod(
							StatType.Int,
							"DreadHornInt",
							m.Int < Math.Abs(malas) ? m.Int / 2 : malas,
							TimeSpan.FromSeconds(60)
						)
					);
				}
		...
		public Mobile Validate(Mobile m)
		{
			...
			if (
				!CanBeHarmful(agro, false) || !agro.Player /*|| Combatant == agro*/
			)
				return null;
