	...
	public class MannequinCompareGump : Gump
	{
			...
			: base(100, 100)
		{
			...
			AddHtmlLocalized(
				...
				18,
				1114513,
				string.Format("#{0}", _SameItem.LabelNumber),
				0x7FFF,
				false,
				false
			); // <DIV ALIGN=CENTER>~1_TOKEN~</DIV>
			...
			for (int i = 0; i < EquipmentItem.Count; i++)
			{
				...
				if (EquipmentItem[i].IsSpriteGraph)
				{
					AddSpriteImage(
						...
						41,
						0x9D3B,
						EquipmentItem[i].SpriteW,
						EquipmentItem[i].SpriteH,
						30,
						30
					);
				...
				else
				{
					...
					if (!EquipmentItem[i].IsBoolen)
						AddHtml(
							...
							100,
							18,
							Color(EquipmentItem[i].Value, EquipmentItem[i].Cap),
							false,
							false
						);
				}
				...
				else
				{
					...
					if (!SelectItem[i].IsBoolen)
					{
						...
						AddHtml(
							...
							100,
							18,
							Color(SelectItem[i].Value, SelectItem[i].Cap, ev),
							false,
							false
						);
					}
	...
	public class MannequinStatsGump : Gump
	{
			...
			: base(100, 100)
		{
			...
			if (_Item != null)
			{
				...
				if (_SameItem != null)
				{
					CompareItem = new List<ItemPropDefinition> { new ItemPropDefinition(_Item, _SameItem) };
			...
			pages
				...
				.ForEach(x =>
				{
					...
					if (lm.Any(l => l.Catalog == x.Catalog))
					{
						...
						List<ValuedProperty> cataloglist = lm.Where(i => i.Catalog == x.Catalog)
							.OrderBy(ii => ii.Order)
							.ToList();

						int columncount =
							x.ColumnLeftCount == 0
								? (int)Math.Ceiling(cataloglist.Count / (decimal)2)
								: x.ColumnLeftCount;
						...
						for (int i = 0; i < cataloglist.Count; i++)
						{
							...
							if (i < columncount)
							{
								...
								AddSpriteImage(
									...
									y - 5,
									0x9D3B,
									cataloglist[i].SpriteW,
									cataloglist[i].SpriteH,
									30,
									30
								);
								...
								else
								{
									AddHtml(
										...
										Color(
											cataloglist[i].Value,
											GetItemValue(CompareItem, label),
											cataloglist[i].Cap
										),
										false,
										false
		...
		public string Color(double ev, double diff, int parmv)
		{
			...
			else if (diff < 0)
			{
				...
				{
					name = string.Format(
						"<BASEFONT COLOR=#800000>{0}/{1} (-{2})</BASEFONT>",
						ev,
						parmv,
						Math.Abs(diff)
					);
