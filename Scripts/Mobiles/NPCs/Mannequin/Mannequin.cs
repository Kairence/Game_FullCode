...
namespace Server.Mobiles
{
	public class Mannequin : BaseCreature
	{
		...
		private List<Layer> SameLayers = new List<Layer>() { Layer.FirstValid, Layer.OneHanded, Layer.TwoHanded };

		public bool CheckSameLayer(Item i1, Item i2)
		{
			return i1 is BaseWeapon
				&& i2 is BaseWeapon
				&& SameLayers.Contains(i1.Layer)
				&& SameLayers.Contains(i2.Layer);
		}
		...
		public static List<ValuedProperty> FindItemsProperty(List<Item> item)
		{
			var ll = System
				...
				.GetTypes()
				.ToList()
				.Where(r => r.FullName.Contains("MannequinProperty") && r.IsClass == true && r.IsAbstract == false)
				.ToList();
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
			...
			if (IsOwner(Owner))
			{
				...
				if (from.Alive && from.InRange(this, 2))
				{
					if (
						from.Race == Race
						|| (from.Race == Race.Elf && Race == Race.Human || from.Race == Race.Human && Race == Race.Elf)
					)
					{
		...
		{
			return item != null
				&& item.Parent is Mobile
				&& ((Mobile)item.Parent).FindItemOnLayer(item.Layer) == item
		...
		public Mannequin(Serial serial)
			: base(serial) { }
