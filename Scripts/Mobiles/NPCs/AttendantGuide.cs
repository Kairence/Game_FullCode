...
namespace Server.Mobiles
{
	...
	{
		private static readonly Dictionary<string, List<GuideVertex>> m_GraphDefinitions =
			new Dictionary<string, List<GuideVertex>>();
				...
				: base(50, 50)
			{
				...
				for (int i = 0; i < size; i += 2)
				{
					...
					if (i + 1 < size)
					{
						...
						this.AddHtmlLocalized(
							305,
							20 + i * 30 / 2,
							200,
							20,
							m_ShopDefinitions[i + 1],
							0x7773,
							false,
							false
						);
		...
		public class GuideVertex : IComparable<GuideVertex>
		{
			...
			{
				get { return this.m_Shops; }
			}
			public bool Teleporter
			{
				get { return this.m_Teleporter; }
				set { this.m_Teleporter = value; }
			}
	...
	public class AttendantGuide : PersonalAttendant
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				Dictionary<int, Vertex> m_Shops = GuideHelper.FindShops(
					this.Region != null ? this.Region.Name : null,
					this.Location
				);
					...
					if (dist < (v.Teleporter ? 1 : 3) || dist > 100)
					{
						...
						else
						{
							Timer.DelayCall<Mobile>(
								...
								new TimerStateCallback<Mobile>(CommandFollow),
								m
							);
				...
				: base(60, 36)
			{
				...
				foreach (KeyValuePair<int, Vertex> kvp in shops)
				{
					...
					this.AddHtmlLocalized(
						...
						213,
						20,
						GuideHelper.FindShopName(kvp.Key),
						0x7FFF,
						false,
						false
					);
