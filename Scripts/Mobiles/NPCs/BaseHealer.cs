...
namespace Server.Mobiles
{
	public abstract class BaseHealer : BaseVendor
	{
		...
		public virtual void OfferHeal(PlayerMobile m)
		{
			...
			{
				this.Say(501230); // You look like you need some healing my child.
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (
				!m.Frozen
				&& DateTime.UtcNow >= this.m_NextResurrect
				&& this.InRange(m, 2)
				&& !this.InRange(oldLocation, 2)
				&& this.InLOS(m)
			)
			{
				if (!m.Alive)
				...
				else if ((m.Hits < m.HitsMax || m.Stam < m.StamMax || m.Mana < m.ManaMax) && m is PlayerMobile)
				{
