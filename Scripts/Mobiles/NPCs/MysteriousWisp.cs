...
namespace Server.Mobiles
{
	...
	public class MysteriousWisp : BaseCreature
	{
		...
		{
			get { return m_ItemCountMin; }
			set { m_ItemCountMin = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int ItemCountMax
		{
			get { return m_ItemCountMax; }
			set { m_ItemCountMax = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int IntensityMin
		{
			get { return m_IntensityMin; }
			set { m_IntensityMin = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int IntensityMax
		{
			get { return m_IntensityMax; }
			set { m_IntensityMax = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int RestockMin
		{
			get { return m_RestockMin; }
			set { m_RestockMin = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public int RestockMax
		{
			get { return m_RestockMax; }
			set { m_RestockMax = value; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public double MutateChance
		{
			get { return m_MutateChance; }
			set { m_MutateChance = value; }
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (from is PlayerMobile)
			{
				...
				if (QuestHelper.CheckDoneOnce(pm, typeof(WishesOfTheWispQuest), null, false))
				{
					...
					if (q == null)
					{
						BaseQuest quest = QuestHelper.RandomQuest(
							pm,
							new Type[] { typeof(WhisperingWithWispsQuest) },
							this
						);
			...
			for (int i = 0; i < count; i++)
			{
				...
				switch (Utility.Random(3))
				{
					...
					case 0:
						...
						break;
					case 1:
						item = Loot.RandomArmorOrShieldOrWeaponOrJewelry(false, true, false);
						break;
					case 2:
						item = Loot.RandomArmorOrShieldOrWeaponOrJewelry(true, false, false);
						break;
				}
				...
				m_ItemTable[item] = (int)(
					(weight + Server.SkillHandlers.Imbuing.GetTotalWeight(item, -1, false, true)) * 31.5
				);
				item.Movable = false;
				...
				: base(40, 40)
			{
				...
				for (int i = 0; i < m_Items.Count; i++)
				{
					...
					if (item != null && !item.Deleted)
					{
						...
						string cost =
							m_Wisp.GetCostFor(item) > -1
								? m_Wisp.GetCostFor(item).ToString("###,###,###")
								: "No Longer Available";
						...
						AddHtml(
							...
							250,
							60,
							"<BASEFONT COLOR=#FFFFFF>" + cost + "</BASEFONT>",
							false,
							false
						);
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				if (idx >= 0 && idx < m_Items.Count)
				{
					...
					if (
						item != null
						&& !item.Deleted
						&& m_Wisp.Backpack != null
						&& m_Wisp.Backpack.Items.Contains(item)
					)
					{
