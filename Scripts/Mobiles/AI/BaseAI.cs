...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		{
			/*
			int range = from.Int / 10;
			if( from.Int > 100 )
				range = 10 + from.Int / 20;
			if( range > 25 )
				range = 25;
			if( range < 1 )
				range = 1;
			if( from is FireElemental || from is WaterElemental || from is AirElemental || from is PoisonElemental || from is BloodElemental )
				range = 22;
			*/
		...
		public virtual void EndPickTarget(Mobile from, IDamageable target, OrderType order)
		{
			...
			if (order == OrderType.Attack)
			{
				if (target is BaseCreature)
				{
					...
					if (
						(bc is IBlackSolen && SolenHelper.CheckBlackFriendship(from))
				...
				if (m_Mobile.CurrentNavPoint >= 0 && m_Mobile.CurrentNavPoint < m_Mobile.NavPoints[map].Count)
				{
					...
					else if (OnAtWayPoint())
					{
						...
						else
						{
							...
							m_Mobile.DebugSay(
								String.Format(
									"I will go to the next navpoint: {0}",
		...
		public virtual bool DoActionFlee()
		{
			...
			if (AcquireFocusMob(36, FightMode.Aggressor, false, false, true) || c != null)
			{
				...
				if (WalkMobileRange(c, 1, true, 24, 36))
				{
		...
		public bool ValidGuardTarget(Mobile combatant)
		{
			return combatant != null
				...
				&& combatant.Alive
				&& (!(combatant is Mobile) || !combatant.IsDeadBondedPet)
		...
		public virtual bool DoOrderAttack()
		{
			...
			)
			{
				...
				if (m_Mobile.FightMode == FightMode.Closest || m_Mobile.FightMode == FightMode.Aggressor)
				{
					...
					var eable = m_Mobile.GetMobilesInRange(36); //m_Mobile.RangePerception);

					double BestAggroScore = 0.0;

					foreach (var aggr in eable)
					{
						if (!m_Mobile.CanSee(aggr))
						{
							continue;
						}
						...
						if (
							(
								newCombatant == null
								|| aggrScore > newScore
								|| (aggrScore == newScore && !aggr.Player && newCombatant.Player)
							) && m_Mobile.InLOS(aggr)
						)
						{
							newCombatant = aggr;
		...
		public virtual bool DoOrderRelease()
		{
			...
			if (m_Mobile.DeleteOnRelease)
			{
				m_Mobile.PrivateOverheadMessage(
					...
					master.NetState
				); // ~1_NAME~ appears to have decided that it is better off without a master!
			}
		...
		private class TransferItem : Item
		{
				...
				: base(ShrinkTable.Lookup(creature))
			{
				...
				if (!Core.AOS)
				{
					...
					ItemID == ShrinkTable.DefaultItemID
		...
		public virtual void WalkRandom(int iChanceToNotMove, int iChanceToDir, int iSteps)
		{
			...
			if (m_Mobile.CanFly && Utility.Random(80 * iChanceToNotMove) <= 8)
			{
				var p = Point3D.Zero;
				...
				for (int i = 0; i < 10; i++)
				{
					...
					if (
						m_Mobile.Home != Point3D.Zero
						&& m_Mobile.RangeHome > 0
			...
			{
				if (Utility.Random(80 * iChanceToNotMove) <= 8)
				{
				...
				foreach (var m in eable)
				{
					...
					else if (!bFacFriend || !m_Mobile.IsFriend(m))
					{
						...
						if (m_Mobile.Summoned && m_Mobile.SummonMaster != null)
						{
							...
							if (
								m_Mobile.SummonMaster is PlayerMobile
