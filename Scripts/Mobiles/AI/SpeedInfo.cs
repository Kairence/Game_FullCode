...
namespace Server
{
	public class SpeedInfo
	{
		...
		public static bool GetSpeedsNew(BaseCreature bc, ref double activeSpeed, ref double passiveSpeed)
		{
			...
			activeSpeed = 1.00 - dex * 0.001;
			if( activeSpeed < 0.001 )
				activeSpeed = 0.001;
		...
		public static double TransformMoveDelay(BaseCreature bc, double delay)
		{
			...
			if (!bc.IsDeadPet && (bc.ReduceSpeedWithDamage || bc.IsSubdued))
			{
				var offset = 1.00;
				if (bc.Hits > 0)
					offset = (double)bc.Hits * 10 / (double)bc.HitsMax; //offset = 1 / Math.Pow(2, (double)bc.Hits * 10 / (double)bc.HitsMax );
		...
		public static bool UseNewSpeeds
		{
			get { return true; }
		}
		...
		private static readonly SpeedInfo[] m_Speeds =
		{
			...
			new SpeedInfo(
				...
				new[]
				{
					...
					typeof(MaddeningHorror),
					typeof(Wight),
					typeof(WolfSpider),
					typeof(UndeadGargoyle),
					typeof(SlasherOfVeils),
					typeof(SavagePackWolf),
