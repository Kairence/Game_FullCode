...
namespace Server.Mobiles
{
	...
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		)
		{
			if (
				!dismounted.Mounted
				&& !Server.Spells.Ninjitsu.AnimalForm.UnderTransformation(dismounted)
				&& !dismounted.Flying
			)
				return;
					...
					else
					{
						...
						switch (m_Type)
						{
							...
							case BlockMountType.RidingSwipe:
								{
									if (
										(!Core.SA && m_Mount == null)
										|| m_Mount is Mobile && ((Mobile)m_Mount).Hits >= ((Mobile)m_Mount).HitsMax
									)
									{
