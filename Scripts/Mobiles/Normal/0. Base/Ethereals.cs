...
namespace Server.Mobiles
{
	public class EtherealMount : Item, IMount, IMountItem, IRewardItem
	{
		...
		{
			get { return Transparent ? TransparentMountedID : NonTransparentMountedID; }
		}

		public int MountedHue
		{
			get { return Transparent ? TransparentMountedHue : NonTransparentMountedHue; }
		}
		...
		[Constructable]
		public EtherealMount(
			int itemID,
			int transMountedID,
			int nonTransMountedID,
			int transHue = 0,
			int nonTransHue = 0
		)
			: base(itemID)
		...
		private class EtherealSpell : Spell
		{
			...
			{
				get { return TimeSpan.FromSeconds(Core.AOS ? 3.0 : 2.0); }
			}
