		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			...
			)
			{
				...
				)
				{
					...
					Timer.DelayCall(
						TimeSpan.FromSeconds(Utility.RandomMinMax(3, 6)),
						new TimerStateCallback<Mobile>(Untangle),
						m
					);
				...
				if (m.InRange(this, 1) && !m_DamageTable.ContainsKey(m))
				{
					...
					m_DamageTable[m] = Timer.DelayCall(
						TimeSpan.Zero,
						TimeSpan.FromSeconds(1.0),
						new TimerStateCallback<Mobile>(DoDamage),
						m
					);
				}
			}
