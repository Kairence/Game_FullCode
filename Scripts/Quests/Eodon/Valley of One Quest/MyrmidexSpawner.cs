...
namespace Server.Items
{
	...
	{
		private Type[] _SpawnList = { typeof(MyrmidexLarvae), typeof(MyrmidexDrone), typeof(MyrmidexWarrior) };
		...
		{
			get { return Utility.RandomMinMax(6, 9); }
		}
		...
		{
			get { return NextSpawn < DateTime.UtcNow; }
		}

		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (
				m.InRange(this.Location, 7)
				...
				for (int i = 0; i < SpawnCount - Spawn.Count; i++)
				{
					Timer.DelayCall(
						...
						() =>
						{
							...
							BaseCreature bc =
								Activator.CreateInstance(_SpawnList[Utility.Random(_SpawnList.Length)]) as BaseCreature;
	...
	public class EodonTribeRegion : Region
	{
		...
		{
			_Zones[0] = new EodonTribeRegion(
				EodonTribe.Jukari,
				new Rectangle2D[] { new Rectangle2D(640, 2046, 115, 115) },
				6
			);
			_Zones[1] = new EodonTribeRegion(
				EodonTribe.Kurak,
				new Rectangle2D[] { new Rectangle2D(291, 1817, 125, 90) },
				6
			);
			_Zones[2] = new EodonTribeRegion(
				EodonTribe.Barrab,
				new Rectangle2D[]
				{
					new Rectangle2D(134, 1767, 33, 20),
					new Rectangle2D(142, 1786, 57, 80),
					new Rectangle2D(145, 1750, 20, 20),
				},
				5
			);
			_Zones[3] = new EodonTribeRegion(
				EodonTribe.Barako,
				new Rectangle2D[] { new Rectangle2D(620, 1677, 95, 100) },
				5
			);
			_Zones[4] = new EodonTribeRegion(
				EodonTribe.Urali,
				new Rectangle2D[] { new Rectangle2D(320, 1551, 160, 72) },
				5
			);
			_Zones[5] = new EodonTribeRegion(
				EodonTribe.Sakkhra,
				new Rectangle2D[] { new Rectangle2D(482, 1375, 200, 200) },
				8
			);
		}
		...
		public int Spawns
		{
			get { return this.GetItemCount(i => i is MyrmidexHill); }
		}
