...
namespace Server.Mobiles
{
	public class MyrmidexQueen : BaseCreature
	{
			...
			for (byte i = 0; i <= 0x7; i++)
			{
				...
				Timer.DelayCall(
					...
					() =>
					{
						...
						Timer.DelayCall(
							TimeSpan.FromSeconds(Utility.RandomMinMax(2, 3)),
							() =>
							{
								Type t = Utility.RandomList<Type>(
									typeof(MyrmidexWarrior),
									typeof(MyrmidexDrone),
									typeof(MyrmidexLarvae)
								);
		...
		public void SpitOoze()
		{
			...
			foreach (Mobile m in eable)
			{
				if (m != this && SpellHelper.ValidIndirectTarget(this, m) && this.CanBeHarmful(m, false))
				{
					...
					Timer.DelayCall(
						TimeSpan.FromSeconds(Utility.RandomMinMax(20, 30)),
						() =>
						{
							ooze1.Delete();
							ooze2.Delete();
		...
		public void RaiseRocks()
		{
			...
			if (target != null)
			{
				...
				{
					case Direction.West:
						r = new Rectangle2D(this.X - 24, this.Y - 2, 20, 5);
						break;
					case Direction.North:
						r = new Rectangle2D(this.X - 2, this.Y - 24, 5, 20);
						break;
					case Direction.East:
						r = new Rectangle2D(this.X + 4, this.Y - 2, 20, 5);
						break;
					case Direction.South:
						r = new Rectangle2D(this.X - 4, this.Y + 4, 20, 5);
						break;
				}
	...
	public class Zipactriotl : BaseCreature
	{
		...
		public override bool TeleportsTo
		{
			get { return true; }
		}
		public override TimeSpan TeleportDuration
		{
			get { return TimeSpan.FromSeconds(15); }
		}
		public override double TeleportProb
		{
			get { return 0.33; }
		}
		...
		public override Mobile GetTeleportTarget()
		{
			...
			foreach (Mobile m in eable)
			{
				if (
					...
					&& CanSee(m)
					&& ((PlayerMobile)m)
						.AllFollowers.Where(pet => !(pet is IMount) || ((IMount)pet).Rider == null)
						.Count() > 0
				)
				{
					list.Add(m);
		...
		public void DoSpecial()
		{
			...
			for (int i = 0; i < _Offsets.Length; i += 2)
			{
				...
				Timer.DelayCall<Point3D>(
					TimeSpan.FromMilliseconds(350 * counter),
					(point) =>
					{
			...
			Timer.DelayCall(
				TimeSpan.FromMilliseconds(Utility.RandomMinMax(300, 350) * (_Offsets.Length / 2)),
				() =>
				{
					...
					{
						if (
							m != this && m is PlayerMobile
							|| (m is BaseCreature && ((BaseCreature)m).GetMaster() is PlayerMobile) && CanBeHarmful(m)
						)
						{
		...
		{
			Math.Cos(300.0 / 180.0 * Math.PI),
			Math.Sin(300.0 / 180.0 * Math.PI),
			Math.Cos(320.0 / 180.0 * Math.PI),
			Math.Sin(320.0 / 180.0 * Math.PI),
			Math.Cos(340.0 / 180.0 * Math.PI),
			Math.Sin(340.0 / 180.0 * Math.PI),
			Math.Cos(000.0 / 180.0 * Math.PI),
			Math.Sin(000.0 / 180.0 * Math.PI),
			Math.Cos(020.0 / 180.0 * Math.PI),
			Math.Sin(020.0 / 180.0 * Math.PI),
			Math.Cos(040.0 / 180.0 * Math.PI),
			Math.Sin(040.0 / 180.0 * Math.PI),
			Math.Cos(060.0 / 180.0 * Math.PI),
			Math.Sin(060.0 / 180.0 * Math.PI),
			Math.Cos(080.0 / 180.0 * Math.PI),
			Math.Sin(080.0 / 180.0 * Math.PI),
			Math.Cos(100.0 / 180.0 * Math.PI),
			Math.Sin(100.0 / 180.0 * Math.PI),
			Math.Cos(120.0 / 180.0 * Math.PI),
			Math.Sin(120.0 / 180.0 * Math.PI),
			Math.Cos(140.0 / 180.0 * Math.PI),
			Math.Sin(140.0 / 180.0 * Math.PI),
			Math.Cos(160.0 / 180.0 * Math.PI),
			Math.Sin(160.0 / 180.0 * Math.PI),
			Math.Cos(180.0 / 180.0 * Math.PI),
			Math.Sin(180.0 / 180.0 * Math.PI),
			Math.Cos(200.0 / 180.0 * Math.PI),
			Math.Sin(200.0 / 180.0 * Math.PI),
			Math.Cos(220.0 / 180.0 * Math.PI),
			Math.Sin(220.0 / 180.0 * Math.PI),
			Math.Cos(240.0 / 180.0 * Math.PI),
			Math.Sin(240.0 / 180.0 * Math.PI),
			Math.Cos(260.0 / 180.0 * Math.PI),
			Math.Sin(260.0 / 180.0 * Math.PI),
			Math.Cos(280.0 / 180.0 * Math.PI),
			Math.Sin(280.0 / 180.0 * Math.PI),
			Math.Cos(260.0 / 180.0 * Math.PI),
			Math.Sin(260.0 / 180.0 * Math.PI),
			Math.Cos(240.0 / 180.0 * Math.PI),
			Math.Sin(240.0 / 180.0 * Math.PI),
