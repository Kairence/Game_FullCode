...
namespace Server.Engines.Quests.RitualQuest
{
	public class Prugyilonus : MondainQuester
	{
		...
		{
			get { return new Type[] { typeof(ScalesOfADreamSerpentQuest) }; }
		}
		...
		public Prugyilonus(Serial serial)
			: base(serial) { }
	...
	public class Bexil : MondainQuester
	{
		public override Type[] Quests
		{
			get { return new Type[] { typeof(CatchMeIfYouCanQuest) }; }
		}
	...
	public class BexilPunchingBag : BaseCreature
	{
		...
		public override void OnThink()
		{
			...
			if (Combatant is Mobile && _NextTeleport < DateTime.UtcNow)
			{
				...
				do
				{
					...
					!map.CanSpawnMobile(p.X, p.Y, map.GetAverageZ(p.X, p.Y))
					|| !Region.Find(p, map).IsPartOf<CatchMeIfYouCanQuest.BexilRegion>()
				);
		...
		public BexilPunchingBag(Serial serial)
			: base(serial) { }
	...
	public class Grubbix : MondainQuester
	{
		public override Type[] Quests
		{
			get { return new Type[] { typeof(FilthyLifeStealersQuest) }; }
		}
