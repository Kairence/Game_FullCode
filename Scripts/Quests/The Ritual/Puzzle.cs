...
namespace Server.Engines.Quests.RitualQuest
{
	public class CrystalLotusPuzzle : Item
	{
		...
		private void DoSequence()
		{
			...
			for (int i = 0; i < seqCount; i++)
			{
				...
				Timer.DelayCall<PuzzleTile, int>(
					TimeSpan.FromSeconds(i * WhiteLength),
					(tile, index) =>
					{
						tile.Hue = White;

						Timer.DelayCall(
							TimeSpan.FromSeconds(WhiteLength),
							t =>
							{
						...
						if (index == seqCount - 1)
						{
							...
							foreach (NetState ns in eable)
							{
								if (
									ns.Mobile != null
									&& ns.Mobile is PlayerMobile
									&& QuestHelper.HasQuest<PristineCrystalLotusQuest>((PlayerMobile)ns.Mobile)
								)
								{
		...
		{
			Region = new Region(
				"Crystal Lotus Puzzle Region",
				Map.TerMur,
				Region.DefaultPriority,
				new Rectangle2D(945, 2858, 66, 62)
			);
			Region.Register();
		}

		public CrystalLotusPuzzle(Serial serial)
			: base(serial) { }
	...
	public class PuzzleTile : Item
	{
		...
		public PuzzleTile(Serial serial)
			: base(serial) { }
