...
namespace Server.Engines.Quests.Haven
{
	...
	public class KillHordeMinionsObjective : QuestObjective
	{
		...
		{
			if (
				creature is HordeMinion
				&& corpse.Map == Map.Trammel
				&& corpse.X >= 3314
				&& corpse.X <= 3814
				&& corpse.Y >= 2345
				&& corpse.Y <= 3095
	...
	public class GetDaemonBoneObjective : QuestObjective
	{
		...
		{
			get { return this.m_CorpseWithBone; }
			set { this.m_CorpseWithBone = value; }
		}
		public override object Message
		{
			get
		...
		public override bool IgnoreYoungProtection(Mobile from)
		{
			...
			if (
				(from is Zombie || from is Skeleton)
				&& from.Map == Map.Trammel
				&& from.X >= 3391
				&& from.X <= 3424
				&& from.Y >= 2639
		...
		{
			if (
				(creature is Zombie || creature is Skeleton)
				&& corpse.Map == Map.Trammel
				&& corpse.X >= 3391
				&& corpse.X <= 3424
				&& corpse.Y >= 2639
