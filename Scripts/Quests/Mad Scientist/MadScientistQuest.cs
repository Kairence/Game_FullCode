	...
	public class MadScientistQuest
	{
		...
		private static void GenQuest_Command(CommandEventArgs e)
		{
			...
			List<Item> toDelete = new List<Item>(
				World.Items.Values.Where(i =>
					i is XmlSpawner && (i.Name == "PerfectTimingSpawner" || i.Name == "PerfectTimingSpawner2")
				)
			);
		...
		public static void BarkIngredient(Mobile from)
		{
			...
			from.PublicOverheadMessage(
				...
				0x3B2,
				1112821,
				String.Format("#{0}", (int)context.CurrentIngredient)
		...
		public class QuestContext
		{
			...
			{
				get { return m_IngredientsLeft; }
			}
			public SutekIngredient CurrentIngredient
			{
				get { return m_CurrentIngredient; }
			}
			public Timer ExpireTimer
			{
				get { return m_ExpireTimer; }
			}
			public ClockworkMechanism Mechanism
			{
			...
			public void StartTimer()
			{
				...
				m_Owner.PublicOverheadMessage(
					MessageType.Regular,
					0x3B2,
					1112821,
					String.Format("#{0}", (int)m_CurrentIngredient)
				); // I need to add some ~1_INGREDIENT~.
