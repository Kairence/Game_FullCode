...
namespace Server.Accounting
{
	...
	public class Account : IAccount, IComparable, IComparable<Account>
	{
		...
		public Account(XmlElement node)
		{
			...
			801번 ~ 950번 계정 공동 포인트
			951번 ~ 999번 그 외 기타 저장
			801번 ~ 860번 : 던전(향 후 진행) 포인트
			861번 ~ 890번 : 각종 자재료 포인트(미구현)
			891번 ~ 950번 : 스킬 저장 포인트(케릭 간 스킬 이동에 사용)
			951번 : 모래 채집
			999번 : 계정 포인트 버전 관리
			*/
		...
		public static int[] LoadPoints(XmlElement node)
		{
			...
			foreach (XmlElement ele in chars.GetElementsByTagName("point"))
			{
				try
				{
