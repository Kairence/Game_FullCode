...
namespace Server.Items
{
	...
	{
		private static readonly Dictionary<Mobile, DualWieldTimer> m_Registry =
			new Dictionary<Mobile, DualWieldTimer>();

		public DualWield() { }

		public static Dictionary<Mobile, DualWieldTimer> Registry
		{
				...
				if (timer.DualHitChance < .75)
				{
					...
					BuffInfo.AddBuff(
						...
						new BuffInfo(
							...
							1151294,
							1151293,
							timer.Expires - DateTime.UtcNow,
							attacker,
							(timer.DualHitChance * 100).ToString()
						)
					);
		...
		{
			if (
				HasRegistry(attacker)
				&& attacker.Weapon is BaseWeapon
				&& m_Registry[attacker].DualHitChance > Utility.RandomDouble()
			)
			{
