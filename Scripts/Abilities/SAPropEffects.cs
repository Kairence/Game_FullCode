...
namespace Server.Items
{
	...
	public class PropertyEffect
	{
		...
		public static List<PropertyEffect> Effects
		{
		...
		private class InternalTimer : Timer
		{
			...
			{
				if (
					m_Effect.Mobile == null
					|| (m_Effect.Mobile.Deleted || !m_Effect.Mobile.Alive || m_Effect.Mobile.IsDeadBondedPet)
				)
				{
					m_Effect.RemoveEffects();
				}
				else if (
					m_Effect.Victim != null
					&& (m_Effect.Victim.Deleted || !m_Effect.Victim.Alive || m_Effect.Mobile.IsDeadBondedPet)
				)
				{
					m_Effect.RemoveEffects();
				}
		...
		public static T GetContext<T>(Mobile from, EffectsType type)
			where T : PropertyEffect
		{
			return m_Effects.FirstOrDefault(e => e.Mobile == from && e.Effect == type) as T;
		}

		public static T GetContext<T>(Mobile from, Mobile victim, EffectsType type)
			where T : PropertyEffect
		{
			return m_Effects.FirstOrDefault(e => e.Mobile == from && e.Victim == victim && e.Effect == type) as T;
		}

		public static IEnumerable<T> GetContexts<T>(Mobile victim, EffectsType type)
			where T : PropertyEffect
		{
	...
	public class SoulChargeContext : PropertyEffect
	{
		...
		public static void CheckHit(Mobile attacker, Mobile defender, int damage)
		{
			...
			if (
				...
				&& shield.ArmorAttributes.SoulCharge > Utility.Random(100)
			)
			{
		...
		public static int GetValue(DamageType type, Mobile from)
		{
			...
			switch (type)
			{
				case DamageType.Kinetic:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterKinetic);
				case DamageType.Fire:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterFire);
				case DamageType.Cold:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterCold);
				case DamageType.Poison:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterPoison);
				case DamageType.Energy:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterEnergy);
				case DamageType.AllTypes:
					return (int)SAAbsorptionAttributes.GetValue(from, SAAbsorptionAttribute.EaterDamage);
			}
			return 0;
		}
	...
	public class SplinteringWeaponContext : PropertyEffect
	{
			...
			: base(from, defender, weapon, EffectsType.Splintering, TimeSpan.FromSeconds(4), TimeSpan.FromSeconds(4))
		{
			...
			BuffInfo.AddBuff(
				defender,
				new BuffInfo(BuffIcon.SplinteringEffect, 1154670, 1152144, TimeSpan.FromSeconds(10), defender)
			);
		}
		...
		public static bool CheckHit(Mobile attacker, Mobile defender, WeaponAbility ability, Item weapon)
		{
			if (
				...
				|| (
					Core.EJ
					&& (
						ability == WeaponAbility.Disarm
						|| ability == WeaponAbility.InfectiousStrike
						|| SkillMasterySpell.HasSpell(attacker, typeof(SkillMasterySpell))
					)
				)
			)
				return false;

			SplinteringWeaponContext context = PropertyEffect.GetContext<SplinteringWeaponContext>(
				attacker,
				defender,
				EffectsType.Splintering
			);
	...
	public class SearingWeaponContext : PropertyEffect
	{
		...
		{
			get { return Utility.RandomMinMax(10, 15); }
		}
		...
		{
			SearingWeaponContext context = PropertyEffect.GetContext<SearingWeaponContext>(
				attacker,
				defender,
				EffectsType.Searing
			);
	...
	public class BoneBreakerContext : PropertyEffect
	{
		...
		public static int CheckHit(Mobile attacker, Mobile defender)
		{
			int mana = (int)(
				...
				* (
					(double)(
						AosAttributes.GetValue(attacker, AosAttribute.LowerManaCost)
						+ BaseArmor.GetInherentLowerManaCost(attacker)
					) / 100.0
				)
			);
			int damage = 0;
			...
			{
				BoneBreakerContext context = PropertyEffect.GetContext<BoneBreakerContext>(
					attacker,
					defender,
					EffectsType.BoneBreaker
				);
	...
	public class SwarmContext : PropertyEffect
	{
		...
		{
			ColUtility.ForEach(
				PropertyEffect.GetContexts<SwarmContext>(victim, EffectsType.Swarm),
				context =>
				{
					context.RemoveEffects();
				}
