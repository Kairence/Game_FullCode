...
namespace Server.Items
{
	...
	public class FrenziedWhirlwind : WeaponAbility
	{
		...
		{
			return from.Skills[SkillName.Ninjitsu].Base > from.Skills[SkillName.Bushido].Base
				? SkillName.Ninjitsu
				: SkillName.Bushido;
		}
		...
		public override void OnHit(Mobile attacker, Mobile defender, int damage)
		{
			...
			var targets = SpellHelper
				.AcquireIndirectTargets(attacker, attacker.Location, attacker.Map, 2)
				.OfType<Mobile>()
				.ToList();
			...
			if (targets.Count > 0)
			{
				...
				foreach (var pm in targets.OfType<PlayerMobile>())
				{
					BuffInfo.AddBuff(
						pm,
						new BuffInfo(BuffIcon.SplinteringEffect, 1153804, 1028852, TimeSpan.FromSeconds(2.0), pm)
					);
				}
				...
				if (defender is PlayerMobile && attacker is PlayerMobile)
				{
					...
					BuffInfo.AddBuff(
						defender,
						new BuffInfo(BuffIcon.SplinteringEffect, 1153804, 1152144, TimeSpan.FromSeconds(2.0), defender)
					);
					Timer.DelayCall(
						TimeSpan.FromSeconds(2),
						mob => mob.SendSpeedControl(SpeedControlType.Disable),
						defender
					);
				}
			...
			private void DoHit()
			{
				foreach (Mobile m in m_List)
				{
					if (m_Attacker.InRange(m.Location, 2) && m.Alive && m.Map == m_Attacker.Map)
					{
						...
						int skill =
							m_Attacker is BaseCreature
								? (int)m_Attacker.Skills[SkillName.Ninjitsu].Value
								: (int)
									Math.Max(
										m_Attacker.Skills[SkillName.Bushido].Value,
