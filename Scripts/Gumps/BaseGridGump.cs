...
namespace Server.Gumps
{
	public abstract class BaseGridGump : Gump
	{
		...
		public int CurrentX
		{
			get { return this.m_CurrentX; }
		}
		public int CurrentY
		{
			get { return this.m_CurrentY; }
		}
		...
		public void AddEntryLabel(int width, string text)
		{
			...
			this.AddLabelCropped(
				this.m_CurrentX + this.TextOffsetX,
				this.m_CurrentY,
				width - this.TextOffsetX,
				this.EntryHeight,
				this.TextHue,
				text
			);
		...
		public void AddEntryHeader(int width, int spannedEntries)
		{
			this.AddImageTiled(
				...
				this.m_CurrentY,
				width,
				(this.EntryHeight * spannedEntries) + (this.OffsetSize * (spannedEntries - 1)),
				this.HeaderGumpID
			);
			this.IncreaseX(width);
		}
		...
		)
		{
			...
			this.AddButton(
				this.m_CurrentX + ((width - buttonWidth) / 2),
				this.m_CurrentY
					+ (
						((this.EntryHeight * spannedEntries) + (this.OffsetSize * (spannedEntries - 1)) - buttonHeight)
						/ 2
					),
				normalID,
				pressedID,
		...
		)
		{
			...
			this.AddButton(
				this.m_CurrentX + ((width - buttonWidth) / 2),
				this.m_CurrentY + ((this.EntryHeight - buttonHeight) / 2),
				normalID,
				pressedID,
				0,
				GumpButtonType.Page,
