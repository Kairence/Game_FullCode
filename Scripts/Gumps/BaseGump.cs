...
namespace Server.Gumps
{
	public abstract class BaseGump : Gump, IDisposable
	{
		...
		public virtual bool CloseOnMapChange
		{
			get { return false; }
		}
		...
		public static T GetGump<T>(PlayerMobile pm, Func<T, bool> predicate)
			where T : Gump
		{
			return EnumerateGumps<T>(pm).FirstOrDefault(x => predicate == null || predicate(x));
		}

		public static IEnumerable<T> EnumerateGumps<T>(PlayerMobile pm, Func<T, bool> predicate = null)
			where T : Gump
		{
			var ns = pm.NetState;
			...
			foreach (
				BaseGump gump in ns
					.Gumps.OfType<BaseGump>()
					.Where(g => g.GetType() == typeof(T) && (predicate == null || predicate(g as T)))
			)
			{
		...
		public static List<T> GetGumps<T>(PlayerMobile pm)
			where T : Gump
		{
			var ns = pm.NetState;
		...
		public void AddHtmlLocalizedCentered(
			int x,
			int y,
			int length,
			int height,
		...
		public void AddHtmlLocalizedAlignRight(
			int x,
			int y,
			int length,
			int height,
		...
		private Dictionary<Dictionary<int, string>, Spoof> _ClilocTooltips =
			new Dictionary<Dictionary<int, string>, Spoof>();
		...
		)
		{
			...
			{
				spoof.Text = String.Concat(
					String.Format("<basefont color=#{0:X}>{1}", titleColor.ToArgb(), title),
					'\n',
