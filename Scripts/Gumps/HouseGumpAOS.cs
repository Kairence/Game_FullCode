...
namespace Server.Gumps
{
	...
	public class HouseGumpAOS : Gump
	{
		...
		private static readonly int[] m_HangerNumbers = new int[] { 2968, 2970, 2972, 2974, 2976, 2978 };
			...
			switch (page)
			{
				case HouseGumpPageAOS.Information:
				{
					...
					AddLabel(
						...
						330,
						LabelHue,
						house.Price.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US"))
					);
				...
				case HouseGumpPageAOS.Customize:
				{
					...
					AddButtonLabeled(
						...
						GetButtonID(5, 8),
						1060004,
						isOwner && house is BaseContestHouse || house is Castle || house is Keep
					);
		...
		public static void SwapHouse_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay && house != null)
			{
				...
				if (
					(house is BaseContestHouse && ((BaseContestHouse)house).HouseType == ContestHouseType.Keep)
					|| house is Keep
				)
				{
					entries = HousePlacementEntry
						.HousesEJ.Where(e =>
							e.MultiID != house.ItemID
							&& (e.MultiID == 0x007C || e.MultiID == 0x147E || e.MultiID >= 0x1484)
						)
						.ToArray();
				}
				else if (
					(house is BaseContestHouse && ((BaseContestHouse)house).HouseType == ContestHouseType.Castle)
					|| house is Castle
				)
				{
					entries = HousePlacementEntry
						.HousesEJ.Where(e =>
							e.MultiID != house.ItemID
							&& (e.MultiID == 0x007E || (e.MultiID >= 0x147F && e.MultiID <= 0x1483))
						)
						.ToArray();
				}
						...
						case 0: // Convert Into Customizable House
						{
							if (isOwner && !isCustomizable)
							{
								...
								else if (m_House.HasAddonContainers)
								{
									...
									m_House.Map == Map.TerMur
									&& !Server.Engines.Points.PointsSystem.QueensLoyalty.IsNoble(from)
								)
								{
									...
									*/
									from.SendGump(
										new WarningGump(
											...
											420,
											280,
											new WarningGumpCallback(SwapHouse_Callback),
											m_House
										)
									);
								}
