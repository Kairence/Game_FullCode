	...
	public class RunebookGump : Gump
	{
		...
		public static string GetLocation(RunebookEntry e)
		{
			...
			if (e.Type == RecallRuneType.Ship)
			{
				...
				Sextant.Format(e.Location, e.Map, ref xLong, ref yLat, ref xMins, ref yMins, ref xEast, ref ySouth)
			)
			{
				loc = string.Format(
					"{0}o {1}'{2}, {3}o {4}'{5}",
		...
		private void AddBackground()
		{
			...
			AddHtmlIntern(380, 40, 50, 18, 19, false, false); // Max charges
		}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (
				Book.Deleted
				|| !from.InRange(Book.GetWorldLocation(), (Core.ML ? 3 : 1))
