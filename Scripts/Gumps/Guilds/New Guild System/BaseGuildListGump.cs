...
namespace Server.Guilds
{
	...
	public abstract class BaseGuildListGump<T> : BaseGuildGump
	{
		...
		{
			get { return (this.m_Filter.Length >= 0); }
		}
		...
		public override void PopulateGump()
		{
			...
			if (this.m_Ascending)
				...
				for (
					int i = this.m_List.Count - 1 - this.m_StartNumber;
					i >= 0 && i >= (this.m_List.Count - itemsPerPage - this.m_StartNumber);
					i--
				)
					this.DrawEntry(this.m_List[i], i, itemNumber++);
		...
		public virtual void DrawEntry(T o, int index, int itemNumber)
		{
			...
			for (int j = 0; j < this.m_Fields.Length; j++)
			{
				...
				this.AddHtmlText(
					...
					f.Width,
					20,
					this.GetValuesFor(o, this.m_Fields.Length)[j],
					false,
					false
				);
				...
				case 6: //Back
				{
					pm.SendGump(
						this.GetResentGump(
							...
							this.m_Ascending,
							this.m_Filter,
							this.m_StartNumber - itemsPerPage
						)
					);
					break;
				}
