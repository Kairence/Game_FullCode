	...
	public class GuildMemberInfoGump : BaseGuildGump
	{
		...
		public override void PopulateGump()
		{
			...
			this.AddHtmlLocalized(
				...
				270,
				26,
				(this.m_Member == this.player.GuildFealty && this.guild.Leader != this.m_Member) ? 1063082 : 1062996,
				0x0,
				false,
				false
			...
			switch (info.ButtonID)
			{
					...
					)
					{
						...
						if (targetRank == RankDefinition.Leader)
						{
							...
							else
							{
								...
								pm.SendGump(
									new GuildMemberInfoGump(this.player, this.guild, this.m_Member, false, true)
								);
							}
						}
						else
				...
				case 3: //Set Guild title
				{
					if (
						...
						&& (playerRank.Rank > targetRank.Rank || this.m_Member == this.player)
					)
					{
				...
				{
					if (
						(playerRank.GetFlag(RankFlags.RemovePlayers) && playerRank.Rank > targetRank.Rank)
