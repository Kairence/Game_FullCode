		...
		public override void PopulateGump()
		{
			...
			if (ActiveWar)
			{
				...
				if (
					...
					&& (activeWar.WarBeginning + activeWar.WarLength) > DateTime.UtcNow
				)
			...
			else if (PendingWar)
			{
				...
				time = Color(
					String.Format("{0:D2}:{1:mm}", war.WarLength.Hours, DateTime.MinValue + war.WarLength),
					0x990000
				);
			...
			switch (info.ButtonID)
			{
						...
						else
						{
							...
							if (otherAlliance != null && this.m_Other.Alliance.IsMember(this.m_Other))
							{
								otherAlliance.AllianceMessage(
									1070769,
									((alliance != null) ? alliance.Name : this.guild.Name)
								); // Guild Message: Your guild is now at war with ~1_GUILDNAME~
					...
					else
					{
						if (activeWar != null)
						{
							...
							if (otherAlliance != null && otherAlliance.IsMember(otherGuild))
							{
								otherAlliance.AllianceMessage(
									1070739,
									((this.guild.Alliance != null) ? this.guild.Alliance.Name : this.guild.Name)
					...
					if (war == null && activeWar == null)
					{
						...
						else if (otherAlliance != null && otherAlliance.Leader != this.m_Other)
						{
							pm.SendLocalizedMessage(
								1063239,
								String.Format("{0}\t{1}", this.m_Other.Name, otherAlliance.Name)
				...
				case 14: //Withdraw Request
				{
					if (!playerRank.GetFlag(RankFlags.AllianceControl))
					{
						...
						alliance != null
						&& alliance.Leader == this.guild
						&& alliance.IsPendingMember(this.m_Other)
					)
					{
						this.m_Other.Alliance = null;
						pm.SendLocalizedMessage(1070752); // The proposal has been updated.
				...
				case 15: //Deny Alliance Request
				{
					if (!playerRank.GetFlag(RankFlags.AllianceControl))
					{
						...
						&& otherAlliance != null
						&& alliance.Leader == this.m_Other
						&& otherAlliance.IsPendingMember(this.guild)
					)
					{
						pm.SendLocalizedMessage(1070752); // The proposal has been updated.
				...
				case 16: //Accept Alliance Request
				{
					if (!playerRank.GetFlag(RankFlags.AllianceControl))
					{
						...
						otherAlliance != null
						&& otherAlliance.Leader == this.m_Other
						&& otherAlliance.IsPendingMember(this.guild)
					)
					{
						pm.SendLocalizedMessage(1070752); // The proposal has been updated.
