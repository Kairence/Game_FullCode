	...
	public class GuildDismissGump : GuildMobileListGump
	{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (info.ButtonID == 1)
			{
				...
				if (switches.Length > 0)
				{
					...
					if (index >= 0 && index < this.m_List.Count)
					{
						...
						if (m != null && !m.Deleted)
						{
							...
							if (
								this.m_Mobile.AccessLevel >= AccessLevel.GameMaster
								|| this.m_Mobile == this.m_Guild.Leader
							)
							{
				...
				info.ButtonID == 2
				&& (this.m_Mobile.AccessLevel >= AccessLevel.GameMaster || this.m_Mobile == this.m_Guild.Leader)
			)
			{
				GuildGump.EnsureClosed(this.m_Mobile);
				this.m_Mobile.SendGump(new GuildmasterGump(this.m_Mobile, this.m_Guild));
