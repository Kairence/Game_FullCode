	...
	public class AddGump : Gump
	{
			...
			: base(50, 50)
		{
			...
			this.AddHtmlLocalized(
				...
				20,
				1061027,
				((this.m_Page + 1) * 10) < searchResults.Length ? 0x7FFF : 0x5EF7,
				false,
				false
			); // Next page
		}
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 2: // Previous page
				{
					if (this.m_Page > 0)
						from.SendGump(
							new AddGump(from, this.m_SearchString, this.m_Page - 1, this.m_SearchResults, true)
						);
		...
		private static void Match(string match, Type[] types, List<Type> results)
		{
			...
			for (int i = 0; i < types.Length; ++i)
			{
				...
				if (
					...
					&& Insensitive.Contains(t.Name, match)
					&& !results.Contains(t)
				)
				{
