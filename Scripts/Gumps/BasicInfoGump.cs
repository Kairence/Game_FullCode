...
namespace Server.Gumps
{
			...
			if (title != null)
			{
				...
				if (title.Number > 0)
					...
					AddHtml(
						...
						275,
						20,
						String.Format("<BASEFONT COLOR=WHITE>{0}</BASEFONT>", title.String),
						false,
						false
					);
				...
				if (body.Number > 0)
					...
					AddHtml(
						...
						275,
						height - 60,
						String.Format("<BASEFONT COLOR=WHITE>{0}</BASEFONT>", body.String),
						false,
						false
					);
			}
	...
	public class BasicConfirmGump<T> : Gump
	{
		...
		public BasicConfirmGump(
			TextDefinition warning,
			Action<Mobile, T> confirmcallback,
			T state,
			Action<Mobile, T> cancelcallback = null,
			int width = 290,
			int height = 95
		)
			: base(100, 100)
		{
			...
			if (warning.Number > 0)
				...
				AddHtml(
					...
					width - 20,
					height - 50,
					String.Format("<BASEFONT COLOR=#FFFFFF>{0}</BASEFONT>", warning.String),
					false,
					true
				);
		}
