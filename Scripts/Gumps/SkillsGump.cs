	...
	public class EditSkillGump : Gump
	{
			...
			: base(GumpOffsetX, GumpOffsetY)
		{
			...
			this.AddLabelCropped(
				...
				EntryHeight,
				TextHue,
				String.Format("{0}  [{1}]", skill.Name, skill.Cap)
			);
					...
					if (this.m_From.AccessLevel >= AccessLevel.GameMaster)
					{
						...
						if (text != null)
						{
							...
							CommandLogging.LogChangeProperty(
								this.m_From,
								this.m_Target,
								String.Format("{0}.Base", this.m_Skill),
								this.m_Skill.Base.ToString()
							);
						}
					}
					else
				...
				if (group == selected)
				{
					...
					this.AddImageTiled(
						...
						(group.Skills.Length * (EntryHeight + OffsetSize))
							- (i < (this.m_Groups.Length - 1) ? OffsetSize : 0),
						BackGumpID + 4
					);
				}
			}
						...
						if (sk != null)
						{
							if (this.m_From.AccessLevel >= AccessLevel.GameMaster)
							{
								this.m_From.SendGump(
									new EditSkillGump(this.m_From, this.m_Target, sk, this.m_Selected)
								);
							}
							else
							{
