...
namespace Server.Gumps
{
	...
	public class CAGObject : CAGNode
	{
		...
		{
			get { return (this.m_Type == null ? "bad type" : this.m_Type.Name); }
		}
	...
	public class CategorizedAddGump : Gump
	{
		...
		public void Initialize(int page)
		{
			...
			int emptyWidth =
				TotalWidth - (PrevWidth * 2) - NextWidth - (OffsetSize * 5) - (OldStyle ? SetWidth + OffsetSize : 0);
			...
			this.AddHtml(
				...
				EntryHeight,
				String.Format("<center>{0}</center>", this.m_Category.Caption),
				false,
				false
			);
