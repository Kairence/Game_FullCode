...
namespace Server.Commands
{
	class Location : IComparable
	{
		...
		{
			string hashString = String.Format("{0}-{1}-{2}-{3}", X, Y, Z, Map.MapID);
	...
	public class GenTeleporter
	{
		...
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(GenTeleporter_OnCommand)
			);
			CommandSystem.Register(
				"TelGenDelete",
				AccessLevel.Administrator,
				new CommandEventHandler(TelGenDelete_OnCommand)
			);
		}
		...
		public class TeleportersCreator
		{
			...
			)
			{
				this.CreateTeleporter(
					new Point3D(xLoc, yLoc, zLoc),
					new Point3D(xDest, yDest, zDest),
					mapLocation,
					mapDestination,
					back
				);
