	...
	public sealed class ObjectConditional
	{
		...
		public bool IsItem
		{
			get
			{
				return (
					...
					|| this.m_ObjectType == typeofItem
					|| this.m_ObjectType.IsSubclassOf(typeofItem)
				);
			}
		}
		...
		public bool IsMobile
		{
			get
			{
				return (
					this.m_ObjectType == null
					|| this.m_ObjectType == typeofMobile
					|| this.m_ObjectType.IsSubclassOf(typeofMobile)
				);
			}
		}
		...
		{
			get { return (this.m_Conditionals != null); }
		}
		...
		public void Compile(ref AssemblyEmitter emitter)
		{
			...
			for (int i = 0; i < this.m_Conditionals.Length; ++i)
				this.m_Conditionals[i] = ConditionalCompiler.Compile(
					emitter,
					this.m_ObjectType,
					this.m_Conditions[i],
					i
