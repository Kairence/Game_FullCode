...
namespace Server.Commands.Generic
{
	...
	public class InterfaceGump : BaseGridGump
	{
		...
		public void Render()
		{
			...
			this.AddEntryHtml(
				40 + (this.m_Columns.Length * 130) - 20 + ((this.m_Columns.Length - 2) * this.OffsetSize),
				this.Center(
					String.Format(
						"Page {0} of {1}",
						this.m_Page + 1,
						(this.m_List.Count + EntriesPerPage - 1) / EntriesPerPage
					)
				)
			);
			...
			for (
				int i = this.m_Page * EntriesPerPage, line = 0;
				line < EntriesPerPage && i < this.m_List.Count;
				++i, ++line
			)
			{
				this.AddNewLine();
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			switch (info.ButtonID)
			{
				...
				{
					if (this.m_Page > 0)
						this.m_From.SendGump(
							new InterfaceGump(this.m_From, this.m_Columns, this.m_List, this.m_Page - 1, this.m_Select)
						);
				...
				default:
				{
					...
					if (v >= 0 && v < this.m_List.Count)
					{
						...
						if (!BaseCommand.IsAccessible(this.m_From, obj))
						{
							...
							this.m_From.SendGump(
								new InterfaceGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Select)
							);
							break;
						}
						...
						if (obj is Item && !((Item)obj).Deleted)
							this.m_From.SendGump(
								new InterfaceItemGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, (Item)obj)
							);
	...
	public class InterfaceItemGump : BaseGridGump
	{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			{
				this.m_From.SendGump(
					new InterfaceGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Item)
				);
				return;
			}
			...
			switch (info.ButtonID)
			{
				...
				{
					this.m_From.SendGump(
						new InterfaceItemGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Item)
					);
				...
				case 3: // Delete
				{
					CommandLogging.WriteLine(
						...
						CommandLogging.Format(this.m_Item)
					);
					this.m_Item.Delete();
					this.m_From.SendGump(
						new InterfaceGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Item)
				...
				case 6: // Bring to pack
				{
					...
					if (
						...
						&& !BaseCommand.IsAccessible(this.m_From, owner) /* !m_From.CanSee( owner )*/
					)
					{
						...
						&& (owner.Map == null || owner.Map == Map.Internal)
						&& owner.Hidden
						&& owner.AccessLevel >= this.m_From.AccessLevel
					)
					{
						this.m_From.SendMessage("You can not get what you can not see.");
					}
	...
	public class InterfaceMobileGump : BaseGridGump
	{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			{
				this.m_From.SendGump(
					new InterfaceGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Mobile)
				);
				return;
			}
			...
			switch (info.ButtonID)
			{
				...
				{
					this.m_From.SendGump(
						new InterfaceMobileGump(this.m_From, this.m_Columns, this.m_List, this.m_Page, this.m_Mobile)
					);
				...
				case 3: // Delete
				{
					if (!this.m_Mobile.Player)
					{
						CommandLogging.WriteLine(
							...
							this.m_From.AccessLevel,
							CommandLogging.Format(this.m_From),
							CommandLogging.Format(this.m_Mobile)
						);
