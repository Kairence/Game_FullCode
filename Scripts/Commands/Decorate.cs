...
namespace Server.Commands
{
	public class Decorate
	{
		...
		public static void GenerateFromFile(string keyName, string path, params Map[] maps)
		{
			...
			Utility.WriteConsoleColor(
				...
				"[Decoration] Generated decoration from {0} complete, including {1} items!",
				path,
				count
			);
		}

		public static void GenerateRestricted(
			string keyName,
			string folder,
			Type restrictType,
			bool derivesFrom,
		...
		)
		{
			...
			Utility.WriteConsoleColor(
				ConsoleColor.Magenta,
				"[Decoration] Added {0} decoration items oftype {1} from folder {2} [{3} files]!",
				count,
				restrictType.Name,
				folder,
				files.Length
	...
	public class DecorationList
	{
						...
						if (m_Params[i].StartsWith("CannonDirection"))
						{
							...
							if (indexOf >= 0)
								direction = (CannonDirection)
									Enum.Parse(typeof(CannonDirection), m_Params[i].Substring(++indexOf), true);
						}
					}
						...
						if (m_Params[i].StartsWith("Content"))
						{
							...
							if (indexOf >= 0)
							{
								content = (BeverageType)
									Enum.Parse(typeof(BeverageType), m_Params[i].Substring(++indexOf), true);
								fill = true;
							}
						}
					}
				...
				else if (m_Type.IsSubclassOf(typeofBaseDoor))
				{
					...
					for (int i = 0; i < m_Params.Length; ++i)
					{
						if (m_Params[i].StartsWith("Facing"))
						{
							...
							{
								facing = (DoorFacing)
									Enum.Parse(typeof(DoorFacing), m_Params[i].Substring(++indexOf), true);
								break;
							}
						}
					}
				...
				else if (m_Type.IsSubclassOf(typeofAddonComponent))
				{
					switch (m_Type.Name)
					{
						default:
							Utility.WriteConsoleColor(
								ConsoleColor.Red,
								"[Decorate] Warning: {0} addon component has null Addon. Will delete after next server start",
								m_Type.Name
							);
							item = (Item)Activator.CreateInstance(m_Type);
							break;
					...
					if (m_Params[i].StartsWith("ContentType"))
					{
						...
						if (indexOf >= 0)
							cont.ContentType = (FillableContentType)
								Enum.Parse(typeof(FillableContentType), m_Params[i].Substring(++indexOf), true);
					}
				}
		...
		{
			return new List<DecorationList>(
				ReadAll(path).Where(dl => dl.m_Type == type || (derivesFrom && dl.m_Type.IsSubclassOf(type)))
			);
		}
