...
namespace Server.Commands
{
	public class CreateWorld
	{
		...
		public static List<CommandEntry> Commands = new List<CommandEntry>(
			new CommandEntry[]
			{
				...
				new CommandEntry(
					"City Loyalty",
					"SetupCityLoyaltySystem",
					"DeleteCityLoyaltySystem",
					Category.System,
					Expansion.SA,
					122
				),
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register(
				...
				new CommandEventHandler(Recreate_OnCommand)
			);
		}

		[Usage("CreateWorld [nogump]")]
		[Description(
			"Generates the world with a menu. If nogump argument is given, no gump will be displayed, all options will be assumed true, and the action will proceed immediately."
		)]
		...
		[Description(
			"Undoes world generation with a menu. If nogump argument is given, no gump will be displayed, all options will be assumed true, and the action will proceed immediately."
		)]
		...
		[Description(
			"Re-generates the world with a menu. If nogump argument is given, no gump will be displayed, all options will be assumed true, and the action will proceed immediately."
		)]
							...
							case CreateWorld.GumpType.Create:
								...
								if (CanGenerate(entry, ref error))
								{
									if (entry.Delay > 0)
									{
										DoDelayedCommand(
											from,
											TimeSpan.FromMinutes(entry.Delay),
		...
		public static bool CanGenerate(CommandEntry entry, ref string error)
		{
			...
			if (entry.CheckID == 127)
			{
				...
				string er = String.Format(
					"<br>- Cannot generate {0}. You need to generate Decorations and Spawners first.",
					entry.Name
				);
				Console.WriteLine(er);
			...
			if (entry.Category == Category.RevampedDungeon)
			{
				...
				{
					string er = String.Format(
						"<br>- Cannot generate {0}. You need to generate Spawners first.",
						entry.Name
					);
					Console.WriteLine(er);
	...
	public class CreateWorldGump : Gump
	{
			...
			: base(50, 50)
		{
			...
			foreach (CreateWorld.CommandEntry entry in CreateWorld.Commands)
			{
				...
				bool created =
					CreateWorldData.CreateTable.ContainsKey(entry.CheckID)
					&& CreateWorldData.CreateTable[entry.CheckID];

				AddLabel(
					20,
					y + 1,
					created ? 200 : 338,
					String.Format("{0} {1}", entry.Name, created ? "[created]" : "[not created]")
				);
				AddCheck(
					210,
					y - 2,
					210,
					211,
					m_Type == CreateWorld.GumpType.Create ? !created : created,
					entry.CheckID
				);
	...
	public class NewCreateWorldGump : BaseGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			AddHtml(
				...
				140,
				20,
				ColorAndCenter("#696969", String.Format("Your Expansion: {0}", Core.Expansion.ToString())),
				false,
				false
			);
			...
			List<CreateWorld.CommandEntry> commands = new List<CreateWorld.CommandEntry>(
				CreateWorld.Commands.Where(c =>
	...
	public static class CreateWorldData
	{
		...
		public static bool HasGenerated(int index)
		{
			switch (index)
			{
				...
				case 109:
					return SHTeleporter.SHTeleporterCreator.FindSHTeleporter(Map.Trammel, new Point3D(5747, 1895, 0))
						!= null;
				case 110:
				...
				case 122:
					return Server.Engines.CityLoyalty.CityLoyaltySystem.Cities != null
						&& Server.Engines.CityLoyalty.CityLoyaltySystem.Cities.Count > 0
						&& Server.Engines.CityLoyalty.CityLoyaltySystem.Cities[0].Stone != null;
				case 123:
