	...
	public class DecorateDelete
	{
		public static void Initialize()
		{
			CommandSystem.Register(
				...
				new CommandEventHandler(DecorateDelete_OnCommand)
			);
		}
						...
						if (this.m_Params[i].StartsWith("CannonDirection"))
						{
							...
							if (indexOf >= 0)
								direction = (CannonDirection)
									Enum.Parse(typeof(CannonDirection), this.m_Params[i].Substring(++indexOf), true);
						}
					}
						...
						if (this.m_Params[i].StartsWith("Content"))
						{
							...
							if (indexOf >= 0)
							{
								content = (BeverageType)
									Enum.Parse(typeof(BeverageType), this.m_Params[i].Substring(++indexOf), true);
								fill = true;
							}
						}
					}
				...
				else if (this.m_Type.IsSubclassOf(typeofBaseDoor))
				{
					...
					for (int i = 0; i < this.m_Params.Length; ++i)
					{
						if (this.m_Params[i].StartsWith("Facing"))
						{
							...
							{
								facing = (DoorFacing)
									Enum.Parse(typeof(DoorFacing), this.m_Params[i].Substring(++indexOf), true);
								break;
							}
						}
					}
			...
			else if (item is SkillTeleporter)
			{
				...
				for (int i = 0; i < this.m_Params.Length; ++i)
				{
					if (this.m_Params[i].StartsWith("Skill"))
					{
						...
						if (indexOf >= 0)
							tp.Skill = (SkillName)
								Enum.Parse(typeof(SkillName), this.m_Params[i].Substring(++indexOf), true);
					}
					...
					if (this.m_Params[i].StartsWith("ContentType"))
					{
						...
						if (indexOf >= 0)
							cont.ContentType = (FillableContentType)
								Enum.Parse(typeof(FillableContentType), this.m_Params[i].Substring(++indexOf), true);
					}
				}
				...
				if (this.m_Params[i].StartsWith("Light"))
				{
					...
					if (indexOf >= 0)
						item.Light = (LightType)
							Enum.Parse(typeof(LightType), this.m_Params[i].Substring(++indexOf), true);
				}
