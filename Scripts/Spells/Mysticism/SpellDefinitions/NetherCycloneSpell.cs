			...
			if (p != null && CheckSequence())
			{
				...
				if (map != null)
				{
					...
					for (int x = effectArea.X; x <= effectArea.X + effectArea.Width; x++)
					{
						for (int y = effectArea.Y; y <= effectArea.Y + effectArea.Height; y++)
						{
							...
							Timer.DelayCall<Point3D>(
								TimeSpan.FromMilliseconds(Utility.RandomMinMax(100, 300)),
								point =>
								{
					...
					foreach (var d in AcquireIndirectTargets(p, 3))
					{
						...
						double damage = (
							((Caster.Skills[CastSkill].Value + (Caster.Skills[DamageSkill].Value / 2)) * .66)
							+ Utility.RandomMinMax(1, 6)
						);
						...
						if (d is Mobile)
						{
							...
							double mod =
								m.Skills[SkillName.MagicResist].Value
						...
						Effects.SendLocationParticles(
							EffectItem.Create(d.Location, map, EffectItem.DefaultDuration),
							0x37CC,
							1,
							40,
							97,
