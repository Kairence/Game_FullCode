...
namespace Server.Spells.Mysticism
{
	public class SpellPlagueSpell : MysticSpell
	{
		...
		private static Dictionary<Mobile, List<SpellPlagueTimer>> m_Table =
			new Dictionary<Mobile, List<SpellPlagueTimer>>();
		...
		public static void OnMobileDamaged(Mobile from)
		{
			if (m_Table.ContainsKey(from) && m_Table[from].Count > 0 && m_Table[from][0].NextUse < DateTime.UtcNow)
			{
				...
				double mod =
					from.Skills[SkillName.MagicResist].Value >= 70.0
						? (from.Skills[SkillName.MagicResist].Value / 1000 * 3)
						: 0.0;
