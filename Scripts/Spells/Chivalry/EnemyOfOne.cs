...
namespace Server.Spells.Chivalry
{
	...
	{
		private static readonly SpellInfo m_Info = new SpellInfo("Enemy of One", "Forul Solum", -1, 9002);
		...
		private static readonly Dictionary<Mobile, EnemyOfOneContext> m_Table =
			new Dictionary<Mobile, EnemyOfOneContext>();
		...
		public static string GetTypeName(Mobile defender)
		{
			if (
				...
				|| (defender is BaseCreature && ((BaseCreature)defender).GetMaster() is PlayerMobile)
			)
			{
	...
	public class EnemyOfOneContext
	{
		...
		{
			double anatomy = m_Owner.Skills.Anatomy.Value * 0.6;
			if (anatomy >= 100)
				anatomy += 8;

			m_DamageScalar = (int)anatomy;

			/*
			var anatomy = (int)m_Owner.Skills.Anatomy.Value;
			...
			if (m_TypeName == null)
			{
				BuffInfo.AddBuff(
					...
					new BuffInfo(
						...
						1075653,
						1075902,
						m_Expire - DateTime.UtcNow,
						m_Owner,
						string.Format("{0}\t{1}", m_DamageScalar, "100"),
						true
					)
				);
			}
			else
			{
				BuffInfo.AddBuff(
					...
					new BuffInfo(
						...
						m_Expire - DateTime.UtcNow,
						m_Owner,
						string.Format("{0}\t{1}\t{2}\t{3}", m_DamageScalar, TypeName, ".", "100"),
						true
					)
				);
			}
