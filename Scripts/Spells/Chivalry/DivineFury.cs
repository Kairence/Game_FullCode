...
namespace Server.Spells.Chivalry
{
	...
	{
		private static readonly SpellInfo m_Info = new SpellInfo("Divine Fury", "Divinum Furis", -1, 9002);
		...
		public override void OnCast()
		{
			if (CheckSequence())
			{
				...
				string args = String.Format(
					"{0}\t{1}\t{2}\t{3}",
					GetAttackBonus(Caster).ToString(),
					GetDamageBonus(Caster).ToString(),
					GetWeaponSpeedBonus(Caster).ToString(),
					GetDefendMalus(Caster).ToString()
				);
				...
				*/
				...
				BuffInfo.AddBuff(
					Caster,
					new BuffInfo(BuffIcon.DivineFury, 1060589, 1150218, TimeSpan.FromSeconds(delay), Caster, args)
				);
		...
		public static int GetWeaponSpeedBonus(Mobile m)
		{
			if (m_Table.ContainsKey(m))
			{
				...
				return 10 + (int)anatomy;
				; //m.Skills[SkillName.Chivalry].Value >= 120.0 && m.Karma >= 10000 ? 15 : 10;
			}
