...
namespace Server.Spells.Chivalry
{
	...
	{
		private static readonly SpellInfo m_Info = new SpellInfo("Consecrate Weapon", "Consecrus Arma", -1, 9002);

		private static Dictionary<Mobile, ConsecratedWeaponContext> m_Table =
			new Dictionary<Mobile, ConsecratedWeaponContext>();
			...
			else if (this.CheckSequence())
			{
				...
				*/
				...
				IEntity from = new Entity(
					Serial.Zero,
					new Point3D(this.Caster.X, this.Caster.Y, this.Caster.Z),
					this.Caster.Map
				);
				IEntity to = new Entity(
					Serial.Zero,
					new Point3D(this.Caster.X, this.Caster.Y, this.Caster.Z + 50),
					this.Caster.Map
				);
				...
				double seconds = (double)weapon.Speed; // this.ComputePowerValue(20);
				...
				*/
				...
				BuffInfo.AddBuff(
					...
					new BuffInfo(
						...
						duration,
						Caster,
						String.Format("{0}\t{1}", context.ConsecrateProcChance, context.ConsecrateDamageBonus)
					)
				);
			}
