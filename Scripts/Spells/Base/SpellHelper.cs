...
namespace Server.Spells
{
	...
	public class SpellHelper
	{
		...
		public static bool CheckField(Point3D p, Map map)
		{
			...
			foreach (Item item in eable)
			{
				...
				if (
					...
					|| t.IsDefined(typeof(DispellableFieldAttribute), true)
				)
				{
					eable.Free();
					return false;
		...
		public static bool AddStatBonus(Mobile caster, Mobile target, bool blockSkill, StatType type)
		{
			return AddStatBonus(
				...
				target,
				type,
				GetOffset(caster, target, type, false, blockSkill),
		...
		public static int GetOffset(Mobile caster, Mobile target, StatType type, bool curse, bool blockSkill)
		{
			if (Core.AOS)
			{
				...
				{
					case StatType.Str:
						return 20; //(int)Math.Ceiling(target.RawStr * percent);
					case StatType.Dex:
						return 20; //(int)Math.Ceiling(target.RawDex * percent);
					case StatType.Int:
						return 20; //(int)Math.Ceiling(target.RawInt * percent);
				}
			}
		...
		public static bool ValidIndirectTarget(Mobile from, Mobile to)
		{
			...
			if (from is BaseCreature)
			{
				...
				else
				{
					...
					if (
						to is BaseCreature
						&& (((BaseCreature)to).Controlled || ((BaseCreature)to).Summoned)
						&& ((BaseCreature)to).GetMaster() is PlayerMobile
					)
					{
						return true;
					}
		...
		public static IEnumerable<IDamageable> AcquireIndirectTargets(
			Mobile caster,
			IPoint3D p,
			Map map,
			int range,
		...
		public static void Summon(
			...
			bool scaleStats,
			bool summoned = true,
			SkillName useSkill = SkillName.Magery
		)
		{
			Map map = caster.Map;
		...
		public static bool RestrictRedTravel
		{
			get { return Config.Get("General.RestrictRedsToFel", false); }
		}
		...
		public static bool IsValidTile(int itemID)
		{
			...
			return (
				...
				|| itemID == 0x1D1F
				|| itemID == 0x1D12
			); // Pentagram center, Abbatoir center, Arcane Circle Center, Bloody Pentagram has 4 tiles at center itemID == 0x307F 심원의 진
		}
		...
		public static void SummonCheck(Mobile Caster)
		{
			...
			foreach (Mobile m in World.Mobiles.Values)
			{
				if (m is BaseCreature)
				{
					...
					{
						if (
							bc is SummonedDaemon
							|| bc is SummonedFireElemental
							|| bc is SummonedWaterElemental
							|| bc is SummonedAirElemental
							|| bc is SummonedEarthElemental
						)
							continue;
						else
							list.Add(m);
		...
		public static void SummonCheckHigh(Mobile Caster)
		{
			var list = new List<Mobile>();
			foreach (Mobile m in World.Mobiles.Values)
			{
		...
		public static bool CheckReflect(
			int circle,
			ref Mobile caster,
			ref IDamageable target,
			DamageType type = DamageType.Spell
		)
		{
			IDamageable c = caster as IDamageable;
		...
		public static bool CheckReflect(
			int circle,
			ref IDamageable source,
			ref IDamageable defender,
			DamageType type = DamageType.Spell
		)
		{
			bool reflect = false;
			...
			if (target.MagicDamageAbsorb > 0 && Utility.RandomDouble() < 0.6 - circle * 0.2)
			{
				reflect = true;
	...
	public class TransformationSpellHelper
	{
		...
		private static readonly Dictionary<Mobile, TransformContext> m_Table =
	...
	public class TransformTimer : Timer
	{
		...
		protected override void OnTick()
		{
			if (
				...
				|| (m_Mobile.Hue != m_Spell.Hue && !BestialSetHelper.IsBerserk(m_Mobile))
			)
			{
