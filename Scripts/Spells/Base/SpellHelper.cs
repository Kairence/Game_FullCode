...
/* Tele To */		{ true,		true,		true,		true,		true,		true,			true,			true,		false,				true,				false,				false,		false, 		true,						true,						true,			true,				false,		false,      true,           false,          false,          false,          true },
        ...
        public static bool IsValidTile(int itemID)
        {
            ...
            return (itemID == 0xFEA || itemID == 0x1216 || itemID == 0x1D10 || itemID == 0x1D0F || itemID == 0x1D1F || itemID == 0x1D12);	// Pentagram center, Abbatoir center, Arcane Circle Center, Bloody Pentagram has 4 tiles at center itemID == 0x307F 심원의 진
        }
		...
		public static void SummonCheck( Mobile Caster )
		{
			...
			foreach ( Mobile m in World.Mobiles.Values )
			{
				if ( m is BaseCreature )
				{
					...
					{
						if( bc is SummonedDaemon || bc is SummonedFireElemental || bc is SummonedWaterElemental || bc is SummonedAirElemental || bc is SummonedEarthElemental )
							continue;
						else
							list.Add( m );
					}
		...
		public static void SummonCheckHigh( Mobile Caster )
		{
			...
			foreach ( Mobile m in World.Mobiles.Values )
			{
				...
				{
					BaseCreature bc = m as BaseCreature;
					if( bc.ControlMaster == Caster && ( bc is SummonedDaemon || bc is SummonedFireElemental || bc is SummonedWaterElemental || bc is SummonedAirElemental || bc is SummonedEarthElemental ) )
					{
						list.Add( m );
					}
				}
