...
namespace Server.Spells.Spellweaving
{
	...
	{
		private static readonly SpellInfo m_Info = new SpellInfo("Attune Weapon", "Haeldril", -1);
		...
		public static void TryAbsorb(Mobile defender, ref int damage)
		{
			...
			else if (m_Table.ContainsKey(defender))
			{
				BuffInfo.AddBuff(
					defender,
					new BuffInfo(
						BuffIcon.AttuneWeapon,
						1075798,
						m_Table[defender].Expires - DateTime.UtcNow,
						defender,
						defender.MeleeDamageAbsorb.ToString()
					)
				);
			}
		}
		...
		public class ExpireTimer : Timer
		{
			...
			public void DoExpire(bool message)
			{
				...
				Timer.DelayCall(
					...
					delegate
					{
						m_Mobile.EndAction(typeof(AttuneWeaponSpell));
					}
				);
