	...
	public class AnimateDeadSpell : NecromancerSpell
	{
		...
		private static readonly CreatureGroup[] m_Groups = new CreatureGroup[]
		{
			...
			new CreatureGroup(
				...
				new SummonEntry[] { new SummonEntry(0, typeof(MoundOfMaggots)) }
			),
			// Mounts
			new CreatureGroup(
				new Type[] { typeof(Horse), typeof(Nightmare), typeof(FireSteed), typeof(Kirin), typeof(Unicorn) },
				new SummonEntry[]
				{
			...
			new CreatureGroup(
				...
				new SummonEntry[] { new SummonEntry(5000, typeof(WailingBanshee)), new SummonEntry(0, typeof(Wraith)) }
			),
			// Dragons
			new CreatureGroup(
				new Type[]
						...
						else if (this.CheckSequence())
						{
							...
							if (map != null)
							{
								...
								Timer.DelayCall(
									...
									new TimerStateCallback(SummonDelay_Callback),
									new object[] { this.Caster, c, p, map, group }
								);
							}
						}
					}
		...
		public static void Register(Mobile master, Mobile summoned)
		{
			...
			Timer.DelayCall(
				TimeSpan.FromSeconds(2.0),
				TimeSpan.FromSeconds(2.0),
				new TimerStateCallback(Summoned_Damage),
				summoned
			);
		}
