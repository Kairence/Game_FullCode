...
namespace Server.Spells.Necromancy
{
	...
	public class SummonFamiliarEntry
	{
		...
		{
			get { return this.m_ReqNecromancy; }
		}
		public double ReqSpiritSpeak
		{
			get { return this.m_ReqSpiritSpeak; }
		}
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (index >= 0 && index < this.m_Entries.Length)
			{
				...
				else if (necro < entry.ReqNecromancy || spirit < entry.ReqSpiritSpeak)
				{
					...
					this.m_From.SendLocalizedMessage(
						1061606,
						String.Format("{0:F1}\t{1:F1}", entry.ReqNecromancy, entry.ReqSpiritSpeak)
					);

					this.m_From.CloseGump(typeof(SummonFamiliarGump));
					this.m_From.SendGump(
						new SummonFamiliarGump(this.m_From, SummonFamiliarSpell.Entries, this.m_Spell)
					);
				}
