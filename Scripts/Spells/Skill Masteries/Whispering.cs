...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Whispering", "", -1, 9002);
		...
		public int EnhancedGainChance
		{
			get { return _EnhancedGainChance; }
		}
		...
		public override bool CheckCast()
		{
			...
			if (
				...
				|| ((PlayerMobile)Caster)
					.AllFollowers.Where(m => !(m is Server.Engines.Despise.DespiseCreature))
					.Count() == 0
			)
			{
		...
		public override void OnCast()
		{
			if (CheckSequence())
			{
				...
				{
					foreach (
						Mobile m in ((PlayerMobile)Caster).AllFollowers.Where(m =>
							m.Map == Caster.Map
							&& Caster.InRange(m.Location, PartyRange)
							&& !(m is Server.Engines.Despise.DespiseCreature)
						)
					)
					{
