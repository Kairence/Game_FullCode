...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Body Guard", "", -1, 9002);
		...
		protected override void OnTarget(object o)
		{
			...
			if (
				protectee is BaseCreature
				&& !((BaseCreature)protectee).Summoned
				&& ((BaseCreature)protectee).GetMaster() is PlayerMobile
			)
				master = ((BaseCreature)protectee).GetMaster();

			if (protectee != null)
			{
				BodyGuardSpell spell =
					GetSpell(s => s.GetType() == typeof(BodyGuardSpell) && s.Target == protectee) as BodyGuardSpell;
	...
	public class AcceptBodyguardGump : Gump
	{
		...
		public AcceptBodyguardGump(Mobile protector, Mobile protectee, BodyGuardSpell spell)
			: base(150, 50)
		{
