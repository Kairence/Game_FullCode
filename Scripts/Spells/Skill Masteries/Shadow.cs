...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Shadow", "", -1, 9002);
		...
		public override bool RevealOnTick
		{
			get { return false; }
		}
		...
		public override void OnCast()
		{
			if (CheckSequence())
			{
				...
				BuffInfo.AddBuff(
					Caster,
					new BuffInfo(BuffIcon.Shadow, 1155910, 1156059, TimeSpan.FromSeconds(duration), Caster)
				); // Increases difficulty to be detected while hidden. <br>Increases difficulty to unhide from taking damage.
			}
		...
		public static double GetDifficultyFactor(Mobile m)
		{
			...
			if (spell != null)
				return (
						(
							spell.Caster.Skills[spell.CastSkill].Value
							+ spell.Caster.Skills[spell.DamageSkill].Value
							+ (spell.GetMasteryLevel() * 40)
						) / 3
					) / 150;
