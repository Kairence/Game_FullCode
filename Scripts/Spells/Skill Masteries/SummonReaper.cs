...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Summon Reaper", "Lartarisstree", 204, 9061);
			...
			if (o is IPoint3D)
			{
				...
				else if (SpellHelper.CheckTown(p, this.Caster) && this.CheckSequence())
				{
					TimeSpan duration = TimeSpan.FromSeconds(
						((Caster.Skills[CastSkill].Value + (ArcanistSpell.GetFocusLevel(Caster) * 20)) / 240) * 75
					);
					BaseCreature.Summon(
						new SummonedReaper(Caster, this),
						false,
						this.Caster,
						new Point3D(p),
						442,
	...
	public class SummonedReaper : BaseCreature
	{
			...
			: base(AIType.AI_Spellweaving, FightMode.Closest, 10, 1, 0.2, 0.4)
		{
			...
			double scale =
				...
				+ (
					(
						caster.Skills[spell.CastSkill].Value
						+ (double)(spell.GetMasteryLevel() * 40)
						+ (double)(ArcanistSpell.GetFocusLevel(caster) * 20)
					)
				) / 1000.0;
