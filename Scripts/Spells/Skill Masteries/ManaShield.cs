...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Mana Shield", "Faerkulggen", -1, 9061);
		...
		public override void OnCast()
		{
			...
			{
				double skill =
					((Caster.Skills[CastSkill].Value + ArcanistSpell.GetFocusLevel(Caster) * 20) / 2)
					+ (GetMasteryLevel() * 20)
					+ 20;
				...
				BuffInfo.AddBuff(
					...
					new BuffInfo(
						...
						Caster,
						String.Format("{0}\t{1}\t{1}", ((int)(Chance * 100)).ToString(), "50")
					)
				); // ~1_CHANCE~% chance to reduce incoming damage by ~2_DAMAGE~%. Costs ~2_DAMAGE~% of original damage in mana.
			}
