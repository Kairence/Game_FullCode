...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Thrust", "", -1, 9002);
		...
		{
			get { return (GetMasteryLevel() * 6) * Phase; }
		}
		public int DefenseModifier
		{
			get { return _DefenseMod; }
			set { _DefenseMod = Math.Min((GetMasteryLevel() * 6) * 3, value); }
		}

		public int DefenseModDisplayed
		{
			get { return Math.Min(18, DefenseModifier); }
		}
		...
		public override void OnCast()
		{
			...
			BuffInfo.AddBuff(
				...
				new BuffInfo(
					...
					String.Format(
						"{0}\t{1}\t{2}",
						AttackModifier.ToString(),
						DefenseModifier.ToString(),
						ScaleMana(30).ToString()
					)
				)
			);
