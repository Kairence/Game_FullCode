...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Holy Fist", "Kal Vas Grav", -1, 9002);
			...
			if (m != null)
			{
				if (CheckHSequence(m))
				{
					...
					int sdiBonus = SpellHelper.GetSpellDamageBonus(
						...
						m,
						CastSkill,
						m is Mobile ? Caster.Player && ((Mobile)m).Player : false
					);
					...
					if (target is Mobile && !CheckResisted((Mobile)target) && ((Mobile)target).NetState != null)
					{
						...
						Server.Timer.DelayCall(
							TimeSpan.FromSeconds(skill / 60),
							() =>
							{
								if (
									!TransformationSpellHelper.UnderTransformation(mob, typeof(AnimalForm))
									&& (
										Core.SA
										|| !TransformationSpellHelper.UnderTransformation(
											mob,
											typeof(Server.Spells.Spellweaving.ReaperFormSpell)
										)
									)
								)
