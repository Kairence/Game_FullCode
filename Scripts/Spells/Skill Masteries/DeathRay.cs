...
namespace Server.Spells.SkillMasteries
{
	public class DeathRaySpell : SkillMasterySpell
	{
		...
		public override int DamageThreshold
		{
			get { return 1; }
		}
		public override bool DamageCanDisrupt
		{
			get { return true; }
		}
		public override double TickTime
		{
			get { return 3; }
		}

		public override int UpkeepCancelMessage
		{
			get { return 1155874; }
		} // You do not have enough mana to keep your death ray active.
		public override int DisruptMessage
		{
			get { return 1155793; }
		} // This action disturbs the focus necessary to keep your death ray active and it dissipates.

		public override TimeSpan ExpirationPeriod
		{
			get { return TimeSpan.FromMinutes(360); }
		}
			...
			if (m != null)
			{
				if (CheckHSequence(m))
				{
					...
					else
					{
						...
						double damage =
							(Caster.Skills[CastSkill].Base + Caster.Skills[DamageSkill].Base)
							* ((double)GetMasteryLevel() * .8);
						...
						BuffInfo.AddBuff(
							...
							new BuffInfo(
								...
								1155896,
								1156085,
								String.Format("{0}\t{1}", ((int)damage).ToString(), m.Name)
							)
						); // Deals ~2_DAMAGE~ to ~1_NAME~ every 3 seconds while in range. Preforming any action will end spell.
