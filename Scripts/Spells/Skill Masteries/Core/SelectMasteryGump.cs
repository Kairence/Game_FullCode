			...
			foreach (SkillName skName in MasteryInfo.Skills)
			{
				...
				if (sk != null && sk.IsMastery && sk.VolumeLearned > 0)
				{
					...
					AddHtmlLocalized(
						...
						200,
						16,
						MasteryInfo.GetLocalization(skName),
						skName == current ? C32216(Red) : C32216(Blue),
						false,
						false
					);
					AddHtmlLocalized(
						...
						16,
						1156052,
						MasteryInfo.GetMasteryLevel(User, skName).ToString(),
						0,
						false,
						false
					);
		...
		public override void OnResponse(RelayInfo info)
		{
			...
			else if (User.Skills[n].Base >= MasteryInfo.MinSkillRequirement)
			{
				...
				User.SendLocalizedMessage(
					1156236,
					String.Format("{0}\t{1}", MasteryInfo.MinSkillRequirement.ToString(), User.Skills[n].Info.Name)
