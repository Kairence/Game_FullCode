...
namespace Server.Spells.SkillMasteries
{
	public abstract class SkillMasterySpell : Spell
	{
		...
		public virtual int RequiredMana
		{
			get { return 10; }
		}
		public virtual bool PartyEffects
		{
			get { return false; }
		}
		public virtual int DamageThreshold
		{
			get { return 45; }
		}
		public virtual bool DamageCanDisrupt
		{
			get { return false; }
		}
		public virtual double TickTime
		{
			get { return 2; }
		}
		public virtual int PartyRange
		{
			get { return 12; }
		}

		public virtual int UpkeepCancelMessage
		{
			get { return 1156111; }
		} // You do not have enough mana to keep your ability active.
		public virtual int OutOfRangeMessage
		{
			get { return 1156098; }
		} // Your target is no longer in range of your ability.
		public virtual int DisruptMessage
		{
			get { return 1156110; }
		} // Your ability was canceled.

		public virtual bool CancelsWeaponAbility
		{
			get { return false; }
		}
		public virtual bool CancelsSpecialMove
		{
			get { return CancelsWeaponAbility; }
		}
		public virtual bool ClearOnSpecialAbility
		{
			get { return false; }
		}

		public virtual bool RevealOnTick
		{
			get { return true; }
		}

		public virtual TimeSpan ExpirationPeriod
		{
		...
		public virtual double BaseSkillBonus
		{
			get
			{
				...
				return (Caster.Skills[CastSkill].Value + Caster.Skills[DamageSkill].Value + (GetMasteryLevel() * 40))
					/ 3;
			}
		}
		...
		public SkillMasterySpell(Mobile caster, Item scroll, SpellInfo info)
		...
		public virtual bool OnTick()
		{
			...
			if (
				(Caster is PlayerMobile && Caster.NetState == null)
				|| Expires < DateTime.UtcNow
				|| !Caster.Alive
				|| Caster.IsDeadBondedPet
			)
			{
				Expire();
			}
		...
		protected void AddPartyMember(Mobile m)
		{
			...
			{
				foreach (
					var pet in ((PlayerMobile)m).AllFollowers.Where(p => !PartyList.Contains(p) && ValidPartyMember(p))
				)
				{
					AddPartyMember(pet);
				}
		...
		{
			foreach (
				var spell in GetSpells(s =>
					s.PartyEffects && s.PartyList != null && (s.Caster == m || s.PartyList.Contains(m))
				)
			)
			{
				if (disband)
		...
		private static Dictionary<Mobile, List<SkillMasterySpell>> m_Table =
			new Dictionary<Mobile, List<SkillMasterySpell>>();
		...
		public static TSpell GetSpell<TSpell>(Mobile m)
			where TSpell : SkillMasterySpell
		{
		...
		public static TSpell GetSpell<TSpell>(Func<SkillMasterySpell, bool> predicate)
			where TSpell : SkillMasterySpell
		{
		...
		public static bool HasSpell<TSpell>(Mobile from)
			where TSpell : SkillMasterySpell
		{
			CheckTable(from);
				...
				if (p != null)
				{
					foreach (PartyMemberInfo info in p.Members)
					{
						...
						if (
							spell != null
							&& spell.PartyEffects
							&& from.InRange(info.Mobile.Location, spell.PartyRange)
							&& spell.CheckPartyEffects(info.Mobile)
						)
							return spell;
					}
				}
		...
		public static void OnDamage(Mobile victim, Mobile damager, DamageType type, ref int damage)
		{
			...
			PerseveranceSpell preserve =
				SkillMasterySpell.GetSpellForParty(victim, typeof(PerseveranceSpell)) as PerseveranceSpell;
						...
						else
						{
							...
							if (left > 0)
								m.SendLocalizedMessage(
									1079335,
									left.ToString("F", System.Globalization.CultureInfo.InvariantCulture)
								); // You must wait ~1_seconds~ seconds before you can use this ability again.
						}
					}
		...
		public class UpkeepTimer : Timer
		{
			...
			public SkillMasterySpell Spell
			{
				get { return m_Spell; }
			}

			public UpkeepTimer(SkillMasterySpell spell)
				: base(TimeSpan.FromSeconds(spell.TickTime), TimeSpan.FromSeconds(spell.TickTime))
			{
				m_Spell = spell;
			}
