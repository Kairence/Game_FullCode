...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("White Tiger Form", "", -1, 9002);
		...
		public override void OnCast()
		{
			...
			else if (TransformationSpellHelper.UnderTransformation(Caster))
			{
				...
				|| (Caster.IsBodyMod && AnimalForm.GetContext(Caster) == null)
			)
			{
				DoFizzle();
			}
			else if (CheckSequence())
			{
				...
				int skills = (int)(
					(Caster.Skills[CastSkill].Value + Caster.Skills[DamageSkill].Value + (GetMasteryLevel() * 40)) / 3
				);
				...
				int bleedMod = (int)(
					(
						(
							Caster.Skills[SkillName.Ninjitsu].Value
							+ Caster.Skills[SkillName.Stealth].Value
							+ (GetMasteryLevel() * 40)
						) / 3
					) / 10
				);
				BuffInfo.AddBuff(
					Caster,
					new BuffInfo(
						BuffIcon.WhiteTigerForm,
						1155911,
						1156060,
						String.Format("{0}\t{1}\t{2}\t{3}", "20", "5", "", bleedMod.ToString())
					)
				);
		...
		public static int GetDamageBonus(Mobile m)
		{
			...
			if (context != null && context.Type == typeof(WildWhiteTiger))
				return (int)(
						(
							(
								m.Skills[SkillName.Ninjitsu].Value
								+ m.Skills[SkillName.Stealth].Value
								+ (MasteryInfo.GetMasteryLevel(m, SkillName.Ninjitsu) * 40)
							) / 3
						) / 10
					) / 2;
			...
			double bleedchance =
				(double)(
					(
						...
						+ attacker.Skills.Stealth.Value
						+ (MasteryInfo.GetMasteryLevel(attacker, SkillName.Ninjitsu) * 40)
					) / 3.0
				) / 15.0;
