...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Focused Eye", "", -1, 9002);
		...
		public override void OnCast()
		{
			...
			{
				_PropertyBonus = (int)(
					(Caster.Skills[CastSkill].Value + Caster.Skills[DamageSkill].Value + (GetMasteryLevel() * 40)) / 12
				);
				...
				BuffInfo.AddBuff(
					...
					new BuffInfo(
						...
						1156003,
						1156004,
						String.Format("{0}\t{1}", _PropertyBonus.ToString(), ScaleUpkeep().ToString())
					)
				); // +~1_VAL~% Hit Chance Increase.<br>Mana Upkeep Cost: ~2_VAL~.
