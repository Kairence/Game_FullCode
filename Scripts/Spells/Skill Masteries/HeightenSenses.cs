...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Heightened Senses", "", -1, 9002);
		...
		public override void OnCast()
		{
			if (CheckSequence())
			{
				...
				BuffInfo.AddBuff(
					Caster,
					new BuffInfo(
						BuffIcon.HeightenedSenses,
						1155925,
						1156062,
						String.Format("{0}\t{1}", PropertyBonus().ToString(), ScaleUpkeep().ToString())
					)
				); // +~1_ARG~% Parry Bonus.<br>Mana Upkeep Cost: ~2_VAL~.
			}
