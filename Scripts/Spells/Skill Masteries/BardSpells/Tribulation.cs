...
namespace Server.Spells.SkillMasteries
{
	public class TribulationSpell : BardSpell
	{
		...
		private static SpellInfo m_Info = new SpellInfo("Tribulation", "In Jux Hur Rel", -1, 9002);
		...
		public TribulationSpell(Mobile caster, Item scroll)
		...
		public void OnTarget(Mobile m)
		{
			...
			else if (CheckHSequence(m))
			{
				...
				BuffInfo.AddBuff(
					m,
					new BuffInfo(
						BuffIcon.TribulationTarget,
						1115740,
						1115742,
						String.Format("{0}\t{1}\t{2}", m_PropertyBonus, m_PropertyBonus, (int)m_DamageChance)
					)
				);
				...
				BuffInfo.AddBuff(
					Caster,
					new BuffInfo(
						BuffIcon.TribulationCaster,
						1115740,
						1151388,
						String.Format("{0}\t{1}\t{2}", m.Name, (int)m_DamageFactor, (int)m_DamageChance)
					)
				);
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(TribulationSpell spell)
