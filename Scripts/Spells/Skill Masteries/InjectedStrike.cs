...
namespace Server.Spells.SkillMasteries
{
	...
	{
		private static SpellInfo m_Info = new SpellInfo("Injected Strike", "", -1, 9002);
		...
		public override bool ClearOnSpecialAbility
		{
			get { return true; }
		}
		public override bool CancelsWeaponAbility
		{
			get { return true; }
		}
				...
				else if (!HasSpell(Caster, this.GetType()))
				{
					if (CheckSequence())
					{
						...
						BuffInfo.AddBuff(
							Caster,
							new BuffInfo(
								BuffIcon.InjectedStrike,
								1155927,
								1156163,
								String.Format("{0}\t{1}", bonus.ToString(), (bonus / 2).ToString())
							)
						);
		...
		protected override void OnTarget(object o)
		{
			...
			else if (o is BaseWeapon && weapon != null && (BaseWeapon)o == weapon && potion != null)
			{
				if (CheckSequence())
				{
					...
					{
						if (
							Caster.CheckTargetSkill(
								CastSkill,
								potion,
								potion.MinPoisoningSkill,
								potion.MaxPoisoningSkill
							)
						)
						{
							ApplyPoison(weapon, potion);
		...
		public override void OnHit(Mobile defender, ref int damage)
		{
			...
			BuffInfo.AddBuff(
				defender,
				new BuffInfo(
					BuffIcon.InjectedStrikeDebuff,
					1155927,
					1156133,
					TimeSpan.FromSeconds(7),
					defender,
					String.Format("{0}\t{1}", malus, Caster.Name)
				)
			);
