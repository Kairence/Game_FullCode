...
namespace Server.Spells.SkillMasteries
{
	public class FistsOfFurySpell : SkillMasteryMove
	{
		...
		{
			get { return new TextDefinition(1155895); }
		} // You ready yourself to unleash your fists of fury!

		public override TimeSpan CooldownPeriod
		{
		...
		public override void OnUse(Mobile from)
		{
			SkillName damageSkill =
				from.Skills[SkillName.Anatomy].Value > from.Skills[SkillName.EvalInt].Value
					? SkillName.Anatomy
					: SkillName.EvalInt;
			...
			Timer.DelayCall(
				...
				() =>
				{
					...
					BuffInfo.AddBuff(
						from,
						new BuffInfo(BuffIcon.FistsOfFury, 1155930, 1156256, TimeSpan.FromSeconds(duration), from)
					);
			...
			if (_Table[attacker].Target == defender)
			{
				...
				if (_Table[attacker].Hit >= 3)
				{
					...
					AOS.Damage(
						defender,
						attacker,
						Utility.RandomMinMax(level + 1, (level * 7) - 1),
						0,
						0,
						0,
						0,
