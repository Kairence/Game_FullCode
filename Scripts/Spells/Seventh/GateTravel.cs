...
namespace Server.Spells.Seventh
{
	public class GateTravelSpell : MagerySpell
	{
		...
		public void Effect(Point3D loc, Map map, bool checkMulti, bool isboatkey = false)
		{
			...
			else if (map != Map.Trammel && !map.CanSpawnMobile(loc.X, loc.Y, loc.Z) && !isboatkey)
			{
		...
		private bool GateExistsAt(Map map, Point3D loc)
		{
			...
			{
				if (item is UserMoongate || item is PublicMoongate)
				{
					_gateFound = true;
					break;
				}
		...
		[DispellableField]
		private class InternalItem : UserMoongate
		{
			[CommandProperty(AccessLevel.GameMaster)]
			public UserMoongate LinkedGate { get; set; }
			...
			public override void UseGate(Mobile m)
			{
				if (
					LinkedGate == null
					|| !(LinkedGate is InternalItem)
					|| !((InternalItem)LinkedGate).BoatGate
					|| !LinkedGate.Deleted
				)
				{
