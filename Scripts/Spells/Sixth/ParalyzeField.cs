	...
	public class ParalyzeFieldSpell : MagerySpell
	{
		...
		public void Target(IPoint3D p)
		{
			if (!Caster.CanSee(p))
			{
				...
				SpellHelper.CheckTown(p, Caster)
				&& SpellHelper.CheckWater(new Point3D(p), Caster.Map)
				&& CheckSequence()
			)
			{
				...
				{
					Timer.DelayCall<int>(
						TimeSpan.FromMilliseconds(i * 300),
						index =>
						{
							Point3D point = new Point3D(
								eastToWest ? pnt.X + index : pnt.X,
								eastToWest ? pnt.Y : pnt.Y + index,
								pnt.Z
							);
				...
				)
				{
					...
					if (Core.AOS)
					{
						duration =
							2.0
							+ (
								(int)(m_Caster.Skills[SkillName.EvalInt].Value / 10)
