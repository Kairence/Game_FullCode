...
namespace Server.Mobiles
{
	public class Banker : BaseVendor
	{
		...
		public static bool Withdraw(Mobile from, int amount, bool message = false)
		{
			...
			if (AccountGold.Enabled && from.Account != null && from.Account.WithdrawGold(amount))
			{
				if (message)
					...
					); // ~1_AMOUNT~ gold has been removed from your bank box.
			...
			if (AccountGold.Enabled && from.Account != null && from.Account.DepositGold(amount))
			{
				if (message)
					from.SendLocalizedMessage(
						1042763,
						amount.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("en-US"))
					); // ~1_AMOUNT~ gold was deposited in your account.
		...
		public static void HandleSpeech(Mobile vendor, SpeechEventArgs e)
		{
			...
			{
				if (
					e.Mobile.Map.Rules != MapRules.FeluccaRules
					&& vendor is BaseVendor
					&& !((BaseVendor)vendor).CheckVendorAccess(e.Mobile)
				)
				{
					return;
				}
						...
						case 0x0001: // *balance*
							{
								...
								if (AccountGold.Enabled && e.Mobile.Account is Account)
								{
									...
									vendor.Say(
										...
										String.Format(
											"{0:#,0}",
											((Account)e.Mobile.Account).GetSecureAccountAmount(e.Mobile)
										),
										0x3BC
									);
								}
