...
namespace Server.Mobiles
{
	public class GenericBuyInfo : IBuyItemInfo
	{
		...
		public bool EconomyItem
		{
			get { return BaseVendor.UseVendorEconomy; }
		}
		...
		public void OnBought(Mobile buyer, BaseVendor vendor, IEntity entity, int amount)
		{
			if (EconomyItem)
			{
				foreach (var bii in vendor.GetBuyInfo().OfType<GenericBuyInfo>())
				{
					if (
						bii.Type == m_Type
						|| (m_Type == typeof(UncutCloth) && bii.Type == typeof(Cloth))
						|| (m_Type == typeof(Cloth) && bii.Type == typeof(UncutCloth))
					)
					{
						bii.TotalBought += amount;
						//vendor.Say( bii.TotalBought.ToString());
					}
				}
			}

			//m_Price += BaseVendor.RegionPrice(vendor);
		...
		private class DisplayCache : Container
		{
			...
			public Dictionary<Type, IEntity> Table
			{
				get { return m_Table; }
			}
