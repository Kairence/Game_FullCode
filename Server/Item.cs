...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
	{
		...
		public void Bounce(Mobile from)
		{
			...
			if (bounce != null)
			{
				...
				if (parent is Item && !((Item)parent).Deleted)
				{
					...
					if (
						p.IsAccessibleTo(from) && (!(root is Mobile) || ((Mobile)root).CheckNonlocalDrop(from, this, p))
					)
					{
		...
		{
			to.Send(
				new MessageLocalizedAffix(m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, number, "", type, affix, "")
			);
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public virtual int DecayMultiplier
		{
			get { return 1; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public virtual bool DefaultDecaySetting
		{
			get { return true; }
		}
		...
		{
			get { return TimeSpan.FromMinutes(m_DDT.TotalMinutes * DecayMultiplier); }
		}
		...
		public virtual bool Decays
		{
			get
			{
				...
				return DefaultDecaySetting
					&& Movable
					&& Visible
					&& !HonestyItem /* && Spawner == null*/
				;
			}
		}
		...
		{
			get { return TimeSpan.FromMinutes((DecayTime - (DateTime.UtcNow - LastMoved)).TotalMinutes); }
		}
		...
		public virtual bool StackIgnoreItemID
		{
			get { return false; }
		}
		public virtual bool StackIgnoreHue
		{
			get { return false; }
		}
		public virtual bool StackIgnoreName
		{
			get { return false; }
		}
		...
		{
			get { return IsLockedDown || IsSecure; }
		}
		...
		public int QuestItemHue
		{
			get { return (HiddenQuestItemHue ? Hue : 0x04EA); }
		}
			...
			if (m_Map != null)
			{
				...
				foreach (NetState state in eable)
				{
					...
					if (m.CanSee(this) && m.InUpdateRange(worldLoc))
					{
						...
						{
							p = Packet.Acquire(
								new MessageLocalized(m_Serial, m_ItemID, type, hue, 3, number, Name, args)
							);
						}
			...
			if (Map != null && state != null)
			{
				...
				if (m != null && m.CanSee(this) && m.InRange(worldLoc, GetUpdateRange(m)))
				{
					...
					else
					{
						if (p == null)
							p = Packet.Acquire(
								new UnicodeMessage(m_Serial, m_ItemID, type, hue, 3, m.Language, Name, text)
							);
		...
		{
			get { return Parent as IEntity; }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public IEntity RootParentEntity
		{
			get { return RootParent as IEntity; }
		}
					...
					if (m_Map != null)
					{
						if (m_Parent == null)
						{
							...
							foreach (NetState state in eable)
							{
								...
								if (
									m.CanSee(this)
									&& m.InUpdateRange(m_Location)
									&& (
										!state.HighSeas
										|| !m_NoMoveHS
										|| (m_DeltaFlags & ItemDelta.Update) != 0
		...
		public void SendLocalizedMessage(int number, string args)
		{
			...
			Packet p = Packet.Acquire(
				new MessageLocalized(m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, number, Name, args)
			);
		...
		public void SendLocalizedMessage(MessageType type, int number, AffixType affixType, string affix, string args)
		{
			IPooledEnumerable eable = Map.GetClientsInRange(Location, Core.GlobalMaxUpdateRange);
			Packet p = Packet.Acquire(
				new MessageLocalizedAffix(m_Serial, m_ItemID, type, 0x3B2, 3, number, "", affixType, affix, args)
			);
		...
		public virtual void OnSingleClick(Mobile from)
		{
			...
			if (ns != null)
			{
				if (Name == null)
				{
					...
					{
						ns.Send(
							new MessageLocalized(m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, LabelNumber, "", "")
						);
					}
					else
					{
		...
		public T GetSocket<T>(Func<T, bool> predicate)
			where T : ItemSocket
		{
			if (Sockets == null)
			{
			...
			return Sockets.FirstOrDefault(s => s.GetType() == typeof(T) && (predicate == null || predicate(s as T)))
				as T;
		}
	...
	public class ItemSocket
	{
		...
		public virtual TimeSpan TickDuration
		{
