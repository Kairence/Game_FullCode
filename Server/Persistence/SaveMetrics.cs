...
namespace Server
{
	public sealed class SaveMetrics : IDisposable
	{
		...
		public SaveMetrics()
		{
			if (!PerformanceCounterCategory.Exists(PerformanceCategoryName))
			{
				...
				if (!Core.Unix)
				{
					...
					catch
					{
						if (Core.Debug)
							Console.WriteLine(
								"Metrics: Metrics enabled. Performance counters creation requires ServUO to be run as Administrator once!"
							);
					}
				}
				else
				{
					...
					Console.WriteLine(
						"WARNING: You've enabled SaveMetrics. This is currently not supported on Unix based operating systems. Please disable this option to hide this message."
					);
					Utility.PopColor();
				}
			}
			...
			this.serializedBytesPerSecond = new PerformanceCounter(
				PerformanceCategoryName,
				"Save - Serialized bytes/sec",
				false
			);
			this.writtenBytesPerSecond = new PerformanceCounter(
				PerformanceCategoryName,
				"Save - Written bytes/sec",
				false
			);
